...
| Phase A `criticalBaseKeys`               | 6555-6580   | 9 ключей для быстрой разблокировки UI: profile, norms, products, hr_zones, dayv2_today, advice_settings, advice_read_today, game, subscription_status, widget_layout_v1. Если UI читает ключ синхронно при первом рендере — он должен быть в этом списке (см. BUGS_HISTORY 2026-05-23 для уроков) |
| `LOCAL_ONLY_STORAGE_*`                   | 1790-1825   | Blocklists для ключей которые НЕ зеркалятся в облако (interceptor проверяет `isLocalOnlyStorageKey` early-return). EXACT*KEYS, PREFIXES, SUFFIXES + явные regex. Любой `heys*\*` ключ зеркалится по default, локальность нужно ОБЪЯВЛЯТЬ.                                                         |
