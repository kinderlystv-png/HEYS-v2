  ...
  function BuilderScreen(props) {
    ...
    function sessionServiceSubtitle() {
      ...
      if (openIdx >= 0 && openExLocal && (openUnitLocal === 'weight_reps' || openUnitLocal === 'bodyweight')) {
        const proposal = SK && SK.pendingPlanProposal ? SK.pendingPlanProposal(training) : null;
        const weightEditCtx = proposal && startedAt > 0 && !completedAt
          ? weightEditSessionContext(exercises, openExLocal, proposal)
          : null;
        if (weightEditCtx) {
    ...
    if (showInterrupted) {
      ...
      return h('div', { className: 'sb-root sb-root--interrupted' },
        ...
        h('main', { className: 'sb-interrupted-scroll' },
          ...
          interruptedNote && h('p', { className: 'sb-interrupted-note' }, interruptedNote)
        )
      );
    }
    ...
    const weightEditCtxRoot = openEx && pendingProposal && startedAt > 0 && !completedAt
      ? weightEditSessionContext(exercises, openEx, pendingProposal)
      : null;
    ...
    return h('div', {
      className: 'sb-root sb-builder-screen'
        ...
        + (weightEditCtxRoot ? ' is-weight-entry' : '')
        + (rest
