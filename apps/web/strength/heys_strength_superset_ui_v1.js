...
;(function (global) {
  ...
  function warmupDropHeadKey(exercise) {
    ...
    // Строка «разминка — ярлык, рабочий — номер»: номера идут только по рабочим
    // подходам, поэтому «4 подхода» всегда значит четыре РАБОЧИХ. Считали
    // разминку внутри этого числа, и «4 подхода · 1 разминочный» читалось как
    // «четыре рабочих и ещё разминка», хотя рабочих было три. Ступени дропа
    // счётчик не трогают: они живут внутри подхода, а не отдельными строками.
    const totalLabel = work + ' ' + ruPlural(work, 'подход', 'подхода', 'подходов');
    if (warmups > 0) {
    ...
    return h('div', { className: 'sb-root sb-triset-work-screen' },
      ...
      h('div', { className: 'sb-tw-scroll' },
        ...
        h('p', { className: 'sb-tw-footnote' },
          '«Местами» меняет участников внутри связки, данные подходов едут за упражнением. '
          + '«Разъединить» превращает участников в обычные упражнения: подходы сохраняются, раунды исчезают.')
      )
    );
  }
  ...
  const PLAN_MOVE_FOOTNOTE = 'Переносите сами, без подтверждения куратора: пока он ответит, день уйдёт, и перенос превратится в пропуск.';
      ...
      return h('div', { className: 'sb-plan-feed' },
        ...
        h('span', { className: 'sb-plan-trace' },
          plan.movedFrom
            ...
            : 'План — это назначение, а не факт: карточка запланированного дня не попадает ни в тоннаж, ни в счётчики, ни в расчёт нагрузки, пока тренировка не начата.'),
        actionError && h('p', { className: 'sb-confirm-text', role: 'alert' }, actionError)
      );
    }
  ...
  /** Границы правки связки для кадра Д3 (describePlanEdit → proposal_ui). */
  function describeSupersetBoundaries(liveExercises, proposedExercises) {
    ...
    proposedBlocks.forEach(function (pb) {
      ...
      if (started) {
        const letter = supersetGroupLetter(pb.groupId);
      ...
      replacements.push({
        ...
        beforeLines: supersetMemberLines(live, lb.indexes, liveRounds),
        afterLines: supersetMemberLines(proposed, pb.indexes, propRounds)
      });
    });
