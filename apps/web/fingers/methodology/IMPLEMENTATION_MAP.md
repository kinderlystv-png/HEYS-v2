...
- [ ] **Периодизация (ч.6)** — `periodization_engine`: мезо/макро-циклы (linear
      ...
      только на 1 день.
- [ ] **UI-плеер на остальные doseShape (Шаг 5)** —
      attempts/circuit/continuous/process → разблокирует non-hang атомы
      (болдер/ARC/кампус/техника/тактика): они в каталоге, но B1.5 их не
      рендерит.
- [ ] Прогрессия/вариативность (1.2/1.3) — `detectPlateau` → смена переменной
...
| 1.2 Прогресс. перегрузка       |   ✅   | 🟡  | `S4_progressionCap` (FTL-кап); полная прогрессия — Фаза 2                               |
| 1.3 Вариативность / плато      |   ✅   | ⬜  | `detectPlateau` — Фаза 2                                                                |
...
| 4. Блоки A–I (каталог)         |   ✅   | ✅  | `block_catalog` 36 атомов × 9 блоков; **non-hang атомы ждут UI Шаг 5**                  |
...
> safety S1–S9**, сборка дня, plumbing MVC/уровня) — в коде. Бэклог:
> периодизация, UI для non-hang doseShape (Шаг 5), прогрессия/вариативность,
> полная тест-батарея, числа бенчмарков.
