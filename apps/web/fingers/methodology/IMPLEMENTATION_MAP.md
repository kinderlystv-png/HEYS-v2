...
`flags.newEngine = true`, но live-wiring аудит ниже показывает, что часть
методологических модулей всё ещё не влияет на реальную сборку сессии. Ниже — что
осталось до «полного режима пальцев». Отмечай `[x]` по мере.
...
- [ ] **Периодизация (ч.6) — live wiring**: `periodization_engine` есть
      (`buildPlan`/`current`/`savePlan`/`loadPlan`, модели 6.1–6.6 и
      `selectModel` 6.4), `engine_router` умеет читать `current()` и
      `session_builder` умеет `plannerContext`, но обычный пользователь не
      создаёт `periodization`-plan: `buildPlan`/`savePlan` не вызываются вне
      самого модуля. Осталось связать UI старта цикла с
      `periodization.buildPlan(...)->savePlan(...)` или заменить отдельный
      `mesocycle` единым `periodization_engine`.
...
- [ ] Прогрессия/вариативность (1.2/1.3) — live enforcement: код есть
      (`detectPlateau`/`nextAxis`/`suggestProgression`,
      `_buildProgressionConstraints`, `_progressionAllowsAtom`), но live-opts не
      несут `recordsByQuality`, поэтому progression-cap сейчас не влияет на
      подбор атомов. Осталось собрать series по quality из реальных записей и
      прокинуть через `engine_router._enrichOpts`.
...
| -------------------- | :--------------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| 0. Сводка школ       |        ✅        | ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | —                                                                                                           |
| 1. Принципы          |        ✅        | 🟡 **1.2**: progression enforcement **не активна** — `recordsByQuality` не прокидывается в live-opts → `_buildProgressionConstraints` всегда `null`. Прокинуть records-by-quality в enrich (`engine_router`). **1.6**: `now`/per-tissue S2 freshness не подаётся в `recommendDay` → S2 в сборке микса не оценивается                                                                                                                                                       | **1.6**: довести ввод readiness/самочувствия (сейчас частично)                                              |
| 2. 9 качеств         |        ✅        | ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Quality labels/tooltip в карточке упражнения и trace: какое качество тренируется и почему оно выбрано       |
| 3. Физиология        |        ✅        | ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Tooltip/annotation для energy system, tissue freshness, danger профиля хвата                                |
| 4. Каталог A–I       |        ✅        | ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | **контент**: фото `/exercises/*.webp` для не-хватовых (болдер-мувы/антагонисты/мобильность) — сейчас эмодзи |
| 5. Протоколы         |        ✅        | ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | ✅                                                                                                          |
| 6. Периодизация      |        ✅        | 🟡 **методологический `periodization_engine` частично инертен**: `current()` уже подключён к роутеру, но `buildPlan`/`savePlan` без live-вызова → у обычного пользователя нет сохранённого plan → фазный clamp (ceiling/volume/focus, включая `selectModel` 6.4) обычно не активируется. Параллельный `mesocycle` клампит только Today-бакет. Связать создание плана (UI buildPlan→savePlan) **или** унифицировать `mesocycle`↔`periodization_engine` как единый источник | **6.x**: визуализация плана мезоцикла / forward-календарь (фазы, deload, retest)                            |
| 7. Уровни            |        ✅        | ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | ✅                                                                                                          |
| 8. Тесты / бенчмарки |        ✅        | **8.2**: заменить 🟠-дефолт бенчмарки на 🟢 Berta 2025 (Tables S2–S5)                                                                                                                                                                                                                                                                                                                                                                                                      | ✅ (8.1 ввод + 8.4 ретест + 8.6 графики готовы)                                                             |
| 9. Безопасность      |        ✅        | **9.4**: вес/RED-S — сейчас advisory-рамка, не жёсткий гейт (по решению)                                                                                                                                                                                                                                                                                                                                                                                                   | **9.4**: surface веса/RED-S в UI (частично)                                                                 |
| 10. Источники        |        ✅        | ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Source/info popover для методологических решений и чисел протокола                                          |

**Итог (после глубокого live-wiring аудита 2026-06-12):** методология написана и
проаудирована везде, но часть «науки» пока не доезжает до реального выбора
сессии. **Движок:** (а) progression 1.2 частично инертна — логика
`_progressionAllowsAtom` есть, но `recordsByQuality` не подаётся в live-opts;
(б) periodization 6 частично инертна — `engine_router` читает
`periodization.current()`, но обычный пользователь не создаёт
`periodization`-plan; (в) readiness 1.6 клампит bucket по cooldown/readiness, но
per-tissue S2 freshness не получает `history`+`now` в генераторе; (г) 8.2 —
нужен 🟢-апгрейд чисел бенчмарков; (д) 9.4 вес/RED-S сознательно advisory. **Что
РЕАЛЬНО работает на живом пути:** выбор `focusQuality` по сохранённому
`leadingLimiter` из тест-батареи (§1.4/§8), потолок bucket по
readiness/cooldown, S4 FTL-кап, MAV-обрезка, aerobic-под-режимы, FDP/FDS
edge-ротация, transfer-мостик M3, `variantSeed` reroll.

**Live-wiring verdicts по спорным цепочкам:**

| Цепочка                     | Вердикт     | Живой caller / отсутствие caller                                                                                                                                                                                                                                                                                                                                                | Влияние                                                                                                                    |
| --------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| 6. Periodization            | PARTIAL/GAP | `engine_router._enrichOpts` зовёт `periodization.current()` (`apps/web/fingers/heys_fingers_engine_router_v1.js:330-335`), `session_builder` применяет `plannerContext` (`apps/web/fingers/heys_fingers_session_builder_v1.js:843-849`, `1021-1024`), но `periodization.buildPlan/savePlan` не вызываются вне самого модуля.                                                    | При ручном/тестовом plan clamp сработает; обычный пользователь plan не создаёт, поэтому фаза мезо не влияет на mix-сессию. |
| 1.4/8 Assessment limiter    | PASS        | `records.assessLatestBattery()` читает сохранённую battery (`apps/web/fingers/heys_fingers_records_store_v1.js:369-376`), router кладёт `assessmentResult.leadingLimiter` в `focusQuality` (`apps/web/fingers/heys_fingers_engine_router_v1.js:314-324`), builder читает `focusQuality` при подборе (`apps/web/fingers/heys_fingers_session_builder_v1.js:614-618`, `649-655`). | Сохранённый лимитер реально меняет приоритет quality в живой mix-сборке.                                                   |
| 1.6 Readiness / S2          | PARTIAL     | MixCard передаёт только cooldown-readiness (`apps/web/fingers/heys_fingers_session_ui_v1.js:1283-1291`); Today/launch отдельно считают readiness (`apps/web/fingers/heys_fingers_session_ui_v1.js:430-438`, `4921-4928`); builder запускает S2 только если есть `o.history` (`apps/web/fingers/heys_fingers_session_builder_v1.js:1001-1008`).                                  | Bucket клампится, но per-tissue 48/72h freshness не защищает живую генерацию mix-сессии.                                   |
| 1.2 Progression enforcement | GAP         | `session_builder.recommendDay()` зовёт `_buildProgressionConstraints()` (`apps/web/fingers/heys_fingers_session_builder_v1.js:881-884`), но `recordsByQuality` встречается только внутри builder (`apps/web/fingers/heys_fingers_session_builder_v1.js:759-770`) и не прокидывается из UI/router.                                                                               | Plateau/axis policy не ограничивает выбор атомов в живой сессии.                                                           |

> ⚠️ **Скрытые пробелы, которые карта раньше прятала как «✅»:** «module
> exists + покрыт тестами» ≠ «работает в проде». Прогрессия (1.2) и периодизация
> (6) имеют код и тесты, но в live-пути их входные данные (`recordsByQuality`,
> сохранённый план) **не поставляются** — поэтому эффект на реальную сборку
> сессии нулевой. Это первоочередные кандидаты на «доведение до живого», прежде
> чем строить новое.

| Подраздел                      | Метод — осталось | Движок — осталось                                                                                                                                                                                                       | UI — осталось                                                                                                   | Live-факт                                                                                                                                            |
| ------------------------------ | :--------------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.1 Специфичность              |        ✅        | ✅                                                                                                                                                                                                                      | Пояснить в trace/tooltip, почему фингерборд-сила дополняется application-блоком board/wall.                     | `focusQuality` меняет порядок quality в builder; transfer-мостик M3 добавляет board/wall application-блок, если есть фингерборд-сила без применения. |
| 1.2 Прогресс. перегрузка       |        ✅        | Собрать `recordsByQuality` из реальной истории, определить `currentAxes`, прокинуть в `engine_router._enrichOpts`; сейчас `_buildProgressionConstraints()` вызывается, но возвращает `null` без входа.                  | —                                                                                                               | PARTIAL: enforcement-код есть, live-данных нет.                                                                                                      |
| 1.3 Вариативность / плато      |        ✅        | Связать plateau-детектор с тем же `recordsByQuality` из 1.2; FDP/FDS edge-ротация и `variantSeed` уже работают.                                                                                                         | ✅                                                                                                              | PARTIAL: ротация и reroll живые, plateau-switch ждёт live-series.                                                                                    |
| 1.4 Лимитер                    |        ✅        | ✅                                                                                                                                                                                                                      | ✅                                                                                                              | PASS: сохранённая тест-батарея читается роутером, `leadingLimiter` становится `focusQuality` и влияет на подбор.                                     |
| 1.5 Техника / железо           |        ✅        | ✅                                                                                                                                                                                                                      | Показать баланс техника/железо в reason/trace: почему добавлен или сохранён skill-блок.                         | `V_skillBalance`/доли и техника как quality работают внутри генератора.                                                                              |
| 1.6 Восстановление             |        ✅        | Прокинуть в `recommendDay` реальную tissue-history и `now`, чтобы `S2_tissueFreshness` работал внутри сборки микса; readiness/cooldown bucket уже клампит интенсивность.                                                | Довести явный ввод readiness/самочувствия и объяснение tissue-freshness.                                        | PARTIAL: Today/launch читают readiness, но generator не получает S2 history.                                                                         |
| 1.7 МЭД (MEV/MAV)              |        ✅        | ✅                                                                                                                                                                                                                      | Показать volume-cap reason: когда MAV срезал объём или MEV объяснил недобор.                                    | MAV режет недельный quality-объём, MEV объясняет недобор без форса нагрузки.                                                                         |
| 1.8 Нагрузка / риск            |        ✅        | ✅                                                                                                                                                                                                                      | Показать load/risk reason: FTL-cap, danger-budget, почему сессия стала легче/короче.                            | S4 FTL-cap + danger-budget работают в builder/router.                                                                                                |
| 2. 9 качеств                   |        ✅        | ✅                                                                                                                                                                                                                      | Quality badge/tooltip в упражнениях и trace: какое качество тренируется и почему оно попало в день.             | `quality_catalog` — источник quality/emphasis/energySystem.                                                                                          |
| 3.1 Сила/выносл. по времени    |        ✅        | ✅                                                                                                                                                                                                                      | Tooltip к длительности/энергосистеме: почему этот workSec считается силой/выносливостью.                        | `deriveEnergySystem(workSec)` есть.                                                                                                                  |
| 3.2 Энергосистемы (под-режимы) |        ✅        | ✅                                                                                                                                                                                                                      | Chip `capacity/power/base` и короткий reason для aerobic/anaerobic под-режима.                                  | `energySubMode` capacity/power + aerobic selector по уровню работают.                                                                                |
| 3.3 Ткани / горлышко           |        ✅        | ✅                                                                                                                                                                                                                      | Tissue/danger tooltip у хвата: A2-риск, FDP/FDS lean, freshness-ограничения.                                    | `grips_catalog` danger + S2/S9; S2 live-gap описан в 1.6.                                                                                            |
...
| 6. Периодизация                |        ✅        | Создать live plan через `periodization.buildPlan(...)->savePlan(...)` или унифицировать отдельный UI-`mesocycle` с `periodization_engine`; сейчас `current()` вызывается, но без сохранённого плана обычно даёт `null`. | Визуализация плана/forward-календарь: фазы, deload, retest, focusQuality, volumeMultiplier.                     | PARTIAL/GAP: live socket есть, user-facing source of plan отсутствует.                                                                               |
...
| 8.2 Бенчмарки                  |        ✅        | Заменить 🟠 дефолты `assessment.BENCHMARKS` на 🟢 Berta 2025 Tables S2–S5 и перепроверить пороги level/limiter.                                                                                                         | —                                                                                                               | Числа работают как временная база, но не финальная научная таблица.                                                                                  |
...
| 9.4 Вес / RED-S                |        ✅        | Решить продуктово: оставить advisory или сделать явный hard-gate для RED-S/веса; сейчас это сознательный invariant, не валидатор.                                                                                       | Surface веса/RED-S в UI: аккуратное предупреждение без назначения веса.                                         | PARTIAL by design.                                                                                                                                   |
| 9.5 Реабилитация / red-flags   |        ✅        | ✅                                                                                                                                                                                                                      | ✅                                                                                                              | `S8_painStop` + `S9`; pain-промпт в UI.                                                                                                              |
| 10. Источники                  |        ✅        | ✅                                                                                                                                                                                                                      | Source/info popover для протоколов, бенчмарков и safety-правил.                                                 | `sourceIds` + `bibliography`; расширять по мере добавления/замены чисел.                                                                             |
...
> 2026-06-11.)_ **Контент-бэклог:** фото `/exercises/*.webp` для не-хватовых
> упражнений (болдер-мувы, антагонисты, мобильность) — сейчас эмодзи-fallback в
> превью микса. **Engine-бэклог:** live `recordsByQuality` для progression 1.2,
> live plan-source для periodization 6, `history`+`now` для S2 freshness 1.6,
> числа бенчмарков Berta (8.2).
...
- выбор по формату×лимитеру + тейпер + maintenance. **Ядро написано, live-source
  плана не подключён:** `periodization_engine` строит макро/мезо-план
  (`buildPlan`), `current` резолвит фазу дня, `engine_router` умеет положить
  `current()` в `plannerContext`, а `session_builder` умеет clamp'ить bucket и
  volume. Но обычный UI пока запускает отдельный `Fingers.mesocycle`, а
  `periodization.buildPlan/savePlan` не вызываются в живом пути. Осталось
  унифицировать `mesocycle` и `periodization_engine` или создать plan из UI.
...
`periodization_engine`. **Фаза:** ядро ✅, live wiring 🟡. **Вопросы:** `Q-6-1`
шаблоны ротации nonlinear/DUP; `Q-6-2` единый источник плана (`mesocycle` vs
`periodization_engine`) и UI forward-календарь.
...
UI/limiter live ✅, бенчмарки 🟡. **Вопросы:** `Q-8-1` заменить дефолты на Berta
2025 Tables S2–S5; `Q-8-2` оборудование CF/тензометр — обязательно или
