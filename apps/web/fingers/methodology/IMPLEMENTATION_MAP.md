...
видимость (**UI**). Ячейка: `✅` — сделано, ничего не осталось; `—` — оси нет
(нет user-surface); иначе — **конкретно что доделать**. Детализация по
подразделам — в таблице ниже.
...
| 5. Протоколы         |        ✅        | ✅                                                                                                                                                                                                                                                                                                                                                                                                                    | ✅                                                                                                          |
| 6. Периодизация      |        ✅        | 🟡 **методологический `periodization_engine` инертен**: `buildPlan`/`savePlan` без live-вызова, плана нет → `periodization.current()`=null → фазный clamp (ceiling/volume/focus, в т.ч. мой `selectModel` 6.4) **не активируется**. Параллельный `mesocycle` клампит только Today-бакет. Связать создание плана (UI buildPlan→savePlan) **или** унифицировать `mesocycle`↔`periodization_engine` как единый источник | **6.x**: визуализация плана мезоцикла / forward-календарь (фазы, deload, retest)                            |
| 7. Уровни            |        ✅        | ✅                                                                                                                                                                                                                                                                                                                                                                                                                    | ✅                                                                                                          |
...
**Итог (после глубокого live-wiring аудита 2026-06-11):** наука (Метод) написана
и проаудирована везде — научных пробелов нет. **Движок — пробелов больше, чем
показывала карта раньше:** (а) **прогрессия 1.2 не активна** — логика
`_progressionAllowsAtom` есть, но источник `recordsByQuality` не подаётся в
live-opts, constraints всегда null; (б) **периодизация 6 инертна** — план
никогда не создаётся (`buildPlan`/`savePlan` без вызова), `current()`→null,
фазный clamp + `selectModel` 6.4 не работают на живых сессиях (Today частично
спасает отдельный `mesocycle`); (в) readiness 1.6 — клампит бакет по
cooldown-строке, но per-tissue S2 freshness и `now` в `recommendDay` не
заводятся; (г) 8.2 — 🟢-апгрейд чисел бенчмарков; (д) 9.4 вес/RED-S сознательно
advisory. **Что РЕАЛЬНО работает на живом пути:** выбор focusQuality по лимитёру
из тест-батареи (§1.4/§8 — авто-читается роутером ✅), потолок бакета по
readiness/cooldown, S4 FTL-кап, MAV-обрезка, aerobic-под-режимы, FDP/FDS
ротация, transfer-мостик M3, variantSeed-reroll. **UI:** визуализация мезоцикла
(6.x), ввод readiness (1.6), surface RED-S (9.4), контент-фото не-хватовых
упражнений (4).
...
| 1.2 Прогресс. перегрузка       |   ✅   |   🟡   |  —  | `S4` FTL-trim — ✅ активен. Прогрессия осей: логика есть (`_progressionAllowsAtom` + `_buildProgressionConstraints`), но **не активна в проде** — `recordsByQuality` не подаётся в live-opts (нет ни в mixOpts, ни в `engine_router._enrichOpts`) → `_buildProgressionConstraints` всегда `null` → cap оси не применяется. **Осталось:** собрать `recordsByQuality` из `Fingers.records` и прокинуть в enrich                                                                                                                                                                                      |
| 1.3 Вариативность / плато      |   ✅   |   ✅   | ✅  | `detectPlateau`+`nextAxis` enforced; FDP/FDS edge-ротация ✅ (`edge_history` кросс-сессионный lean-лог → `session_builder` чередует FDP/FDS хваты по под-нагруженному); **`variantSeed` reroll микса** — циклический сдвиг среди равноценных атомов (та же цель/контекст/safety; seed=0 каноничен), UI-кнопка 🔄                                                                                                                                                                                                                                                                                   |
| 1.4 Лимитер                    |   ✅   |   ✅   |  —  | `assessment` scoreLimiters §3.2 → `leadingLimiter` → фокус мезо                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 1.5 Техника / железо           |   ✅   |   ✅   |  —  | `V_skillBalance` + доли §3.8                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 1.6 Восстановление             |   ✅   |   🟡   | 🟡  | Потолок бакета по cooldown/readiness-строке — ✅. Но **per-tissue S2 freshness и `now` не подаются** в `recommendDay` (mixOpts/enrich их не несут) → `S2_tissueFreshness` по реальной истории тканей в сборке микса не оценивается (зовётся отдельно в Today-баннере и launch-gate, но не прокидывается в генератор). **Осталось:** прокинуть `now` + per-tissue историю в live-opts. UI ввода readiness — частично                                                                                                                                                                                |
...
| 4. Каталог A–I                 |   ✅   |   ✅   | ✅  | `block_catalog` 36×9; **все 6 doseShape рендерятся** (Hang/Reps/Continuous/Attempts/Circuit/Process); превью набора в карточке микса (`MixExerciseList`) с мини-фото. **Пробел контента (не кода):** реальные фото `/exercises/*.webp` есть только для хватов пальцев (halfcrimp/openhand4/fullcrimp/…); болдер-мувы, антагонисты, мобильность фото не имеют → fallback на эмодзи-категорию. Нужны новые ассеты — см. UI-бэклог                                                                                                                                                                    |
| 5.x Протоколы                  |   ✅   |   ✅   | ✅  | атомы доза/loadModel/doseConfidence; рендер через 6 runner'ов                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 6. Периодизация                |   ✅   |   🟡   | ⬜  | `periodization_engine` написан: модели 6.1-6.3/6.5/6.6, фазы clamp интенсивности+объёма+фокуса, `selectModel` 6.4 — **но инертен в проде**: `buildPlan`/`savePlan` без live-вызова → плана нет → `engine_router` зовёт `periodization.current()`=null → `plannerContext` не ставится → clamp/selectModel на живых сессиях не работают. Today-бакет частично клампит **отдельный** `mesocycle` (свой storage+UI). **Осталось (движок):** создать план live (UI buildPlan→savePlan) ИЛИ унифицировать `mesocycle`↔`periodization_engine`. **Осталось (UI):** визуализация плана / forward-календарь |
...
> превью микса. **Engine-бэклог:** числа бенчмарков Berta (8.2). _Движковый
