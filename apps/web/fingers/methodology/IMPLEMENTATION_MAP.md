...
- **1.5 верхний кэп «железо не вытесняет лазание»** ✅ 2026-06-08 → валидатор
  `V_skillBalance`: warn, если доля железа > уровневой (§3.8). Был только нижний
  гард «не одно железо». Реализован и подключён в `runAll` (advisory warn);
  commit `3e8a23dd`.
...
- **6.3 инвариант «сила→ёмкость→аэробика» для ВСЕХ моделей** ✅ 2026-06-08 →
  валидатор `V_energySystemSequence`: ёмкостный мезо не раньше силовой базы —
  linear/nonlinear/DUP, не только DUP. Реализован и подключён в `runAll`
  (advisory warn по primaryEnergySystem недель); commit `3e8a23dd`.
