...
**Две оси доказательности.** Комментарий `# 🟢/🟡/🟠` в конце строки `sourceIds`
— это **`doseConfidence`** (A/B/C): уверенность в _самих числах_ протокола. Сила
источника — отдельно, в [`BIBLIOGRAPHY.md`](BIBLIOGRAPHY.md). Протокол может
ссылаться на 🟢 A работу, но иметь дозу `doseConfidence: B` (экстраполяция).
