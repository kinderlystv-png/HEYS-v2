...
; (function (global) {
    ...
    function buildWeekReport({ dateStr, lsGet, profile, pIndex, endDateStr, filterEmptyDays = false }) {
        ...
        // чтобы месяц мог собираться по дням, не заводя пятую копию правила.
        const countedDates = new Set(visibleDays.map((d) => d.dateStr));
        days.forEach((d) => { d.isCounted = countedDates.has(d.dateStr); });
    ...
    function NormCorrectionCard({ card, onDecide }) {
        ...
        return h('div', {
            ...
            copy.evidenceNote
                ? h('div', { className: 'weekly-wrap-correction__evidence-note' }, copy.evidenceNote)
                : null,
