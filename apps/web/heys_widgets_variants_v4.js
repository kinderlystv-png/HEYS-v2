...
(function (global) {
  ...
  // восьми часов до отбоя. Ответ про кофе есть с 30 августа, но приходит он во
  // ВЧЕРАШНЮЮ запись (строка «кофе спрашивается за вчера»): у сегодняшнего дня
  // кофеин до конца суток честно говорит «нет данных» и в счётчик не входит.
  // Экранное время не заводим — самоотчёт, а чек-ин мы сознательно укорачивали
  // (решение владельца 30 августа).
  ...
  function buildSleepReadyBreakdown(title) {
    ...
    // «Закрыто в среднем N из M»: среднее обязано считаться по ТЕМ ЖЕ пунктам,
    // что стоят в знаменателе. Прежде среднее брало у каждого дня его
    // собственный набор, а знаменатель — сегодняшний: во вчерашних днях кофеин
    // отвечен и пунктов четыре, а сегодня его ответ ещё не пришёл и пунктов
    // три. Достижимой становилась строка «Закрыто в среднем 3,6 из 3» — число,
    // которое человек прочитать не может.
    //
    // Поэтому берём только сегодняшние известные пункты и только те дни, где
    // известны все они: день с половиной набора занизил бы среднее, а другого
    // знаменателя у строки нет.
    const todayKeys = BD_SLEEPREADY_FACTORS
      .filter((f) => today[f.key] != null)
      .map((f) => f.key);
    const closedPerDay = days
      .filter((r) => todayKeys.every((key) => r.state[key] != null))
      .map((r) => todayKeys.filter((key) => r.state[key]).length);
