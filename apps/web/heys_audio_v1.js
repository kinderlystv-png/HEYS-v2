...
(function (global) {
  ...
  // Каноническое имя — `adviceSoundEnabled`; `soundEnabled` живёт только для
  // совместимости с теми, кто трогал одну галочку профиля до 24.08, и потому
  // читается запасным. Отсюда правило для всякого, кто добавит ещё один вход в
  // эту настройку: **писать оба имени сразу**. Контрол, пишущий только
  // `soundEnabled`, после первого же нажатия соседа перестанет влиять на звук и
  // при этом продолжит показывать своё положение — человек увидит
  // «выключено», а совет прозвучит. Ровно это и случилось с галочкой профиля
  // (починено 31.08). Входов сейчас три: тумблер листа «Советы»
  // (day/_advice.js), «Звук совета» в настройках (heys_app_shell_v1.js) и
  // галочка профиля (heys_user_tab_impl_v1.js); все три держит смоук
  // __tests__/advice-sound-two-controls.test.js.
