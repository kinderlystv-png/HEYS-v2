...
export default defineConfig({
  ...
  test: {
    ...
    pool: 'forks',
    // Потолок форков. Без него vitest берёт число ядер (здесь 16) и на полном
    // прогоне роняет сам себя: воркер зовёт onTaskUpdate, главный поток занят
    // приёмом отчётов от полутора десятков форков разом и не отвечает в срок —
    // прогон падает с «[vitest-worker]: Timeout calling onTaskUpdate» при ВСЕХ
    // зелёных тестах. Это блокирует push: pre-push-vitest-cache берёт код
    // возврата vitest, а он ненулевой при unhandled error, даже когда
    // «1443 passed, 0 failed».
    // Замер 6 сентября: 16 форков — ошибка воспроизводится в четырёх прогонах
