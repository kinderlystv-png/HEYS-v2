...
- 56 console.log требуют cleanup 🟡

**Цель:**
- Извлечь ~20,000 строк (85%)
- Оставить 3,000-5,000 строк в main file
- Cleanup console.log до ≤10 (из текущих 56)
...
## 🧪 Стратегия Тестирования (Обязательно!)

### Smoke Tests (После каждой фазы)
**Цель:** Быстрая проверка, что UI не сломан (2-3 минуты)
...
### Regression Tests для Phase 4 (Meal Scoring)
**Цель:** Гарантировать идентичность бизнес-логики до и после рефакторинга

**Перед извлечением (baseline):**
```javascript
// Создать snapshot текущих результатов
const testMeals = [
...
**После извлечения (validation):**
- [ ] Запустить те же тесты
- [ ] Сравнить результаты с baseline
- [ ] Все значения должны совпадать на 100%

### Parallel Execution Strategy (Фазы 2.1-2.3)
**Возможность:** Фазы 2.1, 2.2, 2.3 независимы и могут выполняться параллельно

**Рекомендация:** 
- ✅ Безопасно делать параллельно, если работают разные агенты
- ⚠️ Если один агент — выполнять последовательно для упрощения отладки
...
#### 2.4 MealOptimizerSection (178 строк) — ⚠️ ПРОПУСТИТЬ
- [ ] ⚠️ **РЕКОМЕНДАЦИЯ: НЕ ИЗВЛЕКАТЬ** — оставить в main
- [ ] Причина: слишком специфичен для DayTab, минимальная выгода
...
**Рекомендация:** ✅ **Пропустить эту фазу, перейти сразу к Фазе 3**
