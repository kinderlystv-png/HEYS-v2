...
- **Технические cookies** — необходимы для работы приложения, сессий и
  аутентификации. Не отключаются.
- **Аналитические cookies** — Яндекс.Метрика (счётчик посещений, clickmap,
  переходы по ссылкам). Собирает обезличенную статистику использования сайта,
  данные передаются в ООО «Яндекс» (Россия). Запись действий пользователя
  (Webvisor) отключена.

10.2. Мы НЕ используем cookies сторонних рекламных сетей и пикселей для
таргетированной рекламы.

10.3. Вы можете отключить cookies в настройках браузера или использовать
блокировщики трекеров (расширения для браузера). Это может повлиять на работу
части функций сайта.
