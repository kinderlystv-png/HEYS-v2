    ...
    React.useEffect(() => {
      ...
      const finish = (reason, gated) => {
        ...
        if (gated) {
          ...
          console.info('[HEYS.gate] 🚀 Gated render: all data ready, rendering DayTab + cards in one frame', window.__heysClientSwitchT0 ? '[Δt=' + Math.round(performance.now() - window.__heysClientSwitchT0) + 'ms]' : '');
        } else {
          window.__heysGatedRender = false;
        }
        console.info('[HEYS.sceleton] ✅ DayTab unlocked:', reason, window.__heysClientSwitchT0 ? '[Δt=' + Math.round(performance.now() - window.__heysClientSwitchT0) + 'ms]' : '');
        setLoading(false);
      };
      ...
      const onSyncCompleted = (event) => {
        ...
        if (phase === 'full') {
          // Full sync arrived — open gate immediately, render everything at once
          if (gateTimer) clearTimeout(gateTimer);
          finish('full sync completed (gated)', true);
          return;
        }

        // Phase A arrived — start gate timer to wait for full sync
        if (!phaseAReceived) {
          phaseAReceived = true;
          // Check if postboot modules are already loaded
          ...
          }, window.__heysClientSwitchT0 ? '[Δt=' + Math.round(performance.now() - window.__heysClientSwitchT0) + 'ms]' : '');

          // Start gate timer — if full sync doesn't arrive in time, unlock with Phase A data
      ...
      if (!need) {
        finish('sync not needed (cooldown)', true); // data already in localStorage — gated instant
        return;
      }

      // Quick check: sync already completed for this client
      if (cloud._lastClientSync && cloud._lastClientSync.clientId === clientId) {
        finish('sync already completed', true); // data already in localStorage — gated instant
        return;
      }
                          ...
                          .map((c, idx) => {
                            ...
                            return React.createElement(
                              'div',
                              {
                                ...
                                onClick: () => {
                                  window.__heysClientSwitchT0 = performance.now();
                                  console.info('[HEYS.gate] 👤 Выбор клиента [Δt=0ms]', { clientId: c.id, clientName: c.name });
