...
(function (global) {
  ...
  const WIDGET_TYPES = {
    ...
    healthTrend: {
      ...
      // Строки «состав дефолта» и «вид · тренд здоровья»: плитка Главной —
      // «+8 за 14 дней», подпись «Тренд здоровья · 14 дней». Умолчание 7 давало
      // на новой Главной «7 дней» и считало тренд по окну, которого нет в
      // контракте. Сохранённый период у виджета остаётся его собственным.
      settings: {
        periodDays: { type: 'number', default: 14, label: 'Период (дней)', min: 7, max: 30 }
      }
    },
    ...
    // Виджета ещё нет: строка каталога стоит в состоянии «скоро» и обещает
    // человеку, что он появится (контракт home-widgets, «готовится»). Данные
    // уже считаются — норма 14 г на 1000 ккал живёт в советах; плитку рисуем
    // отдельной задачей, тогда флаг снимается.
    fiber: {
      type: 'fiber',
      name: 'Клетчатка',
      category: 'nutrition',
