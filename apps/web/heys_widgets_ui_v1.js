...
(function (global) {
  ...
  const WidgetCard = React.memo(function WidgetCard({
    ...
    const tryEditRemoveTap = useCallback((event) => {
      ...
      if (!start) return;
      editRemoveTapRef.current = null;
      const x = event?.clientX ?? event?.changedTouches?.[0]?.clientX ?? start.x;
    ...
    return React.createElement('div', {
      ...
      onPointerDown: (e) => {
        ...
        if (isEditMode && !resizeDragRef.current?.active) {
          editRemoveTapRef.current = {
  ...
  function HealthTrendVariantBody({ variantId, widget, data, meta = {} }) {
    ...
    if (isShort || variantId === 'compact') {
      ...
      return React.createElement('div', {
        ...
        // 61-я сборка подтвердила это разделение: три живых кадра плитки
        // (рост, падение, мёртвая зона) перешли на полное имя — они отставали
        // от кода, — а кадр «Компакт» в шторке остался с коротким.
