...
; (function (global) {
    ...
    const MealAddProduct = React.memo(function MealAddProduct({
        ...
        const handleOpenModal = React.useCallback(() => {
                    ...
                    requestAnimationFrame(() => {
                        setTimeout(async () => {
                            try {
                                ...
                                const mealQuality = HEYS.mealScoring?.getMealQualityScore?.(currentMeal, null, optimum, pIndex, null);
                                const mealKcalStatus = (() => {
                                    let status = 'good';
                                    if (mealScore?.ok === false)
                                        status = 'bad';
                                    else if ((mealScore?.issues || []).length > 0)
                                        status = 'warn';
                                    if (mealQuality?.score != null) {
                                        if (mealQuality.score < 50)
                                            status = 'bad';
                                        else if (mealQuality.score < 75 && status !== 'bad')
                                            status = 'warn';
                                    }
                                    return status;
                                })();
                                ...
                                // 🆕 Собираем список продуктов в приёме для отображения
                                const mealItems = (currentMeal.items || []).map((item) => {
                                    const product = getProductFromItem(item, pIndex) || { name: item.name || '?' };
                                    const grams = +item.grams || 0;
                                ...
                                if (modalResult) {
                                    const updatedDay = HEYS.Day?.getDay?.() || currentDay;
                                    openAddModal({ day: updatedDay });
                                }
                            } catch (err) {
            ...
            openAddModal();
        }, [mi, date, getLatestDay, getLatestProducts, setDay, multiProductMode]);
            ...
            React.createElement('div', { className: 'row desktop-add-product', style: { justifyContent: 'space-between', alignItems: 'center' } },
                ...
                React.createElement('div', { className: 'aps-open-buttons' },
                    React.createElement(MealAddProduct, {
                        ...
                        buttonIcon: '➕',
                        buttonClassName: 'aps-open-btn--secondary',
                    ...
                    React.createElement(MealAddProduct, {
                        ...
                        buttonIcon: '📝',
                        buttonClassName: 'aps-open-btn--primary',
