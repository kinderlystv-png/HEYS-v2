...
(function (global) {
  ...
  const openProductPortionsEditor = (product) => {
    ...
    HEYS.StepModal.show({
      // Кадры шагов этой зоны рисуют возврат шевроном и словом «Назад» рядом:
      // «вставка строки», «дополнительно», «вредность» и «правка · порции» —
      // все четыре. Один кадр из пяти, «Продукт · порции», слова не даёт;
      // шапка у шагов общая, и разводить её ради одного кадра значит показать
      // человеку две разные кнопки возврата в одном потоке.
      chevronBack: 'labeled',
      steps: [
        {
          id: 'portions',
  ...
  function ProductSearchStep({ data, onChange, context }) {
    ...
    return React.createElement('div', { className: 'aps-search-step aps-v4-flow' },
      ...
      React.createElement('div', { className: 'aps-fixed-header' },
        // Строка «вид · две кнопки над вкладками», переписана 31 августа: над
        // вкладками ОДНА кнопка — «Новый продукт». Её здесь не было вовсе:
        // создать продукт можно было только из пустого поиска или ненайденного
        // штрихкода — то есть человек, у которого поиск что-то нашёл, создать
        // своё уже не мог. «Фото» не ставим, и теперь это позиция самой строки,
        // а не наше отступление: пока приём не собран, снимку нечему
        // принадлежать — второй вход запрещает строка «единственный вход».
