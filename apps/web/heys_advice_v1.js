...
(function() {
  ...
  /**
   * Генерирует ключ кэша из контекста
   * Включает только поля, влияющие на генерацию советов
   */
  function generateCacheKey(ctx) {
    ...
    const keyParts = [
      ...
      Math.round(ctx.dayTot?.kcal || 0),
      Math.round(ctx.dayTot?.prot || 0),
      Math.round(ctx.dayTot?.fiber || 0),
      Math.round(ctx.dayTot?.simple || 0),
      ctx.mealCount || 0,
      // Состояние
      ctx.hour || 0,
      ctx.currentStreak || 0,
      ctx.hasTraining ? 1 : 0,
      // Вода
      Math.round((ctx.day?.waterMl || 0) / 100), // округляем до 100мл
      // Эмоции (влияют на тон)
      ctx.emotionalState || 'normal',
      // Цель
      ctx.goal?.mode || 'maintenance'
    ];
  ...
  function isCacheValid(ctx) {
    if (!adviceCache.result || !adviceCache.key) return false;
    if (Date.now() - adviceCache.timestamp > ADVICE_CACHE_TTL) return false;
    
    const currentKey = generateCacheKey(ctx);
    return currentKey === adviceCache.key;
  }
  
  /**
   * Инвалидирует кэш (вызывать при product_added, water_added и т.д.)
   */
  ...
  function generateAdvices(ctx) {
    ...
    // 🔄 Refeed day: не показываем советы о переедании если перебор разумный
    const isRefeedDay = day?.isRefeedDay || false;
    const REFEED_OK_RATIO = 1.35; // Допустимый ratio для refeed дня
    const isRefeedExcessOk = isRefeedDay && kcalPct > 1.0 && kcalPct <= REFEED_OK_RATIO;
    ...
    // 🔄 Refeed-aware: не ругаем за перебор до 135% в refeed день
    // ─────────────────────────────────────────────────────────
    
    // Критический перебор (с учётом цели и refeed)
    // 🔄 Refeed: не показываем если перебор в допустимом диапазоне
    if (isCriticallyOver(kcalPct, goal) && !isRefeedExcessOk) {
    ...
    // 🔄 Refeed: не показываем если перебор в допустимом диапазоне
    else if (kcalPct > goal.targetRange.max && !isCriticallyOver(kcalPct, goal) && !isRefeedExcessOk) {
      const overPct = Math.round((kcalPct - 1) * 100);
      advices.push({
        id: 'kcal_excess_mild',
        icon: '⚠️',
    ...
    if (hour >= 21 && isInTargetRange(kcalPct, goal)) {
      advices.push({
        ...
        details: '🌟 Вечером в норме — значит день удался! Сон 7-8 часов закрепит результат.',
        type: 'achievement',
        priority: 37,
        category: 'lifestyle',
        triggers: ['tab_open'],
    ...
    if (caloricDebt && caloricDebt.hasDebt) {
      ...
      // ─────────────────────────────────────────────────────────
      // 📈 EXCESS TIPS — Мягкая коррекция перебора (priority: 26-35)
      // 🔬 Herman & Polivy 1984: Строгие ограничения → срывы
      // 🔬 Tomiyama 2018: Позитивный подход эффективнее наказаний
      // ─────────────────────────────────────────────────────────
      
      const hasExcess = caloricDebt?.hasExcess;
      const excessKcal = caloricDebt?.excess || 0;
      ...
      // Рекомендация активности (главное при переборе!)
      ...
      // Шаги уже компенсировали! (позитивный)
      ...
      // Мягкая коррекция нормы (информативно)
      if (dailyReduction > 50 && !hasDebt && hour < 14) {
        advices.push({
      ...
      // ─────────────────────────────────────────────────────────
      // 😰 EMOTIONAL RISK — Стресс + долг = высокий риск срыва
      // 🔬 Epel 2001: Кортизол → висцеральный жир + тяга к сладкому
      // ─────────────────────────────────────────────────────────
      ...
      // ─────────────────────────────────────────────────────────
      // 🌅 CIRCADIAN CONTEXT — Утренний vs вечерний дефицит
      // 🔬 Van Cauter 1997: Утренний дефицит менее критичен
      // ─────────────────────────────────────────────────────────
      
      const circadianUrgency = hour < 12 ? 'low' : hour < 18 ? 'medium' : 'high';
      
      // Утро — успокоить (ещё рано переживать)
      if (hasDebt && hour < 12 && rawDebt < 600) {
        advices.push({
      ...
      // Вечер + большой долг — срочность
      if (hasDebt && hour >= 20 && rawDebt > 500) {
        advices.push({
      ...
      // ─────────────────────────────────────────────────────────
      // 🥩 PROTEIN DEBT — Белковый долг (отдельно от калорий!)
      // 🔬 Mettler 2010: Белок критичен для сохранения мышц
      // ─────────────────────────────────────────────────────────
      ...
      // ─────────────────────────────────────────────────────────
      // 🏋️ TRAINING DAY CONTEXT — Тренировочный день
      // 🔬 Aragon 2013: Timing белка критичен вокруг тренировки
      // ─────────────────────────────────────────────────────────
      ...
      if (trainingCtx?.isTrainingDay && trainingCtx.nutritionPriority === 'high') {
        ...
        } else if (hasDebt && trainType === 'cardio') {
          advices.push({
        ...
        // Высокая интенсивность — особый приоритет
      ...
      if (bmiCtx && hasDebt) {
        if (bmiCtx.category === 'underweight') {
          advices.push({
    ...
    // ─────────────────────────────────────────────────────────
    // 🔄 REFEED DAY TIPS — Загрузочный день (priority: 15-25)
    // ─────────────────────────────────────────────────────────
    
    if (HEYS.Refeed && HEYS.Refeed.getRefeedAdvices) {
      const refeedAdvices = HEYS.Refeed.getRefeedAdvices({
    ...
    // ═════════════════════════════════════════════════════════
    // 🎯 MEAL QUALITY SCORE СОВЕТЫ (priority: 78-80)
    // ═════════════════════════════════════════════════════════
    
    // Интеграция с Meal Quality Score из heys_day_v12.js
    ...
    if (lastMealWithItems && window.HEYS?.getMealQualityScore && canShowMealAdvice()) {
      ...
      if (quality?.score !== undefined) {
        // meal_quality_excellent — идеальный приём (score >= 85)
        if (quality.score >= 85) {
          advices.push({
        ...
        // meal_quality_good — хороший приём (score 70-84)
        ...
        // meal_quality_poor — плохой приём (score < 50)
        else if (quality.score < 50) {
          // Формируем конкретные рекомендации по бейджам
          const issues = [];
        ...
        // meal_quality_trend — улучшение качества приёмов (по сравнению с вчерашним днём)
        // Используем HEYS.storage для получения вчерашнего дня
        ...
        if (yesterdayDay?.meals?.length > 0) {
          // Средний score вчера vs сегодня
    ...
    if (waterMlP2 >= 2500) {
      advices.push({
        ...
        details: '💦 2.5+ литра воды улучшает метаболизм, кожу и концентрацию. Ты в топ-10% по гидратации!',
        type: 'achievement',
        priority: 83,
        category: 'hydration',
        triggers: ['tab_open'],
    ...
    if (weightsForTrend.length >= 3) {
      ...
      // ═════════════════════════════════════════════════════════
      // 📊 WEIGHT FORECAST ADVICES — Прогноз веса к цели
      // ═════════════════════════════════════════════════════════
      ...
      if (weightGoal && currentWeight && Math.abs(currentWeight - weightGoal) > 0.5) {
        // Расчёт прогноза на основе текущего тренда
        const deficitPct = Math.abs(prof?.deficitPctTarget || day?.deficitPct || 10);
        
        // Реалистичный темп похудения: ~0.75-1% веса в неделю при умеренном дефиците
        // При дефиците 10%: ~0.7 кг/нед, при 20%: ~1.2 кг/нед
        const rateKgPerWeek = (deficitPct / 100) * currentWeight * 0.8; // ~0.8% веса на каждые 10% дефицита
        const weightDiff = currentWeight - weightGoal; // + = надо худеть, - = надо набирать
        
        // Текущий реальный тренд (если есть данные)
        const actualRatePerWeek = (trendPerWeek !== 0) ? Math.abs(trendPerWeek) : rateKgPerWeek;
        
        // Если идём в нужном направлении
        ...
        if (isGoingRight && actualRatePerWeek > 0.1) {
          ...
          if (weeksToGoal <= 52) { // Меньше года
            ...
            // Очень долго — предложить увеличить дефицит
            advices.push({
        ...
        // Если идём в противоположном направлении
        ...
        if (Math.abs(weightDiff) < 2) {
          advices.push({
            ...
            type: 'achievement',
            priority: PRIORITY.ACHIEVEMENT - 1,
            category: 'weight',
            triggers: ['tab_open'],
            ttl: 6000
          });
      ...
      if (yesterdayWeight > 0) {
        ...
        if (Math.abs(diff) > 1.0) {
          ...
          advices.push({
            ...
            details: '💧 Скачки веса ±1кг за день — это вода и гликоген, не жир. Смотри на тренд за неделю.',
            type: 'insight',
            priority: 23,
            category: 'correlation',
            triggers: ['tab_open'],
      ...
      if (weights.length >= 5) {
        ...
        if (stdDev < 0.5 && !sessionStorage.getItem('heys_weight_stable_tip')) {
          advices.push({
            ...
            details: '⚖️ Стабильный вес 7 дней = ты нашёл свой баланс. Потребление = расход.',
            type: 'achievement',
            priority: 9,
            category: 'achievement',
            triggers: ['tab_open'],
    ...
    if (hour >= 18 && waterPct < 0.5) {
      ...
      if (needed > 0) {
        advices.push({
          ...
          details: '💧 Вечерняя гидратация важна для ночного восстановления. Но за 1 час до сна — ограничь воду.',
          type: 'tip',
          priority: 42,
          category: 'hydration',
          triggers: ['tab_open'],
    ...
    if (hoursSinceWater > 2 && hour >= 10 && hour <= 21 && waterPct < 1.0) {
      ...
      advices.push({
        ...
        icon: '🚰',
        text: waterText,
        details: '💧 Вода ускоряет метаболизм на 30% в течение часа после питья. 2 стакана перед едой снижают аппетит и помогают съесть на 75-90 ккал меньше.',
        type: 'tip',
        priority: 44,
        category: 'hydration',
        triggers: ['tab_open', 'product_added'],
    ...
    if (waterPct >= 1.0 && !sessionStorage.getItem('heys_water_done')) {
      advices.push({
        ...
        details: '🏆 Норма воды выполнена! Это улучшает метаболизм, кожу и концентрацию.',
        type: 'achievement',
        priority: 6,
        category: 'hydration',
        triggers: ['tab_open'],
    ...
    if (activeWaveHighGI) {
      advices.push({
        ...
        details: '🔖 Инсулин ещё высокий от предыдущей еды. Сладкое сейчас уйдёт в жир. Дождись окна липолиза.',
        type: 'warning',
    ...
    if (lastMealTime) {
      ...
      if (lastH >= 18 && lastH <= 20 && hour >= 21) {
        advices.push({
          ...
          details: '🌙 Ужин за 2-3 часа до сна — золотой стандарт. Пищеварение не мешает засыпанию, а ночной липолиз запускается вовремя.',
          type: 'achievement',
          priority: 43,
          category: 'timing',
          triggers: ['tab_open'],
    ...
    if (mealCount >= 2) {
      ...
      if (categories.present.has('vegetables') && (categories.counts.get('vegetables') || 0) >= 3) {
        advices.push({
          ...
          details: '🌈 3+ порции овощей в день снижают риск хронических заболеваний на 25%.',
          type: 'achievement',
          priority: 65,
          category: 'achievement',
          triggers: ['product_added'],
    ...
    if (proteinChain?.ready && proteinPct < 0.8) {
      advices.push({
        ...
        details: '🥩 Для мышц: 1.6-2г белка/кг веса. Распредели на 3-4 приёма.',
        type: 'tip',
        priority: 40,
        category: 'nutrition',
        triggers: ['tab_open'],
    ...
    if (fiberChain?.ready && fiberPct < 0.5) {
      advices.push({
        ...
        details: '🌿 25-30г клетчатки/день — идеал. Яблоко = 4г, овсянка = 10г.',
        type: 'tip',
        priority: 41,
        category: 'nutrition',
        triggers: ['tab_open'],
