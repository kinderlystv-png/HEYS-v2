...
(function() {
  ...
  function generateAdvices(ctx) {
    ...
    // === REFEED DAY REMINDER ===
    // Рекомендация отметить день как refeed если система рекомендует, но не отмечено
    if (caloricDebt && caloricDebt.needsRefeed && day.isRefeedDay !== true && hour >= 8 && !sessionStorage.getItem('heys_refeed_reminder')) {
      advices.push({
        id: 'refeed_day_reminder',
        icon: '🍽️',
        text: 'Система рекомендует загрузочный день',
        details: `💰 Накопился долг ${caloricDebt.debt || 0} ккал за последние дни. Отметь сегодня как загрузочный день в утреннем чек-ине, чтобы:\n\n✅ Восстановить метаболизм\n✅ Поднять лептин\n✅ Избежать плато в весе\n\nСистема увеличит норму на ${caloricDebt.refeedBoost || 0} ккал сегодня.`,
        type: 'tip',
        priority: 28,
        category: 'nutrition',
        triggers: ['tab_open'],
        ttl: 8000,
        action: '🍽️ Отметить день',
        onAction: () => {
          // Открываем утренний чек-ин на шаге refeedDay
          if (window.HEYS?.showCheckin?.morning) {
            window.HEYS.showCheckin.morning(() => {
              console.log('[Advice] Refeed day marked via reminder');
            });
          }
        },
        onShow: () => { try { sessionStorage.setItem('heys_refeed_reminder', '1'); } catch(e) {} }
      });
    }
