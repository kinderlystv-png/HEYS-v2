...
; (function (global) {
  ...
  * @see normalizeName
  ...
  // Pure stamping helpers живут в HEYS.sync (heys_sync_merge_v1.js) — UMD-обёртка
  // обеспечивает доступ через window.HEYS.sync в браузере и через require() для тестов.
  // Локальный fallback на случай если sync-модуль ещё не загружен (boot race).
  function getStampDayv2ChangedEntities() {
    const fromSync = global.HEYS && global.HEYS.sync && global.HEYS.sync.stampDayv2ChangedEntities;
    return typeof fromSync === 'function' ? fromSync : null;
  }
        ...
        if (!usedPrefetch) {
          ...
          const fetchPage = async (offset) => {
            ...
            const isOversizedPage = (error) => {
              ...
              // 2026-08-22: маскировку чиним у источника — в спеке шлюза появился
              // блок x-yc-apigateway.cors, и на своих ошибках шлюз теперь отдаёт
              // тот же Origin и credentials, что на успехе (проверено на временном
              // шлюзе: 502 приходит с access-control-allow-origin вместо '*').
              // Эту ветку оставляем сознательно: она страхует старые вкладки,
              // открытые до выкатки, и любой промежуточный прокси, который так и
              // не научился отдавать CORS на ошибке. Цена ошибочного срабатывания —
              // несколько запросов поменьше вместо одного, а не потеря данных.
