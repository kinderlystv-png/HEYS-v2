...
(function (global) {
  ...
  function DragValueSlider({
    ...
    // Заливка шкалы — роль набора, а не число. Строка «вид · выбранная оценка
    // сна» называет --gr2, это --v4-ok-fill: в песочном наборе он и есть
    // #7a8a5e, а в тёмных #8a9a6a. Литерал брал светлое значение во всех
    // наборах — шкала одна не следовала палитре, и ни один гейт этого не видел,
    // потому что значение совпадало ровно на той палитре, где его смотрели.
    const fillColor = fill === 'act'
      ? V4_ACCENT_FILL
      : 'var(--v4-ok-fill, #7a8a5e)';
        ...
        React.createElement('div', {
          ...
          estimateSource === 'estimated_avg' || estimateSource === 'average3'
            ? [
              ...
              // старого к свежему (13 → 14 → 15 августа). Порядок задаём
              // датой, а не разворотом собранного списка: разворот держался
              // на том, каким ходом их собрали, и показывал 15 → 14 → 13.
              ...[...estimateSamples]
                .sort((a, b) => String(a.date).localeCompare(String(b.date)))
                .map((sample) => React.createElement('div', {
                key: sample.date,
                ...
                // --v4-ink-2, а не литерал: в тёмных наборах она считается
                // от чернил набора.
                style: { display: 'flex', justifyContent: 'space-between', marginTop: 11, fontSize: 12, fontWeight: 600, lineHeight: 1, color: 'var(--v4-ink-2, rgba(0,0,0,.56))' }
              },
