...
(function (global) {
    ...
    // Разряд тысяч — тем же пробелом, что во всём остальном продукте. Здесь стоял
    // УЗКИЙ неразрывный (U+202F) — единственное такое место на весь код, во всех
    // прочих тридцати восьми числа идут через toLocaleString('ru-RU') и получают
    // обычный неразрывный. Узкий заметно теснее: в паре с кадром «1 610» читалось
    // как «1610», то есть как число без разряда вовсе.
    const fmtKcal = (n) => Math.round(Number(n) || 0).toLocaleString('ru-RU');
