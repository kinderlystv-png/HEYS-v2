/*
      ┌───────────────────────────────────────────────────────────────────────────────┐
      │ 🗺️ НАВИГАЦИОННАЯ КАРТА index.html (1170 строк)                                │
      ├───────────────────────────────────────────────────────────────────────────────┤
      │ 📋 СТРУКТУРА ФАЙЛА:                                                            │
      │                                                                                │
      │ 1️⃣  ИНИЦИАЛИЗАЦИЯ (строки 82-139)                                             │
      │    ├── Service Worker setup (отключен)                                        │
      │    ├── initializeApp() - проверка загрузки React и HEYS компонентов          │
      │    └── Supabase config initialization                                         │
      │                                                                                │
      │ 2️⃣  ОБЛАЧНЫЕ ОБЁРТКИ КОМПОНЕНТОВ (строки 140-290)                             │
      │    ├── 📅 DayTabWithCloudSync (142-181) - статистика дня + синхронизация     │
      │    ├── 🍽️ RationTabWithCloudSync (185-227) - рацион + синхронизация         │
      │    └── 👤 UserTabWithCloudSync (230-266) - профиль + синхронизация           │
      │                                                                                │
      │ 3️⃣  АНАЛИТИКА (строки 269-480)                                                │
      │    └── 📊 AnalyticsTab (269-450) - производительность, auto-refresh          │
      │                                                                                │
      │ 4️⃣  ГЛАВНОЕ ПРИЛОЖЕНИЕ (строки 482-1140)                                      │
      │    ├── 🚀 App() - корневой компонент                                          │
      │    ├── State management (clients, products, sync)                            │
      │    ├── Supabase integration (fetchClients, addClient, etc.)                  │
      │    ├── Modal окно выбора клиента (если !clientId)                            │
      │    ├── Header с авторизацией и переключением клиентов                        │
      │    ├── Tab navigation (день/рацион/отчёты/профиль/аналитика)                 │
      │    └── Render корневого компонента                                            │
      │                                                                                │
      ├───────────────────────────────────────────────────────────────────────────────┤
      │ 🔍 БЫСТРЫЙ ПОИСК:                                                             │
      │   • Компонент: Ctrl+F "КОМПОНЕНТ: " + название                               │
      │   • Функция: Ctrl+F "function " + название                                   │
      │   • State: Ctrl+F "useState" или "useEffect"                                 │
      └───────────────────────────────────────────────────────────────────────────────┘
      */
