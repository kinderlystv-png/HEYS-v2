...
## Норма воды расходилась между экранами: два читателя профиля искали клиента в разных местах (2026-09-12)

Карточка воды на «Питании» и плитка «Вода» на Главной показывали разную дневную
норму на одних и тех же данных.

**Что видно.** На вкладке «Питание» норма считалась от настоящего веса, плитка
Главной уезжала на запасные 70 кг. У человека 80 кг это 300 мл расхождения на
двух экранах одного дня.

**Гипотеза, которая не подтвердилась.** Первой версией было «обёртка
`HEYS.utils.lsGet` скоупит ключ дважды: `nsKey` дописывает clientId, потом его
дописывает `HEYS.store`». Проверено чтением обеих реализаций и замером на живом
дереве: защита стоит в обоих местах (`nsKey` и `scoped()` возвращают ключ как
есть, если он уже содержит clientId), ключа `heys_<cid>_heys_<cid>_...` в
хранилище не появляется, а в залогиненной сессии все четыре способа чтения
профиля дают одно значение. Двойного скоупа нет.

**Причина.** Два резолвера ключа брали клиента из разных мест. `nsKey`
(`heys_core_v12.js`), если `HEYS.currentClientId` пуст, подхватывает выбор из
`heys_client_current`; `scoped()` (`heys_storage_layer_v1.js`) этого запасного
пути не имел и в том же окне считал, что клиента нет, — то есть читал и писал
неклиентский слот. Окно реальное: между загрузкой страницы и auth-init глобал
ещё не проставлен, а выбор клиента в хранилище уже лежит. Карточка «Питания»
читает профиль через обёртку, плитка Главной — через `HEYS.store`
(`widgets/widget_data.js` → `readStoredValue`), и в этом окне они читали разные
слоты: `heys_<cid>_profile` против `heys_profile`.

Рядом, в том же пути чтения: запасная ветка `lsGet` разбирала значение голым
`JSON.parse`. Значения длиннее 384 символов Store кладёт сжатыми, с префиксом
`¤Z¤`; `JSON.parse` на них падает, исключение уходит в `catch`, наружу
возвращается значение по умолчанию. Та же ошибка в проекте всплывала уже не раз
— ради неё и заведён `Store.readSafe`.

**Чем это грозит.** Оба дефекта молчат: ошибки нет, есть значение по умолчанию.
Наружу выходит не «не загрузилось», а правдоподобное неверное число, и отличить
его от настоящего можно только сравнив два экрана.

**Что сделано.**

1. `scoped()` получил тот же запасной путь, что и `nsKey`: если
   `HEYS.currentClientId` пуст, клиент берётся из `heys_client_current`. Разбор
   выбора кэшируется по сырой строке — `scoped()` горячий.
2. Запасная ветка `lsGet` разбирает значение через `parseStoredRaw`: сжатую
   строку отдаёт в `HEYS.store.decompress`, остальное — в `JSON.parse`.
3. `apps/web/__tests__/ls-client-scope-roundtrip.test.js` гоняет настоящие
   `heys_core_v12.js` и `heys_storage_layer_v1.js`, а не копию логики: держит
   однократный скоуп, round-trip по короткой и по уже скоупленной форме ключа,
   сжатое значение, согласие обоих читателей до авторизации и то, что глобальные
   ключи остаются вне клиентского скоупа. Обе новые проверки проверены на
   красноту: с откатанной правкой каждая падает.

**Чего НЕ меняли.** `ns()` оставлен как был, поэтому выгрузка в облако
(`saveClientKey`) в окне загрузки по-прежнему не заводится: правка чинит выбор
LS-слота, а не маршрут записи в облако.

---

## `tasks_patch` молча снёс семь задач: повторяющийся якорь читался как диапазон (2026-09-08)

Агент правил строку `окно:` у фазы 1 в `projects/heys.md` и вызвал `tasks_patch`
с якорем `- окно: 2026-09-04..2026-09-08`. Такая строка стоит в файле **дважды**
— у фазы 1 и у фазы 2, далеко ниже.

**Что видно.** Инструмент взял первое вхождение началом блока, второе — концом,
и заменил всё между ними: 18 строк, включая саму задачу «Привести код к макету
v4» и шесть соседних задач HEYS. Ошибки не было — ответ говорил «Заменил блок».
Пропажу заметили случайно, `grep`'ом по названию задачи при следующем действии;
текст восстановили из `git show HEAD:projects/heys.md`, сверка сошлась.

**Причина.** В `findAnchorLine` (`yandex-cloud-functions/heys-mcp/lib/tasks.js`)
проверка на неоднозначность стояла только у ветки поиска по подстроке. Точное
совпадение бралось первым попавшимся:

```js
const exactRel = slice.findIndex((line) => line.trim() === needle);
if (exactRel !== -1) return fromIndex + exactRel; // без проверки уникальности
```

Комментарий над функцией при этом обещал обратное — «несколько совпадений —
ошибка с кандидатами, а не молчаливый первый hit». Обещание относилось к
подстроке и на точную строку не распространялось. Порядок опасности оказался
обратным ожидаемому: подстрока звучит как догадка, и её проверяли с самого
начала; точная строка звучит как адрес — и потому её не проверяли вовсе.

**Чем это грозит.** Единственная операция сервера, которая удаляет чужой текст,
делала это без сигнала. `rev` из `tasks_read` защищает от гонки с другой
сессией, но не от того, что твоя собственная правка снесла соседей: ревизия
совпала, запись прошла, ответ успешный. Обнаружение зависело от того, посмотрит
ли кто-нибудь в `git diff` после успешного ответа.

**Что сделано.**

1. Повторяющееся **точное** совпадение якоря — теперь `anchor_ambiguous` с
   номерами строк-кандидатов, как у подстроки. Fail-closed: отказ дороже одного
   лишнего хода, молчаливое удаление — дороже всего. Флага «разрешить
   неуникальный якорь» намеренно нет: его ставят один раз в спешке и оставляют
   навсегда.
2. `patchBlockWithStats` возвращает размер правки, и `tasks_patch` называет его
   в ответе: «Снято строк N (с M), вписано K». Чистая дельта строк здесь не
   годится — замена трёх строк тремя даёт ноль.
3. Копия модуля в `heys-api-rpc/lib/heys_tasks_kv.cjs` синхронизирована: она
   держится руками, и расхождение поймал существующий тест.

**Чего НЕ меняли.** `findSectionLine` тоже берёт первое совпадение при двух
одинаковых заголовках, но она только вставляет строку, а не удаляет диапазон:
результат виден в файле и обратим. Класс «молчаливое удаление диапазона» есть
только у `patchBlock`.

**Проверки:** `heys-mcp/__tests__/tasks.test.cjs` — «patchBlock отказывает,
когда ТОЧНАЯ строка-якорь встречается дважды» (воспроизводит инцидент дословно и
требует номера обоих кандидатов в тексте отказа) и «patchBlock называет размер
правки». Полный прогон `npm test` в `heys-mcp`: 1496 тестов, 0 падений.

**Паттерн.** Проверка, написанная для одной ветки условия, читается как свойство
всей функции — и её комментарий начинает лгать раньше, чем код ломается. Когда
две ветки дают один результат, инвариант ставится на выходе, а не в каждой ветке
по отдельности.

---

## Восемь юридических проверок не выполнялись ни одним раннером (2026-08-31)

Найдено при починке изоляции тестов: единственный файл, падавший даже с
исправленным конфигом, оказался вообще не vitest'овским.

**Что видно.** `apps/web/__tests__/consent-proof-v2.test.js` написан на
`node:test` и лежит среди vitest-тестов. Под конфигом приложения он проходил как
«no tests» — `passWithNoTests: true` пропускает файл, в котором раннер не нашёл
ни одного своего теста. Под корневым конфигом падал «No test suite found», и это
падение тонуло среди 896 других, которые давал `singleFork: true`. Третьего
места нет: `pnpm test:node` берёт только `scripts/**/*.test.mjs`.

Восемь проверок внутри — реестр согласий, forward-only активации версий
пользовательского соглашения, серверное владение хешем документа и временем
принятия, отказ health-гейта при отсутствии хеша — не выполнялись нигде с
момента появления файла.

**Чем это грозит.** Молча не выполняющийся тест хуже отсутствующего: он создаёт
видимость покрытия. Подтверждение пришло сразу — первый же запуск после перевода
на vitest покраснел. Реестр согласий был в порядке, устарел сам тест: он собирал
миграции из списка имён, и в список не дописали активацию push 1.2 (15 августа)
и кураторского push 1.1 (16 августа). Полгода зона выглядела закрытой проверкой,
которой не было.

**Что исправлено.** Раннер заменён на vitest (assert оставлен node'овский — тела
проверок не менялись). Реестр теперь собирается из всех миграций, которые
трогают `legal_consent_registry`, а не из списка имён: новая активация попадает
в проверку сама.

**Паттерн.** Тест, который не падает, не доказывает ничего, пока не показано,
что он вообще запускается. Два способа промолчать в этом репозитории:
`passWithNoTests` для файла без тестов раннера и чужой раннер в папке vitest'а.
Оба закрыты гейтом `vitest-isolation-contract`: в папке нет файлов на
`node:test`, и каждый файл объявляет хотя бы один тест. Проверять покрытие
глазами по названиям файлов — не проверка.
...
ищет вчера через `lsGet`; калорийный путь сервер собирает из блоба сам.

Калорийный NDTE с 2026-08-23 больше не живой снимок по `hoursSince` / wrap
вокруг полуночи: среднее capped `tdeeBoost` за HEYS-день
(`calculateNDTEDayAverage`). Волна по-прежнему зовёт мгновенный
`calculateNDTE({ hoursSince })`.
