...
## Манифест каталога отстаёт от строк — облачный каталог не приезжает (2026-08-22, открыт)
...
**Что осталось.** Найти путь записи, который обновляет строки без пересчёта
манифеста, и закрыть его. Тихий `if (!assembled.ok) return out;` стоит сделать
громким: несобравшийся каталог — это не «данных нет», а «данные есть, но им не
доверяем», и это разные вещи для пользователя.
