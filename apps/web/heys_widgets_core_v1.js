    ...
    init() {
      ...
      if (saved && Array.isArray(saved) && saved.length > 0) {
        ...
        this._widgets = saved
          ...
          // порядок сжимается тем же алгоритмом (канвас v4, строка 41). Так же
          // уходят типы, помеченные inDevelopment: добавить их нельзя, значит
          // и стоять на экране им нечего — иначе «показать и не дать».
          .filter((w) => {
