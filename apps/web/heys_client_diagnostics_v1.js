...
(function (global) {
  ...
  function ensureStyles() {
    ...
    style.textContent = [
      ...
      '.cd-icon-btn,.cd-control,.cd-copy{min-height:44px;border-radius:11px;border:1px solid #dadae8;background:#fff;color:#343553;font:inherit;cursor:pointer}',
  ...
  function Overview(props) {
    ...
    var load = React.useCallback(async function (append) {
      ...
      try {
        ...
        var next = response && response.data || { summary: {}, sessions: [] };
    ...
    async function copyReport() {
      try { await navigator.clipboard.writeText(overviewReport(data)); HEYS.Toast?.success?.('Отчёт скопирован'); }
      ...
      h('div', { className: 'cdo-list' }, (data.sessions || []).map(function (session) {
        ...
        return h('article', { key: key, className: 'cdo-session cdo-session--' + status[1] },
          ...
          isOpen && h('div', { className: 'cdo-detail' },
            h('div', { className: 'cdo-detail-actions' }, h('button', { type: 'button', className: 'cdo-secondary', onClick: function () { show({ clientId: session.client_id, clientName: session.client_name }); } }, 'Открыть клиента')),
  ...
  function renderSession(session) {
    ...
    if (!timeline.childNodes.length) timeline.appendChild(el('div', 'cd-event', 'Структурированных событий нет'));
  ...
  function show(options) {
    ...
    function draw() {
      ...
      sessions.forEach(function (session) { body.appendChild(renderSession(session)); });
    ...
    async function load() {
      ...
      try {
        ...
        if (response && response.error) throw new Error(response.error.message || response.error);
        state.data = response && response.data ? response.data : { sessions: [], logins: [] };
        copy.disabled = false;
        draw();
      } catch (error) {
    ...
    copy.addEventListener('click', async function () {
      ...
      try {
        await navigator.clipboard.writeText(safeReport(options.clientName, state.data));
  ...
  HEYS.ClientDiagnostics = {
    ...
    close: close,
    Overview: Overview,
    _test: { buildOverviewParams: buildOverviewParams, eventLabel: eventLabel, overviewReport: overviewReport }
  };
