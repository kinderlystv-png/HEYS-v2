...
describe('«Питание» · разбор кадров канваса', () => {
  ...
  it('поля кольца и кривой воды равны числам кадра', () => {
    ...
    // Галочка отмеченного дня: строка кадра «Вода · карточка · Кольцо · рисунок
    // 08» файла nutrition-tab всё ещё просит её 7×7. В продукте её нет —
    // владелец карточки назван 13 сентября, это кадр water-add, и он под кривой
    // не рисует ничего: составная строка владельца это «Вода · 7 дней в среднем
    // 2,1 л › −200 › 1,7 л › из 3,0 · осталось 1,3 › +100 › +200 › +330 ›
    // +500». Подписи дней дублировали ось, галочки — заливку точек на самой
    // кривой. Отстающий кадр дизайнер приводит к владельцу; до этого строка
    // остаётся в контракте, а продукт следует владельцу — названное отступление,
    // UI_V4_FINDINGS.md#water-card-ring-vs-strip-layout.
    expect(ask('08')).toContain('7×7 (viewBox 0 0 24 24)');
    expect(js).not.toContain("width: 7, height: 7, viewBox: '0 0 24 24'");
