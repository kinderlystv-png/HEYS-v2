...
const EXCEPTIONS = new Map([
  ...
  ['Чек-ин · остальное · 11|color', 'роль --v4-ink-2 = 55 %; сверка не разворачивает вложенный var с запасным'],
...
const FORK_FRAMES = [
  // Пакет 3 сентября добавил в кадр «вчерашний день» строки 14–15 («Дописать
  // точно», «зазор 8px») между note (12) и подвалом (13): foot сдвинулся с 12
  // на 13, row3/a/b/later — на 15–18. У «дня из пачки» шапка на строку длиннее
  // (05 — прокрутка), поэтому те же роли там 13/14/16–19, не 12/13/15–18.
...
describe('«Утренний чек-ин» · разбор кадров канваса', () => {
  ...
  it('кнопка выбора набрана по кадру: 700 тоном --v4-ink-2', () => {
    ...
    // Пакет 43: кадр и контракт дают --ink-2 (55 %), не прежний mark-1 62 %.
    expect(pill.color).toBe('var(--v4-ink-2, rgba(0, 0, 0, 0.56))');
  });
  ...
  // Класс сменился вместе с переездом карточки в шаг «Сон»: прежний
  // .mc-rest-coffee-why снят, и проверка сторожила бы мёртвое правило.
  it('строка «зачем спрашиваем» набрана своим кеглем, а не пояснением карточки', () => {
    const why = rules.get('.mc-sleep-coffee-why');
