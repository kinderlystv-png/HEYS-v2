...
describe('«Утренний чек-ин» · разбор кадров канваса', () => {
  ...
  // «Остального»: кнопка выбора. В пары вошли форма, фон и кегль; вес и тон
  // расходятся, и молчать об этом нельзя — исключение свойства из пары внешне
  // неотличимо от совпадения.
  it('кнопка выбора: вес и тон надписи расходятся с кадром', () => {
    const pill = rules.get('.mc-pill--choice');
    // Кадр: 700. Продукт: 600. Полужирная надпись на невыбранной кнопке
    // спорила бы с выбранной — там тот же 700 заливкой акцента.
    expect(pill['font-weight']).toBe('600');
    // Кадр: rgba(var(--ink),.62). Продукт: полный тон чернил.
    expect(pill.color).toMatch(/--v4-sand-ink/);
  });
  ...
  it('осознанные отступления не разрослись', () => {
    expect(EXCEPTIONS.size).toBe(54);
  });
});
