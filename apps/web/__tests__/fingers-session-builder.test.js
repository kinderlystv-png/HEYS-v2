...
describe('sessionBuilder: контракт выхода (Риск 2 ревью)', () => {
  ...
  it('explainability-поля упражнения доезжают до UI списка', () => {
    const s = SB().recommendDay({ equipmentTypes: ['full'], age: 25, level: 'intermediate', readiness: 'max' });
    const finger = s.exercises.find((e) => e.atomId && e.atomId.startsWith('fs_'));
    expect(finger).toBeTruthy();
    expect(finger.quality).toBe('finger_strength');
    expect(finger.tissueLoad).toMatch(/moderate|high|max/);
    expect(finger.progressionAxis).toMatch(/volume|edge|load/);
  });
...
describe('sessionBuilder: explicit safety validators', () => {
  ...
  it('history с свежей high-tissue нагрузкой → S2 фильтрует рискованный atom без обвала сессии', () => {
    ...
    // S2 теперь работает до выбора atom: high/max кандидаты с неизвестной
    // legacy-группой не проходят, но builder подбирает безопасную замену.
    expect(s).not.toBeNull();
    expect(s.exercises.find((e) => e.atomId === 'fs_maxhang_20mm_half')).toBeUndefined();
    expect(s.__safetyTrace.issues.some((i) => i.code === 'S2.fresh_tissue_violation')).toBe(false);
    expect(s.__trace.resolution.tissueFreshness.checked).toBe(true);
    expect(s.__trace.resolution.tissueFreshness.blocked).toBeGreaterThan(0);
  });
...
describe('sessionBuilder: _deriveLevel (ревью #4-#5 (b) MVC mapping)', () => {
  ...
  it('mvcPctBW < 70 → beginner', () => {
    expect(SB()._deriveLevel(20)).toBe('beginner');
    expect(SB()._deriveLevel(69.99)).toBe('beginner');
  });

  it('70 ≤ mvcPctBW < 87 → intermediate', () => {
    expect(SB()._deriveLevel(70)).toBe('intermediate');
    expect(SB()._deriveLevel(80)).toBe('intermediate');
    expect(SB()._deriveLevel(86.99)).toBe('intermediate');
  });

  it('87 ≤ mvcPctBW → advanced (cap)', () => {
    expect(SB()._deriveLevel(87)).toBe('advanced');
    expect(SB()._deriveLevel(90)).toBe('advanced');
  });

  it('ревью #4 cap: даже elite-MVC (≥102) → advanced, не elite', () => {
    expect(SB()._deriveLevel(102)).toBe('advanced');
...
describe('sessionBuilder: plumbing Гейта #1 E2E через роутер (ревью #8)', () => {
  ...
  it('beginner MVC (40% BW) → derive beginner — порог 70% работает', () => {
  ...
  it('intermediate MVC (75% BW) → derive intermediate', () => {
    F().records = { getMVC: () => ({ addedKg: 56.25 }) }; // 56.25/75 = 75%
    F().getBodyWeight = () => ({ kg: 75 });
    const opts = { equipmentTypes: ['full'], age: 25, readiness: 'max' };
    const result = R().recommendDay(opts);
...
describe('sessionBuilder: progression constraints', () => {
  ...
  it('non-MVC quality cap is enforced: power speed waits for stored speed axis', () => {
    ...
    const volumeConstraints = SB()._buildProgressionConstraints({
      recordsByQuality,
      currentAxes: { power: 'volume' }
    });
    const speedConstraints = SB()._buildProgressionConstraints({
      recordsByQuality,
      currentAxes: { power: 'speed' }
    });

    const blocked = SB()._pickAtomForSlot('power', {
      equipmentTypes: ['full'],
      profile: profile(),
      progressionConstraints: volumeConstraints
    });
    const allowed = SB()._pickAtomForSlot('power', {
      equipmentTypes: ['full'],
      profile: profile(),
      progressionConstraints: speedConstraints
    });

    expect(blocked).toBeNull();
    expect(allowed).toBeTruthy();
    expect(allowed.quality).toBe('power');
    expect(SB()._atomProgressionAxis(allowed)).toBe('speed');
  });
});
...
describe('sessionBuilder: age/skin soft modifiers', () => {
  beforeAll(setupOnce);

  it('35+ и flapper skin дают warning + мягко режут heavy finger volume', () => {
    const baseOpts = {
      equipmentTypes: ['full'],
      age: 55,
      level: 'advanced',
    ...
    const normal = SB().recommendDay(Object.assign({}, baseOpts, {
      age: 25,
      profile: Object.assign({}, baseOpts.profile, { age: 25, skinStatus: null })
    }));
    const capped = SB().recommendDay(Object.assign({}, baseOpts, {
      skinStatus: 'flapper',
      profile: Object.assign({}, baseOpts.profile, { skinStatus: 'flapper' })
    }));

    const codes = capped.__safetyTrace.issues.map((i) => i.code);
    expect(codes).toContain('V.age.35plus_modifier');
    expect(codes).toContain('V.skin.volume_cap');
    expect(capped.__trace.resolution.softModifiers.multiplier).toBe(0.5);
    expect(capped.__trace.resolution.softModifiers.enforced).toBe(true);
    expect(capped.__trace.resolution.softModifiers.afterFtl)
      .toBeLessThan(capped.__trace.resolution.softModifiers.beforeFtl);
    expect(capped.__trace.resolution.softModifiers.afterFtl)
      .toBeLessThan(normal.__trace.resolution.softModifiers.afterFtl);
    expect(capped.coachReason).toContain('Объем снижен по возрасту или состоянию кожи.');
  });
});
...
describe('sessionBuilder §1.5: skill/железо balance', () => {
  beforeAll(setupOnce);

  it('technique leadingLimiter добавляет skill-floor через live builder', () => {
    const s = SB().recommendDay({
      equipmentTypes: ['full'],
      age: 25,
      level: 'intermediate',
    ...
    expect(s).not.toBeNull();
    const skill = s.exercises.find((e) => e.__role === 'skill');
    expect(skill).toBeTruthy();
    expect(['technique', 'mental']).toContain(skill.quality);
    expect(s.coachReason).toContain('Сохранён навык');
  });
});
