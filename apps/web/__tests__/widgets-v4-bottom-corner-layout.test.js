// home-widgets.v4.dc.html — фактическое поведение нижних углов сетки.
// Контракт 28 августа противоречив: одна строка запрещает 1×1 в обоих нижних
// углах, другая запрещает вообще подгонять раскладку под кнопки, третья всё ещё
// описывает резерв только слева. Пока владелец не выбрал правило, тесты ниже
// фиксируют существующее поведение продукта, а не объявляют его новым эталоном.
//   • движок не ставит плитку 1×1 в левый нижний угол последнего ряда;
//     в правый 1×1 встаёт, содержимое уходит вбок полем 52 px, как у 2×1;
...
//   • неполный последний ряд сейчас прижимается вправо.
//
...
describe('home-widgets v4 · контракт про нижние углы', () => {
  it('явно держит конфликт трёх строк, пока владелец не выбрал одно правило', () => {
    const bothCorners = contractLine('1×1 в нижнем углу не ставится');
    const noAdjustment = contractLine('углы под кнопками при расстановке');
    const leftOnly = contractLine('что под ней');

    expect(bothCorners).toContain('в двух нижних углах последнего ряда');
    expect(noAdjustment).toContain('подгонять не нужно');
    expect(noAdjustment).toContain('обходить его при расстановке не требуется');
    expect(leftOnly).toContain('в левом нижнем углу');
    expect(leftOnly).toContain('число прижато к правому краю');
  });
  ...
  it('строка «2×1 в углу» оставляет формат в углу и двигает только содержимое', () => {
    ...
    // 115,5, а не 117,5: наш вопрос дизайнеру был посчитан при поле сетки 14,
    // в продукте оно 16 (--widget-spacing-md). Шестнадцатая сборка пересчитала
    // — ошибка была наша, и число в контракте теперь верное.
    expect(line).toContain('115,5×64');
 ...
 *
 * Собираем её руками, чтобы правило углов проверялось на чистом входе: сама
 * запись меты в init теперь идёт поверх актуального состояния хранилища
 * (снятие типов больше не затирает поля сетки), и это проверяет свой тест —
 * `widgets-retired-migration.test.js`, «раскладка без meta».
 */
