...
describe('cold-start spinner mark', () => {
  it('puts the 56 mark in #root instead of a chrome skeleton', () => {
    ...
    // 24 августа — дуга 50 обводкой 2,6 с полным кругом фоном. Пятнадцатая
    // сборка канвасов это записала: app-splash «вид диска в загрузчике» и
    // spinners «вид знака» просят ровно 50 / 2,6 / полный круг .28, так что
    // код и контракт снова совпали. Проверка сторожит возвращённый вид, чтобы
    // следующий проход не «починил» его обратно молча.
  ...
  it('в загрузчике нет буквы знака и нет порога 300 мс', () => {
    // Пятнадцатая сборка: строка «вид диска в загрузчике» больше не просит
    // букву — «Буквы в загрузчике нет вовсе, порога 300 мс нет». Сторож
    // остаётся: следующий проход не должен вернуть её «по контракту».
