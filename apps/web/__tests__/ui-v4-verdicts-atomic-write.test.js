...
// Окно между чтением и записью у обоих писателей. Двадцать пять миллисекунд
// меньше разброса СТАРТА двух процессов node под нагрузкой — загрузка рантайма
// занимает сотни миллисекунд, — поэтому окна не пересекались и потеря не
// воспроизводилась: 6 сентября полный прогон дал «expected 0 to be greater than
// 0» при зелёном одиночном. Тест ДЕМОНСТРИРУЕТ гонку, а демонстрация от тайминга
// детерминированной не станет; но окно, заведомо большее разброса старта, делает
// пересечение практически неизбежным — писатель B читает раньше, чем A успевает
// записать. Итерация стоит около секунды, а цикл прерывается на первой потере.
const RMW_DELAY_MS = 800;
