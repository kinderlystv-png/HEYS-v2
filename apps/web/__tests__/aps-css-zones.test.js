...
describe('Классы потока добавления лежат по зонам', () => {
  ...
  //
  // Разбор регуляркой держится на двух условиях, и оба проверяются отдельным
  // тестом ниже: правила внутри `@media` вырезаются вместе с блоком (иначе
  // отзывчивая правка `.aps-open-icon` читалась бы повтором базового правила),
  // а сам блок вырезается целиком только пока внутри него не больше одного
  // уровня вложенности. Условия сегодня выполнены; когда перестанут — упадёт
  // проверка допущений, а не молча соврёт эта.
  ...
  // Долга больше нет: ноль во всех пяти файлах, потолок пуст.
  //
  // Десять объявлений оболочки я сперва записала сюда потолком, решив, что
  // копии переплетены с чужими вставками и распутывать их — отдельная работа.
  // Это оказалось ошибкой границ: я взяла начало блока по комментарию-заголовку,
  // и «вставки» были началом той же копии. Настоящие границы — 85 строк подряд,
  // совпадающие дословно. Урок тот же, что с дублями в 610: прежде чем объявить
  // работу сложной, проверь границы вторым способом.
  const SHADOW_DEBT = {};

  it.each(FILES)('в %s выполнены допущения разбора', (file) => {
    const css = read(file).replace(/\/\*[\s\S]*?\*\//g, '');
    // Скобки парные — иначе вырезание блоков съест лишнее.
    expect((css.match(/\{/g) || []).length, `${file}: непарные скобки`)
      .toBe((css.match(/\}/g) || []).length);
    // Внутри at-rule не больше одного уровня вложенности.
    const deep = [];
    for (const m of css.matchAll(/@[a-z-]+[^{]*\{/g)) {
      ...
      while (i < css.length && d > 0) {
        if (css[i] === '{') max = Math.max(max, (d += 1));
        else if (css[i] === '}') d -= 1;
        i += 1;
      }
      if (max > 2) deep.push(m[0].trim());
    }
    expect(deep, `${file}: вложенный at-rule, разбор регуляркой больше не годится`).toEqual([]);
    // После вырезания в остатке не должно остаться ни одного `@`.
    expect((stripAtRules(read(file)).match(/@[a-z-]+/g) || []).length, `${file}: at-rule уцелел`)
      .toBe(0);
  });
