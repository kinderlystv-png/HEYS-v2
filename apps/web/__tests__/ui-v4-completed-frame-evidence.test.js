...
describe('восемь завершённых кадров: построчное evidence', () => {
  ...
  it('покрывает каждую строку восьми кадров без пакетного вердикта по статусу кадра', () => {
    ...
    // 321 с пакета 59: кадр «Связка · создание» получил сноску про суперсет и
    // два рисунка креста выхода (его дорисовали по находке кодера), а из
    // каталога ушла строка 25. Все три новые строки стоят «?» с пометкой
    // unsupported — доказательства у них построчного нет, и утверждение ниже
    // именно этого и требует: рост числа без разбора превращает их в вопросы,
    // а не в зелёный цвет.
    expect(materialized).toHaveLength(321);
    expect(new Set(materialized.map((row) => row.rowIdentity)).size).toBe(321);
