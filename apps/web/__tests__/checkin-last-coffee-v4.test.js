...
describe('чек-ин · блок «Последний кофе»', () => {
  ...
  it('заметка говорит, что вопрос про вчера, — иначе «вчера» только в заголовке', () => {
    // Строка «кофе спрашивается за вчера» приводит слова заметки дословно, и
    // предложение про вчера в них есть. Без него человек утром отвечает про
    // сегодняшний кофе, которого ещё не пил, и ответ становится намерением.
    // ОТСТУПЛЕНИЕ: составная строка кадра «Чек-ин · сон · текст» этого
    // предложения не содержит — кадр отстал от строки, верна строка.
    const { configs } = loadSteps({ [TODAY]: { date: TODAY } });
    const html = renderStep(configs, {});
    expect(html).toContain('Спрашиваем про вчера — сегодняшний кофе ещё впереди');
    expect(html).toContain('кофе позже восьми часов до отбоя мешает сну');
  });

  it('в шаге «Остальное» от карточки не осталось и слоя своего времени', () => {
    // Переезд считается доведённым, когда на прежнем месте нет ни карточки, ни
    // её слоя: мёртвая ветка переживает переезд молча и всплывает через месяц.
    expect(STEPS_SRC).not.toContain('coffeeFocus');
    expect(STEPS_SRC).not.toContain('openCoffeeLayer');
    expect(STEPS_SRC).not.toContain('clearCoffeeMark');
  });
