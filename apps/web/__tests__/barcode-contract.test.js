...
describe('product barcode contract', () => {
  ...
  it('keeps scanner controls accessible and preserves barcode through moderation UI', () => {
    ...
    expect(addProduct).toContain("setBarcodeModal({ mode: 'search', autoStart: true, cameraStart })");
    expect(addProduct).toContain("setBarcodeModal({ mode: 'product', product: productWithSharedBarcode, autoStart: true, cameraStart })");
    expect(addProduct).toContain("cameraStart: createBarcodeCameraStart()");
    ...
    expect(dayMeals).toContain('window.HEYS?.AddProductStep?.createBarcodeCameraStart?.() || null');
    ...
    expect(dayMeals).toContain('Повторить из недавних');
    expect(dayMeals).toContain('Один продукт');
    expect(dayMeals).toContain('Выбрать и сразу закрыть');
    expect(dayMeals).toContain('Несколько продуктов');
    expect(dayMeals).toContain('Остаться в добавлении');
