/**
 * Сведённый кусок home-widgets: кадр «Белок · нет данных».
 * 1×1 — ключ «Белок», прочерк снизу автоотступом, полосы нет.
 * 68×64 кадра — клетка стенда 1×1, не отдельное правило тела.
 * Клетчатку empty / now / add и белок now / add / meals не ломает.
 */
...
describe('Белок · нет данных — сведённый кусок', () => {
  ...
  const mealsAt = uiSrc.indexOf("if (variantId === 'by_meal')", uiSrc.indexOf('function ProteinVariantBody'));
  const meals = uiSrc.slice(mealsAt, uiSrc.indexOf("if (variantId === 'add')", mealsAt));
  ...
  it('читает пять строк кадра из актуального data-v', () => {
    expect(contractValue(canvas, 'Белок · нет данных · 01'))
    ...
    expect(contractValue(canvas, 'Белок · нет данных · 04'))
      .toBe('«—» — моноцифры: шрифт 600 21px/1 Manrope,sans-serif, цвет var(--ink-3)');
  ...
  it('держит вид 1×1 и прочерк без полосы в живой ветке now', () => {
    ...
    expect(now).not.toContain('v4GoalBar(pct) : v4GoalBar');
    expect(now).not.toContain('widget-v4-protein-add');
    expect(now).not.toContain('widget-v4-protein-meals');
    expect(meals).toContain('приёмов не было');
    expect(fiberMini).toContain('widget-v4-goal-value--empty');
    expect(fiberMini).toContain('hasData ? v4GoalBar(pct) : null');
  });

  it('держит прочерк 21/600 тоном пустого дня; клетчатку empty не ломает', () => {
    ...
    const empty = rules.get('.widget-v4-mini.widget-v4-protein .widget-v4-goal-value--empty');
    expect(empty.color).toContain('--v4-ink-3');
    expect(empty.color).not.toContain('--v4-ink,');

    const fiberEmpty = rules.get('.widget-v4-mini.widget-v4-fiber .widget-v4-goal-value--empty');
    expect(fiberEmpty.color).toContain('--v4-ink-3');
