...
describe('agent staging guard', () => {
  ...
  it('keeps main source-only unless an integration pass is explicit', () => {
    ...
    expect(
      detectStagingMode({ branchName: 'main', repoRoot: '/repo', env: { HEYS_INTEGRATION: '1' } }),
    ).toBe('integration');
  });
  ...
  it('is safe-by-default: unrecognized branches fall back to agent mode', () => {
    ...
    // source-only.
    expect(
      detectStagingMode({ branchName: 'copilot/whatsnew-trigger', repoRoot: '/repo', env: {} }),
    ).toBe('agent');
    expect(detectStagingMode({ branchName: 'feature/x', repoRoot: '/repo', env: {} })).toBe(
      'agent',
    );
  ...
  it('honors explicit HEYS_STAGING_MODE override on any branch', () => {
    expect(
      detectStagingMode({
        ...
        repoRoot: '/repo',
        env: { HEYS_STAGING_MODE: 'integration' },
      }),
    ).toBe('integration');
  });
  ...
  it('allows source-only work on main without treating isolated worktrees as shared-root writers', () => {
    expect(
      assertNotSharedRootDuringParallel({
        mode: 'agent',
        branchName: 'main',
  ...
  describe('main/develop trunk commits', () => {
    ...
    it('allows release-only commits on main (whats-new via push:agent)', () => {
      expect(
        assertMainIsIntegrationOnly({
          branchName: 'main',
          files: ['apps/web/public/whats-new.json'],
          env: {},
        }).ok,
      ).toBe(true);
    });
    ...
    it('allows allowlist drift-fix commits on main', () => {
      expect(
        assertMainIsIntegrationOnly({
          branchName: 'main',
          files: ['scripts/bootstrap-bypass-allowlist.txt'],
          env: {},
        }).ok,
      ).toBe(true);
    });
