...
describe('«Динамика веса» · вид «График» 2×2', () => {
  ...
  it('CSS вида держит числа кадра и берёт цвет ролью, а не литералом', () => {
    ...
    //
    // Проверка сторожила форму записи `var(роль, #hex)` и потому запрещала
    // законную голую `var(роль)` — а та хекса не содержит, то есть правилу не
    // противоречит. 3 сентября это и вышло: правило графика вернули к
    // заливочной роли без запасного (роль объявлена во всех четырёх наборах,
    // запасное там мёртвый груз), и тест упал на починке. Сторожим правило:
    // цвет берётся ролью v4, запасное необязательно. Голую роль без маркера
    // намеренности всё равно не пропустит гейт ролей — это его работа, не эта.
    [
