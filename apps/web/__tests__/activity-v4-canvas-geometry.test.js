...
// Канвас пишет шрифт шорткатом `font: 600 12.5px/1 Manrope`, продуктовый CSS —
...
const PAIRS = [
  ...
  // Пакет 1 сентября переименовал полосу: .bar исчез из канваса целиком, а
  // подложку теперь держит .pbar — вместе с .pfill (заливка) и .pknob
  // (ползунок, которого у продукта своего правила нет). Пара сохранена, а
  // не снята: иначе полоса шагов вышла бы из-под сверки молча.
  ['.pbar', '.activity-v4-steps__track'],
...
const EXCEPTIONS = new Set([
  ...
  // Подложка полосы шагов: пакет 1 сентября назвал три разных числа. Проза
  // строки «вид · шаги» просит чернила 7 %, класс .pbar кадра рисует 12 %,
  // код держит 8 % (--v4-line). Контракт старше кадра, а ступени 7 % в
  // наборе нет — 8 % и есть ближайшая к прозе, 12 % увели бы дальше. Ждёт
  // ответа дизайнера: расхождение прозы и кадра внутри одного пакета.
  '.activity-v4-steps__track|background',
]);
...
// 76, а не 77: подложка полосы ушла в названное исключение (проза строки,
// кадр и код дают 7 / 12 / 8 %). Пар при этом не убавилось — пакет
// 1 сентября переименовал .bar в .pbar, и пара перепривязана, а не снята.
const COVERAGE_FLOOR = 76;
