...
function readLatestPinFunctionSql() {
  const candidates = fs
    ...
    .filter((name) =>
      PIN_FUNCTION_DEFINITION.test(fs.readFileSync(path.join(DATABASE_DIR, name), 'utf8')),
    );
...
describe('контракт кодов отказа на входе клиента', { timeout: 30_000 }, () => {
  ...
  it('каждый код из актуальной verify_client_pin_v3 есть в словаре', () => {
    ...
    const unknown = sqlCodes.filter(
      (code) => !Object.prototype.hasOwnProperty.call(dictionary, code),
    );
    expect(
      unknown,
  ...
  it('каждый код с kind explained имеет собственную ветку на экране входа', () => {
    ...
    expect(
      explained.length,
      'в словаре нет ни одного explained-кода — проверка потеряла смысл',
    ).toBeGreaterThan(0);
  ...
  it('«PIN не подошёл» по умолчанию не показывается на invalid_credentials', () => {
    ...
    expect(
      calls.length,
      'реакция на invalid_credentials вызывается больше одного раза — проверьте ветки',
    ).toBe(1);
  ...
  it('блокировка входа — состояние экрана, а не строка ошибки', () => {
    ...
    // Строка контракта «блокировка» (решение 09.09): короткая ступень —
    // строка ошибки с точным временем открытия, без карточки и куратора.
    // Срок берётся с сервера полем lock, а не угадывается по порогам.
    expect(branch).toContain('res.lock');
    expect(branch).toContain('SHORT_LOCK_MINUTES');
    expect(branch).toContain('Вход откроется в ${lockAt}.');
    // Карточка на короткой ступени не поднимается — иначе вернётся ровно то,
    // из-за чего человек писал куратору из-за паузы в четверть часа.
    expect(branch).toMatch(/lockMin <= SHORT_LOCK_MINUTES[\s\S]*setRateBlocked\(false\)/);
    // Длинная ступень — карточка, и слот ошибки при ней пуст: две подачи
    // одного отказа рядом читались бы как два разных отказа.
    ...
    expect(screenSource).toContain(
      "'heys-auth-lockout__body' }, 'Напишите куратору — он снимет блокировку.'",
    );
    // Новая попытка снимает блокировку: иначе карточка переживёт и смену
    // номера, и успешный вход.
    expect(handler.slice(0, handler.indexOf("if (code === 'rate_limited')"))).toContain(
