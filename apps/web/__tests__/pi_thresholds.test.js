...
describe('пороги: SWR не падает без профиля', () => {
  ...
  beforeEach(() => {
    // Корневой CI гоняет весь suite в одном fork: чужой fake timer
    // оставляет setTimeout мёртвым, и await setTimeout(20) висит 10с.
    vi.useRealTimers();
    store = {};
  ...
  it('SWR с протухшим кэшем и undefined profile не пишет Background compute failed', () => {
    const days = makeDays(10);
    ...
    try {
      ...
      expect(deferred.length, 'SWR должен поставить фоновый пересчёт').toBeGreaterThan(0);
      while (deferred.length) deferred.shift()();

      const swrFails = errors.filter((line) => line.includes('Background compute failed'));
      expect(swrFails, swrFails.join('\n')).toEqual([]);
      expect(Number.isFinite(store.heys_adaptive_thresholds?.thresholds?.proteinPerMealG)).toBe(
        true,
      );
    } finally {
