...
describe('держатель места · правило продукта', () => {
  ...
  it('ни один живой держатель не пульсирует', () => {
    ...
    const live = liveAnimatedPlaceholders().filter((h) => isLiveInProduct(h.selector));
    const allowed = [
      // insights-skeleton живёт во вкладке «Инсайты» — зона помечена
      // «МАКЕТ НЕ СОГЛАСОВАН», контракта у неё ноль строк, реализовывать по
      // ней нельзя. Снимется вместе с появлением её контракта.
      'insights-skeleton',
    ];
    const unexpected = live.filter((h) => !allowed.some((a) => h.selector.includes(a)));
    expect(
