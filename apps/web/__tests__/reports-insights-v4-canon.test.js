...
describe('ярус вместо заголовка блока', () => {
  ...
  it('оговорка к ярусу — 10,5/600 чернил 38 %, а не мелкий текст', () => {
    const r = rule('.reports-v4-tier__note');
    expect(r).toContain('font: 600 10.5px/1');
    // Чернила 38 % — это роль --v4-ink-4, а не литерал: набор переопределяет
    // её в тёмных палитрах, литерал бы там остался чёрным.
    expect(r).toContain('var(--v4-ink-4');
...
describe('роли цвета', () => {
  it('красный — только разрушающее действие, а не «плохое число»', () => {
    ...
    // состояние строки в «Что осталось доступным»: оба про действие.
