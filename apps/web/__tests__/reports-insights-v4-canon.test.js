...
describe('ярус вместо заголовка блока', () => {
  ...
  it('оговорка к ярусу — 10,5/600 чернил 56 %, а не мелкий текст', () => {
    ...
    // Чернила 56 % — каноническая ступень --v4-ink-2, а не литерал: набор
    // переопределяет её в тёмных палитрах, литерал бы там остался чёрным.
    expect(r).toContain('var(--v4-ink-2');
...
describe('моноцифры обязательны', () => {
  ...
  // Крупный кегль у ТЕКСТА, а не у числа. Список поимённый и короткий: каждое
  // имя здесь снимает с проверки живое правило, поэтому попадать сюда должно
  // только то, где цифр не бывает вовсе.
  const КРУПНЫЙ_ТЕКСТ_НЕ_ЧИСЛО = [
    // Заголовок панели «Ещё N» — название блока словами; счёт сигналов рядом
    // стоит своим правилом на 11px.
    '.insights-v4-more__title',
  ];

  it('крупные числа зоны — обязательно моноцифрами', () => {
    // Кегли 20 и больше в зоне почти всегда у чисел: крупный кегль занят ими по
    // строке «шкала кеглей». Исключения называются поимённо выше — иначе
    // проверка молчит на всей зоне разом, стоит в ней появиться заголовку.
    for (const m of CSS.matchAll(/([^{}]*)\{[^}]*font(?:-size)?:[^;]*?(\d\d)(?:\.\d)?px[^}]*\}/g)) {
      if (+m[2] < 20) continue;
      const селектор = m[1].trim().split('\n').pop().trim();
      if (КРУПНЫЙ_ТЕКСТ_НЕ_ЧИСЛО.some((имя) => селектор.includes(имя))) continue;
      expect(m[0], 'крупное число без моноцифр: ' + селектор)
