...
describe('subscription package C · code anchors', () => {
  ...
  it('contact curator screen uses v4 modal row, not legacy gradient CTA', () => {
    const start = SUBS.indexOf('function ContactCuratorScreen');
    const end = SUBS.indexOf('function openCuratorContactModal', start);
    const block = SUBS.slice(start, end);
    expect(block).toContain('sub-contact__row');
    expect(block).toContain('Поддержка HEYS');
    expect(block).toContain('отвечаем в рабочее время');
