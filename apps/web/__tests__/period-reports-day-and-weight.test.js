...
describe('лист отчётов · средний вес', () => {
  // Тест месяца фиксирует системное время — возвращаем настоящее, чтобы
  // соседние проверки в этом файле и в соседних не наследовали заморозку.
  afterEach(() => {
    vi.useRealTimers();
  });
  ...
  it('месяц считается по дням: порог достижим, средние взвешены по дням', () => {
    ...
    //
    // Время зафиксировано, потому что тест сеет данные от «сегодня» и
    // отматывает шестнадцать недель назад, пропуская будущее. На реальной дате
    // результат зависит от дня прогона: 31 августа последний день месяца
    // выпадал из засева, и полнота выходила 30 из 31 вместо единицы — в CI это
    // мигало бы по календарю, а не по коду. Середина месяца выбрана нарочно:
    // при ней внутри окна есть месяцы, засеянные целиком.
    vi.useFakeTimers();
