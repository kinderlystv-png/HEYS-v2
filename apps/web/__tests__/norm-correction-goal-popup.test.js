...
describe('разбор цели · источник поправки', () => {
  ...
  it('холодный старт называет, что будет дальше, а не только считает дни', () => {
    // Полоса без этой строки растёт в никуда: человек видит прогресс и не
    // знает, к чему он.
    expect(SRC).toContain('Пока считаем по формуле. Через неделю проверим расчёт на');
  });

  it('порядок слоёв назван вслух — строки считаются одна из другой', () => {
    // Список выглядит как равноправные слагаемые, а поправка правит не итог
    // дня, а расход, от которого он считается.
    expect(SRC).toContain('Порядок слоёв не произволен');
    expect(SRC).toContain('Норма без долга — ');
  });

  it('«что такое поправка» — второй слой, а не строка в первом', () => {
    // В первом слое остаётся число и дата, с которой оно действует.
    const at = SRC.indexOf('Что такое поправка на факт');
    expect(at).toBeGreaterThan(-1);
    const block = SRC.slice(at - 1600, at + 1400);
    expect(block).toContain("React.createElement('details'");
    expect(block).toContain('развернуть');
    expect(block).toContain('Дефицит при этом не меняется');
    // Показывается только когда поправка применена: в холодном старте
    // объяснять нечего, там своя строка.
    expect(block).toContain('if (!applied) return null');
  });

  it('пилюля даты — пилюля, а не прямоугольник', () => {
    // Строка контракта «пилюля зрелости и опоры»: поля 4/7, радиус 999.
    const at = SRC.indexOf('const ncPill = {');
    const pill = SRC.slice(at, at + 600);
    expect(pill).toContain("padding: '4px 7px'");
    expect(pill).toContain('borderRadius: 999');
  });
});
