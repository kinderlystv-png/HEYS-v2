...
const EXCEPTIONS = new Map([
  ...
  [
    'Вход · Вход клиента · 9|align',
    'высота поля 44 из контракта: при ней baseline уводит текст вверх',
  ],
...
const CLIENT = [
  ...
  [
    10,
    '.heys-auth-shell .heys-auth-field > .heys-auth-prefix',
    ['fontWeight', 'fontSize', 'lineHeight', 'color'],
  ],
  [
    11,
    '.heys-auth-shell input.phone-input-large',
    ['fontWeight', 'fontSize', 'lineHeight', 'color'],
  ],
  ...
  [
    19,
    ['.heys-auth-key', '.heys-auth-key-spacer'],
...
//
// Пакет 50 (9 сентября) разделил кадр на две ступени: решение 31 августа сужено
// до часа и суток, а короткая ступень вернулась строкой — но своим кеглем,
// 12,5 px против 11,5 у отказа кода. Поэтому пары теперь две.
const LOCKOUT_SHORT = [
  // Порядок каскадный: база, затем уточнение — compare сливает правила по
  // списку, и последний перезаписывает предыдущие.
  [
    14,
    [
      '.heys-auth-error-slot',
      '.heys-auth-error-slot:not(:empty)',
      '.heys-auth-error-slot.is-lock:not(:empty)',
    ],
    [
      'marginTop',
      'radius',
...
const LOCKOUT = [
  ...
  [15, '.heys-auth-lockout__title', ['fontWeight', 'fontSize', 'lineHeight', 'color']],
  [16, '.heys-auth-lockout__body', ['fontWeight', 'fontSize', 'lineHeight', 'color', 'marginTop']],
];
...
const CURATOR = [
  ...
  [
    5,
    '.heys-auth-subtitle',
    ['fontWeight', 'fontSize', 'lineHeight', 'color', 'textAlign', 'marginTop'],
  ],
  ...
  [
    11,
    ['.heys-auth-btn', '.heys-auth-btn--primary'],
    [
      'marginTop',
      'radius',
      'background',
...
// 25 с пакета 50: кадр блокировки разделён на две ступени, и короткая добавила
// свою строку — вместо одной карточки сверяются строка и карточка.
...
describe('«Вход» · разбор кадров канваса', () => {
  ...
  it('кадр «Вход · блокировка · 15 минут» совпадает со строкой в слоте ошибки', () => {
    expect(
      compare({
        razbor,
        rules,
        frame: 'Вход · блокировка · 15 минут',
        pairs: LOCKOUT_SHORT,
      }),
    ).toEqual([]);
  });

  it('кадр «Вход · блокировка · час и сутки» совпадает с карточкой блокировки', () => {
    expect(
      compare({
        razbor,
        rules,
        frame: 'Вход · блокировка · час и сутки',
        pairs: LOCKOUT,
      }),
    ).toEqual([]);
  });
  ...
  it('клавиатура и слот ошибки следуют своим строкам, а не кадру', () => {
    ...
    expect(rules.get('html .heys-auth-key.heys-auth-key--delete').background).toMatch(
      /^var\(--v4-surface\b/,
    );
