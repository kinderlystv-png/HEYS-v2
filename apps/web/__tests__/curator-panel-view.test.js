...
describe('панель куратора · строка клиента', () => {
  ...
  it('коридор, нехватка данных и «вы» набраны как данные, а не пилюли', () => {
    ...
    expect(CSS).toMatch(/\.cur-row__age\.is-data[\s\S]{0,260}--v4-ink-2/);
...
describe('панель куратора · окно', () => {
  ...
  it('кадр «Диагностика»: числа выше кнопок, «Штатно» долей в листе', () => {
    ...
    // Подписи плиток строчными — так их пишет кадр: это подписи под числом,
    // а не заголовки.
    for (const key of ['активных клиентов', 'посещений', 'сбоев', 'отклонений']) {
