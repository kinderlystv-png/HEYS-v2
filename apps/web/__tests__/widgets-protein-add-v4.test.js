...
describe('Белок · Добрать — сведённый кусок', () => {
  ...
  it('читает семь строк кадра из актуального data-v', () => {
    ...
    expect(contractValue(canvas, 'Белок · Добрать · 04'))
      .toBe('«112 из 140 г» — моноцифры: шрифт 600 9px/1 Manrope,sans-serif, цвет var(--ink-2)');
    ...
    expect(contractValue(canvas, 'Белок · Добрать · 06'))
      .toBe('«+28» — моноцифры: шрифт 600 19px/1 Manrope,sans-serif, трекинг -.02em, цвет var(--tx)');
  ...
  it('цвет «N из M г» и «+N» — роли, песок ≠ синий', () => {
    expect(rules.get('.widget-v4-protein-add__now').color).toContain('--v4-ink-2');
