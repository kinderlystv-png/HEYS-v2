...
const overlaysSource = fs.readFileSync(
  path.resolve(__dirname, '..', 'heys_app_overlays_v1.js'),
  'utf8',
);
...
function persistAcknowledgedVersion(
  version,
  { localStorageAvailable = true, sessionStorageAvailable = true } = {},
) {
...
function inspectWhatsNewState({ data, runningVersion = '', buildMetaVersion = '' } = {}) {
...
function shouldClearPendingUpdateFlag({ pendingUpdate = false, inspection = null } = {}) {
...
describe("What's New display guarantees", () => {
  ...
  it('does not fetch, render, retry, or mutate seen-state while centrally disabled', () => {
    const inspectStart = modalSource.indexOf('async function inspectUnseen()');
    const fetchStart = modalSource.indexOf(
      'const [data, buildMeta] = await Promise.all',
      inspectStart,
    );
    const disabledReturn = modalSource.indexOf(
      'if (!isEnabled()) return createDisabledInspectionResult()',
      inspectStart,
    );
    expect(disabledReturn).toBeGreaterThan(inspectStart);
    expect(disabledReturn).toBeLessThan(fetchStart);

    const effectStart = appRootSource.indexOf(
      'const whatsNewEnabled = HEYS.ReleaseFeatures?.whatsNewEnabled === true',
    );
    const retrySetup = appRootSource.indexOf('const scheduleRetry = (delayMs)', effectStart);
    expect(effectStart).toBeGreaterThan(-1);
    expect(appRootSource.slice(effectStart, retrySetup)).toContain(
      'if (!whatsNewEnabled || isLandingDemoEmbed)',
    );
    expect(appRootSource.slice(effectStart, retrySetup)).toContain('return undefined');

    expect(overlaysSource).toContain(
      'HEYS.ReleaseFeatures?.whatsNewEnabled === true && !isConsentBlocking',
    );
    expect(modalSource.slice(disabledReturn, fetchStart)).not.toContain(
      'persistAcknowledgedVersion',
    );
  });

  it('returns disabled at runtime without fetch, timers, or seen-state writes', async () => {
    const fetchSpy = vi.fn();
    const setTimeoutSpy = vi.fn();
    const storageWrites = [];
    const storage = {
      getItem: vi.fn(() => ''),
      setItem: vi.fn((key, value) => storageWrites.push([key, value])),
      removeItem: vi.fn((key) => storageWrites.push([key, null])),
    };
    const window = {
      HEYS: { ReleaseFeatures: { whatsNewEnabled: false } },
      React: {
        useState: vi.fn(),
        useEffect: vi.fn(),
        useCallback: vi.fn(),
    ...
    const inspection = await window.HEYS.WhatsNew.inspectUnseen();
    expect(inspection).toMatchObject({ ok: true, hasUnseen: false, reason: 'disabled' });
  ...
  it('wires runtime and session fallbacks into the production modal source', () => {
    ...
    expect(modalSource.slice(persistStart, persistStart + 500)).toContain(
  ...
  it('waits for an active update lock but ignores an expired one', () => {
    ...
    mockLocalStorage.heys_update_in_progress = JSON.stringify({
      timestamp: mockNow - UPDATE_LOCK_TIMEOUT_MS - 1,
    });
