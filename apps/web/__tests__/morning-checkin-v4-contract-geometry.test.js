...
describe('чек-ин v4: геометрия по контракту канваса', () => {
  ...
  it('метка просрочки: только число дней, и только с седьмого дня', () => {
    ...
    // Строка кадра «Чек-ин · замеры просрочены · 30» (57-я сборка) называет
    // метку целиком: «14 дней без замеров». Голое «14 дней» рядом с «4 дня
    // подряд» у соседней карточки читалось как ещё одна серия, то есть похвала.
    expect(at(6)).toBeNull(); // порог контракта — 7 дней
    expect(at(7)).toBe('7 дней без замеров');
    expect(at(14)).toBe('14 дней без замеров');
    expect(at(21)).toBe('21 день без замеров');
    expect(at(22)).toBe('22 дня без замеров');
