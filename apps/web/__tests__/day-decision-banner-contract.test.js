...
describe('баннер решения о дне', () => {
  ...
  // Кадры «День под порогом · выбор» и «День пустой · выбор» задают вопрос
  // словами и на «вы». «Выбери, что делать:» было единственным «ты» на экране.
  it('вопрос задан на «вы» словами кадра', () => {
    expect(BANNER).toContain('Что это было?');
    expect(BANNER).not.toContain('Выбери, что делать');
  });

  // Кадр пишет долю словами и с разделёнными тысячами: «783 ккал из 1 737 —
  // это 45 % нормы». Скобка «(45%)» после числа читалась сноской к нему.
  it('доля названа словами, тысячи разделены', () => {
    expect(BANNER).toContain('— это ${pct} % нормы');
    expect(BANNER).toContain("toLocaleString('ru-RU')");
    expect(BANNER).not.toMatch(/\(\$\{pct\}%\)/);
  });
});
...
describe('карточка подтверждения дня', () => {
  ...
  // Кадры «Мало калорий»: карточка говорит на «вы», тысячи разделены, доля
  // пишется с пробелом перед знаком — «45 % от нормы».
  it('карточка говорит на «вы» и держит форму чисел кадра', () => {
    expect(STATS).toContain('подтвердите их.');
    expect(STATS).not.toContain('подтверди их.');
    expect(STATS).toContain("+ ' % от нормы'");
  });
