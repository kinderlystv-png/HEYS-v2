...
describe('Отчёты и Инсайты v4 — сверка с канвасом', () => {
  ...
  it('отступления названы и не разрастаются молча', () => {
    ...
    // Порядок списка следует порядку ключей в снимке: он и есть источник.
    const DEVIATIONS = [
      'состав фенотипа',
      ...
      // половине строки — скелетон: он собран классами insights-skeleton
      // прежней системы и не повторяет раскладку того, что придёт (шапка
      // зрелости, действие, разбор). Спиннера и слова «загрузка» в нём нет,
      // эта часть контракта выполнена. Переписывать раскладку скелетона —
      // отдельная задача: класс общий для нескольких вариантов зоны.
      'расчёт и отказ',
      // «состав порогов» снят 30 августа — вместо одной таблицы три яруса по
      // источнику числа. Осталось одно отступление: поверхность и поля
      // карточки яруса взяты от соседних блоков зоны (--v4-card, 2/16), а не
      // от .cd канваса (--c1, 2/14). Перекрашивать один блок из одиннадцати
      // одинаковых нельзя — это часть общего прохода по зоне.
      'вид · ярусы порогов'
    ];
