...
describe('Отчёты и Инсайты v4 — сверка с канвасом', () => {
  ...
  it('отступления названы и не разрастаются молча', () => {
    ...
    // 2026-08-29, лист периодов: третье отступление — «одиннадцать дефектов и
    // их судьба». Строка говорит, что 5, 7, 9 и 10 остаются в журнале, но и они
    // закрыты: 5 и 7 ушли переходом месяца на подневный счёт, 9 — снятием
    // запасной ветки входа, 10 — удалением второго входа. Расхождение в пользу
    // кода: закрыты все одиннадцать, журнальный набор пуст. Снимается правкой
    // строки на стороне дизайнера, не кодом.
    const DEVIATIONS = [
      'состав фенотипа',
      'состав порогов',
      'одиннадцать дефектов и их судьба'
    ];
