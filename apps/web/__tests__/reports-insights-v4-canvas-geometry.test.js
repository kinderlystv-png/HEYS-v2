...
describe('Отчёты и Инсайты v4 — сверка с канвасом', () => {
  ...
  it('отступления названы и не разрастаются молча', () => {
    ...
    const DEVIATIONS = [
      ...
      // Блок «Динамика»: у кривой веса осталось то, что строки блока
      // запрещают, и разбор её вида — отдельная задача, не эта. Замер на
      // живом дереве в песочном и синем наборе:
      //   линия рисуется инлайном style={{stroke:'url(#weightLineGrad)'}} —
      //     градиент #fb923c→#f97316, и правило .reports-v4-dynamics-card
      //     .weight-sparkline-line с ролью --v4-ok-fill под ним мёртвое:
      //     применяется только его толщина 2,5;
      //   прогнозная точка на завтра (heys_day_weight_trends_v1.js:305) даёт
      //     пунктир rgb(156,163,175) и треугольник доверия — при строке
      //     «Прогнозов и экстраполяции в блоке нет»;
      //   подписи значений и дней кривой веса рисуются — при строке
      //     «Сетки, осей и подписей значений нет».
      // Три вердикта стояли «=» на этом же тексте и удостоверяли то, чего на
      // экране нет; строка «графики» держит ещё и холст 360×158 / 360×120
      // против «ширина 262, высота 52».
      'состав · Динамика',
      'два запрета',
