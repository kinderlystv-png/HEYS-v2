...
describe('Отчёты и Инсайты v4 — сверка с канвасом', () => {
  ...
  it('отступления названы и не разрастаются молча', () => {
    ...
    const DEVIATIONS = [
      ...
      'Визуал v4 · Отчёты · 75',
      'Визуал v4 · Отчёты · 76',
      // Роль с именем набора у правой колонки ленты и снятый заголовок
      // «Замеры тела» — оба с названной причиной в вердиктах.
      'Визуал v4 · Отчёты · 97',
      'Визуал v4 · Отчёты · 103',
      // Именованные строки контракта, закрытые отступлением. Герой экрана разбора —
      // спор с кадром, записан дизайнеру. «Что из этого следует» — списком фраз
      // вместо двух колонок. Шкала каскада теперь следует live HTML; две
      // расходящиеся prose-строки остаются долгом canvas, а не исключением.
      ...
      // дизайнера №28: число снято и из кадра, и из строки, правило удалено —
      // среднее отклонение от плана это вывод, и его место в Инсайтах.
      // Строки контракта под это решение ещё нет, поэтому вердикт остаётся
      // «≠» до её приезда.
      'Отчёты · нет веса · 09',
      ...
      'Визуал v4 · Отчёты · 42',
      // Нулевая строка матрицы: список действий, который рисует кадр от
      // 29 августа, в продукте не существует. Компонент ReportsV4ZeroActions
      // удалён 30 августа вместе с советом про норму (e8b2601eb), его стили
      // сняты как мёртвые 31 августа, а тест reports-tiers-contract сторожит,
      // чтобы он не вернулся. Пакет 31 августа это ратифицировал: строка
      // «карточка · призыв о замерах» описывает ярус уже с кнопкой
      // «Записать замер». Строка контракта от 29 августа осталась в файле и
      // требует прежнего — отсюда отступление у неё самой и у шести строк
      // кадра, которые её рисуют.
      'месяц нулей — вопрос к норме',
      'Отчёты · нулевая строка матрицы · 29',
