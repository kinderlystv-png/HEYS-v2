...
describe('Отчёты и Инсайты v4 — сверка с канвасом', () => {
  ...
  it('отступления названы и не разрастаются молча', () => {
    ...
    const DEVIATIONS = [
      ...
      // держали вид того, что не рисуется. Снят целиком: канвас spinners
      // запрещает скелетоны «ни одного, нигде», и строка этого контракта —
      // единственное место, где их просят.
      'расчёт и отказ',
      // Тот же скелетон, вторая строка контракта, что его просит. Канвас
      // spinners запрещает скелетоны «ни одного, нигде», и продукт следует
      // ему; строка reports-insights просит полосы на #f4f4f3 — выполнить оба
      // канваса нельзя. Мёртвый SkeletonCard и 110 строк его CSS сняты 31
      // августа: компонент был объявлен, экспортирован и ни разу не вызван,
      // то есть скелетона не было и раньше — были только его стили.
