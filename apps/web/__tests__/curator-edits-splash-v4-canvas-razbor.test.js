...
// Строка повтора имеет собственные поля кадра; фон бейджа также приведён и
// сверяется.
const REPEAT = [
  [9, [`${M}item`, `${M}item--repeat`], ['align', 'padding']],
...
describe('«Кураторские правки» и «Стык» · разбор кадров канваса', () => {
  ...
  // Базовая строка сохраняет поля 10/13, а кадр повтора явно задаёт своему
  // модификатору 11/13/12 и выравнивание по верху. Бейдж «×N» приведён к кадру
  // 31 августа (da55525c7): он лежит
