...
// Решение владельца 12 сентября: пояснения из кадра, обращённые к тому, кто
// делает экран, а не к тому, кто им пользуется, с экранов сняты. Кадр их
// по-прежнему рисует — дизайнеру отдана задача перерисовать.
const CANVAS_CONFLICTS = Object.freeze({
  'А1б': Object.freeze({
    48: 'абзац «шесть правок против шума» описывает саму переделку — снят с экрана',
    'текст 2/2': 'тот же абзац в составной строке',
  }),
  'А2': Object.freeze({
    26: 'абзац «состояние, в котором список живёт» пересказывает экран — снят',
    'текст': 'тот же абзац в составной строке',
    // Строка контракта «состояние живёт в номере» (61-я сборка, решение
    // 14 сентября) прямо отвечает на этот спор: сделанный номер — ЗАЛИВКА,
    // «обводки у него нет нигде». Кадр А2 всё ещё рисует обводку: перерисовать
    // его дизайнер не успел. Контракт старше кадра, поэтому код стоит по
    // контракту, а расхождение названо здесь и записью
    // strength-done-number-fill-or-outline.
    13: 'сделанный номер: контракт требует заливку, кадр ещё рисует обводку',
  }),
});
...
describe('strength builder: спокойные состояния активного списка', { timeout: 45_000 }, () => {
  ...
  it('повторяет контракт кольца Е3: контекст, число, подпись и три действия', () => {
    ...
    // Подпись кольца объясняла, почему оно стоит над кнопкой, и откуда взято
    // число, — разговор про раскладку. Снята решением владельца 12 сентября;
    // само кольцо, его число и три действия остаются под проверкой ниже.
    expect(BUILDER).not.toContain("Кольцо стоит над кнопкой «Завершить»");
