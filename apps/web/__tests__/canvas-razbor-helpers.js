...
// Числа и роли из фразы разбора: «шрифт 600 44px/.9 Manrope», «отступ сверху
// 16px», «фон var(--acs)». Цвет бывает вложенным — var(--ink-3), —
...
const PICK = {
  ...
  // Рамка обрывалась на первой запятой, а она стоит внутри цвета:
  // «var(--ink-30)» приходило как «rgba(var(--ink)». Числа при этом
  // совпадали, и сверка звала расхождением пару, которая сошлась. Берём
  // куски без запятых и скобочные группы целиком, до одного уровня вложения.
...
const ROLE = {
  ...
  // 0.56, а не 0.55: кадр называет РОЛЬ `var(--ink-2)`, а долю держит палитра.
  // Решением дизайнера 7 сентября доля поднята (песочный 56 %, синий 61 %) —
  // по самой тёмной поверхности набора 55 % давало 4,48 и 3,77, ниже порога.
  // Пока здесь стояло 0.55, девяносто проверок сведения краснели на разнице в
  // сотую и выглядели как «кадры отстали», хотя отставал этот словарь.
  '--ink-2': 'rgba(0,0,0,0.56)', '--ink-3': 'rgba(0,0,0,0.45)',
  '--ink-4': 'rgba(0,0,0,0.38)', '--ink-30': 'rgba(0,0,0,0.3)',
  '--sage': '#eaefe0', '--sagetx': '#3f4a2e',
  '--red': '#a83c22', '--warn': '#c9922e', '--ovl': '#d99a63', '--val-bad': '#a83c22'
};
...
function norm(value, localVars) {
  ...
  // Подстановки идут по кругу, пока значение меняется: ступени вложены друг в
  // друга, и одного прохода мало. Своя переменная файла разворачивается в роль
  // набора — `--boot-disc` это `var(--v4-hero, #efe3cf)`, — а роль набора уже
  // некому развернуть, если localVars стоит последней. Так сверка звала
  // расхождением «#efe3cf против var(--v4-hero,#efe3cf)»: один и тот же цвет,
  // не доведённый до конца. Пять кругов с запасом: живая вложенность — две.
  for (let pass = 0; pass < 5; pass += 1) {
    ...
    // Своя переменная сверяемого файла: роли набора и роли канваса разобраны
    // выше, здесь остаётся то, что файл объявил сам.
    if (localVars) {
