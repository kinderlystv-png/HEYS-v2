...
describe('шалфейные роли вкладки «Питание»', () => {
  ...
  // 2026-08-24: раньше здесь проверялось, что --v4-ok-tint и --v4-on-ok-fill
  // заданы ещё и в двух каноничных наборах — иначе в каноничной тёмной сработал
  // бы светлый фолбэк. Каноничная палитра убрана из файла (канон остался только
  // на зеркале stable.heyslab.ru, а миграция в heys_theme_v1.js переписывает
  // сохранённое `classic` на `sand`), поэтому охранять надо то же самое, но по
  // четырём живым наборам: ни одна из двух ролей не должна снова остаться на
  // фолбэке хоть в одном из них — иначе тёмный набор возьмёт светлый литерал.
  it.each(SETS)('%s: обе шалфейные роли заданы, а не оставлены на фолбэке', (id) => {
