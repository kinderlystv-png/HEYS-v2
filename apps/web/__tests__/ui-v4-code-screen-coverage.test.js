...
describe('UI v4 code→canvas screen coverage', () => {
  ...
  it('freezes every current source root as exactly covered, excluded or a confirmed gap', () => {
    ...
    // +5 roots 12 сентября (148→153, gaps 82→87). Один новый: экран вынесенного
    // раздела настроек. Четыре нашлись, когда опись научилась читать корень,
    // собранный шаблонной строкой (`messenger-modal${…}`): лист геймификации,
    // модалка шагов, модалка «Что нового» и сама переписка в счёт охвата не
    // входили вовсе — гейт молчал не потому, что всё сведено, а потому, что
    // их не видел. Все записаны gap: covered требует поимённой привязки к кадрам.
    expect(roots).toHaveLength(153);
