...
describe('Инсайты · разбор кадров канваса', () => {
  ...
  it('отступления инсайтовых кадров названы поимённо', () => {
    ...
    const notEqual = Object.entries(verdicts.rows)
      .filter(([, row]) => row.v === '≠')
      .map(([key]) => key)
      // Граница — префикс, а не «Инсайты · NN»: у подкадров ключ несёт лишнюю
      // ступень («Инсайты · подробно · 06»), и при фильтре по номеру такие ключи
      // не ловились ни здесь, ни в отчётной половине. Пять «≠» кадра «Подробно»
      // проваливались между половинами и красили чужой гейт.
      .filter((key) => key.startsWith('Инсайты · '));
