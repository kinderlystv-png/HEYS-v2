...
describe('Вес · сведённые stop-кадры', () => {
  ...
  it('читает ключевые строки из актуального data-v', () => {
    expect(contractValue(canvas, 'Вес · Как сейчас · 04'))
      .toBe('«91,1» — моноцифры: шрифт 600 26px/1 Manrope');
    expect(contractValue(canvas, 'Вес · Как сейчас · 05'))
      .toBe('«−0,9 за неделю» — моноцифры: шрифт 600 10px/1 Manrope, цвет var(--gr), отступ сверху 7px');
    expect(contractValue(canvas, 'Вес · Только число · 04'))
      .toBe('«91,1» — моноцифры: шрифт 600 21px/1 Manrope, цвет var(--tx), трекинг -.02em');
    expect(contractValue(canvas, 'Вес · Точки и среднее · 05'))
      .toBe('«точки — весы, линия — среднее за 7 дней» — моноцифры: шрифт 600 9px/1 Manrope, цвет var(--ink-2), отступ сверху 5px');
  });
