...
describe('Клетчатка · Добрать — сведённый кусок', () => {
  ...
  it('читает восемь строк кадра из актуального data-v', () => {
    ...
    expect(contractValue(canvas, 'Клетчатка · Добрать · 04'))
      .toBe('«18 из 26 г» — моноцифры: шрифт 600 9px/1 Manrope,sans-serif, цвет var(--ink-2)');
    ...
    expect(contractValue(canvas, 'Клетчатка · Добрать · 06'))
      .toBe('«+8» — моноцифры: шрифт 600 19px/1 Manrope,sans-serif, трекинг -.02em, цвет var(--tx)');
    expect(contractValue(canvas, 'Клетчатка · Добрать · 07'))
      .toBe('«Овощи · Бобовые · Цельные злаки» — моноцифры: шрифт 600 8.5px/1 Manrope,sans-serif, цвет var(--ink-2), отступ сверху 4px');
