 ...
 *
 * Третья часть строки — «XP за прошлый день не начисляется» — закрыта гейтом
 * в heys_gamification_v1.js (он сравнивает detail.date действия с эффективным
 * «сегодня»). Отправитель без даты считается сегодняшним, поэтому со стороны
 * виджетов остаётся одно обязательство: класть дату открытого дня в событие.
 * Его и проверяем — сам гейт держит gamification-xp-past-day-gate.test.js.
 */
describe('стопка на прошлом дне', () => {
  ...
  it('событие воды несёт дату открытого дня — иначе XP уйдёт за вчера', () => {
    // Гейт геймификации читает detail.date и без него считает действие
    // сегодняшним. Оба отправителя стопки — запись и её отмена — обязаны
    // назвать день, в который реально легла вода.
    const fabEvents = (uiSrc.match(/new CustomEvent\('heysWaterAdded', \{[\s\S]*?\n {8,10}\}\)\)/g) || [])
      .filter((block) => /source: 'widgets-fab/.test(block));
    expect(fabEvents.length, 'отправителей воды из стопки не найдено').toBe(2);
    for (const block of fabEvents) {
      expect(block, `событие без даты: ${block}`).toMatch(/date: dateKey/);
    }
  });
