...
describe('бар отмены · контракт undo-bar.v4.dc.html', () => {
  ...
  // Названное отступление от строки «доступность»: контракт печатает
  // «Отменить, осталось N секунд», продукт озвучивает «Вернуть». Слово
  // «Отменить» запрещает соседняя строка «слова на экране» — вместе с
  // «undo» и «вы уверены?», — и две строки контракта спорят между собой.
  // Верна запрещающая: подпись для чтения с экрана не может называть кнопку
  // словом, которого на кнопке нет.
