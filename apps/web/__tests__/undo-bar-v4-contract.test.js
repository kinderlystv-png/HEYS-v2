...
describe('бар отмены · контракт undo-bar.v4.dc.html', () => {
  ...
  // Отступления здесь больше нет. Две строки зоны спорили: «доступность»
  // печатала «Отменить, осталось N секунд», а «слова на экране» это самое
  // слово запрещала — вместе с «undo» и «вы уверены?». 31 августа спор закрыт
  // снятием строки «слово действия» в пользу продукта: «Вернуть» называет
  // исход, а не механику, и не спрашивает, что именно отменяют — действие или
  // сам бар. Проверка осталась той же, изменилась её природа: она закрепляет
  // решение, а не отступление от него.
