...
describe('HEYS.TrialQueue.admin Telegram recovery', () => {
    ...
    it('convertLead uses the atomic prepare wrapper and sends only the lead id', async () => {
        rpc.mockResolvedValue({
            data: {
        ...
        expect(result.success).toBe(true);
        expect(result.pin).toBe('4016');
        expect(result.intake_status).toBe('invite_prepared');
        expect(rpc).toHaveBeenCalledWith('admin_prepare_trial_candidate_from_lead', {
            p_lead_id: 'lead-1',
        });
    });
});
