...
describe('settings sheet FAB chips markup', () => {
    it('renders FAB block inside appearance panel', () => {
        ...
        // Строка контракта «границы блока чипов» задаёт текст плашки дословно:
        // «Список обновится, когда закроете настройки». Прежнее «Кнопки
        // перестроятся» тест сторожил как факт, хотя код уже перевели на текст
        // кадра — сторож пережил правку и краснел на верном тексте.
        expect(shellSrc).toContain('Список обновится, когда закроете настройки');
        expect(shellSrc).not.toContain('Кнопки перестроятся');
