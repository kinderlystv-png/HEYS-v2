...
describe('UI v4 · «что в шапке» (tips) — ровно два адреса', () => {
    ...
    it('третий бокс — облако синхронизации, и он остаётся', () => {
        // Строка «третий бокс»: появляется слева от группы только при проблеме.
        // Скрытие в покое держит CSS (.cloud-sync-indicator.idle), здесь — что
        // сам бокс не потерян вместе с тумблером.
...
describe('UI v4 · «шторка в приложении» (login) — один адрес смены оформления', () => {
    it('канвас всё ещё просит две оси в одной шторке', () => {
        expect(readContractRow('login.v4.dc.html', 'шторка в приложении')).toBe(
            'тот же вход в шапке открывает две оси — палитра и режим; заменяет оба нынешних тумблера (в шапке геймификации и в календаре планирования)',
        );
    });
