...
describe('date-remainders · CSS, палитро-зависимые строки', () => {
  ...
  it('«капсула даты» — погашенная стрелка на 40 %: гаснет значок, а не кнопка', () => {
    // 40 % несёт правило значка, а не кнопки. Решение владельца 13 сентября: при
    // прокрутке ряд капсулы стал плавающим, и через полупрозрачный кружок
    // просвечивал уезжающий контент — рябило ровно то, чем объясняли
    // полноширинную подложку. Кнопка осталась плотной, гаснет её содержимое.
    // Сторож держался за прежнее место числа и краснел на верном коде.
    expect(baseCss).toMatch(
      /\.date-picker--v4 \.date-picker-day-nav--disabled > \*,[\s\S]{0,120}opacity: 0\.4;/,
    );
    expect(baseCss).toMatch(
      /\.date-picker--v4 \.date-picker-day-nav--disabled,[\s\S]{0,600}opacity: 1;/,
    );
  });
