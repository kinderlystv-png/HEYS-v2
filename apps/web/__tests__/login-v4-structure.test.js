...
describe('login v4 canvas structure', () => {
  ...
  it('reserves the error slot in flow instead of a fixed overlay', () => {
    ...
    expect(css).toMatch(/\.heys-auth-error-slot:not\(:empty\)[\s\S]*?max-height:\s*55px/);
...
describe('login v4 frame groups', () => {
  ...
  it('reshapes PEP setup to login card frame without explainer wall', () => {
    ...
    // Стена объяснений снята, но не сам факт. Решение дизайнера 31 августа
    // (строка login «слова на экране»): «Две строки про подпись стоят при
    // СОЗДАНИИ кода, а не позже». Человек придумывает код, которым будет
    // подписывать документы, — сказать это после подписания уже поздно. До
