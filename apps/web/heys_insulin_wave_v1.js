...
(function(global) {
  ...
  // ====================================================================
  // CONSTANTS (extracted to heys_iw_constants.js) - REFACTOR v4.2.0
  // ====================================================================
  
  const I = HEYS.InsulinWave?.__internals;
  
  if (!I || !I._loaded.constants) {
    console.warn('[InsulinWave] Constants module required - load heys_iw_constants.js first');
  }
  ...
  const PROTEIN_BONUS_V2 = I?.PROTEIN_BONUS_V2;
  const detectProteinType = I?.detectProteinType;
  const calculateProteinBonusV2 = I?.calculateProteinBonusV2;
  const WAVE_SHAPE_V2 = I?.WAVE_SHAPE_V2;
  const gaussianComponent = I?.gaussianComponent;
  const calculateComponentParams = I?.calculateComponentParams;
  const generateWaveCurve = I?.generateWaveCurve;
  const AUC_CONFIG = I?.AUC_CONFIG;
  const calculateTrapezoidalAUC = I?.calculateTrapezoidalAUC;
  const calculateIncrementalAUC = I?.calculateIncrementalAUC;
  const calculateFullAUC = I?.calculateFullAUC;
  const INSULIN_PREDICTOR_CONFIG = I?.INSULIN_PREDICTOR_CONFIG;
  const getInsulinLevelAtTime = I?.getInsulinLevelAtTime;
  const predictInsulinResponse = I?.predictInsulinResponse;
  const generatePredictionSummary = I?.generatePredictionSummary;
  const WAVE_SCORING_V2 = I?.WAVE_SCORING_V2;
  const scorePeakHeight = I?.scorePeakHeight;
  const scoreDuration = I?.scoreDuration;
  const scoreWaveShape = I?.scoreWaveShape;
  const scoreAUC = I?.scoreAUC;
  const scoreContext = I?.scoreContext;
  const calculateWaveScore = I?.calculateWaveScore;
  ...
  const IR_SCORE_CONFIG = I?.IR_SCORE_CONFIG;
  const SUPPLEMENTS_BONUS = I?.SUPPLEMENTS_BONUS;
  const getSupplementsBonus = I?.getSupplementsBonus;
  const COLD_EXPOSURE_BONUS = I?.COLD_EXPOSURE_BONUS;
  const getColdExposureBonus = I?.getColdExposureBonus;
  const AUTOPHAGY_TIMER = I?.AUTOPHAGY_TIMER;
  ...
  const LIPOLYSIS_STREAK_KEY = I?.LIPOLYSIS_STREAK_KEY;
  ...
  // === UTILS (extracted to heys_iw_utils.js) - REFACTOR v4.2.0 ===
  const utils = I?._utils || {};
