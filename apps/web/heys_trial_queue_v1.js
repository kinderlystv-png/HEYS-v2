...
(function (global) {
  ...
  function TrialQueueAdmin({ onClose, curatorId }) {
    ...
    /* Только подпись: цвета из этой таблицы не читает никто. Рядом с каждым
       состоянием лежала пара шестнадцатеричных значений прежней системы
       (#e9f7ed, #fff7dd, #fee2e2 и ещё шесть пар) — восемнадцать литералов,
       которые ни разу не доходили до экрана: обращения идут только к [0].
       Одно состояние кто-то успел перевести на роли набора, остальные так и
       стояли; мёртвый цвет тем и опасен, что выглядит рабочим. */
    const INTAKE_STATUS = {
      not_invited: ['Не приглашён'],
      invite_prepared: ['Приглашение подготовлено'],
      invite_sent: ['Приглашение отправлено'],
      invited: ['Приглашение отправлено'],
      in_progress: ['Заполняет'],
      completed: ['Готово к разбору'],
      needs_clarification: ['Нужны уточнения'],
      approved: ['Одобрен'],
      approved_waiting_slot: ['Одобрен, ждёт место'],
      rejected: ['Отказ'],
    };
     ...
     * уместен кругом 44, как в контракте «Очередь · Решения · 21».
     */
    const LeadRow = ({ item }) => {
