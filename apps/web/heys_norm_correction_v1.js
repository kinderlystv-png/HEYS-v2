...
(function (global) {
   ...
   *
   * ЗАКРЫТО 5 сентября (строка «вид кабинета куратора»): числа, слова и порядок
   * строк карточки — здесь; лист-обёртка (.cur-sheet), строка клиента в списке
   * и ярус — curator-cabinet.v4.dc.html (кадры «Панель · есть работа»,
   * «Панель · лист поверх»). Что бы ни рисовало содержимое — кабинет или
   * коннектор куратора, — числа берутся отсюда, и они те же, что увидит клиент.
   */
  ...
  /**
   * Сантиметры для строки списка: «76,5» — запятая, а не точка, и неразрывный
   * пробел перед единицей. Ноль после запятой не срезаем: «76,0» и «76» в
   * одном ряду читаются как разная точность замера, хотя точность одна.
   */
  function formatCm(value) {
    return Number(value).toFixed(1).replace('.', ',') + ' см';
  }
    ...
    if (!isSelf) {
      if (acceptedDecrease) {
        return Object.assign(card, {
            ...
            // Этот кадр уже сообщает применённое решение. Два основания из
            // режима согласования и общий список предохранителей здесь
            // уступают двум строкам результата из канваса: что осталось
            // неизменным и каким будет следующий шаг.
            evidenceRows: null,
            evidenceTitle: null,
            previousNote: null,
            safeguardsLayer: null,
            // Кадр «Сверка · норма снизилась · без обхватов» ставит отсутствие
            // замера первой строкой фактов: экран не должен подразумевать
            // наблюдение, которого не было. У ветки с обхватами этой строки
            // нет — там замеры есть, и называть их отдельно нечего.
            // Три ветки, три первые строки фактов. «Замеров не было» ставит
            // отсутствие замера первой строкой: экран не должен подразумевать
            // наблюдение, которого не было. «Только талия» на том же месте
            // показывает ЧИСЛА и полными чернилами, а не тихим тоном: там есть
            // что показать, и показать надо. У ветки с обхватами первой строки
            // нет — замеры есть, называть их отдельно нечего.
            facts: (appliedDecision.evidence?.kind === 'missing'
              ? [{ label: 'Замер талии', value: 'не было', tone: 'quiet' }]
              : appliedDecision.evidence?.kind === 'waist_only'
                  && Number.isFinite(appliedDecision.evidence?.waistFrom)
                  && Number.isFinite(appliedDecision.evidence?.waistTo)
                ? [{
                    label: 'Талия за три недели',
                    value: formatCm(appliedDecision.evidence.waistFrom)
                      + ' → ' + formatCm(appliedDecision.evidence.waistTo)
                  }]
                : []
            ).concat([
              {
            ...
            copy: {
              ...
              // Веток стабильности три (решение владельца 10 сентября, строка
              // контракта «три ветки стабильности, а не две»). Средняя —
              // «только талия» — заведена НАВСЕГДА, а не до расширения
              // сервера: человек вправе мерить одну талию, и ему нужно
              // объяснение по ЕГО замеру, а не отговорка про недостающие.
              // Прежняя формулировка «стабильность обхватов не подтверждена»
              // снята: она звучала отказом при том, что замер есть и учтён.
              body: appliedDecision.evidence?.kind === 'stable_girths'
                ...
                : (appliedDecision.evidence?.kind === 'missing'
                  ...
                  : appliedDecision.evidence?.kind === 'waist_moved'
                  // Средняя полоса: талия сдвинулась больше половины
                  // сантиметра, но меньше полутора. Карточка не утверждает про
                  // талию НИЧЕГО — ни «держится», ни «замеров не было»: первое
                  // было бы неправдой, второе тоже. Свой кадр у этого состояния
                  // рисует дизайнер; до его прихода показываем ту же карточку с
                  // фразой про один вес, без строки с числами талии.
                  ? 'Три недели вес держится на месте. Значит, наш расчёт расхода для вас завышен — мы поправили его, а не вас.'
                  : 'Три недели вес и талия держатся на месте. Талия — самый показательный обхват: когда состав тела меняется, она отвечает первой. Значит, наш расчёт расхода для вас завышен — мы поправили его, а не вас.'),
              heroCaption: '\u2212' + formatKcal(Math.abs(norms.deltaKcal)) + '\u00a0ккал',
  ...
  function decisionEvidenceFromWindowRows(rows) {
    ...
    // Числа обязательны: строка контракта «вид · карточка «только талия»»
    // требует показать «Талия за три недели → 76,5 → 76,0 см», а не слово.
    // Это единственное доказательство ветки — без него карточка снова
    // превращается в отговорку про недостающие замеры.
    const round1 = (value) => Math.round(value * 10) / 10;
    const waistFrom = round1(points[0].value);
    const waistTo = round1(points[points.length - 1].value);
    // Порог «держится» — 0,5 см, и взят он от ТОЧНОСТИ БЫТОВОГО ЗАМЕРА лентой,
    // а не с запасом от вето в 1,5 см (уточнение дизайнера 10 сентября). Ниже
    // половины сантиметра замер не отличает изменение от собственной
    // погрешности: человек мерит сам, утром и вечером получит разное, лента
    // ложится по-разному. Порог от инструмента переживёт смену вето, порог «с
    // запасом от вето» — нет.
    //
    // Отсюда три полосы, а не две: до 0,5 включительно — «держится»; больше 0,5
    // и меньше 1,5 — изменение есть, но на прямой довод не тянет, и тогда
    // карточка НЕ утверждает про талию ничего; от 1,5 — вето по замеру, и сюда
    // мы уже не попадаем, поправка не применяется.
    const drift = Math.abs(round1(waistTo - waistFrom));
    return {
      kind: drift <= 0.5 ? 'waist_only' : 'waist_moved',
      waistPoints: points.length,
      spanDays,
      waistDrift: drift,
      waistFrom,
      waistTo
