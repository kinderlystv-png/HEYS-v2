...
(function (global) {
   ...
   */
  const EVIDENCE = {
    // Rosenbaum & Leibel 2010: при дефиците расход падает сильнее, чем
    // предсказывает формула по массе. Это и есть причина, по которой расчёт
    // расходится с фактом и нуждается в поправке.
    adaptation: '20107198'
  };
  ...
  function buildWeeklySyncCard({
    ...
    const card = {
      ...
      evidenceRows: (res.status === 'ready' && exp > 0 && res.factPerDay)
        ? [
          { label: 'Формула говорит', value: Math.round(exp) },
          { label: 'Факт говорит', value: res.factPerDay }
        ]
        : null
    };
