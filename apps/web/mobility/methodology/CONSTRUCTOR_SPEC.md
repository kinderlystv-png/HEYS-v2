...
> не как отдельная копия. Общее правило проекта — «Архитектура тренировочных
> режимов: ядро + контент»; канон:
> [`../../_kernel/TRAINING_MODE_REGULATION.md`](../../_kernel/TRAINING_MODE_REGULATION.md).
> Этот документ задаёт мобильность-инстанс `SPORT_CONFIG` (§4 здесь) + доменную
> схему атома и закрывает технические блокеры пула (`Q-atom-1`, `Q-pain-1`,
> `Q-limit-1`, `Q-dose-1`).
>
