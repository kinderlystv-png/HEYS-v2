...
(function () {
  ...
  HEYS.dayCaloricBalance.computeCaloricBalance = function computeCaloricBalance(ctx) {
    ...
    const caloricDebt = React.useMemo(() => {
      ...
      // Расчёт долга и перебора вынесен в heys_day_caloric_debt_core_v1.js:
      // это чистая функция, и её зеркалит MCP, чтобы считать норму сервером, а
      // не брать протухший кэш отрисовки. Здесь остаются советы и аналитика.
      const core = HEYS.dayCaloricDebtCore && HEYS.dayCaloricDebtCore.computeDebtCore
        ? HEYS.dayCaloricDebtCore.computeDebtCore({ date, day, prof, optimum, sparklineData, fmtDate })
        : null;
      if (!core) return null;
      const {
        CFG,
