    ...
    const statsBlock = React.createElement('div', {
      ...
      (() => {
        ...
        return React.createElement('div', {
          ...
          // Кадр «Цикл · график калорий · 10». Ступенька на линии нормы без
          // объяснения читается как ошибка расчёта, а не как поправка: подпись
          // называет направление («вниз, а не вверх») и говорит, что прошлые
          // дни не пересчитываются. Из трёх подписей кадра в продукт идёт
          // только эта — решение дизайнера 03.09; две другие объясняют разметку
          // самого канваса.
          useReportsV4 && hasCycleReportContext && React.createElement('div', {
            className: 'reports-v4-kcal-cycle-footnote'
          }, 'Норма приподнята во второй половине цикла и возвращается к базе с началом периода — ступенька идёт вниз, а не вверх. Каждый день хранит свою норму и назад не пересчитывается.'),
                ...
                return React.createElement('div', null,
                  React.createElement('div', { style: goalStyles.row },
                    React.createElement('span', { style: ncName },
                      'Поправка на факт',
                      React.createElement('span', {
                        style: Object.assign({}, ncPill, { background: 'transparent', color: 'var(--v4-ink-2, rgba(0, 0, 0, 0.56))' })
                      }, 'копим данные')
                    ),
                    React.createElement('span', {
                      style: {
