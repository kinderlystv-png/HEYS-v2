  ...
  plugins: [
    ...
    viteStaticCopy({
      targets: [
        ...
        // Силовой конструктор: экраны грузятся лениво отдельными файлами через
        // heys_day_stats_bundle_loader_v1.js, поэтому обязаны попасть в dist —
        // иначе в проде они дают 404, и приложение молча откатывается на старый
        // инлайновый конструктор.
        {
          src: 'strength',
          dest: '.'
        },
        // Общее ядро тренировочных режимов: полноэкранный слой тоже ленивый.
        {
          src: '_kernel',
          dest: '.'
        }
