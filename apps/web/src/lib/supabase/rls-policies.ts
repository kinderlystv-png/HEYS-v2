...
export type UpdateUserPreference = Partial<Omit<UserPreference, 'id' | 'user_id' | 'created_at' | 'updated_at'>>;
...
export class PermissionError extends RLSError {
  constructor(required_permission: Permission, current_role?: UserRole) {
    super(
      `Недостаточно прав доступа. Требуется: ${required_permission}`,
      'PERMISSION_DENIED',
...
export class RoleError extends RLSError {
  constructor(required_role: UserRole, current_role?: UserRole) {
    super(
      `Недостаточный уровень роли. Требуется: ${required_role}`,
      'ROLE_INSUFFICIENT',
