...
;(function(global){
  ...
  // === Import modal components (Phase 2 refactor) ===
  const DayModals = HEYS.DayModals || {};
  const ZoneMinutesPicker = DayModals.ZoneMinutesPicker;
  const SleepQualityPicker = DayModals.SleepQualityPicker;
  const DayScorePicker = DayModals.DayScorePicker;

  // === Import chart components (Phase 3 refactor) ===
  const DayCharts = HEYS.DayCharts || {};
  const WeightSparkline = DayCharts.WeightSparkline;
...
const mainBlock = React.createElement('div', { className: 'area-main card tone-violet main-violet', id:'main-violet-block', style:{overflow:'hidden'} },
  ...
  // (авто-высота убрана; таблица сама уменьшена по строкам / высоте инпутов)
    ...
    const renderSparkline = (data, goal) => {
      // KcalSparkline извлечён в модуль heys_day_charts/KcalSparkline.js
      if (typeof window.HEYS !== 'undefined' && 
          window.HEYS.DayCharts && 
          typeof window.HEYS.DayCharts.renderSparkline === 'function') {
        return window.HEYS.DayCharts.renderSparkline(data, goal);
      }
      
      // Fallback: simple error message
      console.error('KcalSparkline module not loaded');
      return React.createElement('div', { 
    ...
    const statsBlock = React.createElement('div', { className: 'compact-stats compact-card' },
      ...
      weightSparklineData.length >= 1 && React.createElement('div', { 
        ...
        WeightSparkline && React.createElement(WeightSparkline, {
          data: weightSparklineData,
          trend: weightTrend,
          onPointClick: (type, point, x, y) => {
            if (type === 'weight' && point) {
              haptic();
              handleDateChange(point.date);
            }
          }
        }),
    ...
    return React.createElement(React.Fragment, null,
      ...
      ZoneMinutesPicker && React.createElement(ZoneMinutesPicker, {
        isOpen: showZonePicker,
        zoneIndex: zonePickerTarget?.zoneIndex,
        value: pendingZoneMinutes,
        kcalPerMin: kcalMin,
        onConfirm: confirmZonePicker,
        onCancel: cancelZonePicker,
      ...
      // Sleep Quality Picker Modal (используем вынесенный компонент)
      showSleepQualityPicker && React.createElement(SleepQualityPicker, {
        isOpen: showSleepQualityPicker,
        value: pendingSleepQuality,
        note: pendingSleepNote,
        sleepHours: day.sleepHours,
        existingNote: day.sleepNote,
        onConfirm: () => {
          setDay(prev => ({ ...prev, sleepQuality: pendingSleepQuality, sleepNote: pendingSleepNote }));
          setShowSleepQualityPicker(false);
        },
        onCancel: () => {
          setPendingSleepQuality(day.sleepQuality || 0);
          setPendingSleepNote(day.sleepNote || '');
          setShowSleepQualityPicker(false);
        }
      }),
      
      // Day Score Picker Modal (используем вынесенный компонент)
      showDayScorePicker && React.createElement(DayScorePicker, {
        isOpen: showDayScorePicker,
        value: pendingDayScore,
        comment: pendingDayComment,
        autoScore: calculateMealAverages(day.meals).dayScore,
        onConfirm: () => {
          setDay(prev => ({ ...prev, dayScore: pendingDayScore, dayComment: pendingDayComment }));
          setShowDayScorePicker(false);
        },
        onCancel: () => {
          setPendingDayScore(day.dayScore || 0);
          setPendingDayComment(day.dayComment || '');
          setShowDayScorePicker(false);
        }
      }),
