      ...
      return React.createElement(React.Fragment,{key:meal.id},
        ...
        React.createElement('div', { className: 'mobile-products-list' },
          ...
          isMealExpanded(mi, (day.meals||[]).length, day.meals, displayIndex) && (meal.items || []).map(it => {
            ...
            // Фон карточки по полезности: 0=красный(вредный), 5=голубой(средний), 10=зелёный(полезный)
            const getHarmBg = (h) => {
              if (h == null) return '#fff';
              // h теперь: 0=вредный, 5=средний, 10=полезный
              if (h <= 1) return '#fecdd3';  // 0-1: красный — вредный
              if (h <= 2) return '#fee2e2';  // 2: розовый
              if (h <= 3) return '#fecaca';  // 3: светло-розовый
              if (h <= 4) return '#e0f2fe';  // 4: очень светло-голубой
              if (h <= 5) return '#bae6fd';  // 5: голубой — средний
              if (h <= 6) return '#7dd3fc';  // 6: более насыщенный голубой
              if (h <= 7) return '#d1fae5';  // 7: светло-мятный
              if (h <= 8) return '#a7f3d0';  // 8: мятный
              if (h <= 9) return '#6ee7b7';  // 9: зелёный
              return '#34d399';              // 10: насыщенный зелёный — полезный
            };
            const harmBg = getHarmBg(harmVal);
            
            // Бейдж полезности/вредности (новая логика: 0=вредный, 10=полезный)
            const getHarmBadge = (h) => {
              if (h == null) return null;
              if (h <= 2) return { emoji: '⚠️', text: 'вредный', color: '#dc2626' };
              if (h >= 8) return { emoji: '🌿', text: 'полезный', color: '#059669' };
              return null;
            };
