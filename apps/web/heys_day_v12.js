...
;(function(global){
  ...
  const MealOptimizerSection = React.memo(function MealOptimizerSection({ meal, totals, dayData, profile, products, pIndex, mealIndex, addProductToMeal }) {
    ...
    const [optExpanded, setOptExpanded] = React.useState(true); // Сразу показываем все советы
...
// console.log('[PullRefresh] ⚠️ Sync not available | clientId:', clientId, '| cloud:', !!cloud);
    ...
    // === БЛОК СТАТИСТИКА ===
    // PR-1: Build VM and actions for stats rendering
    const statsActions = {
      openExclusivePopup,
      haptic,
      setDay,
    ...
    // Build stats VM (structured data)
    const statsVm = HEYS.dayStatsVm?.build?.({
      prof, day, dayTot, optimum, normAbs, weight, cycleDay, 
    ...
    // Temporary data object (for gradual migration to VM)
    const statsData = {
      day, prof, dayTot, optimum, displayOptimum, displayRemainingKcal,
      tdee, bmr, eatenKcal, stepsK, householdK, train1k, train2k, train3k,
      tefKcal, dayTargetDef, baseExpenditure, weight, TR, caloricDebt,
      sparklineData, currentRatio, displayRatioStatus, normAbs, cycleDay,
      ndteData, tefData, chartPeriod, balanceCardExpanded, showConfetti,
      shakeEaten, shakeOver, displayTdee, displayHeroOptimum, displayHeroEaten,
      displayHeroRemaining, eatenCol, displayRemainCol
    };
    
    // Render stats block using extracted module
    const statsBlock = HEYS.dayStats?.render?.({ 
      React, 
      vm: statsVm, 
    ...
    // PR-2: Extract to module with ctx/actions pattern
    const waterCtx = { day, prof };
    const waterActions = { setDay, haptic };
    const waterCard = HEYS.dayWater?.render?.({ 
      React, 
      ctx: waterCtx, 
      actions: waterActions 
    }) || React.createElement('div', { style: { padding: '12px' } }, '⚠️ Water module not loaded');

    // === Activity Card (Активность) ===
    // PR-2: Extract to module with ctx/actions pattern
    const activityCtx = { 
      day, prof, stepsK, householdK, totalHouseholdMin, 
      visibleTrainings, trainingsBlock 
    };
    const activityActions = { 
      setDay, haptic, openHouseholdPicker, openTrainingPicker 
    };
    const compactActivity = HEYS.dayActivity?.render?.({ 
      React, 
      ctx: activityCtx, 
