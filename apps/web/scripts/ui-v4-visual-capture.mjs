    ...
    if (item.id === 'home-widgets-default') {
      ...
      await page.evaluate(() => {
        ...
        const homeCanvasData = {
          ...
          healthTrend: {
            // Кадр «Главная · дефолтная раскладка» и строка «вид · тренд
            // здоровья»: «+8» за 14 дней. Стояло −1 за 7 — число другого кадра.
            hasData: true,
            delta: 8,
            periodDays: 14,
            // Оценки, а не готовые точки: продукт отдаёт именно их, а
            // проекцию делает сам вид. Эти семь дают ровно полилинию кадра
            // «2,18 11,16 20,17 29,12 38,9 47,6 56,4» — стенд проверяет
            // рабочий путь, а не заготовку мимо него.
