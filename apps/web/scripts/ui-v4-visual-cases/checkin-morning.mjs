...
function checkinCase({ id, label, oid, at, profile, days, today: todayExtra, ls, yesterdayVerify, walk, expectStep, notes, tab = 'diary', themeId = 'sand' }) {
  ...
  return {
    ...
    checkin: {
      ...
      walk: walk || [],
      expectStep,
      probe: `(() => { const lsGet = HEYS.utils.lsGet; const prof = lsGet('heys_profile', {}); const pIndex = HEYS.dayUtils.buildProductIndex(HEYS.products.getAll()); const rows = []; for (const d of ['2026-08-15','2026-08-14','2026-08-13','2026-08-12','2026-08-11']) { const day = lsGet('heys_dayv2_' + d, null); let kcal = 0; (day && day.meals || []).forEach((m) => (m.items || []).forEach((it) => { const g = +it.grams || 0; const pr = pIndex.byName.get(String(it.name||'').trim().toLowerCase()) || pIndex.byId.get(String(it.product_id).toLowerCase()); const src = pr || it; if (src.kcal100 != null) kcal += (+src.kcal100 || 0) * g / 100; })); const norm = HEYS.dayNorm.kcal(day || {}, prof, {}); rows.push({ d, has: !!day, meals: (day && day.meals || []).length, kcal: Math.round(kcal), norm, ratio: norm ? +(kcal/norm).toFixed(3) : null, ok: HEYS.ratioZones.isSuccess(norm ? kcal/norm : 0) }); } return { rows, metrics: HEYS.dayCalendarMetrics.getCurrentStreak(), pIdx: pIndex.byId.size, today: HEYS.dayUtils.todayISO() }; })()`,
    },
    ...(notes ? { notes } : {}),
  };
}
