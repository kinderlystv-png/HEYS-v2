...
// шаги 10 000 (два бонуса по 250) = 3 020 → 3,0 л, как в кадрах «норма».
// Коэффициент 30, а не 28: продукт берёт женский 28 только при
// profile.sex === 'female', а стенд, как и приложение, хранит пол словом в
// profile.gender.
//
// Карточка воды на «Питании» на тех же данных пишет «из 2,9 л»: норму ей даёт
// расчёт по профилю, прочитанному через HEYS.utils.lsGet, а тот на ключе
// профиля возвращает пустой объект (nsKey добавляет клиента, дальше store.get
// скоупит второй раз) — и расчёт падает на запасные 70 кг. Замер 12 сентября:
// в localStorage обе формы ключа держат weight 74, HEYS.store.get отдаёт 74,
// HEYS.utils.lsGet — undefined. Стенд это не лечит: кадр «Кольцо» подписан
// «из 3,0», и расхождение должно быть видно в паре, а не спрятано данными.
