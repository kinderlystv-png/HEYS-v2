...
export const HOME_BASE_DATA = Object.freeze({
  ...
  // Вода: стенд подменяет ответ целиком, поэтому мало подать объём — иначе
  // всё остальное приходит пустым. «К этому часу» тогда считался по реальным
  // часам машины (снял в 9:30 — «+1700», снял вечером — другое число), а «Ритм
  // дня» рисовал пустую плитку без столбиков: снимок зависел от часа съёмки.
  //
  // Час зафиксирован на 18:00, как в кадре. Столбики заданы в тех же
  // пропорциях, что рисует кадр «Вода · Ритм дня» (11 · 18 · 8 · 22 · 14 и
  // низкий серый провал), и в сумме дают те же 1700.
  water: {
    drunk: 1700,
    target: 2700,
    pct: 63,
    hoursSinceWater: 3,
    lastWaterTime: '15:00',
    checkHourLabel: 'к 18:00',
    expectedMlNow: 1850,
    expectedPctNow: 69,
    deficitMlNow: -150,
    rhythmBins: [256, 419, 186, 512, 327, 0],
    isClosedDay: false,
  },
  steps: {
    hasData: true,
