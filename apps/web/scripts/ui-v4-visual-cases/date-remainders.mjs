...
const today = (themeId) => dateCase('date-today', 'Дата · сегодня', themeId, {
  // Ниже «Осталось на сегодня» начинается вкладка «Питание» — своя зона.
  frameTextUntil: 'Осталось на сегодня',
  frameTextUntilWhy: 'строка канваса «границы»: ниже капсулы начинается вкладка «Питание» со своей зоной',
...
const otherDay = (themeId) => dateCase('date-other-day', 'Дата · чужой день', themeId, {
  // Зона отвечает за капсулу, стрелки и календарь (строка «границы»): всё
  // ниже «Итога дня» кадр рисует обстановкой, а на деле там лента приёмов —
  // её описывает nutrition-tab. Машинная сверка ниже этой черты не идёт.
  frameTextUntil: 'Итог дня',
  frameTextUntilWhy: 'строка канваса «границы»: зона называет капсулу, стрелки, сетку календаря и содержимое шторок; ниже кадр рисует вкладку «Питание» — её описывает nutrition-tab',
