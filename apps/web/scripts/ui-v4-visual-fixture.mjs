...
export function buildUiV4VisualSnapshot(item = {}) {
  ...
  return {
    ...
    // `_custom: true` — «продукт этого клиента». Без пометки хранилище считает
    // строку ссылкой на общий каталог, не находит базу и кладёт заготовку без
    // нутриентов: каталог на стенде есть, а калории всех позиций нулевые —
    // отсюда «Питание не ведётся» и «0 / 2 дней» на вкладке «Отчёты».
    products: [...PRODUCTS, ...(item.fixtureProducts || [])].map((product) => ({ _custom: true, ...product })),
  };
}
