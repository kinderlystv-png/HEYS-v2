(function () {
  ...
  const SCIENCE_INFO = {
    ...
    // === Умный планировщик — Sprint 3 science engine ===
    SMART_PLANNER: {
      name: 'Умный планировщик — научное обоснование',
      short: 'Планировщик строит расписание приёмов пищи на основе 6 принципов доказательной нутрициологии: хроно-питание, синтез белка (MPS), гликемическая нагрузка, анаболическое окно, качество сна и личный инсулиновый ритм.',
      details: 'Каждый приём рассчитывается индивидуально с учётом времени суток (хроно-питание), состава рациона (белок/углеводы/GL), тренировочной активности (анаболическое окно) и истории питания (адаптивная инсулиновая волна). Распределение энергии между приёмами динамически корректируется по хроно-ритму: больше — в период пика пищеварительной активности (11:00–15:00), меньше — вечером. Белок на каждый приём оптимизирован под порог MPS: 0.4 г/кг, но не более 40 г на приём. После тренировки первый приём в анаболическом окне (до 2ч) получает повышенные нормы белка и углеводов. Последний приём перед сном ограничивается по гликемической нагрузке (GL < 10) и включает sleep-friendly продукты. Smart-алгоритм извлекает личный медианный интервал между приёмами из истории 14 дней.',
      formula: 'S1 Хроно: ratio(время) = MORNING 0.33 / LUNCH 0.38 / SNACK 0.20 / EVENING 0.28\n  Итог = 70% chrono_ratio + 30% sleep_ratio\n\nS2 MPS: optimal_prot = min(40, weight_kg × 0.4) г на приём\n  При prot < optimal → boost до оптимума\n\nS3 GL: targetGL = hoursToSleep < 3 ? GL<10 : GL<20\n\nS4 POST_WORKOUT (2ч после тренировки):\n  prot = min(weight×0.35, budget.prot×0.85)\n  carbs = min(weight×1.0, budget.carbs×0.85)\n\nS5 PRE_SLEEP: scenario = PRE_SLEEP если hoursToSleep < 4\n  Hint: dairy, nuts, legumes, poultry\n\nS6 Adaptive wave: median(gap 2–6ч из 14д) → effectiveProfile.insulinWaveHours',
      sources: [
        { label: 'Garaulet & Gómez-Abellán, 2014 — Timing of food intake and obesity', pmid: '23877420', level: 'A' },
        { label: 'Areta et al., 2013 — Protein ingestion and muscle protein synthesis', pmid: '23459753', level: 'A' },
        { label: 'Ludwig, 2002 — The glycemic index: Physiological mechanisms', pmid: '12002800', level: 'A' },
        { label: 'Ivy, 2004 — Regulation of muscle glycogen repletion after exercise', pmid: '15212750', level: 'B' },
        { label: 'Halson, 2014 — Sleep in elite athletes and nutritional interventions', pmid: '24435400', level: 'B' }
      ],
      evidenceLevel: 'A',
      confidenceScore: 0.88,
      interpretation: 'Планировщик активен, когда в дне есть хотя бы один приём пищи и остался бюджет КБЖУ. Приёмы оптимизируются автоматически — достаточно следовать рекомендациям.',
      priority: 'HIGH',
      category: 'TIMING',
      actionability: 'TODAY',
      impactScore: 0.85,
      whyImportant: '🧠 6 принципов доказательной науки в каждом приёме. Хроно · MPS · GL · Анаб.окно · Сон · Личный ритм.'
    }
  };
