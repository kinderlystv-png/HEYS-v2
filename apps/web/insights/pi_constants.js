...
(function () {
  ...
  const SCIENCE_INFO = {
    ...
    ADAPTIVE: {
      ...
      details: 'Целевой процент дефицита в HEYS снижает калорийность относительно вашей нормы дня; слишком жёсткий режим дольше переносится хуже. При длительном агрессивном дефиците организм снижает расход как защитную реакцию — это и есть адаптивный термогенез. На практике это выглядит как плато: усилий больше, а динамика слабее. Метрика помогает вовремя увидеть этот сценарий и выбрать умную стратегию (рефид, мягче дефицит, выше белок, лучше сон), вместо бесконечного ужесточения. Ключевая идея — устойчивость важнее краткосрочного экстремума.',
    ...
    WEIGHT_PREDICTION: {
      ...
      details: 'Линейный прогноз сглаживает случайные колебания веса (вода, соль, углеводы), чтобы увидеть реальный тренд. Часть быстрых скачков массы тела связана с гликогеном и водой, а не с долей жира за сутки — поэтому оценка прогресса устойчивее по недельному тренду, чем по одному взвешиванию. Практическая ценность не в точности прогноза, а в раннем обнаружении плато или отклонения от цели. Если прогнозируемый наклон не соответствует желаемому темпу, это сигнал к коррекции стратегии. Для надёжности нужно минимум 7-10 точек данных, а интерпретировать лучше в недельном окне, игнорируя дневные флуктуации. Прогноз НЕ учитывает будущие изменения поведения — он экстраполирует текущую траекторию.',
      formula: 'Линейная регрессия:\n  slope = Σ((day - avgDay)(weight - avgWeight)) / Σ(day - avgDay)²\n  forecast = currentWeight + slope × daysAhead',
      source: 'Статистический анализ временных рядов; динамика массы тела при изменении энергии — Hall et al.',
      sources: [
        { pmid: '7608935', level: 'C', title: 'Time-series forecasting — Linear regression approach' },
        { pmid: '21872751', level: 'A', title: 'Hall et al., 2011 — Modeling human body composition and energy balance' }
      ],
      evidenceLevel: 'C',
    ...
    NEXT_MEAL: {
      ...
      short: 'Помогает спланировать ближайший приём пищи с учётом голода, самочувствия и целей дня.',
      details: 'Рекомендация учитывает голод, самочувствие, текущий баланс дня, состав недавней еды и ваши поведенческие паттерны. Оценка реакции на состав последнего приёма служит дополнительным контекстом и не запрещает есть. В результате проще выбрать подходящий состав и придерживаться плана питания.',
      formula: 'Рекомендация основана на:\n  • Голоде и самочувствии\n  • Калорийном балансе дня\n  • Составе недавней еды\n  • Истории паттернов питания\n\nПоказывает практичный вариант следующего приёма.',
    ...
    TRAINING_WINDOW: {
      ...
      details: 'Метрика сопоставляет доступную энергию, время последнего приёма пищи, тип нагрузки и самочувствие. Это помогает выбрать слот, в котором тренировку легче качественно выполнить. При нерегулярном графике полезнее учитывать переносимость и план дня, чем искать универсально «идеальное» время.',
      formula: 'Факторы выбора времени:\n  • Самочувствие и доступная энергия\n  • Тип и длительность тренировки\n  • Циркадный ритм работоспособности\n  • Время и состав последнего приёма пищи',
    ...
    INSULIN_WAVE_STATUS: {
      name: 'Окно для сжигания жира',
      short: 'Показывает ориентировочное время до окна и длительность паузы после его начала.',
      details: 'Таймер помогает выдержать паузу между приёмами, если голода нет и следующий приём не запланирован. Расчёт строится по составу еды и близкой активности, но не измеряет уровень инсулина, липолиз или расход жира. При голоде, слабости или по плану можно есть раньше.',
      formula: 'Статусы оценки:\n  • Расчёт до окна\n  • Ориентировочное окно открыто\n  • Время с начала окна\n\nДлительность зависит от состава еды, полноты данных и близкой активности.',
      ...
      evidenceLevel: 'B',
      confidenceScore: 0.85,
      interpretation: 'Если голода нет, таймер помогает продолжить паузу; при голоде или по плану можно есть раньше.',
      priority: 'MEDIUM',
      category: 'METABOLISM',
      actionability: 'TODAY',
      impactScore: 0.65,
      whyImportant: 'Помогает осознанно выдерживать паузу между приёмами без жёсткого запрета на еду.'
    },
    ...
    NEAT_ACTIVITY: {
      ...
      details: 'NEAT часто недооценивают, хотя именно он создаёт заметный вклад в расход при сидячей работе. Малые изменения (больше ходьбы, бытовая активность) могут существенно улучшить баланс энергии. Основной рычаг дефицита в HEYS по-прежнему — питание относительно нормы дня; активность дополняет и помогает переносимости режима, но не заменяет контроль калорийности.',
      formula: 'Avg household minutes + trend (linear). Порог: <20 низко, 20-40 умеренно, 40-60 хорошо, 60+ отлично',
      source: 'Levine et al., 2005; Tudor-Locke et al., 2011 — NEAT и шаги',
      sources: [
        { pmid: '14722404', level: 'A', title: 'Levine et al., 2005 — Non-exercise activity thermogenesis (NEAT)' },
        { pmid: '21471817', level: 'B', title: 'Tudor-Locke et al., 2011 — Daily step recommendations' }
      ],
    ...
    HYPERTROPHY_COMPOSITION: {
      ...
      formula: 'Сравниваем измерения (талия/biceps/thigh) на фоне:\nбелок ≥1.6 г/кг/день + силовые тренировки.\nВес стоит, а талия уходит или обхваты растут = рекомпозиция.',
      sources: [
        {
    ...
    MEAL_TIMING: {
      ...
      short: 'Описывает частоту приёмов и интервалы между ними.',
      details: 'Паттерн показывает фактическую частоту питания и интервалы между приёмами. При коротком интервале следующий приём может начаться до конца расчётной длительности предыдущего, а длинные интервалы у некоторых людей усиливают голод; ни один интервал сам по себе не определяет изменение жировой массы. Частоту выбирайте по голоду, самочувствию, медицинским ограничениям и способности соблюдать рацион.',
      ...
      interpretation: 'Сопоставляйте интервалы с голодом, самочувствием и соблюдаемостью плана; универсального обязательного интервала нет.',
      priority: 'HIGH',
      category: 'TIMING',
      actionability: 'TODAY',
      impactScore: 0.75,
      whyImportant: 'Интервалы помогают разбирать голод, самочувствие и удобство выбранного режима питания.'
    },
    WAVE_OVERLAP: {
      name: 'Короткие интервалы между едой',
      short: 'Показывает случаи, когда следующий приём добавлен до завершения оценки предыдущего.',
      details: 'Метрика оценивает длительность по составу каждого приёма и отмечает короткие интервалы между соседними приёмами. Это описательный контекст частоты и состава питания, а не измерение уровня инсулина и не признак набора или снижения жировой массы. Частоту приёмов выбирайте по голоду, самочувствию, медицинским ограничениям и способности соблюдать рацион.',
      ...
      interpretation: 'Показатель описывает частоту пересечений; вывод о снижении жировой массы делайте по долгосрочному тренду.',
      priority: 'HIGH',
      category: 'TIMING',
      actionability: 'TODAY',
      impactScore: 0.80,
      whyImportant: 'Помогает сопоставить состав и частоту приёмов с голодом, самочувствием и соблюдаемостью плана.'
    },
    LATE_EATING: {
