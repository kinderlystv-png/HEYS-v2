...
(function (global) {
  ...
  function initModule() {
    ...
    function isEmptyPatternFinding(p) {
      ...
      // Формулировки «ничего не нашли» у детекторов разные, и список ведётся
      // по факту, а не по догадке: «связь … пока не видна» появилась 12
      // сентября при чистке текстов и мимо прежнего списка прошла на первый
      // экран. «Недостаточно белка» сюда не попадает — это настоящая находка.
      return /не выявлен|пока не видн|недостаточно данных|пока не наблюда|мало данных|не хватает данных/i.test(text);
    }
          ...
          h('div', { className: 'insights-v4-nutrition__rhythm-line' },
            ...
            (function () {
              try {
                ...
                if (plan.closed || !until) return ' Следующего времени сегодня нет.';
