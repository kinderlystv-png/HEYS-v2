...
(function (global) {
  ...
  const DEFAULT_PROFILE = {
    ...
    deficitPctTarget: 0,
    // Поправка на факт — действующее значение и дата применения. Два
    // скаляра, а не объект: профиль сливается перекрытием с родительской
    // меткой времени, и вложенный объект при расхождении устройств может
    // склеиться половинами в значение, которого не было ни у кого.
    // История решений живёт отдельным client-scoped ключом: она растёт, а
    // профиль читается на каждом расчёте нормы.
    normCorrectionFactor: 1,
    normCorrectionAppliedAt: '',
  ...
  const PROFILE_VALIDATORS = {
    ...
    // Границы те же, что у расчёта: за ними вероятнее ошибка данных, чем такой
    // обмен, и норму по такому значению двигать нельзя даже если оно как-то
    // попало в профиль.
    normCorrectionFactor: v => {
      if (v === '' || v === null || v === undefined) return 1;
      const n = Number(v);
      if (isNaN(n)) return 1;
      return Math.max(0.90, Math.min(1.15, n));
    }
  };
        ...
        React.createElement(ProfileSection, {
          ...
          React.createElement('div', { className: 'profile-section__fields' },
            ...
            (() => {
              ...
              // Единый источник BMR — HEYS.TDEE.calcBMR (Mifflin). h тут в метрах → ×100 в см.
              const bmr = (HEYS.TDEE && HEYS.TDEE.calcBMR)
