...
(function (global) {
  ...
  I.WAVE_SHAPE_V2 = C?.WAVE_SHAPE_V2 || {
    ...
    components: {
      ...
      hepatic: {
        // ⚠ v4.3 (2026-05-14): MISNOMER. Caumo 2000 (PMID 10780864) описывает
        // 2-фазную секрецию инсулина (быстрая+медленная). 3-я компонента
        // относится к ПЕЧЁНОЧНОМУ КЛИРЕНСУ инсулина — это экспоненциальный спад,
        // а не Gaussian peak. Текущая реализация (Gaussian с peakOffset=0.70)
        // моделирует поздно-фазную секрецию хвоста, что даёт похожую форму
        // визуально, но физиологически некорректно.
        //
        // Практический impact: baseAmplitude=0.05 (5% вклада) — эффект мал.
        // Полный refactor (Gaussian → exp(-t/tau), tau≈30-45мин per Caumo) —
        // отдельная задача (большая работа, ломает snapshot тесты visualisation).
        // TODO R15: переименовать в `clearance` и переписать на exp decay.
        peakOffset: 0.70,    // Поздно в волне (имитация late-phase secretion)
        sigma: 0.35,         // Широкий
        baseAmplitude: 0.05, // 5% — практический impact низкий
        // Модификаторы
        insulinResistanceBoost: 1.5, // IR увеличивает позднюю секрецию
        alcoholBoost: 1.3    // Алкоголь — известный модификатор печёночного метаболизма
      }
    },
  ...
  // ============================================================================
  // ⚠ v4.3 (2026-05-14): HONEST LABEL.
  // Composite score weights (peak 0.25 + duration 0.20 + shape 0.20 + AUC 0.20
  // + context 0.15 = 1.0) и пороги (excellent ≥85 / good ≥70 / fair ≥50) —
  // АВТОРСКАЯ ЭВРИСТИКА. Не валидировано против reference standard.
  //
  // Научный консенсус для сравнения волн (Brouns 2005, PMID 16034360):
  // использовать iAUC (incremental AUC) как единичную метрику, а не composite.
  // Пик и AUC коррелируют (большой пик обычно = большой AUC) → composite веса
  // частично дублируют информацию.
  //
  // Composite сохранён для UI «детальная разбивка», но в коде помечен как
  // эвристика. Кандидат на refactor → iAUC-only (R15).
  // ============================================================================
  ...
  // 📊 ФАЗЫ ВОЛНЫ — детальная модель ИНСУЛИНОВОГО ответа (не глюкозного!)
  // ⚠ v4.3 (2026-05-14): уточнение что моделируется ИНСУЛИН, не глюкоза.
  //   • Инсулиновый peak (1-я фаза секреции): 10-30 мин (Caumo 2000 PMID 10780864,
  //     DeFronzo 1989). rise=20мин — середина диапазона.
  //   • Глюкозный peak (OGTT): 30-45 мин — это ДРУГАЯ кривая.
  //   • Plateau 30-90 мин и Decline 60-120 мин — для инсулина (быстрый клиренс).
  //   • Lipolysis (липолиз): после возврата инсулина к baseline — жиросжигание
  //     возобновляется (Campbell 1993).
  I.WAVE_PHASES = C?.WAVE_PHASES || {
    rise: {
      baseMinutes: 20,        // Базовое время подъёма ИНСУЛИНА (Caumo 2000, 1-я фаза)
  ...
  // 🌶️ SPICY FOOD — капсаицин и термогенез
  // ⚠ v4.3 (2026-05-14): магнитуда снижена 0.96 → 0.99 (-4% → -1%).
  // Ludy, Mattes & Moore 2012 meta-analysis (Chem Senses, PMC3257466):
  // капсаицин даёт незначимый прирост energy expenditure и «no significant
  // effects on 2-h postprandial blood glucose». Direct insulin wave evidence
  // тоже null. Прежние -4% были на верхней границе допустимого; -1%
  // оставляем как символический thermogenic flag (не клинический эффект).
  // TODO R15: рассмотреть удаление механики совсем.
  I.SPICY_FOOD = C?.SPICY_FOOD || {
    multiplier: 0.99,  // -1% (символически; literature ≈0% on glucose response)
    patterns: [
  ...
  // 💧 HYDRATION — дегидратация и метаболизм глюкозы
  // ⚠ v4.3 (2026-05-14): TRIGGER CALIBRATION revisited.
  //   - Carroll 2018 (J Appl Physiol, PMID 30496706, n=16): 1.9% body mass loss
  //     дал NULL effect на glucose/insulin OGTT у здоровых.
  //   - Систематический обзор 2020 (PMC7551868): «acute body water deficits
  //     do not impact circulating insulin» у здоровых subjects.
  //   - Эффект значим только у T2DM (Johnson et al.).
  //   - IOM 2004 «30 мл/кг» относится к TOTAL water (вода + еда + чай + кофе),
  //     а не к чистой питьевой воде. Триггер «<30 мл/кг ЧИСТОЙ воды» поэтому
  //     срабатывает у большинства людей, которые НЕ дегидратированы (значимая
  //     часть воды поступает из пищи и кофе/чая).
  // КАЛИБРОВКА:
  //   - Магнитуды снижены: severe 0.08→0.05, moderate 0.05→0.03, mild 0.03→0.01.
  //     Литература (Carroll 2018) поддерживает ~0% у здоровых; оставляем
  //     символический штраф как поведенческий nudge для тяжёлых случаев.
  //   - Пороги срабатывания (severe <30%, moderate <50%) — оставлены, но
  //     помечены: trigger чувствительный, может ловить юзеров не реально
  //     дегидратированных. UI должен использовать формулировку «недостаточно
  //     питьевой воды», не «обезвоживание».
  I.HYDRATION_BONUS = C?.HYDRATION_BONUS || {
    severe: { maxPct: 30, bonus: 0.05 },    // <30% нормы → +5% (Carroll 2018 → ~0% у здоровых)
    moderate: { maxPct: 50, bonus: 0.03 },  // 30-50% → +3%
    mild: { maxPct: 70, bonus: 0.01 },      // 50-70% → +1% (символически)
  ...
  // 🍟 TRANS FATS — v4.3 (2026-05-13): магнитуды снижены, time scale mismatch.
  // Salmerón 2001 (NHS, PMID 11382654) — это 14-летняя эпидемиология (+39%
  // риска T2DM при хроническом потреблении). Применять +15% к ОДНОЙ волне
  // инсулина за 2г trans-fat — перенос chronic risk на acute response.
  // Острые исследования (Lefevre 2005, Tholstrup 2010, Bendsen 2011) у
  // метаболически здоровых субъектов НЕ показывают значимых изменений
  // постпрандиального инсулина — только при ≥20%E trans × 6 нед у IR.
  // Решение: магнитуды снижены примерно вдвое, как advice-флаг хронической
  // экспозиции, а не как механистический штраф к острой волне. TODO:
  // перенести в `userProfile.transFat30dAvg` (хронический счётчик).
  I.TRANS_FAT_BONUS = C?.TRANS_FAT_BONUS || {
    high: { threshold: 2, bonus: 0.06 },    // 2+ г → +6% (было +15%, нет ack-evidence)
    medium: { threshold: 1, bonus: 0.03 },  // 1-2 г → +3%
    low: { threshold: 0.5, bonus: 0.01 },   // 0.5-1 г → +1% (символический флаг)
    none: { threshold: 0, bonus: 0.00 }     // <0.5 г → нет эффекта
  };

  // ═══════════════════════════════════════════════════════════════════════════
  // 🆕 v3.1.0 (2025-12-10) → v4.3 (2026-05-14): источник заменён, цитаты ниже peer-reviewed.
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  // Sources:
  //   - Flood-Obbagy & Rolls 2009 (Appetite 52(2):416-22, PMID 19110017):
  //     цельное яблоко vs apple sauce vs apple juice → разная сатиация и
  //     glucose response. Whole > sauce > juice по satiety/glucose stability.
  //   - PMID 36364827 (Nutrients 2022): juice gastric T½ ≈ 38 мин, whole
  //     apple ≈ 65 мин (~40% быстрее у liquid form).
  //   - PMID 12519741 (Eur J Clin Nutr 2003): liquid glucose peak ~39 мин,
  //     liquid+solid ~67 мин.
  // ⚠ Конкретные множители (×1.30 / ×1.15 / ×0.85) — heuristic калибровка
  // в этих диапазонах; точные значения зависят от food matrix.
  ...
  // v4.3 (2026-05-13): coldShower снижен до 0 (не -5%).
  // Van Marken Lichtenbelt 2009 / Hanssen 2015 показали +43% Si — НО это
  // 10 дней при 14-15°C по 6ч/сутки С ДРОЖЬЮ. Без дрожи (Hanssen 2021)
  // эффект исчезает. Острый эффект 3-мин холодного душа на постпрандиальную
  // волну в течение следующих 5 часов в literature НЕ описан — invented.
  // Cold bath/swim оставлены как защитимые (длительность + температура +
  // дрожь начинают приближаться к Hanssen-протоколу).
  I.COLD_EXPOSURE_BONUS = C?.COLD_EXPOSURE_BONUS || {
    coldShower: { bonus: 0.0, minutes: 3, desc: '🧊 Холодный душ → нейтрально (нет RCT для острого окна)' },
    coldBath: { bonus: -0.05, minutes: 10, desc: '🧊 Ледяная ванна → -5% (было -10%, в нижней границе допустимого)' },
    coldSwim: { bonus: -0.08, minutes: 5, desc: '🧊 Моржевание → -8% (с дрожью, кумулятивный эффект)' },
    // Длительность эффекта: ~4-6 часов после экспозиции
    effectDurationHours: 5
  };

  // 🔄 AUTOPHAGY — v4.3 (2026-05-13): фазы переименованы, магнитуды снижены.
  //
  // ⚠ ПРЕЖНЯЯ ВЕРСИЯ заявляла «Аутофагия активна с 16ч». Реальный консенсус:
  //   • Jamshed 2019 (PMC6627766): 18-час eTRF → +22% экспрессии LC3A
  //     УТРОМ натощак. Это экспрессия ГЕНА, не аутофагический поток.
  //   • Vendelbo 2014: 36ч голодания — даже СНИЖЕНИЕ LC3-II в нетренированной
  //     скелетной мышце. Аутофагия в людях тканеспецифична.
  //   • Anton 2018 «metabolic switch» 12-18ч — это переход на кетоны,
  //     НЕ аутофагия (часто путают).
  // Реальный consensus для аутофагического ПОТОКА у людей: 24-72ч + ткане-
  // специфично. Бонусы +10/+15/+18% — invented без RCT-источника.
  //
  // Лейблы фаз заменены с «Аутофагия активна / Глубокая» на нейтральные
  // «Низкое инсулиновое состояние / Кетоз вероятен», бонусы снижены к
  // эвристическим. TODO: явный disclaimer в UI о тканеспецифичности.
  I.AUTOPHAGY_TIMER = C?.AUTOPHAGY_TIMER || {
    ...
    // +2/+5/+8/+10 чтобы не перевешивать другие реально измеримые факторы.
    sensitivityBonus: {
      early: 0.02,    // +2% (heuristic)
      active: 0.05,   // +5% (heuristic)
      deep: 0.08,     // +8% (heuristic)
      extended: 0.10  // +10% (heuristic — verified human autophagy markers тканеспецифичны)
    }
  };
  ...
  I.calculateNDTE = (params) => {
    ...
    // ⚠ v4.3 (2026-05-14): порог поднят 200 → 300 kcal.
    // Magkos 2008 (PMID 17635103): значимый HOMA-IR effect только выше ~3.77 МДж
    // (≈ 900 kcal). Ниже порога эффект ниже шума. 200 kcal давало ложно-позитивный
    // bonus за бытовую активность (быстрая прогулка, лёгкая зарядка). Поднимаем
    // до 300 kcal (Magkos curve начинает становиться значимой) — это всё ещё
    // консервативно, истинный плато порога ~500-900 kcal.
    if (hoursSince >= I.NDTE.maxWindowHours || trainingKcal < 300) {
