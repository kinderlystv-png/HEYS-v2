...
- Зависимости для controlled WebView, secure token storage, биометрии, network
  state, app metadata и push уже объявлены в `package.json`, но shell/WebView,
  session storage и notification flows ещё не реализованы.
...
| `react-native-webview`, `expo-secure-store`, `expo-local-authentication`, `expo-notifications`, `expo-network`, `expo-application` объявлены в mobile dependencies, но feature-code ещё не добавлен. | local files     | `sed -n '1,220p' apps/mobile/package.json` + `rg -n "react-native-webview                     | expo-secure-store                                                      | expo-local-authentication | expo-notifications" apps/mobile/app apps/mobile/src; true` | Confirmed: dependencies declared; no app/src feature usage yet. |
