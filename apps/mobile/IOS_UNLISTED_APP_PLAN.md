...
| Auth/security                  | ✅ Mobile token хранится через SecureStore; logout чистит local session и вызывает WebView cookie logout.        | Happy-path smoke с реальным curator/demo account на iPhone.                                     |
...
| Client PIN/native login scope  | ⚠️ Native login сейчас curator email/password; client PIN остаётся web-side контуром.                            | Для клиентского iOS-релиза решить: ship curator-only или добавить native PIN + client exchange. |
...
Отдельное решение до клиентского релиза: текущий native login использует curator
email/password. Если App Store build выдаётся именно клиентам, нужен либо
утверждённый curator-only first release, либо отдельная реализация native
PIN-login + client WebView session exchange.
...
| Logout screen clears native session after calling API logout in WebView context.                                                 | local files     | `sed -n '1,220p' apps/mobile/app/auth/logout.tsx`; `rg -n "auth/logout" apps/mobile/app/_layout.tsx apps/mobile/app/settings/index.tsx`                                                                        | Confirmed: logout route posts to `/auth/curator-logout`, then clears SecureStore and returns to login. |
| Native login currently targets curator email/password auth.                                                                      | local files     | `rg -n "apiClient.post<LoginApiResponse>" apps/mobile/src/features/auth/api.ts`; `rg -n "async function handleLogin                                                                                            | FROM curators" yandex-cloud-functions/heys-api-auth/index.js`                                          | Confirmed: mobile login calls `/auth/login`; backend handler queries `curators`. |
