...
| Push is not active in the first iOS release.                                                                                     | local files       | `bash -lc 'if rg -n "expo-notifications" apps/mobile/package.json apps/mobile/app/settings/index.tsx apps/mobile/src -g "!node_modules"; then exit 1; else echo no-active-push; fi'`                           | Confirmed: no active package dependency, import or permission request remains.                                                                                               |
