...
> Актуальное решение по auth UI (2026-07-11): отдельная React Native форма
> логина удалена как дублирующая. Единственный пользовательский вход — фирменный
> синий HEYS-вход внутри controlled WebView. Guest, expired session, logout и
> login deep link открывают `/web`. Упоминания native login ниже сохранены как
> исторический план и не являются текущим требованием.
...
- iOS `bundleIdentifier`, `buildNumber`, `scheme` и Face ID usage description
  заданы.
- Display name для iOS - `HEYS`; первый iOS-релиз ограничен iPhone-only
  (`supportsTablet: false`), пока iPad не прошёл отдельный QA.
- Expo placeholder icon/splash заменены на HEYS-branded RGB assets.
...
- Зависимости для controlled WebView, secure token storage, биометрии, network
  state и app metadata объявлены в `package.json`. Push-зависимость и active
  permission prompt не входят в первый релиз без продуктового сценария.
...
## Completion audit по DoD 2026-07-02

| DoD item                                                                                    | Current status                                                                              | Evidence                                                                                                                   | Remaining gate                                                       |
| ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| `apps/mobile` собирает iOS build                                                            | 🟡 Локальный iOS JS export проходит; EAS iOS build не запускался без Apple credentials/2FA. | `cd apps/mobile && npx expo export --platform ios --output-dir /tmp/heys-mobile-export`                                    | EAS iOS build через Apple Developer account.                         |
| `app.json` содержит production iOS identity, scheme и assets                                | ✅ Закрыто локально.                                                                        | Facts rows: iOS identity and HEYS-branded assets.                                                                          | Нет локального gate.                                                 |
| `eas.json` содержит iOS production/submit path                                              | ✅ Закрыто локально.                                                                        | Facts row: EAS profiles include iOS config.                                                                                | Apple credentials при реальном build/submit.                         |
| Native shell реализует boot/loading/offline/error/settings/support/privacy/account deletion | ✅ Закрыто локально.                                                                        | Files under `apps/mobile/app` include boot, web, settings, logout and account deletion routes; iOS export passes.          | Real iPhone smoke.                                                   |
| Auth хранит session в SecureStore/Keychain                                                  | ✅ Закрыто локально.                                                                        | Facts row: SecureStore session layer exists.                                                                               | Real device Keychain smoke.                                          |
| WebView uses allowlist and is not general browser                                           | ✅ Закрыто локально.                                                                        | Facts row: WebView navigation policy blocks unsafe schemes and allows configured hosts.                                    | Real iPhone WebView navigation smoke.                                |
| Web session exchange не передаёт long-lived token в URL/localStorage                        | ✅ Backend and mobile contract closed.                                                      | Facts rows: backend exchange handlers, production DB support, deployed API contract.                                       | Real iPhone happy path with demo PIN.                                |
| Deep links have auth guard                                                                  | 🟡 Custom scheme and route parser exist; full universal links are deferred.                 | Facts row: deep-link web paths go through session exchange return URL.                                                     | Universal Links require Apple associated domain setup.               |
| Logout чистит native and web session                                                        | ✅ Закрыто локально.                                                                        | Facts row: logout screen clears native session and both web cookie roles.                                                  | Real iPhone cookie jar smoke.                                        |
| Client/curator/PIN contexts do not mix                                                      | ✅ Contracted and checked at source/API level.                                              | Facts rows: native login supports client PIN and curator auth; backend exchange supports separate curator/client subjects. | Real iPhone role-switch smoke.                                       |
| App Store metadata заполнена                                                                | 🟡 Release packet draft prepared, not entered into App Store Connect.                       | `apps/mobile/APP_STORE_RELEASE_PACKET.md`                                                                                  | App Store Connect owner action.                                      |
| Demo account works for reviewer                                                             | 🟡 PIN contract was live-smoked; reviewer-safe demo credentials still need to be chosen.    | Production PIN/exchange smoke previously passed; release packet has placeholders.                                          | Product owner chooses demo phone/PIN and safe sample data.           |
| Account deletion can be initiated in app                                                    | ✅ Закрыто локально и live-smoke проверяет invalid-session rejection.                       | Facts rows: native account deletion route and live invalid-session smoke.                                                  | Real valid-session deletion smoke only with disposable demo account. |
| TestFlight smoke on real iPhone                                                             | ⬜ External gate.                                                                           | Not run.                                                                                                                   | Apple Developer, EAS build, TestFlight device.                       |
| App Review approved                                                                         | ⬜ External gate.                                                                           | Not submitted.                                                                                                             | App Store Connect submission.                                        |
| Apple unlisted link получен и проверен                                                      | ⬜ External gate.                                                                           | Not requested.                                                                                                             | App Review approval first, then Apple unlisted request.              |
| Клиентская инструкция установки готова                                                      | 🟡 Draft ready with placeholder link.                                                       | `apps/mobile/CLIENT_INSTALL_GUIDE.md`                                                                                      | Insert final unlisted App Store link after Apple approval.           |
...
| Expo/EAS iOS config            | ✅ `app.json` содержит `HEYS`, `com.heys.mobile`, build number, `heys://`, iPhone-only scope и production icon/splash. `eas.json` содержит iOS profiles.                           | Apple credentials/2FA при реальной EAS iOS build.                                                |
| Native shell                   | ✅ Boot flow, login, WebView shell, settings, native loading/error states, logout screen.                                                                                          | Smoke на iPhone/simulator с реальными аккаунтами.                                                |
| Auth/security                  | ✅ Client PIN login и curator email/password login хранят session в SecureStore; WebView получает HttpOnly cookie через one-time exchange; logout чистит client и curator cookies. | Happy-path smoke на реальном iPhone/TestFlight.                                                  |
...
| Push                           | ⬜ Не входит в первый релиз: active permission prompt и `expo-notifications` dependency убраны.                                                                                    | Вернуть только после утверждённого сценария, backend token registration и APNs/Expo credentials. |
| App Store compliance           | 🟡 В приложении есть support/privacy/terms, native account deletion screen с подтверждением, release packet и deletion guide подготовлены.                                         | Перенести metadata в App Store Connect, добавить screenshots, privacy labels, demo account.      |
...
| Client PIN/native login scope  | ✅ Native client PIN login и client WebView session exchange реализованы; backend/migration задеплоены и contract-tested.                                                          | Добавить App Review demo credentials и пройти real PIN smoke на iPhone/TestFlight.               |

Главное, что не упустить: backend/gateway для `/auth/mobile/session-exchange`
задеплоен, а client PIN exchange закрыт contract-test и схемой production DB. До
релиза всё ещё нужен happy-path smoke на реальном iPhone/TestFlight с demo
phone/PIN: native login -> exchange URL -> HttpOnly cookie в iOS WebView -> web
...
10. ✅ Реализовать native client PIN login + client WebView exchange.
11. ⬜ Прогнать simulator smoke.
12. ⬜ Прогнать real iPhone/TestFlight smoke.
13. ✅ Подготовить App Store metadata и Review Notes draft.
...
| iOS identity, display name, iPhone-only scope and custom scheme are set.                                                         | local files       | `sed -n '1,220p' apps/mobile/app.json`; `cd apps/mobile && npx expo config --type public`                                                                                                                                                                                               | Confirmed: `name: HEYS`, `bundleIdentifier`, `buildNumber`, `scheme`, `supportsTablet: false`.                                                                               |
| App icon, adaptive icon, splash icon and favicon are HEYS-branded RGB assets, not Expo placeholders.                             | local files       | `file apps/mobile/assets/icon.png apps/mobile/assets/adaptive-icon.png apps/mobile/assets/splash-icon.png apps/mobile/assets/favicon.png`                                                                                                                                               | Confirmed: PNG files are RGB, expected sizes are 1024x1024 for app/splash assets and 48x48 for favicon.                                                                      |
...
| Logout screen clears native session and both web cookie roles.                                                                   | local files       | `sed -n '1,220p' apps/mobile/app/auth/logout.tsx`; `rg -n "auth/logout" apps/mobile/app/_layout.tsx apps/mobile/app/settings/index.tsx`                                                                                                                                                 | Confirmed: logout route revokes native session, posts to client and curator logout endpoints, then clears SecureStore.                                                       |
| Native login supports client PIN and curator email/password auth.                                                                | local files       | `rg -n "loginWithPin" apps/mobile/src/features/auth/api.ts apps/mobile/app/auth/login.tsx`; `rg -n "kind: 'client'" apps/mobile/src/features/auth/api.ts`                                                                                                                               | Confirmed: mobile login has client PIN mode and curator mode.                                                                                                                |
| Push is not active in the first iOS release.                                                                                     | local files       | `bash -lc 'if rg -n -e "expo-notifications" -e "Push-уведомления" -e "requestNotificationPermission" -e "getNotificationReadiness" apps/mobile/package.json apps/mobile/app/settings/index.tsx apps/mobile/src -g "!node_modules"; then exit 1; else echo no-active-push; fi'`          | Confirmed: no active package dependency, UI row, import or permission request remains.                                                                                       |
| Account deletion can be initiated from native Settings after explicit confirmation.                                              | local files       | `rg -n -e "settings/account-deletion" -e "deleteAccount" -e "delete_my_account" -e "Удаление аккаунта" apps/mobile/app apps/mobile/src/features/auth/api.ts`; `rg -n "delete_my_account" yandex-cloud-functions/heys-api-rpc/index.js database/2026-05-20_compliance_overhaul_fix2.sql` | Confirmed: native Settings opens account deletion route; route calls session-safe `delete_my_account` only after confirmation.                                               |
| Live account deletion endpoint rejects invalid sessions without deleting data.                                                   | live smoke        | `node` smoke script against `/rpc?fn=delete_my_account` with a fake `p_session_token`                                                                                                                                                                                                   | Confirmed: HTTP 200 with `success:false` and `invalid or expired session`.                                                                                                   |
...
| Backend source has DB-backed one-time mobile session exchange handlers for curator and client sessions.                          | local files       | `rg -n "CLIENT_SESSION_EXPIRES_IN" yandex-cloud-functions/heys-api-auth/index.js`; `rg -n "mobile_web_session_exchanges" yandex-cloud-functions/heys-api-auth/index.js`                                                                                                                 | Confirmed: issue and consume handlers store hashed exchange tokens, support client sessions and mark consume.                                                                |
...
| Production DB table for mobile exchange tokens supports curator and client subjects.                                             | production DB     | `bash scripts/db/psql.sh -c "SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_schema='public' AND table_name='mobile_web_session_exchanges' ORDER BY ordinal_position;"`                                                                          | Confirmed: table includes nullable `curator_id` and nullable `client_id` with one-subject check migration applied.                                                           |
...
| Production API is deployed for client-capable mobile exchange.                                                                   | deploy + contract | `./deploy-all.sh heys-api-auth`; `node yandex-cloud-functions/heys-api-auth/__tests__/mobile-session-exchange.contract.test.cjs`; DB migration command above                                                                                                                            | Confirmed: auth function deploy health check passed, contract covers curator/client exchange, DB supports nullable `client_id`. Real PIN smoke still needs demo credentials. |
