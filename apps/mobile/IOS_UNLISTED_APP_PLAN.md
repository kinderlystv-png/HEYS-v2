...
- Backend source содержит mobile session exchange routes для WebView cookie
  bridge; production deploy/API Gateway update ещё требует отдельной команды.
...
## Ревью фактической реализации 2026-07-01
...
| Auth/security                  | ✅ Mobile token хранится через SecureStore; logout чистит local session; WebView получает exchange URL. | Deploy `heys-api-auth` и проверить cookie bridge live.                                        |
...
| Deep links                     | ✅ `scheme: heys`, parser и root listener добавлены.                                                    | Universal Links требуют домен и `apple-app-site-association`.                                 |
...
| App Store compliance           | 🟡 В приложении есть support/privacy/terms/account deletion links.                                      | Заполнить App Store metadata, screenshots, privacy labels, demo account, review notes.        |
...
| Client rollout                 | ⬜ В коде не требуется.                                                                                 | Подготовить клиентскую инструкцию после получения App Store link.                             |

Главное, что не упустить: до релиза нельзя считать WebView-auth завершённым,
пока `/auth/mobile/session-exchange` не задеплоен и не проверен на реальном
домене с `HttpOnly` cookie в iOS WebView.
...
| Backend source has mobile session exchange handlers.                                                                             | local files     | `rg -n "handleMobileSessionExchange" yandex-cloud-functions/heys-api-auth/index.js`                                                  | Confirmed: issue and consume handlers exist.                                     |
