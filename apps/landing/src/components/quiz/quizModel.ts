...
export interface Choice<T extends string> {
  ...
  label: string;
  /**
   * Вторая строка карточки ответа. Голый ярлык заставляет человека подобрать,
   * куда себя отнести; описание опыта превращает выбор категории в узнавание —
   * а узнавание и продаёт разбор (решение владельца 2026-08-09, вариант 8b).
   *
   * Живёт в модели, а не в разметке: тексты — часть вопроса, и версия лендинга
   * их не определяет.
   */
  hint?: string;
}
...
export const TRIGGER_CHOICES: ReadonlyArray<Choice<TriggerCode>> = [
  { code: 'stress', label: 'Стресс или эмоции', hint: 'Ем, когда тревожно или всё навалилось' },
  {
    code: 'fatigue',
    label: 'Усталость или недосып',
    hint: 'К вечеру нет сил, и режим рассыпается',
  },
  {
    code: 'social',
    label: 'Компания, кафе, праздник',
    hint: 'С людьми получается иначе, чем дома',
  },
  {
    code: 'all_or_nothing',
    label: 'Один промах — и режим уже не важен',
    hint: 'После срыва проще бросить, чем продолжить',
  },
  {
    code: 'unknown',
    label: 'Не понимаю',
    // Связано с кодом: описывает ветку `when` ниже, где спрашивают время
    // суток. Уберут ветку или сменят вопрос — эта строка соврёт первой, и на
    // макете это не видно. Держать наравне с ценами из `PRICING`.
    hint: 'Спросим иначе — по времени суток',
  },
];
...
export const SEGMENTS: Record<SegmentCode, SegmentResult> = {
  ...
  emotional: {
    ...
    curator:
      'Куратор держит ваш контекст и помогает разобрать, что стало триггером, — без оценок.',
  },
  social: {
    title: 'Социальный срыв',
    explanation:
      'Срыв запускает среда: компания, кафе, праздник. Решает подготовка, а не характер.',
