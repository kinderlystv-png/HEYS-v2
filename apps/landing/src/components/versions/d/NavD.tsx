  ...
  useEffect(() => {
    const applyScroll = () => {
      ...
      if (sticky) {
        ...
        // Galaxy S9+). Звать там уже некуда: реквизиты, документы и контакты.
        //
        // Но порог другой, чем у CTA. По первому же появившемуся пикселю
        // гасить нельзя: на FAQ раскрытие вопроса меняет высоту страницы,
        // подвал прыгает через край кадра туда-обратно, и пилюля мигает вместе
        // с ним. Ждём, пока подвал займёт больше половины экрана — к этому
        // моменту человек уже читает реквизиты, а не листает вопросы, и
        // дрожание высоты выше по странице до порога не дотягивается.
        const footer = document.querySelector<HTMLElement>('footer');
        const deepInFooter = footer ? footer.getBoundingClientRect().top < vh * 0.4 : false;

        const visible = !wide && y > from && !ownCtaInView && !deepInFooter;
