...
const TARIFFS: ReadonlyArray<Tariff> = [
  {
    ...
    fit: {
      title: 'Кому подходит',
      text: 'Тот же дневник, что у Pro. Ведёте сами — история останется, если позже понадобится сопровождение.',
    },
...
const PLAN_COMPARISON: ReadonlyArray<readonly [string, string, string, string]> = [
  ...
  ['Кто ведёт дневник тренировок', 'Вы', 'Вы', 'Куратор'],
  ['Программа тренировок', 'Нет', 'Нет', 'На четыре недели'],
  ['Адаптация нагрузки', 'Сами', 'Не входит', 'Куратор помогает изменить ближайшую тренировку'],
  ['Обновление программы', 'Нет', 'Нет', 'По результатам выполнения, не чаще раза в неделю'],
  [
    'Контроль техники',
    'Сами',
    'Нет',
    'Куратор разбирает упражнения текущей программы по коротким видео',
  ],
...
const SUPPORT_DETAILS: ReadonlyArray<{
  ...
  // сравнивать не с чем.
  //
  // В спойлере осталось только то, чего на первом слое нет вовсе: как куратор
  // ведёт тренировки и в какие часы отвечает.
  {
    title: 'Как работает куратор',
    paragraphs: [
      'В Pro Спорт куратор видит тренировочный дневник и помогает перестроить ближайшую тренировку, если ситуация изменилась. Основная программа обновляется по результатам выполнения, но не чаще одного раза в неделю.',
      'Для разбора техники достаточно короткого видео одного рабочего подхода. На старте куратор проверяет упражнения новой программы, дальше вы присылаете новые или вызывающие сомнение движения. В ответ — несколько главных замечаний: что уже получается, что изменить и нужно ли показать упражнение повторно.',
    ],
    ...
    <Section id="pricing" index="06" label="Форматы и тарифы" tone="warm">
      ...
      <div className="-mx-3 min-[561px]:mx-0">
        ...
        {/* Шапка Self ниже остальных: под ней сразу подзаголовок, а у Pro и
          ...
          У Pro обводка в цвет собственной шапки и кнопки, но приглушённая —
          `rgba(18,40,62,0.42)` при 1.5px против 1px у соседей. Сплошной
          `#12283E` оказался слишком контрастным: рамка перетягивала внимание
          с содержимого карточки на собственный контур (замечание владельца
          2026-08-09). Толщина осталась — она и делает Pro заметнее, а цвет
          лишь поддерживает родство с шапкой. Без неё выходило наоборот: у Self и
          Pro Спорт рамки были, у основного формата только тень, и главный
          тариф оказывался выделен слабее второстепенных (замечание владельца
          2026-08-09). Рамка смыкается со своей шапкой, и карточка читается
          обведённой целиком — как выбранный вариант.

          Рамки соседей при этом не ослаблены: у Self `rgba(16,24,38,0.16)`, у
          Pro Спорт лиловая `#C9C7E0` — они по-прежнему отделены от фона, просто
          не спорят с основным форматом за внимание.
            ...
            return (
              <div
                ...
                className={`flex flex-col overflow-hidden rounded-[22px] bg-white min-[561px]:rounded-[26px] ${
                  tariff.id === 'pro'
                    ? 'border-[1.5px] border-[rgba(18,40,62,0.42)] shadow-[0_18px_44px_rgba(10,17,25,0.12)] lg:order-2'
                    : tariff.id === 'proSport'
