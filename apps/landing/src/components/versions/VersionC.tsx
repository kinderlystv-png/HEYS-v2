...
export default function VersionC() {
  ...
  const content = {
    ...
    hero: {
      ...VARIANTS.A.hero,
      // Первый экран обещает не снятую рутину, а вникание. Причина: «дневник
      // ведёт куратор» — операционное преимущество, воспроизводимое любым
      // сервисом с ассистентом, и на странице оно и так звучит пять раз ниже.
      // Отличает HEYS то, что человек уточняет контекст, прежде чем советовать,
      // — это и выносим в заголовок. «Рядом» использовать нельзя: «куратор
      // всегда рядом» в чёрном списке `COPY_VOICE`.
      headline: 'Куратор ведёт ваш дневник — и разбирается, почему неделя пошла так.',
      subheadline:
        'Присылаете фото, голосовое или пару фраз. Куратор заносит еду, сон и нагрузку, уточняет, что происходило в трудные дни, и только потом предлагает следующий шаг.',
      subheadlineDesktopLines: [
        'Присылаете фото, голосовое или пару фраз.',
        'Куратор заносит еду, сон и нагрузку, уточняет, что происходило в трудные дни, и только потом предлагает следующий шаг.',
      ],
      // Заголовок обещает — кнопка ведёт посмотреть, и её подпись называет
      // ровно то, что человек увидит. Прямую заявку на первый экран не ставим:
      // это запрещено записью `COPY_VOICE` от 2026-06-27, действие закрыто
      // липкой панелью.
      ctaPrimary: 'Посмотреть разобранную неделю',
