...
export default function VersionC() {
  ...
  const content = {
    ...
    hero: {
      ...VARIANTS.A.hero,
      // Версия A обещает механику, версия C — результат недели, поэтому первый
      // экран говорит и то, и другое: кто ведёт дневник и что человек получает.
      // Заголовок обещает, блок «разобранная неделя» показывает — поэтому его
      // заголовок формулирует показ, а не повторяет обещание.
      headline: 'Дневник ведёт куратор — вы получаете разобранную неделю.',
      subheadline:
        'Присылаете фото, голосовое или короткое сообщение. Куратор заносит еду, сон и нагрузку, видит, где сбивается ритм, и предлагает один выполнимый шаг.',
      subheadlineDesktopLines: [
        'Присылаете фото, голосовое или короткое сообщение.',
        'Куратор заносит еду, сон и нагрузку, видит, где сбивается ритм, и предлагает один выполнимый шаг.',
      ],
