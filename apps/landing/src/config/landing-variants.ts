      ...
      items: [
        {
          ...
          a: 'Вы получаете 7 дней в полноценном тарифе Pro. Куратор перенесёт первые приёмы в дневник, посмотрит ритм недели и разберёт с вами, где день начинает сбиваться и какой шаг взять дальше. Привязывать карту не нужно, автосписаний нет.',
        },
        {
        ...
        {
          q: 'Подойдёт ли HEYS при поездках и непредсказуемом графике?',
          a: 'Да, если заранее учитывать реальный ритм. Куратор смотрит на поездки, нагрузку и пропуски приёмов пищи в контексте недели и помогает подготовить простой запасной вариант, когда привычный план не работает.',
        },
        {
          q: 'Обязательно ли брать куратора?',
          a: 'Не обязательно — у нас три варианта.\n\n<strong>Self</strong> — если вам легко даётся дисциплина и нужен только дневник с базовым контролем. Вы ведёте записи сами: КБЖУ, динамика, виджеты. Куратора в этом формате нет.\n\n<strong>Pro</strong> — главный формат HEYS: куратор берёт на себя рутину дневника, помогает увидеть паттерны недели и спокойно вернуться в режим после срыва.\n\n<strong>Pro Спорт</strong> — всё из Pro плюс персональное ведение тренировок. Один специалист составляет программу на четыре недели, раз в неделю вместе с вами разбирает питание, нагрузку и восстановление, а между встречами делает одну ограниченную корректировку. Пилот доступен только после личного согласования; бесплатной недели в нём нет.\n\nНа 7-дневный бесплатный тест вы получаете именно тариф Pro.<br><br><a href="#trial" class="inline-block px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 transition-colors shadow-sm">Оставить заявку на неделю Pro (0 ₽) →</a>',
        },
        {
        ...
        },
        {
          ...
          a: 'Не нужно восстанавливать дневник задним числом или компенсировать пропуски жёсткими ограничениями. Куратор посмотрит на свежий контекст и поможет вернуться с одного выполнимого шага.',
        },
        {
