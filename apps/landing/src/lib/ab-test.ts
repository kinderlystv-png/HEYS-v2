...
export const HERO_VARIANTS: Record<string, { headline: string; subheadline: string }> = {
  control: {
    headline: 'Оцифруйте свой метаболизм. Поведенческий каскад HEYS вместо слепых диет.',
    subheadline: 'Обычные приложения считают калории. Мы строим ваш персональный baseline по 10 факторам, учитываем ваши циркадные ритмы и спасаем еще до того, как вы почувствовали истощение.',
  },
  variant_a: {
    headline: 'Устали от диет и откатов? Пора попробовать систему Адаптивного Каскада.',
    subheadline: 'Мы объединили предиктивную аналитику Crash Risk Score и заботу живого человека. HEYS анализирует ваше состояние и помогает вырулить, когда всё идёт не по плану.',
  },
  variant_b: {
    // Старый заголовок — оставлен как benchmark для A/B теста
    headline: 'Верните контроль над своим состоянием. Научный подход вместо силы воли.',
    subheadline: 'Алгоритмы HEYS не ругают за съеденное на ночь. С помощью байесовского анализа мы находим ваши индивидуальные паттерны, рассчитываем вероятности и плавно меняем привычки.',
  },
  variant_c: {
    headline: 'Энергия, вес, ясность — под вашим управлением. Без силы воли.',
    subheadline: 'Обычные приложения слепо считают калории. HEYS строит личную карту рисков и предвещает "ямы", когда нужна пауза. Вы просто ведете дневник, алгоритм анализирует, а куратор страхует.',
  },
}
