...
Общая настройка окружения: **[docs/dev/ONBOARDING.md](../docs/dev/ONBOARDING.md)**
...
```
Если получите `Permission denied`, выполните `chmod +x start_modern_heys_demo.sh`.
...
./quick_test.sh
```
Если получите `Permission denied`, выполните `chmod +x quick_test.sh`.
...
- **Интернет** (для загрузки React библиотек, если не закешированы)
