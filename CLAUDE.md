...
- **Расхождение с дизайном не теряется и не чинится молча.** Всё, что меняет
  ...
  `✓` принято · `—` снято · `!` решено без дизайнера, с причиной). Дефект кода,
  где контракт прав и однозначен, туда не пишется — он просто чинится и попадает
  в протокол экрана. Копим и отдаём дизайнеру одним заходом: по накоплении
  заводится отдельная задача в задачнике, а не письмо на каждую строчку.
