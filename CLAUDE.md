...
- **`--rehash` без пересмотра вердиктов красит гейт в зелёный, ничего не
  проверив.** Пересъёмка отпечатков по строкам, чьи вердикты ещё не
  пересматривали, сводит новый текст строки со старым вердиктом: расхождение
  исчезает из виду вместе с поводом его искать. Опаснее прочих случаев ложной
  зелени тем, что команда выглядит рутинной уборкой и её зовут не думая. Порядок
  один: сначала перечитать изменённые строки и поправить код или вердикт, и
  только потом `--rehash <зона>`. Зона в середине переноса **должна** стоять
  красной — это состояние, а не дефект; зелёная при непересмотренных вердиктах —
  ложь.
