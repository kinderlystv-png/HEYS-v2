...
entry только для technical-изменений, закоммитит follow-up, прогонит тесты и
выполнит push. Для user-facing/UI/runtime правок агент обязан идти через
`pnpm push:ready`, чтобы вручную подтвердить смысл релиза и скриншоты.
