...
- **Цвет — ролью, и это проверяется машинно.** `pnpm ui:v4:check` держит два
  гейта: перекраска классики (`scripts/ui-v4-check-classic-drift.mjs`) и
  неопределённые роли (`scripts/ui-v4-check-undefined-roles.mjs`).
  Неопределённая роль — это цвет, который молча не следует набору: список
  известных мест заморожен и может только уменьшаться.
