...
1. **Проверяет staged** — если ничего не застейджено, но в worktree есть dirty
   файлы, отказывается работать и показывает что было бы захвачено. Это страховка
   от пустого/случайного коммита, **не** запрет на чужие файлы: сборщик осознанно
   стейджит нужную группу (`git add <группа>` или `git add -A`).
2. Коммитит staged. Pre-commit в `integration`-режиме пересобирает **только
   бандлы, затронутые правкой в коммите** (через `bundle:legacy:auto`) — неважно,
   чьи это были файлы.
...
отдельной ветке и НЕ попадать в main вместе с общим потоком (эксперимент,
долгий рискованный рефактор под отдельный review). Если это именно твой случай:
