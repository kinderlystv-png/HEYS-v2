...
export function validateAction(action:ActionDefinition):void {
  ...
  if(action.schemaVersion!==1||action.version!==1)fail(`action.${action.id}.version`,'expected schema/version 1');
  ...
  action.conditionalEffects.forEach((item,index)=>{if(item.evaluateAt!=='pre_action')fail(`action.${action.id}.conditionalEffects[${index}]`,'must evaluate pre_action');validateCondition(item.when);item.effects.forEach((effect,effectIndex)=>validateEffect(effect,`conditional[${index}].effects[${effectIndex}]`));});
  unique(action.scheduledEffects.map((item)=>item.id),`action.${action.id}.scheduledEffects`);const rollKeys:string[]=[];
  for(const scheduled of action.scheduledEffects){if(!snake.test(scheduled.id))fail('scheduled.id','snake_case required');if(scheduled.trigger.kind==='condition')validateCondition(scheduled.trigger.condition);else if(scheduled.trigger.kind==='after_steps'){integer(scheduled.trigger.steps,`scheduled.${scheduled.id}.trigger.steps`);if(scheduled.trigger.steps<1)fail(`scheduled.${scheduled.id}.trigger.steps`,'must be positive');}else{integer(scheduled.trigger.dayOffset,`scheduled.${scheduled.id}.trigger.dayOffset`);integer(scheduled.trigger.minuteOfDay,`scheduled.${scheduled.id}.trigger.minuteOfDay`);if(scheduled.trigger.dayOffset<0||scheduled.trigger.minuteOfDay<0||scheduled.trigger.minuteOfDay>1439)fail(`scheduled.${scheduled.id}.trigger`,'invalid at_time bounds');}scheduled.effects.forEach((effect,index)=>validateEffect(effect,`scheduled.${scheduled.id}.effects[${index}]`));}
  ...
  action.geometryRules.forEach((rule,index)=>{validateCondition(rule.when,`action.${action.id}.geometryRules[${index}]`);if(!rule.reason)fail('geometryRule.reason','required');});
}
...
export function validateEvent(event:EventTemplate):void {
  ...
  if(!event.copy?.title||!event.copy.situation)fail(`event.${event.id}.copy`,'title and situation required');validateCondition(event.trigger,`event.${event.id}.trigger`);
