...
- **TypeScript with legacy-relaxed package tsconfig**, ESM (`import`/`export`)
...
- **Типизация**: package tsconfig intentionally keeps some strict flags relaxed
  (`noUnusedLocals`, `exactOptionalPropertyTypes`, `noUncheckedIndexedAccess`,
  `noImplicitAny`). Не ужесточай их случайно в feature-правках; если нужно
  tightening, делай отдельным scoped refactor.
