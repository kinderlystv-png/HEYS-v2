...
COMMENT ON TABLE public.consents_archive_prehash IS
  'Согласия тестового периода без хэша документа (до 2026-07-28). Доказательством согласия не являются: текст версии не сохранён. Персональные данные — включать в выгрузку по запросу субъекта и в срок хранения наравне с consents. РЕШЕНИЕ 2026-08-11: FK с ON DELETE CASCADE выбран сознательно — архив перестаёт быть свидетельством навсегда и умирает вместе с клиентом. Размен в пользу приватности: хранить данные субъекта мимо процедуры удаления хуже, чем потерять запись, которая всё равно ничего не доказывает.';
