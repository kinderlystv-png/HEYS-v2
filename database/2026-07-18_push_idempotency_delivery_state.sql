...
BEGIN
  IF NOT EXISTS (
    SELECT 1
      FROM pg_constraint
     WHERE conname = 'push_idempotency_status_check'
       AND conrelid = 'public.push_idempotency'::regclass
  ) THEN
    ALTER TABLE public.push_idempotency
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.push_idempotency TO heys_admin;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'heys_rpc') THEN
    GRANT SELECT, INSERT, UPDATE, DELETE ON public.push_idempotency TO heys_rpc;
  END IF;
END $$;
