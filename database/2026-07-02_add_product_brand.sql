...
COMMENT ON COLUMN public.shared_products.brand IS
  'Optional product brand / manufacturer label shown separately from product name.';

CREATE INDEX IF NOT EXISTS idx_shared_products_brand_trgm
  ON public.shared_products USING GIN (brand gin_trgm_ops)
...
BEGIN
  ...
  INSERT INTO shared_products (
    ...
    v_user_id,
    p_product_data->>'name',
    NULLIF(TRIM(REGEXP_REPLACE(COALESCE(p_product_data->>'brand', ''), '\s+', ' ', 'g')), ''),
    v_name_norm,
    v_fingerprint,
...
BEGIN
  ...
  INSERT INTO shared_products (
    created_by_user_id, name, brand, name_norm, fingerprint, barcode, barcodes,
