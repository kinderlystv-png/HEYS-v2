...
--
-- Сырьё живёт в Cloud Logging (одна JSON-строка `t="mcp_call"` на вызов) и
-- хранится 14 суток. Сюда суточный джоб складывает только агрегат: ни одного
-- идентификатора человека, ни аргументов, ни тел. При этом условии таблицы не
-- попадают в перечень обработки ПДн — если кто-то добавит client_id или
-- curator_id, квалификация меняется и запись обязана попасть в перечень со
-- сроком хранения.
--
