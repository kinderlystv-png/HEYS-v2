...
// Решение по этой описи живёт ОТДЕЛЬНЫМ файлом, а не здесь: этот документ
// генерируется целиком, и любая вписанная в него договорённость исчезает при
// первой же пересборке — молча и без следа.
W('**Разметки «место за местом» не будет** — решение дизайнера 4 сентября:');
W('размечать надо значение, а не место. Прежняя инструкция «пометьте `герой` у');
W('семейств» не действует.', '');
W('Что делать с каждым видом значения, какие роли названы и где перекраска');
W('запрещена до кадров — [`UI_V4_BARE_LITERALS_DECISION.md`](UI_V4_BARE_LITERALS_DECISION.md).');
W('Числа по вёдрам — `node scripts/ui-v4-list-bare-literals.mjs --buckets`.', '');
