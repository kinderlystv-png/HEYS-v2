...
function assertIntegrationRunConfirmed() {
  ...
  writeError(
    'This helper merges branches, rebuilds generated/release artifacts, and creates integration commits.',
  );
  writeError(
    'Preview only: pnpm agents:integrate --dry-run --branches=... --title="..." --items=\'[...]\'',
  );
  process.exit(2);
}
...
function parseBranches() {
  ...
  if (raw.trim() === 'auto') {
    ...
    if (!hasFlag('--yes') && !hasFlag('--dry-run')) {
      writeError(
        'Re-run with --yes to confirm integrating the branches above (or pass explicit --branches=...).',
      );
      process.exit(2);
    }
    return discovered;
...
function ensureReleaseArgs() {
  ...
  if (!title || !itemsJson) {
    writeError(
      'Missing release text. Provide --title and either --items or --item-title/--item-description.',
    );
    process.exit(2);
  }
...
function main() {
    ...
    if (result.status !== 0) {
      ...
      if (conflicts) {
        ...
        conflicts
          .split('\n')
          .filter(Boolean)
          .forEach((file) => writeError(`  - ${file}`));
      }
      if (!hasFlag('--dry-run')) {
        run('git', ['merge', '--abort'], { mutates: true });
        if (startRef) run('git', ['reset', '--hard', startRef], { mutates: true });
        writeError(
          `Rolled back to ${startRef || 'pre-integration HEAD'}. Resolve the conflict on "${branch}" and re-run.`,
        );
      }
      process.exit(1);
    }
  ...
  commitIfStaged(
    'chore(build): sync generated artifacts for agent integration',
    GENERATED_ADD_PATHS,
  );
  ...
  if (isWhatsNewEnabled()) {
    ...
    runRequired(
      'git',
      ['add', '--', 'apps/web/public/whats-new.json', 'apps/web/public/whats-new'],
      { mutates: true },
    );
    ...
    writeLine("What's New disabled centrally; integration skipped release metadata.");
  }
