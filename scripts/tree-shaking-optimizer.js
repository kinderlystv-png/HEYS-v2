...
function generateRecommendations(importPatterns, optimizations) {
  ...
  if (importPatterns.starImports.length > 0) {
    recommendations.push(
      `Replace ${importPatterns.starImports.length} star imports with named imports`,
    );
  }

  if (importPatterns.largeLibraryImports.length > 0) {
    recommendations.push(
      `Optimize ${importPatterns.largeLibraryImports.length} large library imports`,
    );
  }
...
async function main() {
  ...
  console.log(
    `🔍 Import patterns analyzed: ${Object.values(results.analysis).reduce((a, b) => a + b, 0)}`,
  );
