...
export function renderMarkdown(summary) {
  const lines = [
    ...
    '`v === "≠"`) по зонам контракта. **Отправлено дизайнеру 5 сентября** —',
    'решением владельца, не дожидаясь закрытия strength-builder: 732 строки из 933',
    'лежат в 22 сведённых зонах и меняться не будут, дорасти может только доля',
    'strength-builder (201 строка, 59 строк ещё без вердикта).',
    '',
    '## Что просим',
    '',
