...
function acquireShipLock(branch, message) {
  ...
  if (ageMs > SHIP_LOCK_TTL_MS) {
    err(
      `[ship] ⚠️  Found ship-lock from PID ${existing.pid} but it's ${ageSec}s old (>${SHIP_LOCK_TTL_MS / 1000}s TTL) — stealing.`,
    );
  ...
  err(
    `[ship]      kill -9 ${existing.pid} && rm ${path.relative(ROOT_DIR, SHIP_LOCK_PATH)}    # if hung`,
  );
...
function cleanupStaleGitLocks() {
  ...
  const presentLocks = lockNames.filter((n) => {
    try {
      fs.statSync(path.join(gitDir, n));
      return true;
    } catch {
      return false;
    }
  ...
  if (liveGitPids) {
    err(
      `[ship] ⚠️  stale lock(s) detected (${presentLocks.join(', ')}) but live git PID(s) ${liveGitPids} — not removing.`,
    );
    err(
      `[ship]    Wait for that git process to finish, or kill it manually if you know it's hung.`,
    );
    return;
  }
...
function main() {
  ...
  out(
    `[ship] branch=${branch}  type=${type}  user-facing=${isUserFacing}${flags.dryRun ? '  (dry-run)' : ''}`,
  );
  ...
  if (isWhatsNewEnabled()) {
    ...
    if (isUserFacing) {
      ...
      run(
        process.execPath,
        [PREPARE_RELEASE, '--auto', '--force-technical', '--covered-commits=auto'],
    ...
    if (staged) {
      ...
      const wnSubject =
        subject.length > subjectBudget
          ? subject.slice(0, subjectBudget - 1).trimEnd() + '…'
          : subject;
      run('git', ['commit', '-m', `${HEADER_PREFIX}${wnSubject}`], {
        label: '💾 commit: whats-new',
      });
    } else {
      out('[ship] whats-new already up-to-date — no release commit needed.');
    }
  } else {
    out("[ship] What's New disabled centrally — release metadata unchanged.");
  }
