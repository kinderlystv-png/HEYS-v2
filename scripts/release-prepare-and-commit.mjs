...
function runInteractiveNodeScript(scriptPath, args = []) {
  ...
  if (result.status !== 0) {
    throw new Error(
      `Command failed: node ${path.relative(ROOT_DIR, scriptPath)} ${args.join(' ')}`.trim(),
    );
  }
}
...
function main() {
  if (!isWhatsNewEnabled()) {
    writeLine("What's New is disabled centrally; no files were staged or committed.");
    return;
  }
  ensurePathsExist();
  ...
  writeLine(
    'Push выполняй только после явной команды пользователя; pre-push guards должны пройти.',
  );
}
