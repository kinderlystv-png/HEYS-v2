...
function runCli() {
  ...
  if (rehashAt >= 0) {
    ...
    for (const [key, value] of state.current) {
      ...
      if (existing.h && existing.h !== next && existing.v !== '?') {
        ...
        patchZoneRow(zoneId, key, (row) => {
          ...
          row.h = next;
          // Поля разобранного вердикта уходят вместе с ним. Причина отступления,
          // ссылка на решение и вид неприменимости описывают вердикт, который
          // только что сняли: на строке «ждём» они читаются как «строка
          // разобрана». Прежний факт при этом не теряется — он переписан выше в
          // текст вердикта. К 14 сентября таких полей накопилось 98 в десяти
          // зонах, и гейт семантики краснел на каждом.
    ...
    if (changed.length) {
      // Самое опасное: строка переписана, а факт под ней остался от прежнего
      // текста. Вердикт снят в долг — здесь только перечисляем, что перечитать.
      console.log(`  вердикт снят, перечитайте: ${show(changed)}`);
    }
    return;
  }
