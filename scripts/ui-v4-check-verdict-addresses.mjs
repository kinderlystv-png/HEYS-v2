...
const BASELINE = {
  ...
  beyond: 0,
  absent: 15,
  // 2 сентября: 122 → 154 → 153. Ветка сведения UI v4 правила 123 файла, и
  // строки уехали от обычных правок выше по файлу — имя доказательства
  // осталось в том же файле, но дальше окна в 60 строк. Однозначное (имя
  // встречается в файле ровно один раз) починено автоматом: 160 → 154, плюс
  // absent вернулся к 15. Оставшиеся адреса имеют имя в файле несколько раз;
  // выбрать нужное за человека нельзя, и подстановка первого попавшегося была
  // бы ложным адресом. Минус один — возврат раскладки плитки воды: адреса её
  // строк переписаны вручную вместе с вердиктами.
  moved: 153,
};
