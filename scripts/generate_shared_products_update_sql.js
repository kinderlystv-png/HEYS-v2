// generate_shared_products_update_sql.js — dry-run SQL generator for shared_products
const fs = require('fs');
const path = require('path');

const inputPath = process.argv[2];
if (!inputPath) {
    console.error('Usage: node generate_shared_products_update_sql.js <jsonPath>');
    process.exit(1);
}
...
for (const product of products) {
    ...
    for (const col of columns) {
        ...
        const value = getValue(product, col);
        if (value === undefined || value === null || value === '') stats.nullCounts[col] += 1;
