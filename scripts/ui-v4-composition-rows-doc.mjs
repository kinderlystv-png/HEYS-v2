  ...
  const lines = [
    ...
    ...(meta.excluded && meta.excluded.length
      ? [
          ...
          '',
          'Строки оставлены в файле, чтобы решение не потерялось и не всплыло',
          'третий раз. Разбирать их заново не нужно — все семь закрыты.',
          '',
          '| Строка | Чем закрыта |',
          '| --- | --- |',
          ...CYCLE_RESOLVED.map(([key, how]) => `| \`${key}\` | ${how} |`),
        ]
      : []),
...
// Семь строк Цикла, которые 10 сентября сначала были перенесены в «цвет и роль»,
// а затем разобраны дизайнером поштучно. Здесь они лежат уже с исходом, а не с
// ожиданием: четыре были нашей правкой и сведены, две несли ещё и форму и тоже
// сведены, одна снята как не расхождение вовсе. Восьмой строки, о которой писал
// дизайнер, не существует — ошибка его счёта, признана им же.
const CYCLE_RESOLVED = [
...
const CYCLE_COLOR_ROLE = new Set(CYCLE_RESOLVED.map(([key]) => key));
