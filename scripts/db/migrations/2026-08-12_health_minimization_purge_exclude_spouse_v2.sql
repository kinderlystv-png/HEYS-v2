-- Health minimization purge v2: spouse exclude expands beyond cycle.
-- Owner decision 2026-08-12: Александра (4545ee50-4f5f-4fc0-b862-7ca45fa1bafc)
-- keep cycle + measurements + supplements (семейный аккаунт, не клиент,
-- исключение до первого клиента). Replaces purge_health_minimization_data_v1.
...
BEGIN
  ...
  -- ---- MEASUREMENTS / SUPPLEMENTS (skip excluded clients) ----
  UPDATE public.client_kv_store kv
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'kept_reason', 'owner_exception_spouse_family_account_cycle_measurements_supplements',
...
COMMENT ON FUNCTION public.purge_health_minimization_data_v1(TEXT, BIGINT, BIGINT, BIGINT, BIGINT, UUID[]) IS
  'Destructive health-minimization purge. Excludes cycle+measurements+supplements for p_exclude_client_ids (default: spouse family account). Owner command required.';
