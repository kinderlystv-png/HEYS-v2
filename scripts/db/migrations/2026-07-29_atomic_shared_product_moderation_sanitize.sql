-- 2026-07-29: harden typed product mapping for atomic moderation.
--
-- Pending product_data can contain browser-local metadata such as
-- id="p_..." and timestamps. Those keys are not part of the moderation patch
-- contract and must never reach jsonb_populate_record for shared_products.
-- Keep the original atomic behavior while filtering through an explicit
-- allowlist of columns that the UPDATE below can apply.
...
BEGIN
  ...
  ELSE
    ...
    ELSE
      ...
      -- Explicit product-update contract: browser metadata (notably local
      -- string ids like "p_...") must not be cast into typed table columns.
      SELECT coalesce(jsonb_object_agg(entry.key, entry.value), '{}'::jsonb)
      INTO v_patch
      FROM jsonb_each(v_patch) AS entry(key, value)
      WHERE entry.key = ANY(ARRAY[
        'name', 'brand', 'brand_fingerprint', 'name_norm',
        'barcode', 'barcodes',
        'simple100', 'complex100', 'protein100', 'badfat100', 'goodfat100',
        'trans100', 'fiber100', 'gi', 'harm', 'category', 'portions',
        'description', 'sodium100', 'omega3_100', 'omega6_100', 'nova_group',
        'additives', 'nutrient_density', 'is_organic', 'is_whole_grain',
        'is_fermented', 'is_raw',
        'vitamin_a', 'vitamin_c', 'vitamin_d', 'vitamin_e', 'vitamin_k',
        'vitamin_b1', 'vitamin_b2', 'vitamin_b3', 'vitamin_b6', 'vitamin_b9',
        'vitamin_b12', 'calcium', 'iron', 'magnesium', 'phosphorus',
        'potassium', 'zinc', 'selenium', 'iodine'
      ]::text[]);
...
-- scripts/db/migrations/2026-07-29_atomic_shared_product_moderation.sql.
