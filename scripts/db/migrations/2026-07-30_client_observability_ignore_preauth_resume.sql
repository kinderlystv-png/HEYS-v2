-- Exclude stale foreground-resume telemetry that never reached a client context
-- from visit metrics. Real warnings and failures remain visible.
-- ROLLBACK: restore client_app_visit_summary_v1 from
-- 2026-07-25_client_observability_signal_quality.sql.
...
SELECT
  ...
  CASE
    ...
    WHEN bool_or(
        ...
        AND event_context->>'visit_kind' = 'resume'
        AND event_context->>'auth_state' = 'pending'
      )
      AND bool_or(
      ...
      AND count(*) FILTER (
        WHERE event_name IS NOT NULL
          AND event_name NOT IN ('visit_started', 'app_foregrounded')
      ) = 0
      AND NOT COALESCE(bool_or(
        (level = 'error' AND NOT transient_api_retry)
        OR level = 'warn'
        OR event_status IN ('degraded', 'timeout', 'failed')
        OR event_name IN ('boot_failed', 'app_runtime_failed', 'sync_cycle_failed', 'write_failed')
      ), false)
      AND max(client_ts) < now() - interval '90 seconds' THEN 'ignored'
...
SELECT * FROM traced_visits WHERE outcome <> 'ignored'
UNION ALL
SELECT * FROM login_without_trace;

COMMENT ON VIEW public.client_app_visit_summary_v1 IS
  'Structured visits with routine deviations filtered and stale pre-auth resume-only telemetry excluded from visit metrics.';
