...
export const LEGACY_SCHEMA_BASELINE = Object.freeze({
  ...
  'home-widgets': Object.freeze({
    ...
    // по всем свойствам после починки Manrope — вердикт стал «=».
  ...
  'tab-activity': Object.freeze({
    ...
    // 15.09, сборки 71–72: ещё четыре «≠» и три «—» ушли тем же путём —
    // дизайнер переписал строки подписи расхода и ряда зон, вердикты снялись
    // в «?». Как и прежде, это НЕ прогресс типизации: долг не разобран, он
    // вышел из счёта вместе со знанием. Затянуть храповик всё равно обязаны —
    // он считает текущее состояние, а не намерение.
    typedMismatch: [18, '169a78ffed5cab91'],
    notApplicable: [59, '3bfb3f7b0ecb4406'],
  }),
  tips: Object.freeze({
    // 06.09, вечер: 344 legacy «≠» типизированы owner-decision по критерию
    // дизайнера (см. UI_V4_FINDINGS_HISTORY#legacy-mismatch-owner-decision-2026-09-06).
