...
/**
 * Храповик дублей меток кадров по зонам. Одна метка на два кадра означает, что
 * лист приёмки описывает один кадр дважды: кодер сводит код со второй копией и
 * пишет «сошлось» про разметку, которой на канвасе не видно. В `tips` такой ряд
 * из десяти скрытых кадров-дубликатов дал 129 фантомных строк из 507 и прожил
 * месяц — дизайнер удалил его пакетом 42.
 *
 * Порог только вниз: 6 сентября замер по всем 28 канвасам даёт ноль и по
 * кадрам, и по строкам. Появление дубля роняет гейт, а не пропускается молча.
 *
 * Обёртка `ui-v4-check-reverse-coverage-gate.mjs` ссылалась на эту константу с
 * коммита 0e337d5a7 и падала на импорте — то есть проверка была не «зелёной», а
 * мёртвой: `pnpm ui:v4:check` обрывался на ней три недели.
 */
export const DUPLICATE_FRAME_BASELINE = Object.freeze({
  tips: Object.freeze([0]),
});
