...
function pickSearchMatch(query, matches) {
  ...
  // Последняя ветка перед переспросом: карточки-близнецы. В общей базе живут
  // дубли одного продукта с разными названиями — «люля куриная на гриле» и
  // «на шпажках», 154 и 152 ккал. Различить их человек не может, потому что
  // различать нечего: разница ниже погрешности домашних весов.
  //
  // Сравнение — по составу целиком (тем же sameAggregateComposition с допуском
  // 0.05, что и схлопывание own/shared), а НЕ по калорийности. Калории —
  // производная и вырожденная: 10 г белка и 3.3 г жира дают одинаковые ~30
  // ккал, так что допуск по ним схлопнул бы яичный белок со сливками.
  //
  // Проверка стоит последней намеренно: выше неё ветки точного имени и
  // единственного own — надёжные пути, и перебивать их близостью состава
  // нельзя. Кандидаты возвращаются рядом с выбранным, чтобы вызывающий мог
  // назвать вслух, какую карточку взял.
  const top = scored.slice(0, 5);
  const twins = top.every((s) => sameAggregateComposition(s.product, best.product));
  if (twins && top.length > 1) {
    return { ok: true, product: best.product, twins: top.map((s) => s.product) };
  }
