    ...
    case 'initialize': {
      ...
      return rpcResult(id, {
        ...
        instructions: ctx.instructions || [
          ...
          '2а. «Как вчера» или «такой же перекус»: heys_get_day за дату-источник → meal_id из текста → heys_log_meal с copy_meal: { date, meal_id, count при «два»/«три» }. Не копируй граммы из текста get_day вручную. Новые позиции из той же реплики передавай полем items рядом с copy_meal — они попадут в тот же приём, второй вызов не нужен.',
