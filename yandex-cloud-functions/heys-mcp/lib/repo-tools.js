 ...
 *
 * Когда звать. Fallback, когда локального репозитория нет или нужен срез
 * main/прода. Если исходники HEYS открыты в сессии — сначала локальный
 * grep/read. Узкий инструмент: вопрос про внутреннее устройство HEYS —
 * формула, порядок расчёта, где живёт правило, что делает функция. На вопросы
 * про клиентов, еду, задачи и переписку он не нужен — правило в curator.js.
 */
...
const REPO_TOOL_SCHEMAS = [
  {
    ...
    description: 'Найти в исходниках HEYS (read-only срез main), где и как что-то устроено. Fallback: когда локального репо нет или нужен main/прод; при открытом heys-v2 — локальный grep/read. Только внутреннее устройство — «как считается», «откуда берётся», «где задано»; не для клиентов, дневников и задач. Находки помечены видом файла — отвечай по «коду», не по фикстуре.',
    inputSchema: {
      type: 'object',
      properties: {
