    ...
    if (Number.isFinite(pieces) && pieces > 0) {
      ...
      if (!perPiece) {
        throw new ToolError(
          ...
          `${label}: у продукта «${product.name}» не задан вес одной штуки. СНАЧАЛА ПОИЩИ, он мог называть его раньше: tasks_search по имени продукта (не по слову, которым он его зовёт) и, если поиск вернул пустое с непустым skipped, греп по зеркалу задачника — пустой ответ там означает «не смотрел», а не «нет». Не нашёл — тогда спроси, сколько граммов в одной штуке или сколько весит упаковка и сколько в ней штук. Передай piece_grams — вес сохранится в карточку, и дальше «штуки» будут считаться сами.`,
...
const ITEM_SCHEMA = {
  ...
  properties: {
    ...
    piece_grams: { type: 'number', description: 'Вес одной штуки в граммах. Нужен только если в карточке продукта его ещё нет: инструмент попросит, а полученное значение сохранит в карточку. Если куратор называет вес упаковки и число штук («330 г, 8 сосисок»), посчитай сам и передай сюда — иначе цифра останется в переписке и через неделю её будут искать заново.' },
    new_product: {
      type: 'object',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        protein_coeff_g_per_kg: { type: 'number', description: 'Цель по белку в граммах на килограмм веса, 1.2–2.4 — именно она задаёт норму белка. Не задана — считается по режиму: дефицит 1.8 (ж 1.6), поддержание 1.6 (ж 1.4), набор 1.8 (ж 1.6), плюс 0.2 в день с тренировкой. Передай 0, чтобы снять своё значение и вернуть расчёт по режиму. Доля белка в процентах (protein_pct) норму белка НЕ задаёт — она влияет только на то, как остаток калорий делится между углеводами и жиром.' },
      },
    },
  },
  {
