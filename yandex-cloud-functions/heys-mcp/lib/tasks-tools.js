...
const TASKS_TOOL_SCHEMAS = [
  ...
  {
    name: 'tasks_search',
    description: 'Поиск по всему задачнику сразу — задачи, журнал, дни, деньги, разовые отчёты. Возвращает найденные строки вместе с соседними, чтобы было видно контекст. Вызывай ПЕРЕД разбором новой вводной: в журнале почти всегда уже есть прошлый разговор по этой теме. Если что-то в проход не поместилось, ответ называет эти файлы поимённо — «ничего нет» по непрочитанному говорить нельзя, сначала загляни в них через tasks_read.',
    inputSchema: {
      type: 'object',
      properties: {
...
const TASKS_WRITE_SCHEMAS = [
  ...
  },
  {
    ...
    description: 'Закрыть содержательный обмен перед финальным ответом: обязательно дописать стенограмму обеими репликами целиком, а если из разговора следует решение, отвергнутая версия, открытый вопрос или новый следующий шаг — тем же вызовом дописать краткий вывод в журнал. Это последний пишущий вызов хода. Факты о жизни сюда не смешиваются: сначала запиши каждый из них через tasks_learn с kind «факт», затем вызови checkpoint. Без journal_block допустим только обмен, из которого ничего устойчивого не следует. Запись задним числом — через date (ISO или «вчера»): иначе блок уедет в сегодняшний transcript/, даже если в шапке стоит вчерашнее время.',
    inputSchema: {
      type: 'object',
      properties: {
