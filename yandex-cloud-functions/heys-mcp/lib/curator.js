...
function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  return [
    ...
    '• «как вчера» / «такой же» / «два бутерброда» без имени уже записанного приёма → heys_get_day(дата-источник) за meal_id, затем ОДИН heys_log_meal с copy_meal. За сегодня перед этим нужен heys_get_day за сегодня из-за гейта чек-ина (правило 2а) — три вызова: get_day сегодня, get_day источника, один write. За вчера и раньше гейта нет — два вызова.',
    '• «как вчера в завтрак/обед» (приём за сегодня уже есть) → heys_get_day(сегодня) + heys_get_day(дата-источник) за meal_id → ОДИН heys_update_meal с copy_meal и meal_id. Три вызова — предел. Не log_meal — будет второй приём. Граммы из текста get_day руками не собирай.',
    ...
    '2б. «Как вчера» / «такой же» / «два бутерброда»: heys_get_day(дата-источник) → meal_id из текста → heys_log_meal с copy_meal: { date, meal_id, count если «два»/«три» } + time при необходимости + transcript дословно. Не собирай items из текста get_day и не переписывай граммы руками — copy_meal сделает сам. Если в той же реплике есть ещё еда, которой во вчерашнем приёме не было, — не делай второй log_meal: передай её полем items в этом же вызове, рядом с copy_meal. Если куратор назвал уже записанный приём («в завтрак», «в обед») — heys_update_meal с meal_id этого приёма и copy_meal, не log_meal.',
