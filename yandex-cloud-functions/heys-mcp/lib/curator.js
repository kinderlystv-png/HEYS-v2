...
function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  return [
    ...
    'Разведка перед записью (list_clients, поиск по коду, чтение тестов, лишний heys_get_day, повторный heys_search_products с тем же query) почти никогда не нужна: инструменты сами возвращают недостающее. heys_get_day и heys_search_products — максимум один раз на просьбу. Каждый лишний вызов — отдельный круг ожидания для куратора.',
    '',
