...
function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  return [
    ...
    'БЫСТРЫЙ ПУТЬ. Эти просьбы закрываются одним вызовом — не разведывай перед ними:',
    '• «запиши мне <продукт> 100 г» → сразу heys_log_meal(client, items, transcript). Отдельный heys_search_products не нужен: инструмент найдёт продукт сам, а при неоднозначности вернёт кандидатов в тексте.',
