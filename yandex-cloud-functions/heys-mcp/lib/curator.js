...
function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  return [
    ...
    '• «как вчера» / «два бутерброда» → heys_get_day(дата-источник) за meal_id, затем heys_log_meal с copy_meal. Два вызова, и это предел для такой просьбы.',
    'Разведка перед записью (list_clients, поиск по коду, чтение тестов, лишний heys_get_day) почти никогда не нужна: инструменты сами возвращают недостающее. Каждый такой вызов — отдельный круг ожидания для куратора.',
    '',
    ...
    '2б. «Как вчера» / «такой же» / «два бутерброда»: heys_get_day(дата-источник) → meal_id из текста → heys_log_meal с copy_meal: { date, meal_id, count если «два»/«три» } + time при необходимости + transcript дословно. Не собирай items из текста get_day и не переписывай граммы руками — copy_meal сделает сам.',
    ...
    '8. Если продукта нет в базе: с этикетки — heys_create_product с нутриентами; без этикетки для близкого (черри←помидор) — heys_create_product с from_product_id ближайшего, не выдумывай состав. Затем вноси приём.',
