...
const TASKS_INSTRUCTIONS = [
  ...
  'З6. Каждый содержательный обмен закрывай tasks_checkpoint последним пишущим вызовом перед финальным ответом. В transcript_block передавай его реплику ДОСЛОВНО и целиком, без правок и пересказа, и свой ответ так же полно; инструмент сам выберет transcript/ГГГГ-ММ-ДД.md и откажет, если нет одной из сторон. Раньше от твоего ответа брались одни выводы, и это оказалось дырой: 04.08 марка машины прозвучала в твоём разборе аккумулятора, в стенограмму не попала и уцелела случайно — журнальной записью часом позже. Деталь, из которой сделан вывод, теряется первой, а нужной оказывается через день. Поэтому сохраняй всё содержательное: числа, марки, названия, артикулы, цены, сроки, отвергнутые версии и причину отказа. Сжимать можно только форму — таблицу разверни строками, повтор убери. Заголовок обмена — время «## ЧЧ:ММ» или диапазон «## ЧЧ:ММ–ЧЧ:ММ», реплики помечай «**Кин:**» и «**Claude:**». Вывод терминала, логи и промежуточную возню сжимай до строки-сводки в квадратных скобках. Это не журнал: стенограмма — сплошная запись разговора и единственное место, где деталь лежит целиком; журнал — только то, что из разговора следует. Факты о жизни сначала записывай отдельно через tasks_learn с kind «факт», затем делай checkpoint. Пропустил обмен — допиши следующим ходом с пометкой «задним числом». Формат — transcript/README.md.',
