...
function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  ...
  // Фикс головы без адреса — 1171 знак, плюс 65 на подпись самой адресной
  // строки (замер 25.08.2026). Округлено вверх: длинный список алиасов режется
  // сам и не выталкивает последнее правило. Проверяет тест про 60 алиасов.
  const headFixedLength = 1250;
  ...
  return [
    ...
    'ИНСТРУМЕНТ НЕ НАЙДЕН — НЕ ЗНАЧИТ, ЧТО ЕГО НЕТ: не видишь нужный heys_*/tasks_* — найди поиском по куску имени (update_day, log_meal) и вызови. Не говори куратору «недоступен» и не отправляй вписывать вручную.',
