...
const TRANSCRIPT_ARG = {
  type: 'string',
  description: 'Дословная полная реплика куратора, из которой следует эта запись. Коннектор автоматически сохранит её вместе с подтверждённым результатом инструмента в стенограмму; не пересказывай и не сокращай.',
};
    ...
    for (const name of WRITE_TOOLS) {
      ...
      tools[name] = async (args = {}) => {
        ...
        if (!transcript) {
          throw new ToolError(
            'transcript_required',
            'Запись не сделана: передай transcript с дословной полной репликой куратора. Коннектор сохранит её вместе с результатом записи, чтобы в стенограмме не было немого провала.',
          );
        }
