...
function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  return [
    ...
    '• «как вчера» / «такой же» / «два бутерброда» → heys_get_day(дата-источник) за meal_id, затем ОДИН heys_log_meal с copy_meal. Два вызова — предел для такой просьбы.',
    ...
    '2б. «Как вчера» / «такой же» / «два бутерброда»: heys_get_day(дата-источник) → meal_id из текста → heys_log_meal с copy_meal: { date, meal_id, count если «два»/«три» } + time при необходимости + transcript дословно. Не собирай items из текста get_day и не переписывай граммы руками — copy_meal сделает сам. Если в той же реплике есть ещё еда, которой во вчерашнем приёме не было, — не делай второй log_meal: передай её полем items в этом же вызове, рядом с copy_meal.',
