...
const TASKS_INSTRUCTIONS = [
  ...
  'З6. Каждый содержательный обмен, который не записывается через пишущий HEYS-инструмент, закрывай tasks_checkpoint последним пишущим вызовом перед финальным ответом. Для записи в дневник передавай обязательный transcript — инструмент сам положит дословную реплику куратора и подтверждённый результат в transcript/ГГГГ-ММ-ДД.md. В обычном checkpoint передавай обе реплики целиком, без пересказа; заголовок — «## ЧЧ:ММ», стороны — «**Кин:**» и «**Claude:**». Стенограмма хранит разговор, journal — только вывод. Факт о жизни сначала запиши через tasks_learn с kind «факт». Пропустил обмен — допиши следующим ходом с пометкой «задним числом». Формат — transcript/README.md.',
