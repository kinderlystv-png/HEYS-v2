...
function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  return [
    ...
    '2а. Гейт чек-ина за сегодня проверяет сам сервер при записи еды: чек-ин не закрыт — heys_log_meal вернёт checkin_required, тогда сначала heys_checkin. Предварительный heys_get_day ради статуса не нужен (в его тексте статус по-прежнему виден строкой «Чек-ин:»). Прошлые и будущие даты пишутся без гейта.',
