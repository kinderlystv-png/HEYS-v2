...
const TASKS_INSTRUCTIONS = [
  ...
  'З5. После разбора запиши его в журнал (tasks_append в journal/ГГГГ-ММ.md) в том же ходе — но пиши не ход разговора, а то, что из него следует: к чему пришли, какая версия отвергнута и почему, что осталось открытым, ссылка на заведённую задачу. Ход разговора целиком лежит в стенограмме, и второй его копией журнал быть не должен: за четыре дня он вырос до размера самой стенограммы, то есть перестал экономить чтение. Отвергнутое называй прямо («версия про клемму снята замером 4.37 В») — иначе поиск вернёт снятое наравне с верным, и какое из двух действует, будет видно только тому, кто прочитает весь день. Весь смысл tasks_context держится на том, что в журнале есть вывод прошлого разговора, — но туда он попадает только так. Захват без разбора («просто запиши») журнал не трогает.',
  'З6. В том же ходе дописывай стенограмму — tasks_append в transcript/ГГГГ-ММ-ДД.md: его реплику ДОСЛОВНО и целиком, без правок и пересказа, и свой ответ так же полно. Раньше от твоего ответа брались одни выводы, и это оказалось дырой: 04.08 марка машины прозвучала в твоём разборе аккумулятора, в стенограмму не попала и уцелела случайно — журнальной записью часом позже. Деталь, из которой сделан вывод, теряется первой, а нужной оказывается через день. Поэтому сохраняй всё содержательное: числа, марки, названия, артикулы, цены, сроки, отвергнутые версии и причину отказа. Сжимать можно только форму — таблицу разверни строками, повтор убери; Заголовок обмена — время «## ЧЧ:ММ» или диапазон «## ЧЧ:ММ–ЧЧ:ММ», реплики помечай «**Кин:**» и «**Claude:**». Тема вместо времени не пройдёт — tasks_append отклонит запись, не создав её: по времени день восстанавливается, по теме — только глазами. Вывод терминала, логи и промежуточную возню сжимай до строки-сводки в квадратных скобках. Это не журнал: стенограмма — сплошная запись разговора и единственное место, где деталь лежит целиком; журнал — только то, что из разговора следует. Ты в неё пишешь, а читаешь по делу: контекст поднимай из журнала, а за подробностями дня иди сюда. Пропустил обмен — допиши следующим ходом с пометкой «задним числом». Формат — transcript/README.md.',
