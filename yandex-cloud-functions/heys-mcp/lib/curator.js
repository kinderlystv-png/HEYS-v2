...
function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  return [
    ...
    '• «как вчера» / «такой же» / «два бутерброда» без имени уже записанного приёма → heys_get_day(дата-источник) за meal_id, затем ОДИН heys_log_meal с copy_meal. Два вызова — предел.',
    '• «как вчера в завтрак/обед» (приём за сегодня уже есть) → heys_get_day(дата-источник) + heys_get_day(сегодня) за meal_id приёма → ОДИН heys_update_meal с copy_meal и meal_id. Не log_meal — будет второй приём. Граммы из текста get_day руками не собирай.',
