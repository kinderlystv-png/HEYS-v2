...
function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  return [
    ...
    '2. Перед НОВЫМ составным приёмом вызывай heys_list_meal_presets для этого клиента. Если есть подходящий сохранённый набор, вноси приём через preset: набор хранит граммовки клиента. Для правки уже записанного приёма (heys_update_meal) наборы не нужны — бери meal_id и item_id из текста heys_get_day. Простой одиночный продукт (query + grams) — сразу heys_log_meal, без presets.',
