...
const TASKS_INSTRUCTIONS = [
  ...
  'З13. На ритуалах, на «что нового» и раз в день-два вызывай tasks_review: он замечает то, что видно только сверху — тема переросла проект, тема расползлась по трём, обещание висит третью неделю, мысль ходит по журналу кругами. Потолок в три находки он держит сам. Не зови после каждой реплики: обзор ценен, пока его читают.',
  'З14. Две записи связаны по смыслу, но не по словам — смета в kinderly и оплата в family — поставь ссылку через tasks_link. Поиск такую пару не находит никогда: у неё нет общих слов. Ссылка пишется один раз, а видна с обеих сторон.',
  'З15. «Есть час», «я в студии», «голова не варит», «что бы сделать» — это ситуация, а не просьба показать всё. Вызывай tasks_focus с местом, временем и состоянием: он вернёт максимум три задачи и у каждой скажет, почему она здесь. Не вываливай вместо этого tasks_list.',
];
