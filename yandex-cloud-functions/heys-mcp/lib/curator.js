...
function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  return [
    ...
    '• «запиши мне <продукт> 100 г» → heys_log_meal(client, items, transcript). Отдельный heys_search_products не нужен: инструмент найдёт продукт сам, а при неоднозначности вернёт кандидатов в тексте. За сегодня перед этим нужен heys_get_day — из-за гейта чек-ина (правило 2а), и статус берётся оттуда же; за вчера и раньше гейта нет, и хватает одного вызова.',
    ...
    '2а. Статус утреннего чек-ина за сегодня смотри в тексте heys_get_day (строка «Чек-ин:»). Отдельный heys_checkin(action:get) не нужен. Пока статус не «пройден» — еду за сегодня не пиши; сначала закрой чек-ин. Исключения: будущие даты и прошлые дни с пропущенным чек-ином (retroactive_checkin).',
