...
/**
 * Инструменты, чей ответ несёт `day_after` — состояние дня по версии сервера.
 *
 * Правило «сверь day_after с тем, что ожидал» жило в инструкции и было ровно
 * тем местом, где клиент её обрывал (замер 22.08.2026): до модели доезжало
 * «инструмент возвращает day_». Описания инструментов доезжают целиком —
 * значит правилу место здесь, рядом с тем, кто это поле и возвращает.
 */
const DAY_AFTER_TOOLS = new Set([
  'heys_log_meal',
  'heys_update_meal',
  'heys_delete_meal',
  'heys_add_water',
...
const DAY_AFTER_RULE = ' В ответе day_after — сверь с ожидаемым, не сошлось — скажи куратору; stale_write_blocked = записи нет.';
...
function buildCuratorSchemas({ requireTranscript = false } = {}) {
  ...
  const schemas = TOOL_SCHEMAS.map((schema) => {
    ...
    return {
      ...
      description: (schema.name === 'heys_create_product'
        ...
        + (DAY_AFTER_TOOLS.has(schema.name) ? DAY_AFTER_RULE : ''),
      inputSchema: {
        ...schema.inputSchema,
        properties: {
  ...
  schemas.push({
    ...
    // Голос в переписке принадлежит куратору. До 22.08.2026 описание говорило
    // обратное — «внёс еду → напиши клиенту», — и это был единственный канал,
    // который до модели доезжал: правило 25 «отвечать по решению куратора»
    // лежало в хвосте инструкции, обрезанном клиентом на 2048 символах.
    description: 'Ответить клиенту в мессенджере приложения. Пиши клиенту только если куратор прямо попросил ответить. После записи в дневник, не вместо неё.',
    inputSchema: {
      type: 'object',
...
/**
 * Сколько инструкции реально доезжает до модели.
 *
 * Замер 22.08.2026: сервер отдал 45 316 символов, клиент (Claude Code) обрезал
 * на 2048 — посреди слова, на правиле про `day_after`. Всё, что ниже, модель не
 * видела никогда: «БЫСТРЫЙ ПУТЬ», «СЧЁТЧИК ВЫЗОВОВ», запреты разведочных
 * вызовов. Полгода правил писались в стол, а разборы трейсов списывали это на
 * непослушание модели.
 *
 * Отсюда бюджет: критическое обязано уместиться в голову с запасом к
 * замеренному потолку. Запас — потому что 2048 это факт одного клиента и одной
 * его версии, а не контракт: у ChatGPT-эндпоинтов лимит свой, и он может
 * измениться без предупреждения.
 *
 * Правило размещения (README, «Инструкции доезжают не целиком»): правило
 * кладётся в самый слабый канал, который ещё справляется с задачей. Про один
 * инструмент — в его описание; реактивное — в текст ответа; сюда попадает
 * только то, что действует ДО первого вызова и не привязано к инструменту.
 */
const INSTRUCTIONS_HEAD_BUDGET = 1800;

/**
 * Алиасы — самая ценная строка головы (запись не тому клиенту — худшая ошибка
 * режима) и единственная, чья длина растёт сама: с каждым новым клиентом и
 * каждым новым прозвищем. Поэтому режем именно её, а не правила рядом.
 */
function fitAddressLine(addressLine, budgetLeft) {
  if (!addressLine || addressLine.length <= budgetLeft) return addressLine;
  const tail = '… остальные — heys_list_clients.';
  if (budgetLeft <= tail.length) return tail;
  const cut = addressLine.slice(0, budgetLeft - tail.length);
  const lastSep = cut.lastIndexOf('; ');
  return `${lastSep > 0 ? cut.slice(0, lastSep + 1) : cut} ${tail}`;
}

function curatorInstructions(curatorName, withTasks = false, nowMs = Date.now(), withRepo = false, addressLine = '') {
  // Голова считается первой и с известным бюджетом: остальное дописывается
  // ниже и обрезается клиентом без потери критического.
  //
  // Состав головы пересобран 25.08.2026 по телеметрии за 7 дней (2301 вызов):
  // адресация свою войну выиграла (heys_list_clients — 1 вызов за неделю), а
  // повторные вызовы живы (search_products → search_products 70 раз), потому
  // что правило про них лежало ниже обрыва. Место перераспределено в пользу
  // того, что серверная механика поймать не может. Снимок прежнего набора — в
  // OPTIMIZATION_LOG.md, запись «снимок головы инструкции перед перестройкой».
  // Фикс головы без списка алиасов — девять правил, 1348 знаков вместе с
  // подписью адресной строки (замер 25.08.2026). Значение подобрано перебором:
  // 1450 — минимальное, при котором все девять правил остаются в бюджете и при
  // 60, и при 200 алиасах, с задачником и без. Проверяет тест про 60 алиасов.
  const headFixedLength = 1450;
  const fittedAddress = fitAddressLine(addressLine, Math.max(120, INSTRUCTIONS_HEAD_BUDGET - headFixedLength));
  return [
    `Дневники HEYS: еда, вода, сон, тренировки клиентов куратора${curatorName ? ` (${curatorName})` : ''}.`,
    '',
    fittedAddress
      ? `Адресация из памяти (передавай прямо в параметр client, не ищи): ${fittedAddress}`
      : (withTasks
    ...
    'В пишущие инструменты передавай client_id, точное имя или алиас («мне»); часть чужого имени не принимается.',
    'ЗАПРЕТ ДНЕВНИКОВОЙ АРХЕОЛОГИИ: просьба записать с алиасом — сразу heys_* с client=этот алиас (жена = жене). Перед записью запрещены list_clients, grep по файлам, чтение тестов ради параметров'
      + (withTasks ? '; tasks_context для «кто такой мне/жена» — сервер отклонит такой вызов.' : '.'),
    'КРИТИЧЕСКОЕ ПРАВИЛО РЕЖИМА: каждая запись адресная. Не знаешь КОМУ — heys_list_clients и уточни у куратора. Клиента по догадке не выбирай: запись в чужой дневник — худшая ошибка этого инструмента.',
    'В каждый пишущий вызов — обязательный transcript: дословная реплика куратора целиком, иначе сервер отклонит запись до её выполнения.',
    'ИНСТРУМЕНТ НЕ НАЙДЕН — НЕ ЗНАЧИТ, ЧТО ЕГО НЕТ: не видишь нужный heys_*/tasks_* — найди поиском по куску имени (update_day, log_meal) и вызови. Не отправляй куратора вписывать вручную. Не нашёл — так и скажи, назвав, что искал.',
    'ОДИН ВЫЗОВ НА РЕПЛИКУ: повтор вызова с теми же аргументами — ошибка, а не осторожность: нужное уже в первом ответе. Несколько приёмов, дат или задач — одним вызовом со списком, не цепочкой.',
    'ОТВЕТ НА ЗАПИСЬ НЕ ПРИШЁЛ — не повторяй её вслепую: пишущий вызов не идемпотентен, второй создаст дубль. Один heys_get_day на дату записи и смотри, попала ли.',
    // ── Ниже этой строки начинается то, что до модели может не доехать ──
    // Всё, что выше, обязано умещаться в INSTRUCTIONS_HEAD_BUDGET. Проверяет
    // тест «критическое умещается в бюджет инструкции».
    ...(withTasks ? ['Стенограмма: transcript инструмент сохранит сам вместе с подтверждённым результатом, отдельный tasks_checkpoint для такого технического ответа не нужен. Если после записи даёшь самостоятельный разбор или решение, сохрани и его обычным tasks_checkpoint.'] : []),
    ...
    '• «убери жим из плана» / «добавь ещё подход» / «поставь 70 вместо 75» → heys_get_training(дата) за id упражнений и подходов, затем ОДИН heys_update_training с exercises_add / exercises_remove / exercises_patch. Два вызова — предел; весь список упражнений заново не диктуй, правка адресная. Клиент план уже открыл — heys_propose_training_edit.',
    ...
    '• «как вчера в завтрак/обед» (приём за сегодня уже есть) → heys_get_day(сегодня) + heys_get_day(дата-источник) → ОДИН heys_update_meal с copy_meal и meal_id целевого приёма. Три вызова — предел. Не log_meal: он заведёт второй приём, если названный завтрак дальше 10 минут от текущего времени, а если ближе — молча дольёт копию в него.',
    ...
    '• СКЛЕЙКА ПО ВРЕМЕНИ (сервер, не на твоё усмотрение): запись в пределах 10 минут от уже записанного приёма ложится В НЕГО — meal_id остаётся прежним, в ответе строка «Дописал в приём». Это не отменяет правила «одна реплика — один вызов»: лишний круг стоит куратору ожидания, а склейка лишь спасает дневник от цепочки приёмов по одной позиции. Совпал весь состав до грамма — придёт possible_duplicate: это не сбой записи, а вопрос куратору «ел второй раз?»; повторять вызов с allow_duplicate: true можно только после его ответа.',
    ...
    '',
    'Каталог продуктов:',
    '15. Карточку продукта heys_update_product по умолчанию правит там, где ошибка и живёт: продукт из общей базы, у которого у клиента нет своей версии, правится в ОБЩЕЙ карточке — исправление увидят все; личная карточка и та, что клиент уже переписал под себя, правятся лично. Инструмент всегда называет в ответе, какую базу изменил; нужна другая — scope:"client" или scope:"shared". Очередь модерации (heys_moderate_products) это про клиентские публикации, а не про правку куратором уже опубликованной карточки, и обходить её здесь нечего. Рецепт (product.recipe) — только Type B, в общей базе состава нет. Save рецепта молча меняет карточку вперёд; прошлые дни не трогает. Исправление прошлого — heys_reapply_recipe после вопроса человеку (dry_run, затем confirm). Эволюция состава (майонез→сметана) = save без ретро. Правка карточки ингредиента рецепт не пересчитывает: heys_update_product сам назовёт блюда, которые от неё разошлись, — обнови каждое пустым recipe_patch:{}.',
...
function createCuratorContext({
  ...
  function apiForClient(clientId) {
    return {
      ...
      // Правка общей карточки — кураторская операция: REST-шлюз требует
      // curator-JWT и клиентской сессией сюда не попасть в принципе.
      async updateSharedProduct(productData) {
        return api.updateSharedProduct(curatorJwt, productData);
      },
    };
  }
...
module.exports = {
  createCuratorContext,
  buildCuratorSchemas,
  curatorInstructions,
  CLIENTLESS_TOOLS,
  INSTRUCTIONS_HEAD_BUDGET,
};
