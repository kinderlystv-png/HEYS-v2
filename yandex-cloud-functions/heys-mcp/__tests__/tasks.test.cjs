...
test('patchBlock отказывает, когда ТОЧНАЯ строка-якорь встречается дважды', () => {
  // Инцидент 08.09 / heys: строка «- окно: ...» стоит в projects/heys.md у
  // фазы 1 и у фазы 2. Патч взял первое вхождение началом, второе концом и
  // снёс 18 строк между ними — вместе с задачей «Привести код к макету v4» и
  // шестью соседними. Ошибки не было: проверка на неоднозначность работала
  // только для подстроки, а точное совпадение бралось первым молча.
  const text = [
    '- [ ] Привести код к макету v4',
    '  - окно: 2026-09-04..2026-09-08',
    '- [ ] Соседняя задача',
    '- [ ] Фаза 2',
    '  - окно: 2026-09-04..2026-09-08',
  ].join('\n');
  assert.throws(
    () => tasks.patchBlock(text, {
      from: '- окно: 2026-09-04..2026-09-08',
      to: '- окно: 2026-09-04..2026-09-08',
      replacement: '  - окно: 2026-09-08..2026-09-17',
    }),
    /anchor_ambiguous:from:2:/,
  );
  // Кандидаты названы номерами строк — иначе якорь нечем уточнить.
  try {
    tasks.patchBlock(text, { from: '- окно: 2026-09-04..2026-09-08', replacement: 'x' });
    assert.fail('ожидался отказ');
  } catch (e) {
    assert.match(String(e.message), /2:.*\|.*5:/);
  }
});

test('patchBlock называет размер правки — сколько снял и сколько вписал', () => {
  // Патч на «замени одну строку», снявший восемнадцать, обязан быть виден в
  // ответе. Чистая дельта строк это скрывает: замена трёх строк тремя даёт ноль.
  const text = '## Раздел\n- один\n- два\n- три\n## Конец\n';
  const out = tasks.patchBlockWithStats(text, {
    from: '- один',
    to: '## Конец',
    replacement: '- новый',
  });
  ...
  assert.deepEqual(out.removedLines, ['- один', '- два', '- три']);
  assert.match(out.text, /- новый\n## Конец/);

  const one = tasks.patchBlockWithStats(text, { from: '- два', replacement: '- два·' });
...
test('checkpoint отклоняет знаки без слов вместо реплики Кина', async () => {
  // 4 сентября блок 03:20 встал в стенограмму с «**Кин:** ?»: сторона непустая,
  // гейт пропустил, обмен потерян насовсем. У стороны Claude проверки на
  // подмену содержания были с самого начала, у стороны Кина — ни одной.
  const api = liveTasksApi();
  for (const stub of ['?', '…', '—', '?!']) {
    await assert.rejects(
      () => session(api).tasks_checkpoint({
        transcript_block: `## 03:20

**Кин:** ${stub}
**Claude:** разобрал вопрос и записал вывод: три поиска, два чекпоинта, порядок по времени держится.`,
      }),
      (e) => e.code === 'verbatim_transcript_required',
      `заглушка «${stub}» должна быть отбита`,
    );
  }
  assert.equal(api.kv[tasks.keyForPath(TRANSCRIPT_TODAY)], undefined);
});

test('короткая реплика Кина и явный пропуск проходят', async () => {
  // Ловится не краткость, а отсутствие слов: «да», «б», «давай.» — обычные
  // реплики, и отбивать их значило бы менять один вид потери на другой.
  // Невосстановимая реплика пишется словами и явно.
  for (const kin of ['да', 'б', 'давай.', 'делай сам', '[реплика не восстановлена]']) {
    const api = liveTasksApi();
    const saved = await session(api).tasks_checkpoint({
      transcript_block: `## 03:20

**Кин:** ${kin}
**Claude:** разобрал вопрос и записал вывод: три поиска, два чекпоинта, порядок по времени держится.`,
    });
    assert.equal(saved.structured.checkpoint, true, `реплика «${kin}» должна быть принята`);
  }
});
