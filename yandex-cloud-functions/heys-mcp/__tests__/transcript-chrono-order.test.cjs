...
/**
 * Порядок держится инвариантом, а не дисциплиной вызывающих.
 *
 * 04.09: tasks_checkpoint писал стенограмму chrono, а tasks_append — append,
 * хотя заголовок стенограммы проверял прямо перед записью. 29 файлов из 35
 * лежали вразнобой, до 23 разрывов в дне. Первая попытка закрыть это проверкой
 * по форме кода была зелёной на возвращённом дефекте — insertBlockByTime
 * оставался в лямбде и удовлетворял её. Поэтому правило стоит в самой записи:
 * форму кода обмануть можно, дельту — нет.
 */
test('дельта mode=append в стенограмму не проходит вовсе', () => {
  const kv = require(RPC_LIB);
  for (const p of ['transcript/2026-09-04.md', 'archive/transcript_2026-09-04_part1.md']) {
    const file = kv.ensureFile(null, p);
    file.text = '## 20:10\n\nвечер';
    assert.throws(
      () => kv.applyDeltaToFile(file, 'append', '## 14:20\n\nдень', Date.now()),
      /transcript_requires_chrono/,
      `${p}: append прошёл — блок ляжет в конец независимо от своего времени`,
    );
  }
  // Журнал и проекты пишутся append законно — инвариант их не трогает.
  const journal = kv.ensureFile(null, 'journal/2026-09.md');
  journal.text = '## 2026-09-04 10:00 · раз';
  assert.ok(kv.applyDeltaToFile(journal, 'append', '## 2026-09-04 11:00 · два', Date.now()));
});
