...
test('блок правил не раздувается бесконечно', () => {
  ...
  assert.ok(bytes < 37300, `дубли вернулись: блок снова ${bytes} байт`);
});
...
test('сделанное снимается тем же инструментом и в том же ходе', () => {
  ...
  // Правило не должно спорить с запретом ставить галочки за него.
  assert.match(rule, /ставит только он/);
  const ticks = TASKS_RULES().find((l) => /Сам галочки не ставь/.test(l));
  assert.ok(ticks, 'запрет на галочки никуда не делся');
});
