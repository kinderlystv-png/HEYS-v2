...
test('блок правил не раздувается бесконечно', () => {
  ...
  // 2026-09-03: 42634 → 42968. Новых правил нет, вырос З17 — тот, что про
  // закрытие дня. Появился tasks_slot_done, и без строки о нём правило учит
  // ровно тому, что сломалось: отметить дело можно было только закрытием дня,
  // потому что галочку ставил лишь tasks_close_day, а он обязан писать заметку
  // «> как прошло» — признак закрытого дня для доски. 3 сентября отметка трёх
  // утренних дел объявила день законченным, и одиннадцать ещё не наступивших
  // пунктов стали читаться как «не состоялись». Правило без этой строки
  // оставляет модель ровно в той же развилке, где ошибка уже случилась.
  assert.ok(bytes < 42990, `дубли вернулись: блок снова ${bytes} байт`);
});
...
test('критическое умещается в бюджет инструкции целыми строками', () => {
  ...
  for (const marker of [
    ...
    // Резервные направления: что делать, когда инструмента не видно и когда
    // ответ на запись не пришёл. Обе строки бесполезны, если их обрежет —
    // они действуют ровно в тот момент, когда остальное уже не сработало.
    'ИНСТРУМЕНТ НЕ НАЙДЕН',
    'ОДИН ВЫЗОВ НА РЕПЛИКУ',
    'ОТВЕТ НА ЗАПИСЬ НЕ ПРИШЁЛ',
  ]) {
...
test('длинный список алиасов режется сам, а не выталкивает правила', () => {
  ...
  // Последнее правило головы — самое уязвимое: длинный адрес выталкивает
  // именно его. 25.08 так дважды терялось «ОТВЕТ НА ЗАПИСЬ НЕ ПРИШЁЛ», пока
  // headFixedLength не подобрали перебором.
  assert.ok(delivered.includes('ОТВЕТ НА ЗАПИСЬ НЕ ПРИШЁЛ'), 'последнее правило головы пережило длинный список');

  const huger = Array.from({ length: 200 }, (_, i) => `«оченьдлинныйалиас${i}» → Клиент Длинное Имя Отчество ${i}`).join('; ');
  const wide = curatorInstructions('Куратор', true, Date.now(), true, huger);
  const wideHead = wide.slice(0, INSTRUCTIONS_HEAD_BUDGET);
  const wideDelivered = wideHead.slice(0, wideHead.lastIndexOf('\n'));
  assert.ok(wideDelivered.includes('ОТВЕТ НА ЗАПИСЬ НЕ ПРИШЁЛ'), 'и вчетверо более длинный список тоже');
});
...
test('ответ клиенту — по просьбе куратора, а не следом за записью', () => {
  // Инверсия 22.08.2026: правило «голос в переписке — куратора» лежало в хвосте
  // инструкции, который клиент обрезает, а доезжающее описание предписывало
  // обратное — «внёс еду, напиши клиенту».
  const reply = buildCuratorSchemas({ requireTranscript: true })
    .find((schema) => schema.name === 'heys_reply_message');

  assert.match(reply.description, /только если куратор прямо попросил/);
  assert.ok(!/клиент должен видеть/.test(reply.description), 'старая формулировка звала отвечать самому');
});
