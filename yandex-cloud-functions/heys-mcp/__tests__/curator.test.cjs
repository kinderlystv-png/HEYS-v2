...
test('инструкция ведёт коротким путём и не требует разведки перед записью', () => {
  ...
  // Быстрый путь не должен обещать того, что запрещает гейт чек-ина: еда за
  // сегодня требует heys_get_day, и молчание об этом столкнуло бы два правила
  // лбами — модель либо нарушит гейт, либо пойдёт разведывать наугад.
  assert.ok(
    !/закрываются одним вызовом/.test(text),
...
test('блок правил не раздувается бесконечно', () => {
  ...
  assert.ok(bytes < 42569, `дубли вернулись: блок снова ${bytes} байт`);
});
