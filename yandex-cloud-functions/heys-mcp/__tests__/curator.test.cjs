...
test('правило про ресурсы называет оба и не требует их у каждого слота', () => {
  const rule = TASKS_RULES().find((line) => /takes/.test(line) && /whose/.test(line));
  assert.ok(rule, 'признаки слота названы в правилах');
  assert.match(rule, /машина/);
  assert.match(rule, /ребёнок/);
  // Необязательность — главное в этой паре полей: обязательное не заполняется.
  assert.match(rule, /только когда он сам это сказал/);
  assert.match(rule, /не занимает ничего/);
});

test('занятость ресурса подана фактом, а вывод из неё берётся у него', () => {
  const rule = TASKS_RULES().find((line) => /Занятость ресурса/.test(line));
  assert.ok(rule, 'правило говорит, что делать с занятым ресурсом');
  assert.match(rule, /факт, а не запрет/);
  ...
  assert.match(rule, /знает он один/);
  assert.match(rule, /ОДИН раз/);
  assert.match(rule, /tasks_learn/);
  // Порядок вызовов такие правила не предписывают.
  assert.doesNotMatch(rule, /сначала вызови/i);
});
