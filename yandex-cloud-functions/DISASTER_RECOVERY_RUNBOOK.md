...
**Какой источник строк брать.** Если строки в облаке целы и отстал только
манифест — берите живую базу (`--from-live`), а не снимок: восстановление из
снимка потеряет позиции, добавленные после него. Снимок нужен, когда потеряны
сами строки.

```bash
# строки целы, разошёлся только сторож
node restore-products-catalog.js --client-id <UUID> --from-live [--apply]
```
...
### Известное состояние прода (2026-08-22, закрыто)

У двух из трёх клиентов пара была рассогласована в живой базе: строки
обновлялись, манифест оставался от прошлой публикации.

| Клиент     | Было (строк / `rowCount`) | Стало   | Вердикт `assemble()` |
| ---------- | ------------------------- | ------- | -------------------- |
| `4545ee50` | 296 / 278                 | 296/296 | `complete`           |
| `ccfe6ea3` | 151 / 146                 | 151/151 | `complete`           |
| `02e1aff8` | 7 / 7                     | 7/7     | `complete`           |

Причина устранена в пути записи: инструменты MCP писали строки без манифеста,
теперь оба ключа пишет `products.saveOverlayRows` (см.
`apps/web/BUGS_HISTORY.md`). Пары вылечены `--from-live --apply` 2026-08-22;
прежнее состояние осталось рядом под ключами `*__before_restore_20260822085129`
и `*__before_restore_20260822085143`. Проверено независимым запросом к базе: у
всех трёх клиентов число строк равно `rowCount` манифеста. Сторож
`products_catalog_shrink` после лечения — `clean`: рост каталога тревогу не
поднимает.
