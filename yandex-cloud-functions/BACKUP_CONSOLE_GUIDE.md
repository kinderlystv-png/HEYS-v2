...
3. Установите следующие параметры:
   ...
   **Время начала резервного копирования:**
   - Установите: `22:00` UTC (факт кластера `heys-production` на 2026-08-15)
   - Примечание: это время начала окна резервного копирования

   **Срок хранения резервных копий:**
   - Установите: `14 дней` (факт `backup_retain_period_days` кластера)
...
1. **Per-Client KV Snapshots** (heys-client-daily-backup) — ежедневный снапшот
   `client_kv_store` per-client в S3 с retention 365 дней. Используется для
   точечного восстановления данных одного клиента (а не всей БД). См.
   `heys-client-daily-backup/README.md`.

2. **Репликация в другой регион** (для катастрофоустойчивости)
   - Настройка cross-region replica
   - Автоматическое failover

> Ранее предлагалось использовать самописную функцию `heys-backup` (pg_dump → S3
> с retention 7 дней). Удалена 2026-05-22: дублировала встроенный YC PG backup
> (retention 14 дней, PITR), который более надёжен и не требует поддержки.
...
**Примечание**: Эта задача выполняется вручную через веб-интерфейс и не может
быть автоматизирована в коде.
