...
# 3. ОБЯЗАТЕЛЬНО: передеплоить функции, читающие секрет
./deploy-all.sh --group all
```

> ⚠️ **Ждать «5 минут» бесполезно — тёплые контейнеры новое значение не
> подхватят.** Проверено 2026-08-03:
>
> - `shared/secrets.js` мемоизирует `initPromise` на всё время жизни контейнера:
>   overlay из Lockbox в `process.env` выполняется один раз при cold start и
>   больше не повторяется. 5-минутный TTL в `shared/lockbox-client.js` работает
>   только внутри самого клиента и на это не влияет.
> - Даже если бы overlay повторялся, пароль БД всё равно остался бы старым:
>   `shared/db-pool.js` собирает конфиг пула из `process.env` один раз при
>   первом `getPool()` и держит пул модульным синглтоном.
>
> Итог: единственный надёжный способ применить ротацию — redeploy (или иной
> рестарт всех инстансов) функций. До этого часть тёплых контейнеров продолжит
> работать со старым секретом, а по паролю БД — получать ошибки аутентификации.
...
4. **Чеклист для будущей ротации секретов:**
   ...
   - После добавления версии **обязателен redeploy функций**: `secrets.js`
     кеширует overlay на всё время жизни контейнера, поэтому тёплые инстансы
     сами новое значение не подтянут (подробности — раздел «Ротация секретов»)
