...
- В production **обязательно** заполните `TELEGRAM_ALLOWED_USER_IDS` (минимум один ID куратора). Пустой список допускается только в dev.
- Настройте `API_ALLOWED_ORIGINS`, чтобы backend принимал запросы только с нужных доменов (например, `https://curator.heysapp.com`).
