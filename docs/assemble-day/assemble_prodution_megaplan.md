...
> кураторской интеграции Текущий gate: Sprint 21 `DONE`; Sprint 22 `PARTIAL` —
> продолжается перевод каталога
...
> **Готово: 8 из 21 core-спринта (Sprint 0–5, 14 и 21); Sprint 22 в работе.**
> Текущий этап — **Sprint 22**: механизм готов, остался перевод каталога
> ситуаций на условия состояния. Отдельного приёмочного спринта больше нет: по
> `D75` владелец тестирует игру по ходу разработки, а из меню игры доступен
> сброс кампании. Очередь идёт по порядку v2.0 (§3), а не по возрастанию
> номеров.

| Статус           | Спринты                       | Что это означает                                       |
| ---------------- | ----------------------------- | ------------------------------------------------------ |
| ✅ `DONE`        | 0–5, 14, 21                   | Реализованы и зафиксированы в журнале §6               |
| 🟡 `PARTIAL`     | 22                            | Начат; остаток описан в журнале §6                     |
...
|   3 |     22 | Ситуации по состоянию вместо слот-грида      | 🟡 `PARTIAL`     | Механизм готов, понедельник переведён; 36 из 42 ещё по позиции   |
|   4 |     15 | Играбельный длинный цикл и UI периодов       | ⬜ `NOT STARTED` | Месяц возобновляем, имеет итог, первый слой остаётся простым     |
|   5 |     23 | QA-профиль под длинные кампании              | ⬜ `NOT STARTED` | Инварианты + регрессионные зёрна + длинные прогоны (`D74`)       |
|   6 |      8 | Самостоятельность семьи                      | ⬜ `NOT STARTED` | Поверх новой основы; D21–D23 реализованы компактно               |
|   7 |      9 | Экономика и карьера                          | ⬜ `NOT STARTED` | Setup reducer, честный горизонт, версионированный price-book     |
|   8 |     10 | Расширение вариативности                     | ⬜ `NOT STARTED` | Безусловный после `D73`: вопрос «сколько», а не «нужно ли»       |
...
- Контентные спринты 8, 9 и 10 начинаются только после Sprint 23: расширение
...
**Статус:** ⬜ `NOT STARTED` — шаг 8 очереди v2.0. После `D73` спринт
безусловный: вариативность принята как продуктовая цель, вопрос только в объёме.
...
**Статус:** ⬜ `NOT STARTED` — шаг 4 очереди v2.0, после Sprints 14, 21 и 22.
...
**Статус:** 🟡 `PARTIAL` 2026-07-31 — механизм готов, перевод каталога не
завершён. Подробности и остаток — в журнале §6.
...
**Статус:** ⬜ `NOT STARTED` — шаг 5 очереди v2.0, после PASS Sprint 15.
Заменяет Sprint 7 по `D74`.
