...
> кураторской интеграции Текущий gate: экономический контур в движке; остаток —
> интерфейс выбора формата и периодов
...
| ---------------- | ----------------------------- | ------------------------------------------------------ |
| ✅ `DONE`        | 0–5, 8, 10, 14, 21, 22, 23    | Реализованы и зафиксированы в журнале §6               |
| 🟡 `PARTIAL`     | 9, 15                         | Движок готов; интерфейса для новых шагов пока нет      |
...
|   6 |     15 | Играбельный длинный цикл и UI периодов       | 🟡 `PARTIAL`     | 30 дней проходятся и сохраняются; UI периодов не переделан             |
|   7 |      8 | Самостоятельность семьи                      | ✅ `DONE`        | 2026-08-01: у близких своя загрузка, окно и взаимный путь              |
|   8 |      9 | Экономика и карьера                          | 🟡 `PARTIAL`     | Setup-шаг, ритм дохода и цель есть; UI выбора формата нет              |
|   9 |     11 | QA расширенного билда                        | ⬜ `NOT STARTED` | По профилю Sprint 23, не по прежним 10 000 × 7                         |
|  10 |     12 | Наблюдение владельца по расширенному билду   | ⬜ `NOT STARTED` | Не gate по `D75`: фиксирует выводы, не разрешает следующий шаг         |
