...
> · очередь под симулятор жизни (`D70`–`D75`)<br> Дата актуализации:
> 2026-07-31<br> Область: базовая вымышленная кампания до персонального режима и
> кураторской интеграции Текущий gate: расширенный билд проверен профилем;
> следующий разрешённый prompt — Sprint 12
...
> **Готово: 12 из 21 core-спринта (Sprint 0–5, 8, 10, 14, 21, 22 и 23); Sprints
> 9 и 15 частично — движок готов, интерфейса для выбора формата и для периодов
> пока нет.** Следующий этап — довести интерфейс этих двух спринтов. Отдельного
...
| ---------------- | ----------------------------- | ------------------------------------------------------ |
| ✅ `DONE`        | 0–5, 8–11, 14, 15, 21–23      | Реализованы и зафиксированы в журнале §6               |
| ⬜ `NOT STARTED` | 8–12, 15–20, 22, 23           | Не начаты; порядок исполнения задан §3, а не номером   |
...
|   9 |     11 | QA расширенного билда                        | ✅ `DONE`        | 2026-08-01: профиль отработан, отчёт `reports/qa-profile-v1.0.json`    |
|  10 |     12 | Наблюдение владельца по расширенному билду   | ▶️ `NEXT`        | Не gate по `D75`: фиксирует выводы, не разрешает следующий шаг         |
...
**Статус:** ⬜ `NOT STARTED` — шаг 10 очереди v2.0. По `D75` это наблюдение, а
не gate: оно фиксирует выводы и не разрешает следующий шаг.
...
Это наблюдение владельца, а не gate (`D75`): оно фиксирует выводы, но не разрешает и не блокирует следующий шаг. Владелец самостоятельно проходит обновлённый расширенный билд без кода, документации и diagnostics во время решений, затем перезапускает кампанию через сброс из меню и проходит её другой стратегией. Результат запиши отдельной versioned редакцией отчёта наблюдения.
