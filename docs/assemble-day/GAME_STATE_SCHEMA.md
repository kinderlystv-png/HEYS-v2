...
interface EmploymentState {
  format: 'office' | 'remote' | 'project' | null;
  chosenAtStepIndex: number | null;
}
```

Формат выбирается отдельным шагом до планирования недели и дальше не меняется:
`format` и `chosenAtStepIndex` устанавливаются вместе. Шаг создаёт ритм дохода
на весь горизонт кампании, ставит `economy.financialGoal` и оставляет три записи
в журнале. Контент читает формат через путь `employment.format`.

## Состояние близких

У партнёра и ребёнка есть собственные `load` (0–100) и окно доступности
`windowFromMin`/`windowToMin`. Загрузка растёт, когда нагрузку переносят на
близкого, снижается, когда её забирают на себя, и спадает на границе дня.
Производный контекст отдаёт `partnerLoad`, `partnerAvailableNow` и
`childAvailableNow`, поэтому контент сравнивает окно с часами через контекст, а
не через сравнение двух путей состояния.
