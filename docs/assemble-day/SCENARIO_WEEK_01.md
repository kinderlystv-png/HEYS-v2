...
> Статус: implementation-контракт сценария<br> Набор документов: 0.56<br>
> Обновлено: 2026-08-01<br> Владелец: контент первого вертикального среза

> **Что изменилось после Sprints 15, 22 и 10.** Кампания больше не равна этой
> неделе: `CAMPAIGN_DAYS = 336`, авторская неделя задаёт первые семь дней,
> дальше идут якоря без назначенных ситуаций. Ни одна ситуация не открывается
> позицией в сценарии — все триггеры это условия на состояние и время, инвариант
> проверяется `findCursorBoundEvents`. Манифест ниже описывает авторский контент
> недели и её временные якоря, но не порядок прохождения: порядок теперь решает
> отбор.
...
Scenario v6 продлевает production-календарь до полного модельного года: 12
месяцев × 4 недели × 7 дней. После 38 якорей авторской недели оставшиеся якоря
не назначают ситуацию по позиции — их заполняет тот же state/time-driven отбор.
Scenario v4 ранее добавил content-owned теги `planned_work_window`,
...
В длинном routine-хвосте anti-spiral gate при критическом голоде допускает
только ситуации с фактически доступным food-стабилизатором. Выход
`prepare_simple_meal` включён в существующие бытовые окна, а не в одну
универсальную ситуацию: иначе годовая кампания схлопывается в один повторяющийся
сюжет. Бытовые fallback-ситуации также предлагают `rest_short`. Если деньги и
учтённые порции закончились, `prepare_simple_meal` сохраняет мягкий выход с
ценой во времени и усилии — без скрытого долга и новой экономической сущности.
