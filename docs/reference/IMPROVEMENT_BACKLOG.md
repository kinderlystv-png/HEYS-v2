...
для `verify-full` PostgreSQL и не вызывает worker ради проверки живости. RPC и
REST проверяются до трёх раз; recovery-deploy разрешён только плановому monitor,
поэтому transient сбой во время push/manual проверки не создаёт второй rollout.
...
| F11 | Scheduled health monitor выполняет внешний strict dead-man; recovery требует 3 попытки                 | `sed -n '1,190p' .github/workflows/api-health-monitor.yml && node --test yandex-cloud-functions/__tests__/function-inventory.test.cjs yandex-cloud-functions/__tests__/check-heys-ops-status.test.cjs`                                                                                                                                                                                                                                      | source contract обновлён; run `29640220848` был зелёный 2026-07-18   |
