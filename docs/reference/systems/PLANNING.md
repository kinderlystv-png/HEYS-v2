...
Color Trail сохраняет клеточный typed-array движок, но рисует территорию, след и
змей через сглаженный Canvas-слой. Пересечение открытого следа соперника лишь
помечает разрез: поражение засчитывается после успешного возвращения атакующей
змеи на свою территорию. Если соперник успел замкнуть собственный след раньше,
разрез отменяется. Свой след разрешено пересекать, а контакт голов не вызывает
скрытого сброса. Завершённый разрез удаляет отделённые от последней домашней
точки острова чужой территории. После поражения территория полностью исчезает, а
та же змея спустя короткую паузу получает новую базу вдали от места гибели;
одновременно в мире по-прежнему не больше четырёх змей. Текущий раунд ведёт
ограниченный in-memory журнал кадров и решений движка; пользователь может
скопировать самодостаточный JSON, но журнал не сохраняется и не отправляется.
