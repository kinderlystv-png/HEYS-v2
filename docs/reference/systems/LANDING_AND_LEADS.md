...
невалиден по самой оферте: п. 8.2 `docs/legal/user-agreement.md` признаёт
подписание ПЭП только **после успешной аутентификации** (ключ ПЭП — телефон и
PIN), а лендинг работает с анонимным посетителем. Теперь модалка собирает только
согласие на обработку ПДн и явно сообщает, что условия использования принимаются
при входе в приложение; акцепт `user_agreement`/`payment_oferta` остаётся за
приложением.
