...
> **Статус:** source-контракты проверены 2026-07-27; контракт акцепта в
> `PurchaseModal` уточнён 2026-07-31 **Охват:** формы заявки, согласия, gateway,
> сохранение и дедупликация, атрибуция, Telegram-уведомление и передача лида в
> curator flow **Не подтверждено:** production deployment/runtime, доставка
> Telegram и Метрики, фактическая обработка заявок командой
...
- optional marketing consent 1.4, отсутствие которого передаётся как `null`.

С 2026-08-14 форма фиксирует `privacy_version=1.8`: публичная политика описывает
работу оператора и не заменяет in-app согласие `personal_data` 1.0. После
активации пакета 1.11 серверный trigger сверяет версию заявки с реестровой
записью `privacy_policy`, а не с `personal_data`. Пользовательское соглашение и
согласие на персональные данные на лендинге не принимаются: их клиент
подписывает после PIN-входа. Согласие на здоровье 1.5 изъято из обязательного
набора.
...
**Контракт акцепта уточнён 2026-07-31 (решение владельца по `22` п. 3.14).** До
этой даты `PurchaseModal` расходился с описанным выше правилом: чекбокс
формулировался как «даю согласие … **и принимаю условия использования**», а в
payload уходил `consent.user_agreement_version`. Сервер это поле не валидировал
и не сохранял (`heys-api-leads` пишет только `consent_privacy_version`,
`consent_method`, `consent_user_agent`, `consent_marketing_version`), то есть
акцепт изображался в UI, но нигде не фиксировался. Дополнительно он был бы
невалиден по самой оферте: п. 8.1 `docs/legal/user-agreement.md` относит ПЭП к
согласиям в сервисе, а акцепт оферты — к пункту 1.3 (кнопка на экране согласий
после входа). Лендинг работает с анонимным посетителем и оферту не акцептует.
Модалка собирает только согласие на обработку данных заявки; акцепт
`user_agreement`/`payment_oferta` остаётся за приложением.
...
- Телефонный контракт остаётся российским (`+7`): обе текущие формы и API README
  ...
  contract.
- Production wiring, актуальные environment secrets и доставка внешних событий
  этим source-аудитом не доказаны.
- **Закрытое мобильное меню остаётся в порядке табуляции** (проверено настоящими
  нажатиями `Tab` на `/v/c/` 2026-07-31, не программным фокусом): первый `Tab`
  на странице уводит фокус внутрь `#mobile-navigation`. Закрытое состояние —
  `display: block`, `visibility: visible`, `opacity: 1` у контейнера,
  `pointer-events: none`, `aria-hidden="true"`, атрибута `inert` нет; прячут его
  только прозрачность вложенных обёрток и `pointer-events`. Мышь такое меню
  игнорирует, клавиатура — нет. Следствия: пользователь клавиатуры проходит пять
  невидимых пунктов до контента, а `aria-hidden` на фокусируемом содержимом —
  нарушение WCAG 4.1.2 (фокус в «немой» зоне). Лечится `inert` при
  `open === false` либо `visibility: hidden` вместе с `opacity`. Файл —
  `apps/landing/src/components/MobileNavMenu.tsx`; задача, которая им владела,
  завершена и заархивирована, поэтому дефект остаётся неназначенным.
- **Видео первого экрана — самый тяжёлый ресурс лендинга.**
  `hero-curator-demo.webm` весит 1,95 МБ и загружается целиком сразу: у элемента
  `preload="auto"` при том, что постер (`hero-curator-demo-poster.jpg`) есть. На
  мобильном интернете это главный кандидат в проблему первого экрана. Изменять в
  одиночку нельзя: hero-демо — общий контракт всех версий лендинга и
  заблокировано решением `22` п. 3.15, смена `preload` должна быть осознанной.
