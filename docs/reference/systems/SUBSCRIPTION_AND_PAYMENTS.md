...
16. `heys_registration_in_progress` нельзя считать авторитетнее завершённого
    ...
    flow.
17. Регистрация и утренний чек-ин — **два мастера**. До `canWrite` план = 4
    profile + терминальный `welcome` (три конца: доступ открыт / ждём куратора /
    неделя с даты по `todayISO` после 03:00), без morning ledger и без `dayv2`.
    «Начать утренний чек-ин» или live `heys:subscription-changed` перемонтируют
    дневной план. Крестика у утра нет; после complete — Главная.
18. Активный `trial` не рендерит subscription status, срок или CTA на основном
    экране; эти данные доступны только во втором слое настроек.
19. Поздняя загрузка StepModal повторно регистрирует `payment_required` через
    каноническое событие `document:heys-stepmodal-ready`; альтернативное имя или
    `window` target не являются частью контракта.
20. Повторное исполнение lazy chunk не очищает существующий StepModal registry:
...
- **Того же этапа ждёт маршрут `GET /payments/order-preview`** (6 сентября 2026,
  коммит `55d7c4c6c`). Он отдаёт `projected_period_end` с
  `period_kind: "estimate"` до оплаты; после платежа приходит
  `confirmed_period_end` с `"confirmed"`. Поля намеренно разные: одно на оба
  случая читалось бы как подтверждённый срок там, где есть только прогноз.
  Раньше клиент считал дату сам («сегодня + 30»), и это снято — срок зависит от
  тарифа и даты платежа, а не от дня открытия экрана. Арифметика живёт на
  сервере и покрыта тестом
  `computeProjectedPeriodEnd uses calendar months like PostgreSQL INTERVAL`:
  прогноз обязан совпадать с тем, что посчитает `INTERVAL` в базе, иначе после
  оплаты дата поедет.
- Пока маршрута нет в проде, строка срока на экране заказа **не рисуется
  вовсе**, и это штатное поведение, а не дефект: клиент показывает только
  пришедшее. Сухой прогон деплоя 6 сентября остановлен предполётной проверкой на
  `LOCKBOX_PAYMENTS_SECRET_ID` — секрета ещё не существует, и гейт отказал
  правильно. Тот же дефект инфраструктуры записан в журнале решений
  (`маркетинг/15`, строка 53) с 8 августа: функции платежей нет в облаке вовсе.
