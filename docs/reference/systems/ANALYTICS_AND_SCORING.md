...
> **Статус:** основные source-пути проверены 2026-07-18 **Охват:** субъективная
> оценка дня, Status/Unified Day Score, Predictive Insights, Early Warning и
> relapse risk **Не подтверждено:** научная валидность моделей,
...
Inline-сценарии «Что если» в слое «Подробно» открываются после 14 заполненных
дней. Они передают в `pi_whatif.js` уже наблюдённые patterns с
`requireObserved:true`; legacy-подстановка умеренного baseline для этой
пользовательской поверхности запрещена. Пара «было → станет» пересчитывается
владельцем `HEYS.PredictiveInsights.calculateHealthScore` и подписана как оценка
дня из паттернов, а не HEYS Score. Если нужный pattern или владелец score
недоступен, UI показывает причину вместо выдуманного числа.

Прогноз веса второго слоя считается на фиксированном окне 30 дней и возвращает
до семи последних фактических замеров отдельным `series`. UI строит недельную
точку из `projectedWeight`, а месячный темп берёт из неокруглённого
`monthlyChange` владельца; при отсутствии любого из этих полей или минимум двух
датированных фактических точек v4-карточка не рисует выдуманный график.
Наблюдённый ряд и прогнозная точка остаются различимыми в разметке.
...
9. Подпись графика калорий говорит о ступеньке особого периода только при
   включённом `healthFeatures.isCycleTrackingEnabled(profile)`; без
   cycle-контекста график сохраняет общее пояснение столбиков и линии плана.
10. Пользовательский what-if не строит пару результата из legacy moderate
    defaults: нужны наблюдённые patterns и канонический score owner.
...
| A7  | `scoreHistory` нормализует подтверждённую шкалу 1–10 в единый output 0–100                | `rg -n 'getStoredScore100                                                                                                     \| buildScoreHistory' apps/web/heys_predictive_insights_v1.js && pnpm vitest run apps/web/**tests**/predictive-score-history-scale.test.js`                                                                                                          | проверено 2026-07-18 |
...
| A15 | Inline what-if требует 14 дней и наблюдённые patterns; пару считает canonical score owner | `pnpm --dir apps/web test -- __tests__/insights-v4-whatif-inline.test.js`                                                                                                                                                                                                                                                                                                          | проверено 2026-09-01 |
| A16 | V4-прогноз веса читает фиксированные 30 дней и получает фактический `series` от владельца | `pnpm --dir apps/web test -- __tests__/insights-v4-weight-prediction.test.js`                                                                                                                                                                                                                                                                                                      | проверено 2026-09-01 |
