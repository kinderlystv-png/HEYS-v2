...
> production docs-contract v0.38 синхронизирован 2026-07-30 **Охват:** package
> API, registries, reducer, RNG/hash, scenario manifest v4, policies,
> интерактивный UI, checkpoint D7, tests, полный v0.2 report и текущие
> production gates  
> **Не подтверждено:** full QA v0.4, человеческая причинность/интерес,
...
- Sprints 0–5
  ...
  registry v0.2, opaque game seed, bounded checkpoint v2, доказанный click-only
...
- Следующий разрешённый шаг — Sprint 6: formative human gate.
- Full QA v0.4 выполняется затем в Sprint 7 на отдельном runner.
...
| Исполнимые уточнения v0.34                    | [`TECHNICAL_CONTRACT_ADDENDUM.md`](../../assemble-day/TECHNICAL_CONTRACT_ADDENDUM.md)                                                                                                                                                                                                                  |
...
7. UI-состояние выбора не сохраняется: checkpoint создаётся только после
   успешного reducer-step. Envelope v2 содержит opaque game seed, revision,
   contract versions, state hash и компактный ledger подтверждённых решений;
   полное состояние, journal и последний summary восстанавливаются replay через
   тот же reducer и сверяются по hash.
...
| AD13 | Checkpoint v2 хранит компактный ledger подтверждённых решений, а trace воспроизводит полные engine-стадии и совпадает с state hash; replay-safe v1 мигрирует после следующего шага, partial/privacy-unsafe v1 отклоняется неизменным                                                           | `pnpm --dir apps/web exec vitest run __tests__/planning-game-assemble-day.test.js --no-coverage`                                                                                                                                                                                | проверено 2026-07-30                                                                    |
...
| AD17 | Канонический current route одинаков: Sprints 0–5 DONE → Sprint 6 human gate                                                                                                                                                                                                                    | `rg -n -e 'Sprints 0.*5.*DONE' -e 'Sprint 6' docs/assemble-day/{README.md,12_ROADMAP.md,assemble_prodution_megaplan.md}`                                                                                                                                                        | проверено 2026-07-30                                                                    |
