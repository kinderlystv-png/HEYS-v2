...
> **Production evidence:** bot source recovery wave 2 опубликован; function
> health, polling heartbeat и общий canary прошли после deploy<br> **Не
> подтверждено:** curator webhook state и реальное callback-нажатие; Mini App
> backend отсутствует в найденном gateway/source-контуре
...
- Gateway по-прежнему содержит webhook routes, хотя operational protocol
  ...
  heartbeat подтверждает polling worker, но Telegram webhook state требует
  отдельной Bot API проверки.
...
| T2  | Ожидаемые Mini App endpoints встречаются только во frontend/docs                                             | `rg -n --glob '!**/node_modules/**' --glob '!**/dist/**' -e '/api/telegram/auth/verify' -e 'X-Telegram-Init-Data' .`                                                 | проверено 2026-07-17      |
...
| T6  | Client имеет отдельный poll, а Start timer под одним lease опрашивает Start и curator tokens с offset commit | `rg -n -e 'handleStartBotPoll' -e 'runStartBotPoll' -e 'runCuratorBotPoll' -e 'offset: lastUpdateId' yandex-cloud-functions/heys-bot-client/index.js`                | проверено 2026-07-18      |
| T7  | Webhook paths требуют отдельные secret checks                                                                | `rg -n -e 'verifyWebhookSecret' -e 'HEYS_START_WEBHOOK_SECRET' -e 'TELEGRAM_WEBHOOK_SECRET' yandex-cloud-functions/heys-bot-client/index.js`                         | перепроверено 2026-07-18  |
| T8  | Start contact создаёт lead и минимизированный handoff                                                        | `rg -n -e 'createStartLeadFromContact' -e 'sendStartLeadHandoff' -e 'record_funnel_event' yandex-cloud-functions/heys-bot-client/index.js`                           | перепроверено 2026-07-18  |
