...
- `mcp_telemetry` — свёртка вчерашних вызовов MCP из `mcp_call_events` в
  `mcp_call_daily` и `mcp_seq_daily`; retention сырья 180 дней; идёт также
  внутри `daily_cleanup`;
...
Телеметрия MCP разведена на три слоя. Горячий путь `heys-mcp` печатает stdout
(Cloud Logging, консоль) и **await-записывает** ту же строку в `mcp_call_events`
через `insert_mcp_call_event` (Bearer `MCP_TELEMETRY_SECRET`, таймаут ≤300 мс).
Состав — белый список `RECORD_FIELDS` в
`yandex-cloud-functions/heys-mcp/lib/telemetry.js`.

`tasks_mcp_trace` и суточная задача `mcp_telemetry` читают **Postgres**
(retention сырья 180 дней). Агрегат — `mcp_call_daily` / `mcp_seq_daily`
(вызовы, p50/p95/max, суммарное время, ошибки, холодные старты; пары «инструмент
→ следующий инструмент»). Пары нужны потому, что главная потеря в MCP не
медленный вызов, а лишний круг, который на одиночных вызовах не виден. Запись
идемпотентна: повторный прогон за ту же дату перезаписывает строки. Отчёт —
`node scripts/mcp-stats.mjs --days N`, сортировка по суммарному времени. Stdout
в folder default log group остаётся для консоли (3 суток); агрегаты без ПДн —
бессрочно.
