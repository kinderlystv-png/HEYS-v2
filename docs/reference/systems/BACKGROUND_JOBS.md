...
- `mcp_telemetry` — свёртка вчерашних вызовов MCP из Cloud Logging в
  `mcp_call_daily` и `mcp_seq_daily`; идёт также внутри `daily_cleanup`;
...
`client_log_trace` имеет целевой retention 30 дней. Maintenance пока только
считает кандидатов старше 30 дней: destructive delete намеренно остаётся
выключенным до отдельного owner/legal решения. Документация и новая schema
миграция используют один порог, поэтому dry-run не маскирует расхождение 14/30.
...
Внутренний heartbeat watcher строит lifecycle incident по полному набору
фактически stale-строк. Шестичасовой `stale_alerted_at` latch ограничивает
только повторные уведомления и не считается признаком восстановления; incident
закрывается лишь когда stale-строк действительно не осталось.
...
Телеметрия MCP разведена на два слоя. Горячий путь `heys-mcp` печатает на каждый
вызов инструмента одну строку чистого JSON `t="mcp_call"` в stdout — рантайм
уносит её в Cloud Logging сам, без соединений к БД и без таймеров: инстанс Cloud
Function замерзает сразу после ответа, поэтому отложенный flush не выполнился бы
никогда. Состав строки задан белым списком `RECORD_FIELDS` в
`yandex-cloud-functions/heys-mcp/lib/telemetry.js`; аргументы, тела ответов,
тексты исключений и идентификаторы людей в неё не попадают конструктивно, а не
по договорённости. Подключения различаются псевдонимом `session_id` —
необратимый срез хэша от токена и id инстанса, потому что stateless-транспорт
своего `Mcp-Session-Id` не ведёт.

Второй слой — суточная задача `mcp_telemetry`: читает Logging за прошедшие сутки
и складывает агрегат в `mcp_call_daily` (вызовы, p50/p95/max, суммарное время,
ошибки, холодные старты) и `mcp_seq_daily` (пары «инструмент → следующий
инструмент»). Пары нужны потому, что главная потеря в MCP не медленный вызов, а
лишний круг, который на одиночных вызовах не виден. Запись идемпотентна:
повторный прогон за ту же дату перезаписывает строки. Отчёт —
`node scripts/mcp-stats.mjs --days N`, сортировка по суммарному времени.
Retention: сырьё живёт в folder default log group (`e23ndggvq798r3v3eepq`,
сейчас 3 суток) — эту группу не укорачивать, по ней разбирают 429/503 и
dead-man. Агрегаты без идентификаторов бессрочны. Отдельная группа на 14 суток —
после выдачи `logging.writer`/`logging.reader` сервис-аккаунту функций.
...
| --- | ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B1  | Общий inventory и ops checker покрывают 9 automations и 17 timer expectations                           | `node --test yandex-cloud-functions/__tests__/function-inventory.test.cjs yandex-cloud-functions/__tests__/check-heys-ops-status.test.cjs`                                                                                                                                                                                                                                                                                                                                              | проверено 2026-08-03; 25/25. С 2026-08-03 `check-heys-ops-status.test.cjs` входит в `test-functions.sh`, то есть выполняется автоматически перед деплоем любой cloud function |
...
| B8  | Backup читает KV и account в одной `REPEATABLE READ` транзакции (`snapshotClientBundle`)                | `rg -n 'snapshotClientBundle\|BEGIN TRANSACTION ISOLATION LEVEL REPEATABLE READ' yandex-cloud-functions/heys-client-daily-backup/index.js`                                                                                                                                                                                                                                                                                                                                              | исправлено 2026-07-18; факт досье обновлён 2026-08-15                                                                                                                         |
