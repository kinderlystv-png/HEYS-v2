...
> **Статус:** основные source-контракты проверены 2026-07-17<br> **Охват:**
> генерация и показ советов, outcomes/snooze/commitments, XP, достижения, daily
> missions и cloud sync игры<br> **Не подтверждено:** полный набор правил,
> баланс наград, browser UX и production cloud reconciliation
...
| E8  | Game cloud sync блокирует запись при failed precheck и использует merge-save со stale-write outcome | `rg -n -e 'cloud_precheck:failed_blocked' -e 'mergeSaveKV' -e 'stale_write_blocked' apps/web/heys_gamification_v1.js yandex-cloud-functions/heys-api-rpc/lib/heys_sync_merge_v1.cjs` | проверено 2026-07-17 |
...
| E12 | Найдены только точечные advice UI/render-loop tests, не полный rules suite                          | `find apps/web/__tests__ -maxdepth 1 -type f -name '*advice*test*' -print`                                                                                                           | проверено 2026-07-17 |
