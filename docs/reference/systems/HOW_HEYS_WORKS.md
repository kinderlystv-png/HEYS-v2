...
При первом клиентском входе выданный куратором код заканчивает свою роль после
создания собственного кода доступа. На первом шаге
[`ClientAccessCodeSetup`](../../../apps/web/heys_client_access_code_setup_v1.js)
пользователь одновременно придумывает код и добровольно принимает соглашение
ПЭП; подтверждение кода уже не просит согласие повторно и явно сообщает, что оно
принято на предыдущем шаге. Слабый или несовпавший код возвращает к первому
вводу с причиной и следующим действием, не создавая сессию с новым кодом.
...
| Нагрузка     | Плотная история 42 дней, тоннаж, зоны и RPE         | Раздельная динамика силовой/кардио, восстановление и совет     | `TrainingKernel.load/strength` → `collectTrainingLoadSummary`                                   |
...
Вкладка `Актив` считает производную нагрузку на чтении, не сохраняет CTL/ATL и
не смешивает килограммы с MET-минутами. Первый слой показывает только
неблокирующий совет при трёх последовательных днях одной мышечной группы;
причина и два равнозначных выхода остаются видимыми. Второй слой раскрывает
динамику окон 42/7 дней и вклад закрытой силовой сессии. Неоткалиброванные
абсолютные баллы пользователю не показываются; готовность названа словом, а
время восстановления считается от относительной тяжести сессии, тоннажа и RPE.
