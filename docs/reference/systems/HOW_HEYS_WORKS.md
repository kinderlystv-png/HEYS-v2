...
Logout является такой же сменой data context: перед очисткой фиксируется факт
активного клиента, затем runtime и memory cache обнуляются, а
`heys:client-changed` с пустым `clientId` принудительно перемонтирует
React-дерево. Поэтому следующий PIN-вход на той же странице не может считаться
первой активацией и переносить живой state предыдущего клиента.
...
В client observability эти сигналы не смешиваются: подтверждённая Phase A
фиксируется как `initial_sync_phase_a_ready`, а `initial_sync_ready` означает
только успешный `phase: 'full'`. Повторный или hot-sync сигнал не создаёт новое
событие готовности в том же посещении.
...
Каркас ожидания выбирается по тому же `defaultTab`, который читает tab-state из
client-scoped профиля. Единый реестр `HEYS.AppSkeletons` описывает композиции
дневника, отчётов, активности, виджетов, инсайтов, задач, продуктов и настроек;
его используют и pre-React boot-экран, и runtime fallback ленивой вкладки. Для
задач дополнительно учитывается `defaultTasksSubtab`, неизвестный tab/subtab
переходит в полноэкранный безопасный fallback. Нижняя навигация остаётся вне
runtime-каркаса и не перемонтируется при раскрытии содержимого.
...
Follow-up зарядки после первого приёма проходит внутри одной модалки: сначала
пользователь выбирает статус, затем интенсивность выполненной зарядки или
причину её отсутствия, включая замену тренировкой в первой половине дня. До
второго ответа terminal-state не сохраняется; «Сделаю позже» закрывает модалку
одним нажатием и откладывает повтор до следующего приёма пищи.
...
Крестик в карточке «ВЕС НА УТРО» сбрасывает поля утреннего чек-ина выбранного
...
локальный `saved_local`/`synced` в `planned`. Сам merge не повышает `updatedAt`,
если строки не изменились, поэтому повторный hot-sync остаётся идемпотентным.
Для записи через `Store.set` облачный handoff выполняется один раз самим Store;
универсальный localStorage-interceptor на время этой записи подавлен и не
создаёт второй immediate upload критического ключа. После записи в localStorage
слой sync очищает соответствующий memory cache и отправляет предметные события:
