...
> Статус: проверено по текущей реализации 17 июля 2026 года.<br> Охват: основной
> web-сценарий — запуск, вход, выбор клиента, дневник, вычисления, локальное
> хранение, облачная синхронизация и рекомендации.
...
возвращаются в очередь на retry.
...
| Upload временно не удался         | Запись возвращается в pending-очередь и повторяется с backoff                                                  |
| Авторизация истекла               | Бессмысленный retry останавливается до новой сессии; локальные данные не выбрасываются автоматически           |
| Phase A не удалась                | Sync продолжает полную загрузку; UI имеет локальный fallback/failsafe                                          |
...
| --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| Runtime входит через `AppEntry.start`, затем `initializeApp`                | `rg -n -e 'AppEntry.start' -e 'initializeApp' apps/web/heys_app_v12.js apps/web/heys_app_entry_v1.js apps/web/heys_app_initialize_v1.js`  | Подтверждена цепочка входа и инициализации                                             |
| React root меняет key по `heys:client-changed`                              | `rg -n -e 'createRoot' -e 'heys:client-changed' -e 'key: reactKey' apps/web/heys_app_initialize_v1.js`                                    | Подтверждены mount и client-key remount                                                |
| Auth-init поддерживает local/offline, PIN и curator restore                 | `rg -n -e 'initLocalData' -e 'setPinAuthClient' -e 'verifyCuratorToken' -e 'getCurrentClientBySession' apps/web/heys_app_auth_init_v1.js` | Подтверждены все три ветви                                                             |
| Gate вызывает защищённый `switchClient`                                     | `rg -n -e 'buildGate' -e 'switchClient' apps/web/heys_app_gate_flow_v1.js apps/web/heys_storage_supabase_v1.js`                           | Подтверждены UI-вход и cloud-транзакция                                                |
...
| День инициализируется из scoped dayv2 и не autosave-ится до hydration       | `rg -n -e 'getInitialDay' -e 'useDayAutosave' -e 'disabled: !isHydrated' apps/web/heys_day_init_v1.js apps/web/heys_day_tab_impl_v1.js`   | Подтверждено                                                                           |
| Autosave ставит metadata и отправляет `heys:data-saved`                     | `rg -n -e 'schemaVersion' -e 'updatedAt' -e '_sourceId' -e 'heys:data-saved' apps/web/heys_day_hooks.js`                                  | Подтверждено                                                                           |
| Totals, нормы и dayScore вычисляются локально                               | `rg -n -e 'calculateDayTotals' -e 'computeDailyNorms' -e 'calculateDayAverages' apps/web/heys_day_calculations.js`                        | Подтверждены три чистых расчётных входа                                                |
| TDEE-контекст вызывает `HEYS.TDEE.calculate`                                | `rg -n -e 'TDEE.*calculate' -e 'buildEnergyContext' apps/web/heys_day_energy_context_v1.js`                                               | Подтверждено                                                                           |
...
| Upload использует pending-очередь и Yandex RPC                              | `rg -n -e 'enqueueClientSave' -e 'scheduleClientPush' -e 'saveClientViaRPC' apps/web/heys_storage_supabase_v1.js`                         | Подтверждены enqueue, batch scheduler и отправка                                       |
| Download и upload имеют разные события                                      | `rg -n -e 'heysSyncCompleted' -e 'heys:data-uploaded' apps/web/heys_storage_supabase_v1.js`                                               | Подтверждено                                                                           |
