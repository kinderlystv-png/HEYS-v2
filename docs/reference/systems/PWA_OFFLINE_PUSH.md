...
контента, автоматически открытой hunger/check-in модалки или route-level экрана
`subscription-loading`. Последний считается допустимым visible frame: ожидание
subscription RPC не должно запускать blank-screen recovery. Бюджет 15 секунд
