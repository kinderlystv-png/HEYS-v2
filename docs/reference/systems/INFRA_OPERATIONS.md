...
- Consent-proof migration № 9 подготовлена и является обязательной для обоих
  deploy workflow, но ещё pending в production до отдельно разрешённого release.
...
| I15 | Frontend/backend блокируются на pending migrations, а web artifact проверяет legal-версии в runtime bundles | `node --test scripts/db/migrate.test.mjs apps/web/__tests__/consent-proof-v2.test.js`; `node scripts/verify-legal-release.mjs --dist=apps/web/public`                                                                                                             | source проверен 2026-07-27; migration № 9 pending    |
