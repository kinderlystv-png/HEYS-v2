...
> **Статус:** core source-контракты проверены 2026-07-18<br> **Охват:** shared
> catalog, client overlay, merge/sync, commit gate, moderation entrypoints и
> поиск в основном web flow<br> **Не подтверждено:** production catalog
...
При включённом overlay commit gate требует client id и cloud API, сохраняет весь
overlay key, затем читает его обратно. Если immediate readback ещё не видит
строку, локальный overlay применяется по успешному save acknowledgement. Для
HTTP 413 есть особая ветка: mutation сохраняется локально и считается queued,
поэтому успешный UI-result не обязательно означает немедленное присутствие
строки в cloud.
...
- Overlay cloud sync сохраняет весь массив одним KV payload. Commit gate уже
  имеет специальную 413 queue ветку — размер key является реальным пределом
  масштабирования, а не гипотезой.
...
| P3  | Missing individual shared base возвращает disabled placeholder, не raw selectable Type A | `rg -n -e 'hasMealNutrientSource' -e '_nutrientsPending' -e '_selectionDisabled' apps/web/heys_products_overlay_v1.js apps/web/heys_add_product_step_v1.js`                                                                                                                                          | проверено 2026-07-18                                                                       |
...
| P6  | Persistent meal product проходит nutrient resolver и overlay/cloud commit gate           | `rg -n -e 'resolveMealProduct' -e 'ensurePersonalProductCommitted' -e 'ensureMealProductReady' apps/web/heys_products_overlay_v1.js apps/web/heys_core_v12.js && pnpm vitest run apps/web/__tests__/overlay-cloud-snapshot-suppress.test.js apps/web/__tests__/product-commit-gate-contract.test.js` | проверено 2026-07-18                                                                       |
...
| P10 | `@heys/search` не импортируется активным web product flow                                | `rg -n -e '@heys/search' -e 'SmartSearchEngine' apps/web --glob '!**/node_modules/**' --glob '!**/dist/**'`                                                                                                                                                                                          | проверено 2026-07-17: только dependency/config/dynamic registry, не AddProductStep runtime |
