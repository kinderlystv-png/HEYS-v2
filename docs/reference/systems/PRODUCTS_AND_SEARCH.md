...
**Масса проверяется двумя правилами (решено 2026-09-03).** Физика одна: 100 г
продукта — это БЖУ и клетчатка плюс вода, зола, органические кислоты и этанол,
поэтому истинная сумма никогда не превышает 100 г. Всё, что выше, — артефакт
этикетки; бюджет округления по ТР ТС 022/2011 и EU 1169/2011 составляет 0.5 г на
объявленное поле, то есть не больше 3 г на шесть полей.

| Правило                                  | Формула                            | Лимит |
| ---------------------------------------- | ---------------------------------- | ----- |
| `shared_products_mass_within_100g`       | БЖУ + клетчатка                    | 105 г |
| `shared_products_macro_mass_within_100g` | БЖУ без клетчатки и без трансжиров | 103 г |

Второе правило (миграция `2026-09-03_shared_products_macro_mass_rule.sql`)
добавлено потому, что клетчатка — единственное поле без перекрёстной проверки:
она даёт массу, но 0 ккал, поэтому `shared_products_energy_plausible` на
неверной клетчатке не срабатывает никогда, и правило массы остаётся единственным
барьером. У БЖУ, наоборот, лишние 5 г запаса не нужны (самые плотные карточки
каталога — подсолнечное масло 99.9 г и сахар 99.8 г), а карточка с БЖУ 104 г и
нулевой клетчаткой проходила все прежние проверки, хотя такого продукта не
существует. Разделение делает отказ адресным: сработало правило БЖУ — врут
макросы; прошло правило БЖУ, но не прошло правило полной массы — вопрос к
клетчатке.

**Порог 105 намеренно не поднимали.** Разбор 2026-09-03 по двум независимым
срезам (экспорт общего каталога 2026-04, 288 карточек; выгрузка прода
2026-08-14, 236 карточек): полоса 100–105 г пуста в обоих, а всё выше 105 — уже
разобранный мусор, починенный миграциями 2026-08-02 (семена чиа 123.5 г —
клетчатка дважды; четыре карточки Chika Layers 107.5–109.0 г — полиолы записаны
в клетчатку). Карточка «Клетчатка яблочная MEDUTEUT» с этикеткой на 107 г лежит
НИЖЕ этих батончиков, поэтому одним порогом от них не отделяется: поднять лимит
под неё значит вернуть в каталог тот же класс ошибок. Сама этикетка недостоверна
— сушёный порошок из яблочных выжимок содержит 8–10 г влаги и около 2 г золы,
которым при сумме 107 г места не остаётся. Путь для такой этикетки — личная
карточка (CHECK стоит только на `public.shared_products`, `client_kv_store` им
не ограничен) либо согласованное с этикеткой значение углеводов.

Нарушение этих CHECK — бизнес-отказ, не падение БД. `heys-api-rpc` мапит
SQLSTATE `23514` в HTTP 200 `{ success: false, code: 'CHECK_VIOLATION', error }`
с текстом про сумму и лимит; коннектор и вкладка каталога показывают `error`, а
не сырой `"Database error"`. Личная карточка клиента при этом уже создана: общая
база отказала, overlay нет. `heys_update_product` общую карточку не
перепубликует — это личный override, не баг публикации. Числа в SQL и в
`shared/pg-check-violation.js` обязаны совпадать — это сторожит тест
`SQL thresholds match the constants the curator-facing message quotes`.
...
| P36 | Масса делится на два правила: БЖУ ≤ 103 г и БЖУ + клетчатка ≤ 105 г; числа SQL = числа JS | `node --test yandex-cloud-functions/heys-api-rpc/__tests__/pg-check-violation.test.js && node --test scripts/db/migrate.test.mjs`                                                                                                                                                                                                                                                              | 17/17 + 9/9 pass 2026-09-03; миграция `2026-09-03_shared_products_macro_mass_rule` НЕ применена |
