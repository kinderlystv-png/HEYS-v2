...
- `pnpm audit` красный: есть high/moderate advisories, в основном в
  dev/build/test цепочке; это не повод строить тяжелый security program, но
  повод точечно обновить зависимости или задокументировать исключения.
...
| Testing & QA                  | Yellow/Red   | Критические sync-тесты зеленые; regression и audit красные.                                          | command results below                                                                                       |
...
| P1       | Разобрать `pnpm audit` high advisories.                     | Уязвимые dev/build/test зависимости могут быть использованы в dev server или supply-chain пути. | Точечно bump/override `form-data`, `vite`, `dompurify` и связанные dev deps, либо зафиксировать unreachable justification. | `pnpm audit` green или остается только documented non-runtime risk. | Не вводить SCA platform; достаточно lockfile hygiene и audit notes. |
...
- Triage `pnpm audit`; update dependencies only where needed.
...
| Dependency audit currently fails with high/moderate advisories.                                                                        | `pnpm deps:check`                                                                                                           | Command output                                                                        | Command exit 1; 10 vulnerabilities, including high advisories for `form-data` and `vite`.       | High       |
...
| `pnpm deps:check`                                                                                                                                                                                 | Exit 1; 10 vulnerabilities reported, including 2 high.                                                                                            |
