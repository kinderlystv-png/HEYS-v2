...
> ⚠️ **Начинать с раздела 0.** Разделы 1–12 — исходный срез 2026-06-17 и
> сохраняются как история: часть их «красных» пунктов уже закрыта. Живые статусы
> живут не здесь, а в
> [`маркетинг/22`](../маркетинг/22_План_реализации_маркетинга.md) § 1.10 и
> [`маркетинг/25`](../маркетинг/25_Roadmap_Ф0_Ф1.md).

## 0. Актуализация 2026-07-31

### Роль документа

Это **методология аудита и исходный 10-прогонный срез 2026-06-17**, а не трекер
статусов. Рекомендации этого аудита операционализированы в
[`маркетинг/22`](../маркетинг/22_План_реализации_маркетинга.md) § 1.10 «Product
release readiness» (`22:346–366`) — именно там источник правды по тому, что
закрыто, а что нет. Ниже — только сверка: какие утверждения аудита устарели и
куда идти за текущим статусом.

Порядок использования: методологию и right-sizing принципы (§ 3, 4) брать
отсюда; статус конкретного пункта — из `22` § 1.10; release-gate R0–R3 — из
`25`.

### Что изменилось с 2026-06-17

Сверка выполнена 2026-07-31 по `маркетинг/22`, `маркетинг/25`,
`docs/reference/IMPROVEMENT_BACKLOG.md`, `docs/reference/IMPROVEMENT_HISTORY.md`
и прямой проверке source.

| Утверждение аудита 2026-06-17                                                                | Состояние на 2026-07-31                                                                                                                                                                                                                                                                                                     | Где живёт статус                                                                                                    |
| -------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| P0: `pnpm test:regressions` красный на cookie-only PIN logout                                | **Устарело.** Закрыто 2026-07-26: `test:regressions` 87/87, targeted `task-005` 46/46, устаревшие assertions синхронизированы с контрактом                                                                                                                                                                                  | `22` § 1.10.1                                                                                                       |
| P0: live-строгость `HEYS_WRITE_CONTEXT_STRICT=1` / `HEYS_REST_READ_STRICT=1` не подтверждена | **Устарело для REST-пути.** Подтверждено 2026-07-26: оба флага в target env, IDOR-матрица (401/403/200) пройдена, readiness `ready=true` для SEC-004/SEC-024. **Оговорка:** strict-env выдаётся только функции `heys-api-rest` (`yandex-cloud-functions/deploy-all.sh:479-482`); для `heys-api-rpc` подтверждения флипа нет | `22` § 1.10.3, `docs/legal/operator/heys-r0-live-evidence-2026-07-26.md`                                            |
| P0: production-like PIN/curator sync smoke не выполнен                                       | **Устарело.** Выполнен 2026-07-26 на двух синтетических клиентах через production API; silent data loss не обнаружен                                                                                                                                                                                                        | `22` § 1.10.2                                                                                                       |
| P0: legal/ПДн prerequisites открыты                                                          | **Устарело для R0.** Gate R0 закрыт 2026-07-26; R1 (оплаты) 🟡, R2/R3 ⬜ — остаются отдельными gate                                                                                                                                                                                                                         | `25` § gates, `32`                                                                                                  |
| P1: `pnpm audit` high advisories                                                             | **Закрыто** 2026-06-17 точечным dependency refresh                                                                                                                                                                                                                                                                          | `22` § 1.10.6                                                                                                       |
| P0: production-like data-loss smoke обязателен до первого клиента                            | **Уточнено 2026-07-31:** все пять сценариев имеют защитный механизм и покрыты тестами (88 тестов зелёные), в том числе fail-closed «dayv2 не уходит батчем без server-side merge». Живой smoke выполнен 2026-07-31 на локальном стеке под триальным клиентом: все пять сценариев пройдены, потери данных нет                | `22` § 1.10.4, [`DATA_LOSS_RECOVERY_COVERAGE_2026-07-31`](implementation/DATA_LOSS_RECOVERY_COVERAGE_2026-07-31.md) |
| P1: broad training test isolation failure (`kernel-training-focus-ui.test.js`)               | **Актуально, открыто**                                                                                                                                                                                                                                                                                                      | `22` § 1.10.7                                                                                                       |
...
| P1: support diagnostics card                                                                 | **Актуально, открыто**                                                                                                                                                                                                                                                                                                      | `22` § 1.10.9                                                                                                       |
...
| P1 (док-долг): `ARCHITECTURE.md` говорит `storage_audit_enforce` default `false`             | **Актуально, drift сохраняется:** source `apps/web/heys_feature_flags_v1.js:63` = `true`, док `apps/web/ARCHITECTURE.md:144` = `false` (проверено 2026-07-31)                                                                                                                                                               | `22` § 1.10.10                                                                                                      |

### Актуальный scorecard — сверка 2026-07-31

Это снимок сверки, а не источник статусов: при расхождении прав `22`/`25`.

| Area                          | 2026-06-17   | 2026-07-31       | Чем подтверждено                                                                                                                                                                                         |
| ----------------------------- | ------------ | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Product & Release Readiness   | Yellow       | **Green/Yellow** | Release-readiness gate закрыт для R0 2026-07-26; открыты P1-пункты 1.10.4/1.10.5/1.10.9                                                                                                                  |
| Architecture & Modularity     | Green/Yellow | Green/Yellow     | Без изменений; док-drift `storage_audit_enforce` не устранён                                                                                                                                             |
| Data, Sync, Auth & Storage    | Yellow/Red   | **Green/Yellow** | 1.10.1 и 1.10.2 закрыты 2026-07-26; активных P0–P3 в проверенном backlog нет (2026-07-18)                                                                                                                |
| Security & Privacy            | Yellow/Red   | **Green/Yellow** | strict/IDOR по REST-пути подтверждены 2026-07-26, R0 закрыт, security-доки актуализированы 2026-07-31. SEC-027 (strict-покрытие `heys-api-rpc`) закрыт как `accepted-risk` решением владельца 2026-07-31 |
| Reliability                   | Yellow       | Yellow           | Data-loss/recovery smoke (1.10.4) всё ещё не выполнен                                                                                                                                                    |
| UX, UI & Copy                 | Yellow       | Yellow           | Focused walkthrough состояний по-прежнему не зафиксирован как выполненный                                                                                                                                |
| Training Modules              | Green/Yellow | Green/Yellow     | Mobility map зелёная; broad-glob isolation (1.10.7) открыт                                                                                                                                               |
| Testing & QA                  | Yellow/Red   | **Green/Yellow** | `test:regressions` 87/87 2026-07-26; остаётся 1.10.7 и незакреплённый регулярный pack                                                                                                                    |
| Performance & Maintainability | Yellow       | Yellow           | Клиентский Android smoke (1.10.8) открыт                                                                                                                                                                 |
| Release Operations            | Green/Yellow | Green/Yellow     | Без изменений                                                                                                                                                                                            |

### Что осталось открытым по этому аудиту

По статусам `22`/`25` на 2026-07-31 ни один пункт аудита не блокирует R0.
Открыты P1-пункты, статус которых ведётся в `22` § 1.10: data-loss/recovery
smoke (1.10.4), регулярный pre-release pack (1.10.5), broad training isolation
(1.10.7), Android/DayTab smoke (1.10.8), support diagnostics card (1.10.9),
документационный drift `storage_audit_enforce` (1.10.10).
...
осознанный риск (SEC-027 `accepted-risk` в `docs/SECURITY_REVIEW.md`) — флип не
дал бы защиты generic session-путям, а cross-client запись по ним невозможна
конструктивно (`client_id` резолвится server-side из session-токена).

Отдельно: платежи и масштабирование закрываются не этим документом, а gate R1/R2
в `25`/`32`.

### Новые расхождения, найденные при сверке 2026-07-31

Не входили в аудит 2026-06-17; проверены напрямую по source в день сверки.

| Расхождение                                                                                                                                                                                                                                                                                                             | Проверка                                                                             |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Evidence-ссылки этого документа на `todo.md:103` и `todo.md:245` (§ 2, § 11) больше не действительны: `todo.md` переписан 2026-07-18 и содержит 99 строк                                                                                                                                                                | `wc -l todo.md`                                                                      |
| `docs/REFACTOR_REACT_MEMO_DAY_TAB.md` с 2026-07-18 помечен как исторический план, который нельзя выполнять как готовое ТЗ; ссылка на него в § 2 и § 11 больше не указывает на действующий план                                                                                                                          | `docs/REFACTOR_REACT_MEMO_DAY_TAB.md:3-8`                                            |
| Новый док-drift ядра: в `apps/web/_kernel/` 19 модулей, таблица «Что уже считается ядром» перечисляет 13; не перечислены `active_session`, `onboarding`, `progression`, `router`, `timer`, `training_focus_ui`                                                                                                          | `ls apps/web/_kernel/*.js` против `apps/web/_kernel/KERNEL_EXTRACTION_PLAN.md:21-34` |
| Documentation gap § 9 «связать методологию с `RELEASE_PROCESS.md`» не закрыт: на документ ссылаются только `docs/README.md` и `маркетинг/22`                                                                                                                                                                            | `rg -l release-readiness-methodology`                                                |
| ~~Security-доки рассинхронизированы с `22`/`25`~~ — **устранено 2026-07-31:** `docs/SECURITY_REVIEW.md` актуализирован: SEC-004 закрыт для REST, SEC-024/L3 IDOR закрыт, компонент «Авторизация / мультитенантная изоляция» переведён 🔴 P0 → 🟡 P1; исходные записи журнала сохранены                                  | `docs/SECURITY_REVIEW.md` § «Приложение C — история документа»                       |
| ~~L6-1 / L6-2 висят как launch-blocker~~ — **устранено 2026-07-31:** добавлен § 7.2a с фактическим статусом (L6-1 mitigated watchdog'ом, L6-2 fixed+deployed cron-cleanup'ом) и остатком `DRY_RUN=0`                                                                                                                    | `docs/SECURITY_REVIEW_l6_baseline.md` § 7.2a                                         |
| ~~Strict-режим покрывает не весь backend~~ — **решено 2026-07-31:** заведено находкой **SEC-027** и закрыто как `accepted-risk` (вариант B, решение владельца). Флип не дал бы защиты generic session-путям, а cross-client запись по ним невозможна конструктивно; зафиксированы остаточный риск и триггеры пересмотра | `docs/SECURITY_REVIEW.md` § «Решение по SEC-027»                                     |

Первые четыре строки — документационный долг этого документа и смежных планов.
Три зачёркнутые закрыты 2026-07-31: две — правкой самих security-документов,
третья — решением владельца по SEC-027 (`accepted-risk`, с зафиксированным
остаточным риском и триггерами пересмотра). Детали и порядок повторного открытия
— в `docs/SECURITY_REVIEW.md`, § «Решение по SEC-027». Статусы задач ведёт `22`.

## 1. Executive Summary

> Срез 2026-06-17. Три из четырёх P0 закрыты 2026-07-26 — см. § 0.
...
- Для публичной работы с реальными клиентами остаются legal/security задачи: РКН
  updateform по HEYS принят порталом 2026-06-17, но публичная запись ещё требует
  публикации/сверки; также нужна внутренняя политика обработки ПДн.
...
> Срез 2026-06-17. Актуальная сверка — в § 0. Evidence-ссылки на `todo.md:103` и
> `todo.md:245` устарели: файл переписан 2026-07-18.
...
| P0       | Закрыть user-only legal blockers: публикацию/сверку РКН-изменений и внутреннюю политику ПДн.                                                      | Риск выпуска с реальными персональными данными без базовых legal оснований.    | РКН updateform принят порталом 2026-06-17; дождаться публикации, сверить запись и утвердить внутреннюю политику. | Статус задач отмечен как done в release checklist.                                                                    | Не внедрять enterprise GRC; только обязательные документы для текущего масштаба. |
...
| Priority | Recommendation                                              | Risk addressed                                                                                  | Minimal sufficient fix                                                                                     | Acceptance criteria                                                | Overengineering guardrail                                           |
| -------- | ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------- |
| P0       | Закрыть legal ПДн задачи до real-client launch.             | Регуляторный риск обработки персональных данных.                                                | РКН updateform принят порталом; дождаться публикации/сверки и утвердить внутреннюю политику обработки ПДн. | Статусы внесены в release checklist; номер/ключ хранятся вне репо. | Не покупать DLP/SIEM на текущем этапе.                              |
...
> Срез 2026-06-17. Блокеры 1–3 закрыты 2026-07-26, блокер 4 (data-loss smoke)
> остаётся открытым как P1. Текущее состояние — § 0 и `22` § 1.10.
...
> План 2026-06-17. Step 1 выполнен (кроме data-loss smoke), Step 2 частично.
> Текущая очередь запуска ведётся не здесь, а в `22` § 1.10 и `25`.
...
> Результаты прогонов 2026-06-17. Красные строки (`test:regressions`, targeted
> task-005) с тех пор зелёные — см. § 0.
