...
### Session 7 — UI Sync Stability & Animation Fixes (2026-02-26)

**Контекст:** При фоновой синхронизации (Phase B) или принудительном обновлении
(`forceReload`), пересчёт `normAbs` вызывал изменение `optimum`. Это приводило к
сбросу анимации прогресс-бара в 0 и повторному заполнению (double-fill flicker),
даже если `eatenKcal` не менялся.

**Проблема:** `useEffect` в `useDayAnimations` реагировал на любое изменение
`optimum`, запуская полную анимацию с нуля. При медленной сети (throttling) это
выглядело как "моргание" интерфейса после загрузки бандлов и применения
синхронизированных данных.

**Решение:** Добавлен механизм дискриминации "реальных действий" пользователя от
"фоновых обновлений" (sync/recalc):

1. Введены `prevKcalRef` (для отслеживания изменения съеденных калорий) и
   `prevDateTabRef` (для отслеживания смены дня/вкладки).
2. Вычисляется флаг
   `isRealAction = (eatenKcal !== prevKcalRef.current) || (dateTabKey !== prevDateTabRef.current)`.
3. Если это реальное действие — запускается полная анимация от 0 до target.
4. Если это фоновое обновление (изменился только `optimum`) — бар мгновенно
   телепортируется в новую позицию без сброса в 0.

**Изменённые файлы:**

- `apps/web/heys_day_animations.js` (исходник хука `useDayAnimations`)
- `apps/web/public/boot-day.bundle.7320c50778ec.js` (обновлён бандл)

**Результат:**

- Устранён визуальный глитч (double-fill) при получении `heys:day-updated`
  (batch) от `fetchDays`.
- На throttled-сети (Mid-tier mobile) интерфейс стабилен: после Skeleton UI и
  загрузки бандлов, применение исторических данных (History Guard) и пересчёт
  CascadeCard не вызывают повторных анимаций прогресс-бара.
