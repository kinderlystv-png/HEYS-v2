...
### ⚠️ Источник правды при проверке секрета — Lockbox, а не `.env`

**Правило: локальный `.env` НЕ является доказательством того, какое значение
действует в проде.** Он расходится с Lockbox молча и в обе стороны. Проверено
2026-08-02 (SEC-035): в `yandex-cloud-functions/.env` четыре ключа оказались
протухшими (его `HEYS_START_BOT_TOKEN` отвечал `Unauthorized`), а файл с
названием `.env.backup-before-phase3-20260522-185935` вопреки имени содержал
**действующие** `JWT_SECRET`, `HEYS_ENCRYPTION_KEY`, `INTERNAL_CRON_TOKEN` и
`VAPID_PRIVATE_KEY`.

Цена ошибки конкретная: сверка утёкшего значения с `.env` дала вывод «значение
устарело, ротация не нужна», тогда как ключ был живым и открывал доступ к
`heys-backups` с выгрузками клиентских данных.

**Как проверять, живо ли утёкшее значение** (сравнение, без печати значений):

```bash
# Lockbox — источник правды. heys-app-secrets: e6qrvefs3vn66jiamfk4
#                            heys-s3:          e6qnjm2ks2n1ubiaiki6, e6qsj7eie5446321chr2
yc lockbox payload get --id <secret-id> --format json
```

Сравнивать программно и выводить только «совпадает / отличается». **Никогда не
печатать значение секрета целиком** — даже частичный вывод в транскрипт агента
или в лог считается утечкой и требует ротации (инцидент 2026-06-14 с
`HEYS_START_BOT_TOKEN`, инцидент 2026-08-02 с S3-ключом).

Для Telegram-ботов дополнительная проверка живости: `getMe` с токеном из
Lockbox, а не из `.env`.

### Свойство проверяется поведением, а не названием

За 2026-08-02 два дефекта подряд оказались невидимы при проверке по описанию:

- **SEC-035** — файл назывался `.env.backup-before-phase3-…`, а содержал
  действующие `JWT_SECRET`, `HEYS_ENCRYPTION_KEY`, `INTERNAL_CRON_TOKEN` и
  `VAPID_PRIVATE_KEY`; «текущий» `.env` при этом был мёртвым.
- **SEC-037** — локальная доска куратора описана в шапке файла и в досье как
  «read-only страница на localhost» и печатает `http://localhost:4777` при
  старте, но `listen` вызван без хоста, то есть слушает все интерфейсы и отдаёт
  данные о здоровье клиентов без единой проверки.

**Правило:** имя файла, комментарий, лог и документация — это гипотеза, а не
факт. Проверять тем, что показывает поведение:

```bash
lsof -nP -iTCP -sTCP:LISTEN | grep node   # на чём реально слушает процесс
curl -s -o /dev/null -w '%{http_code}\n' http://<lan-ip>:<port>/<endpoint>
```

Для секрета — сравнение с Lockbox (см. выше), для сетевой службы — запрос с
другого адреса, для гейта — запрос без учётных данных.

**Статус в документе — тоже гипотеза.** Правило распространяется и на нашу
собственную документацию, включая этот файл. В ту же ночь 2026-08-02: строка
SEC-034 стояла `open (cleanup)`, когда код уже был удалён в том же дереве, а
протокол реализации утверждал «не задеплоено» после выката. Оба расхождения
нашлись не чтением документа, а сверкой с фактическим состоянием — `git show` на
`origin/main`, `git branch -r --contains <sha>`, живой запрос к эндпоинту.

Практический вывод для аудита: прежде чем сослаться на статус в
`SECURITY_REVIEW.md` или в протоколе как на факт, проверить его источником.
Особенно когда над деревом работает несколько сессий: документ отражает момент,
когда его писали, а не момент, когда его читают.
