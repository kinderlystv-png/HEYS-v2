...
| SEC-024 | 2026-06-14 | L3   | API / REST / IDOR     | **Critical (launch-blocker)** | 🚨 Live L3.3 проба показала: `GET /rest/client_kv_store?client_id=eq.<UUID>&k=eq.heys_profile&select=v` **без любого Authorization-header** → HTTP 200 + **полный профиль другого клиента** (`birthDate`, `firstName`, `lastName`, `gender`, `weight`, `cycleTrackingEnabled`, `subscription_*`, `trial_*`). Anon read **всех 557 ключей** Александры. Также **anon POST** — `POST /rest/client_kv_store -d '{"client_id":"<чужой>","k":"...","v":{...}}'` без auth → запись успешно создаётся в БД (signed `success:true,processed:1`). Атакующий не нуждается в auth — только UUID клиента (36 chars, утекает в email-ссылках, screenshot'ах, log файлах, Referer). Нарушение 152-ФЗ + полная компрометация мультитенант-изоляции. Root cause: `YandexAPI.rest()` (`apps/web/heys_yandex_api_v1.js:401`) дизайн-фактически не передавал Authorization header; heys-api-rest GET handler не имел auth-проверки. POST уже имел `validateContextForWriteRest`, но в **warn-mode** (`HEYS_WRITE_CONTEXT_STRICT=0`) пропускал no-context writes silent. | curl `GET https://api.heyslab.ru/rest/client_kv_store?client_id=eq.4545ee50-...&k=eq.heys_profile&select=v` → 200 + PII; curl POST без auth → 200 success | **fixed write + partial read** (2026-06-14): **(a) Write-block deployed:** `HEYS_WRITE_CONTEXT_STRICT=1` для heys-api-rest (deploy-all.sh +5 строк env-block). Live-проба anon POST → HTTP 200 `{processed:0, blocked:1, reason:context_required}` + audit-event `context_required allowed=false rest_strict`. Safe: 0 real REST writes без context_id за 7 дней audit'a. **heys-api-rpc остаётся в warn-mode** до 2026-06-21 (SEC-004 plan). **(b) Read-block warn-mode deployed:** `heys-api-rest/index.js` +110 строк `enforceClientKvAuthForGet` middleware — резолвит `X-Session-Token`/`Authorization: Bearer <session>` header → `client_sessions.token_hash = sha256($1::bytea)` → определяет authedClientId → сравнивает с queried client_id → mismatch → audit `rest_read_cross_client_warn`. Курaторские JWT (3-part dot-separated) пропускаются с warn-log (v2: добавить curator-JWT verify). **(c) Client-side fix:** `apps/web/heys_yandex_api_v1.js` rest() передаёт `X-Session-Token: <token>` если LS-токен есть. Bundle rebuilt. Live-проба warn-mode 2026-06-14: anon GET → warn + проход; sessioned own GET → проход без warn; sessioned cross-client GET → warn `authed=ccfe6ea3 requested=4545ee50` + проход. **(d) Flip STRICT 2026-06-16** (≥48ч observation): `HEYS_REST_READ_STRICT=1` → mismatch → 401/403. **Pre-flip checklist:** audit-log за 48ч показывает 0 unexpected warns; bundle deployed клиентам; curator-JWT verify добавлен. См. также [[project-write-contexts]]. |

> Следующий `SEC-025`. Добавляй новые находки сюда; не переписывай существующие.
