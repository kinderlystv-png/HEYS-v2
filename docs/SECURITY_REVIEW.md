...
> отозван, heys*rpc получил явный EXECUTE на все 70 функций, heys_rest — на
> `safe_upsert_client_kv`. Verify-SELECT в обеих миграциях вернул 0 строк. Логи:
> `security-reports/migrations/2026-06-08_M{1,2}*\*.txt`. SEC-015/016 →
> **fixed**. Smoke-test приложения (PIN-вход, curator-sync, REST KV, messages,
> payments) — пользователю в браузере; rollback (если что-то всплывёт) — внутри
> M2.
...
| ✅ P1  | SAST `ENFILE` починен (прунинг каталогов); осталось убрать `continue-on-error` + тюнить ruleset                                                  | SAST бежит (4516 файлов), но шумен и пока не блокирует                                 | SEC-002, SEC-014 |
| **P1** | REST POST `/rest/shared_products`: отдавать явный 403/405, не утечку `code:42601`                                                                | Прод-ответ раскрывает внутреннюю ошибку БД                                             | SEC-003          |
...
| SEC-003 | 2026-06-08 | L1   | API REST             | Medium   | Прод REST POST `/rest/shared_products` отдаёт `{code:42601,...}` (внутренняя ошибка БД) вместо 403/405                                                                                                                                                                                                                                                                                                                                                              | `bash scripts/security-smoke-test.sh` (8 passed, 1 failed)                                                | open                                                                                                                                                         |
