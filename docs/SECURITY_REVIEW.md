...
| **P1** | БД-hardening: **миграции готовы** — `…_harden_secdef_search_path.sql` (safe, apply сразу) + `…_revoke_public_execute_secdef.sql` (после стейдж/верификации; rollback внутри) | Crown-jewel функции без search_path; PUBLIC-execute ломает least-privilege             | SEC-015, SEC-016 |
