...
| SEC-006 | 2026-06-08 | L0   | Photos / S3           | Medium   | Объекты пишутся `ACL: public-read` → знание URL = доступ к фото (потенциально PII)                                                                                                                                                                                                                                                                                                                                                                                                                           | static read `heys-api-photos`                                                                                                                  | **confirmed live** (2026-06-08, L5.1 проба: `yc storage bucket get heys-photos` → `anonymous_access_flags.read=true`, `list=false`. То есть list через `aws s3 ls` заблокирован, но прямой `GET <URL>` работает по знанию пути. Защита держится только на entropy именования; рекомендуется или снять anonymous read, или signed URLs — отдельный change в YC infra) (статус остаётся open до закрытия).                                                                                                                                                                                                                                                                                |
