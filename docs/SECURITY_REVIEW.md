...
- **Последнее обновление**: 2026-08-02 — разбор находок security-review ветки
  MCP-коннектора и REST-каталога: заведены **SEC-028…032**, из них SEC-029
  (SQL-инъекция через имя колонки) закрывает пункт SQLi из чек-листа L4. Все
  правки лежат в рабочем дереве, не закоммичены и не задеплоены; ход работ и
  порядок деплоя —
  [`docs/implementation/SECURITY_HARDENING_PROTOCOL_2026-08-02.md`](implementation/SECURITY_HARDENING_PROTOCOL_2026-08-02.md).
  Предыдущее обновление: 2026-07-31 — документационная сверка статусов SEC-004 /
  SEC-024 / L6 с фактическим состоянием (evidence 2026-07-26). Полный
  содержательный аудит — 2026-06-08; исходные записи журнала сохранены, новые
  факты добавлены датированными строками. Живые статусы задач ведёт
  [`маркетинг/22`](../маркетинг/22_План_реализации_маркетинга.md) § 6Б.
...
| **L1** | Прогон инструментов: `pnpm audit`, `dependency-check`, SAST, `security-smoke-test`                                                                                          | ✅     | SAST ENFILE починен (SEC-002); SEC-010 закрыт для prod/runtime после `esbuild >=0.28.1`; остался общий L7 hardening                                                                                |
...
| **L3** | Auth/доступ динамика: brute-force PIN/login, IDOR-подмена `client_id`/`context_id`, прод-значение `HEYS_WRITE_CONTEXT_STRICT`, CORS curl — **готовый чек-лист → Раздел 10** | 🟡     | L3.1/L3.2 live PASS 2026-06-14 после SEC-023 fix; остальное — чек-лист Раздела 10                                                                                                                  |
| **L4** | Инъекции/XSS динамика: пробы в meals/messages/products/FAQ/lead, трассировка источника FAQ-HTML, SQLi в REST-фильтрах — **готовый чек-лист → Раздел 10**                    | 🟡     | 2026-08-02: SQLi в REST-фильтрах закрыт статически (**SEC-029**, whitelist имён колонок, контрактный тест). Остальное по чек-листу Раздела 10 не прогонялось; динамические пробы после деплоя — ⬜ |
...
| **L7** | Hardening CI/CD: убрать `continue-on-error`, сделать gitleaks/audit/SAST блокирующими, SBOM                                                                                 | ✅     | 2026-06-14: SAST и dependency-check блокируют CI; gitleaks workflow blocking; SBOM job генерирует CycloneDX artifact                                                                               |
...
Ежеквартально, а также перед paid-scale или крупным изменением обработки ПДн,
прогонять короткий повтор security review:

| Блок              | Что перепроверить                                                                                | Evidence                                |
| ----------------- | ------------------------------------------------------------------------------------------------ | --------------------------------------- |
| Live-пробы        | L3/L4/L5 чек-лист из Раздела 10: brute-force, expired token, IDOR, SQLi/XSS, S3/webhook spoof    | curl/psql output или новая строка SEC   |
| SAST baseline     | `node scripts/security/sast-scan.js`; новые critical/high вне baseline не допускаются            | CI run / local report                   |
| Dependency triage | `pnpm audit --prod` + dev/build reachability для high/critical                                   | triage note в журнале                   |
| L6/152-ФЗ         | backup success markers, restore-drill sample, deletion/S3 cleanup, retention jobs, DSAR register | `SECURITY_REVIEW_l6_baseline.md` update |
| Secrets           | gitleaks/secret scan, webhook secrets, active tokens, Lockbox env                                | report без вывода секретов              |

Результат review: обновить дату в шапке, статусы слоёв L1-L7 и строки `SEC-NNN`;
если проверка дала FAIL, завести новую находку вместо устного комментария.

### Немедленный backlog (P0/P1)

| Приор.                                   | Действие                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Почему                                                                                                                                                                 | Находка          |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| ✅ P0 (остаток → SEC-027, accepted-risk) | Включить `HEYS_WRITE_CONTEXT_STRICT=1` в проде (после суток Phase B и чистки ложных `data_loss_audit`). По threat-model — топ-риск №1 (cross-client writes/IDOR). Самый дешёвый P0: env-var flip + sutki soak. — **Закрыто для `heys-api-rest` 2026-07-26** (production strict write+read, IDOR-матрица пройдена; `22` § 6Б.3/6Б.4, `docs/legal/operator/heys-r0-live-evidence-2026-07-26.md`). Остаток по `heys-api-rpc` вынесен в SEC-027 и принят как осознанный риск 2026-07-31. | Главная защита от межклиентского затирания работает в режиме «логируем, но пропускаем». Reviewer (`SECURITY_REVIEW_tier2.md` §R1): pre-launch блокер, не «желательно». | SEC-004          |
| ✅ P0                                    | Зависимости: triage runtime vs dev-only + hardening reachable build-chain advisory (`esbuild >=0.28.1`) — закрыто 2026-06-14.                                                                                                                                                                                                                                                                                                                                                        | Prod audit clean: 0 advisories / 0 high / 0 critical; full audit без high/critical.                                                                                    | SEC-010          |
...
| 🟡 P1                                    | Auth куратора: MFA (TOTP) + per-account lockout; задать ротацию `JWT_SECRET`                                                                                                                                                                                                                                                                                                                                                                                                         | 2026-06-14: source-fix + DB migration applied для MFA/lockout; pending source commit + штатный deploy `heys-api-auth`, ротация JWT отдельно                            | SEC-011          |
...
| ✅ P1                                    | Telegram bot secret_token check. **Активирован 2026-06-14** для `@heys_start_bot` (`/start-bot/webhook`): токен ротирован через BotFather revoke, `HEYS_START_WEBHOOK_SECRET` сгенерён (64 hex chars), функция передеплоена, `setWebhook` вызван. Live-проба: spoof POST без header → HTTP 403 `{"error":"forbidden"}` PASS. PIN-бот `/bot/webhook` всё ещё warn-only (отдельный токен, активация после live-bot launch).                                                            | DoS/spoof закрыт для HEYS Старт; PIN-бот pending до активации.                                                                                                         | SEC-020          |
| ✅ P1                                    | PIN brute-force lock. Live-проба 2026-06-14 сначала показала 12× `invalid_credentials` без lock; применена `database/2026-06-14_fix_pin_rate_limit_lock.sql`, повторная проба: 5× `invalid_credentials`, затем `pin_rate_limited`, `locked_until` выставлен.                                                                                                                                                                                                                         | Старый `check_pin_rate_limit` ставил lock и сразу `RAISE`, поэтому UPDATE откатывался вместе с транзакцией.                                                            | SEC-023          |
| ✅ P1 pre-launch                         | L3 IDOR sanity-check: cross-client read/write isolation. **Не предполагать, проверить.** Нужны 2 PIN/phone пары от user'а. — **Выполнено 2026-07-26** на двух синтетических клиентах: анонимное чтение B → 401, A→B → 403, A→A и curator→B → 200; cross-write с capability A не изменил B. Reviewer (§R2): главный незакрытый unknown для мультитенант-приложения.                                                                                                                   | Запуск без verified изоляции = неприемлемо для health-данных                                                                                                           | L3.3/L3.4        |
...
| --- | ------------------------------------------- | --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ----- |
| 1   | Аутентификация (curator JWT, PIN, sessions) | 🟡  | PBKDF2-100k пароль, bcrypt PIN, session-токены хешем, HttpOnly+Secure cookie, rate-limit; SEC-011 source+DB fixed                                                      | MFA/per-account lockout ещё не опубликованы в `heys-api-auth`; JWT 24ч без refresh/rotation                                                                    | P1     | ~     |
| 2   | Авторизация / мультитенантная изоляция      | 🟡  | client_id из `context_id` на сервере, `write_contexts`, RPC-only, `curator_write_lock`; strict write+read live для `heys-api-rest` (2026-07-26), IDOR-матрица пройдена | `heys-api-rpc` остаётся warn-mode на merge/curator-путях — SEC-027 `accepted-risk` (2026-07-31); cross-client запись по session-путям невозможна конструктивно | P1     | ✓     |
| 3   | БД: роли, RLS, SECURITY DEFINER             | 🟡  | (L2 ✓) 0 прямых table-грантов anon/PUBLIC; FK CASCADE на 19 табл.; write_contexts USING(false); RLS on 49/49 tables                                                    | Остаток: не включать `FORCE ROW LEVEL SECURITY` без перевода runtime с owner-роли и явных role/context policies                                                | P1     | ✓     |
...
| 10  | Rate limiting / anti-abuse                  | 🟡  | PIN 10/15мин, логин 10/15мин в БД, лимит лидов, anti-enumeration; per-account lockout source+DB ready                                                                  | Нет CAPTCHA; обход ротацией IP; per-account lockout ждёт deploy auth-функции; спуф `X-Forwarded-For`                                                           | P2     | ~     |
...
| 12  | Целостность синхронизации / защита данных   | 🟡  | shrink-guard, tombstones, central stamper, `isNonClientDataKey`, lint-гейты; SEC-007 source-fixed                                                                      | Остаётся широкий ceiling write-merge/CAS для device-clock конфликтов                                                                                           | P1     | ~     |
...
| 14  | Зависимости / supply chain                  | 🟢  | gitleaks, pre-commit/pre-push lint'ы, pnpm-lock; dependency-check blocking; SBOM artifact; prod audit clean после SEC-010                                              | Остаются dev/build-only low/moderate advisories без high/critical; `^`-диапазоны требуют регулярного review                                                    | P2     | ✓     |
| 15  | SAST / Security CI                          | 🟢  | Workflow SAST/dep/secret на PR+schedule; SAST baseline-ratchet; dependency-check blocking; SBOM artifact                                                               | Остаточный риск — поддерживать baseline и scheduled runs; внешняя L8-проверка отдельно                                                                         | P2     | ✓     |
| 16  | Landing app                                 | 🟡  | Static export, lead API: consent/honeypot/rate-limit серверный; `next@15.5.18`, landing prod audit clean                                                               | FAQ `dangerouslySetInnerHTML`; iframe sandbox allow-same-origin+scripts; нет security-headers                                                                  | P2     | ~     |
...
**До публичного запуска НЕ закрыты (срез 2026-06-08/14 по reviewer'у
`SECURITY_REVIEW_tier2.md`; актуализация — сразу под списком):**
...
- **SEC-010** — закрыт для prod/runtime 2026-06-14: `esbuild >=0.28.1`,
  `pnpm audit --prod --json` = 0 advisories, full audit без high/critical.
- **L6** — restore-drill, 152-ФЗ retention/deletion-cascade, RPO/RTO baseline не
  начат. Юридическое требование для перс-данных.
- **SEC-020 активирован 2026-06-14** для `@heys_start_bot`: токен revoked в
  BotFather + ротирован в `.env`, `HEYS_START_WEBHOOK_SECRET` сгенерён,
  `setWebhook` вызван, live-проба spoof'a → HTTP 403 PASS. PIN-бот
  `/bot/webhook` всё ещё warn-only до активации (отдельный токен,
  `TELEGRAM_WEBHOOK_SECRET` пуст).

**Актуализация 2026-07-31 (что из списка выше уже закрыто):**

- **SEC-004** — закрыт для `heys-api-rest`: production
  `HEYS_WRITE_CONTEXT_STRICT=1` 2026-07-26, readiness `ready=true`,
  warn-счётчики пусты. Остаток по `heys-api-rpc` → **SEC-027**: принят как
  `accepted-risk` решением владельца 2026-07-31 (ниже).
- **L3 IDOR** — verified 2026-07-26 на двух синтетических клиентах: anon → 401,
  A→B → 403, A→A и curator→B → 200, cross-write не изменил B.
- **SEC-006** — решение владельца от 2026-06-14: Вариант B (accepted-risk +
  `Cache-Control: private, no-store` + `referrer: no-referrer`).
- **L6** — baseline проведён 2026-06-14/15
...
Живые статусы задач ведёт `22`, не этот файл.

**Непройденные слои:** L3 (частично — L3.5 done), L4 (частично — L4.1/4.2/4.6
done), L5 (частично — L5.1/5.3/5.4/5.5 done), L6 (не начат), L8 (внешний пентест
— третья сторона). L7 закрыт 2026-06-14. — _срез 2026-06-14; L3.1/L3.2/L3.3 и L6
с тех пор выполнены, см. актуализацию выше._
...
| SEC-004 | 2026-06-08 | L0   | Tenant isolation        | High                          | `HEYS_WRITE_CONTEXT_STRICT` по умолчанию off → write-context в warn-only (пишет audit, но пропускает запись)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `heys-api-rpc/index.js:569`, `heys-api-rest/index.js:114`                                                                                                                                                                                                                                                                  | **Phase B+ deployed, мониторинг 7д** (2026-06-14, solo-code-trace раскрыл: warn-mode НЕ писал в audit при `!ctxId` — silent pass. Это значит «0 событий context_required» в 30д-дампе ничего не доказывало (no-context cases были silent). **Деплоен patch** (commit `47c2819e`): добавлен INSERT `context_missing_warn` (allowed=true) в Phase B-ветке в `heys-api-rest/index.js:128+` + `heys-api-rpc/index.js:584+`. Через 7 дней проверить: `SELECT count(*) FROM data_loss_audit WHERE action='context_missing_warn'`. Если 0 → flip STRICT=1 safe. Если >0 → видно каких клиентов/endpoints обновить до flip'a. Триаж за 30д до patch'a: 204 cross-client попыток блокированы (`dayv2_content_dup`=141, `blob_blocked`=63), 0 FP — reviewer-критерий выполнен на видимой выборке. **Апдейт 2026-07-26:** flip выполнен для `heys-api-rest` — production `HEYS_WRITE_CONTEXT_STRICT=1`, повторный `security:strict-readiness` дал `ready=true`, warn-счётчики пусты (`22` § 6Б.3, R0-evidence 2026-07-26). **Апдейт 2026-07-31:** статус уточнён — flip покрывает только `heys-api-rest`; `heys-api-rpc` остаётся warn-mode, остаток вынесен в SEC-027.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| SEC-007 | 2026-06-08 | L0   | Sync integrity          | Medium                        | L3 server-revision pull-gate возвращал `true` без `server_revision` → эффективного гейтинга не было; LWW на device-clock                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `heys_storage_supabase_v1.js`; `heys_sync_queue_runtime_pure_v1.js`; tests `sync-queue-runtime-pure` + `hot-sync-curator-path`                                                                                                                                                                                             | **source-fixed** (2026-06-14, `shouldApplyByRevision({ requireRemoteRevision:true })` теперь блокирует no-revision pull, если localRevision уже известна; storage включает этот режим после активного `_lastMarkerCheckRevision` и пишет `_serverRevisionGateBlocks` diagnostics. Проверки: `vitest run apps/web/__tests__/sync-queue-runtime-pure.test.js apps/web/__tests__/hot-sync-curator-path.test.js --no-coverage` → 28/28 PASS; `node --check` для sync/storage PASS. Не bundled/deployed без отдельной команды.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| SEC-010 | 2026-06-08 | L1   | Deps                    | High                          | `pnpm audit`: 160 advisories (5 critical / 72 high / 63 moderate / 20 low); не разнесены runtime vs dev                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `pnpm audit --audit-level moderate`; 2026-06-14: `pnpm audit --prod --json`, `pnpm audit --json`, `pnpm-lock.yaml`                                                                                                                                                                                                         | **fixed for prod/runtime** (2026-06-14: solo-триаж 6Б.6 показал 0 runtime-reachable high/critical; затем выполнен build-chain hardening: `package.json` override `esbuild >=0.28.1`, lockfile резолвит `esbuild@0.28.1`. Evidence: prod audit clean — 0 advisories / 0 high / 0 critical; full audit — 0 high / 0 critical, только dev/build-only low/moderate `tsup`, `js-yaml`, `ajv`, `brace-expansion`.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| SEC-011 | 2026-06-08 | L0   | Auth                    | Medium                        | Нет MFA для кураторов; lockout только per-IP, не per-account; ротация `JWT_SECRET` не задокументирована                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `heys-api-auth/index.js`; `database/2026-06-14_curator_mfa_account_lockout.sql`; `yandex-cloud-functions/heys-api-auth/__tests__/mfa-totp.test.js`                                                                                                                                                                         | **source+DB fixed, deploy pending** (2026-06-14): добавлены TOTP endpoints `/auth/mfa/status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | setup | enable | disable`, login принимает `mfa_code`, TOTP secret хранится AES-GCM ciphertext, неверный пароль/TOTP увеличивает `curators.failed_login_attempts`, lockout 10/15мин через `login_locked_until`; web login/API передают optional code 2FA. Prod DB migration applied: 6 колонок подтверждены через `information_schema`. Проверки: `node --check yandex-cloud-functions/heys-api-auth/index.js`; `node --check apps/web/heys_yandex_api_v1.js apps/web/heys_storage_supabase_v1.js apps/web/heys_app_hooks_v1.js`; `node --test yandex-cloud-functions/heys-api-auth/**tests**/mfa-totp.test.js`→ 3/3 PASS. Pending: source commit + штатный deploy`heys-api-auth`; deploy-script отказывает dirty source без commit. JWT_SECRET rotation policy остаётся отдельной частью SEC-011. |
...
| SEC-006 | 2026-06-08 | L0   | Photos / S3             | Medium                        | Объекты пишутся `ACL: public-read` → знание URL = доступ к фото (потенциально PII)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | static read `heys-api-photos`                                                                                                                                                                                                                                                                                              | **accepted-risk Вариант B + 2 митигации deployed** (2026-06-14, по решению основателя «1-4 делай, 1 рекомендуемо»): (a) `heys-api-photos/index.js:233`: добавлен `CacheControl: 'private, no-store, max-age=0'` в PutObjectCommand — фото не оседают в shared caches/CDN; (b) `apps/web/index.html:6`: `<meta name="referrer" content="no-referrer">` — photo URL не уезжают в Referer при click на внешние ссылки; (c) deployed (commit `9c531204`). Защита держится на ~318 бит entropy ключа + 2 митигациях против URL leakage. Полная обоснование: [`SECURITY_REVIEW_sec006_recommendation.md`](SECURITY_REVIEW_sec006_recommendation.md). **Триггеры пересмотра на Вариант A** (signed URLs): 100+ клиентов, sharing feature, compliance audit, real URL leak incident.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| SEC-020 | 2026-06-08 | L5   | API / Telegram bot      | Medium                        | Live-проба: `POST /bot/webhook` без любых credentials → `{ok:true}`. Функция `heys-bot-client` обрабатывает Telegram-update без верификации `X-Telegram-Bot-Api-Secret-Token`. Атакующий может: (a) спамить bot-users fake-сообщениями (DoS bot rate-limit на Telegram стороне), (b) race-claim`claim_pin_token_chat` с угадаемым/утечённым UUID (122-bit entropy ≈ невозможно brute, но утечка ссылки в email/screenshot реалистична).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | curl `POST /bot/webhook` с `{update_id:111, message:{chat:{id:-1001}, text:"/test"}}` → HTTP 200 `{ok:true}`                                                                                                                                                                                                               | **fixed + ACTIVATED 2026-06-14** для `@heys_start_bot` (`/start-bot/webhook`): (1) старый `HEYS_START_BOT_TOKEN` ротирован — basis: засветился в chat-логах при grep по `.env` (incident-driven cleanup); user revoke в BotFather → новый токен в `.env:28`; (2) `HEYS_START_WEBHOOK_SECRET` сгенерён `openssl rand -hex 32` (64 hex chars в `.env:29`); (3) функция `heys-bot-client` передеплоена через `./deploy-all.sh`; (4) `setWebhook` вызван — Telegram теперь шлёт `X-Telegram-Bot-Api-Secret-Token` header при каждом update; (5) live-проба curl-spoof'ом: POST `/start-bot/webhook` без header → HTTP 403 `{"error":"forbidden"}`, с неверным header → HTTP 403 PASS. **PIN-бот** `/bot/webhook` всё ещё warn-only (`TELEGRAM_WEBHOOK_SECRET` пуст) — закроется одновременно с активацией PIN-бота. См. [[feedback-no-secret-echo]].                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| SEC-021 | 2026-06-14 | L6   | Backup chain            | High                          | Backup-chain имеет 27-дневную дыру с 2026-04-14 по 2026-05-10 (нет daily backups). Если клиент существовал и удалился в этот период — данные невосстановимы. Влияет на 152-ФЗ DSAR (могут не суметь восстановить данные субъекту по запросу).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `aws --endpoint-url=https://storage.yandexcloud.net s3 ls s3://heys-backups/client-daily/` → видимый gap                                                                                                                                                                                                                   | **mitigated (watchdog deployed)** (2026-06-14, root-cause investigation: function `heys-client-daily-backup` создана 2026-03-29, видимых versions только 4 от 2026-05-22 — гипотеза: accidental delete versions 2026-04-14, trigger продолжал срабатывать без deployed code (silent failure), восстановили 2026-05-22. Старые backups невосстановимы. **Mitigation deployed** (commit `03d0acfd`): миграция `database/2026-06-14_backup_run_log.sql` (таблица success-маркеров) + `heys-client-daily-backup/index.js:495+` INSERT success-marker после каждого run'а + `heys-cron-security-alerts/index.js` правило `backup_chain_gap`. 2026-06-15 rule уточнён на gap-based критерий: alert, если последний `ok/partial` run старше 30 часов, без false-positive периода накопления первых 5 daily runs. Локальный guard: `pnpm security:l6-watchdogs`. Сам backup-chain исторически имеет дыру, но дальше будет early-detected.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| SEC-022 | 2026-06-14 | L6   | Photos / 152-ФЗ §14     | Medium                        | При `DELETE FROM clients WHERE id = <uuid>` 18 FK CASCADE удаляют все ПДн из БД ✅, но photos в S3 `heys-photos/<cid>/...` **НЕ удаляются** (нет связи S3 ↔ БД на уровне FK/триггеров). Это нарушение 152-ФЗ §14 («право на удаление»): ПДн (фото еды могут содержать PII — лицо, документы) остаётся после deletion аккаунта. Дополнительно: storage bloat.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `database/2026-05-11_add_clients_fk_cascade.sql` + grep `S3` cleanup в `heys-cron-*` → no orphan cleanup                                                                                                                                                                                                                   | **fixed + deployed** (2026-06-14, commits `07b2cdf1`+`25175635`): новая cloud function `heys-cron-photo-cleanup` (id `d4ej3g8h2enrf46bj58q`), миграция `database/2026-06-14_photo_cleanup_log.sql` (audit-log таблица), timer trigger `heys-cron-photo-cleanup-timer` (`a1s67dnisamn2gd8f7f0`, cron `0 6 ? * MON *` = понедельник 06:00 UTC). **Safety: DRY_RUN=1 по умолчанию** (env), soft-grace 7 дней (orphan должен быть detected в predыдущем run'е тоже), hard cap 100 prefix'ов / run. Manual invoke 2026-06-14 PASS: `scannedPrefixes:2, orphanCandidates:[]`. Для активации реальных удалений выставить `DRY_RUN=0` в env-vars функции после 1-2 weeks of dry-run observation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| SEC-023 | 2026-06-14 | L3   | Auth / PIN rate-limit   | High                          | Live L3.1 probe показала, что PIN attempts пишутся (`attempts=12`), но `locked_until` остаётся `NULL`: старый `check_pin_rate_limit` пытался поставить lock и затем `RAISE`, из-за чего UPDATE откатывался.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 12× `POST /rpc?fn=verify_client_pin_v3` с `+71234567890` → `invalid_credentials`; DB: `pin_login_attempts.attempts=12`, `locked_until IS NULL`                                                                                                                                                                             | **fixed + live-verified** (2026-06-14): применена `database/2026-06-14_fix_pin_rate_limit_lock.sql`; lock выставляется в `increment_pin_attempt` до обычного JSON-ответа, `check_pin_rate_limit` только читает lock и бросает exception на следующих попытках. Повторная проба: 5× `invalid_credentials`, затем 3× `pin_rate_limited:15 minutes`; DB: `attempts=5`, `locked=true`, `security_events`: `pin_failed=5`, `pin_locked=1`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| SEC-017 | 2026-06-08 | L2   | БД: RLS                 | Medium                        | RLS включён лишь на 4 из 45 таблиц; **выключен на `clients`, `client_kv_store`, `client_sessions`, `payments`, `consents`, `subscriptions`, аудит-таблицах**. `auth.uid()`-политики из миграций НЕ задеплоены (нет в `pg_policies`). DB-уровень не даёт defense-in-depth — вся изоляция держится на RPC + write_contexts (single layer)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `database/2026-06-14_enable_public_rls_defense_in_depth.sql`; live `pg_class.relrowsecurity`                                                                                                                                                                                                                               | **fixed as owner-bypass RLS hardening** (2026-06-14): миграция включила RLS на всех ordinary `public` tables без `FORCE ROW LEVEL SECURITY`, чтобы текущий runtime `PG_USER=heys_admin` не ломался, а non-owner direct grants стали fail-closed без явных policies. Evidence: after migration `rls_on=49 rls_off=0 total=49`; critical owner/runtime read smoke: `clients=4`, `client_kv_store=1488`, `payments=0`, `consents=88`; `relforcerowsecurity=true` → empty. Next hardening step, если runtime переведут с owner-роли: добавить явные role/context policies и только затем рассматривать `FORCE`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| SEC-024 | 2026-06-14 | L3   | API / REST / IDOR       | **Critical (launch-blocker)** | 🚨 Live L3.3 проба показала: `GET /rest/client_kv_store?client_id=eq.<UUID>&k=eq.heys_profile&select=v` **без любого Authorization-header** → HTTP 200 + **полный профиль другого клиента** (`birthDate`, `firstName`, `lastName`, `gender`, `weight`, `cycleTrackingEnabled`, `subscription_*`, `trial_*`). Anon read **всех 557 ключей** Александры. Также **anon POST** — `POST /rest/client_kv_store -d '{"client_id":"<чужой>","k":"...","v":{...}}'` без auth → запись успешно создаётся в БД (signed `success:true,processed:1`). Атакующий не нуждается в auth — только UUID клиента (36 chars, утекает в email-ссылках, screenshot'ах, log файлах, Referer). Нарушение 152-ФЗ + полная компрометация мультитенант-изоляции. Root cause: `YandexAPI.rest()` (`apps/web/heys_yandex_api_v1.js:401`) дизайн-фактически не передавал Authorization header; heys-api-rest GET handler не имел auth-проверки. POST уже имел `validateContextForWriteRest`, но в **warn-mode** (`HEYS_WRITE_CONTEXT_STRICT=0`) пропускал no-context writes silent. | curl `GET https://api.heyslab.ru/rest/client_kv_store?client_id=eq.4545ee50-...&k=eq.heys_profile&select=v` → 200 + PII; curl POST без auth → 200 success                                                                                                                                                                  | **fixed (warn-mode read + strict write, curator v2 deployed)** (2026-06-14): **(a) Write-block deployed:** `HEYS_WRITE_CONTEXT_STRICT=1` для heys-api-rest. Live-проба anon POST → blocked + audit `context_required`. Safe: 0 real REST writes без context_id за 7 дней audit'a. heys-api-rpc остаётся warn-mode до 2026-06-21. **(b) Read-block warn-mode deployed:** `heys-api-rest/index.js` +110 строк `enforceClientKvAuthForGet` middleware (X-Session-Token → client_sessions sha256 → compare с queried client_id → mismatch → audit). **(c) Client-side fix:** `apps/web/heys_yandex_api_v1.js` rest() передаёт `X-Session-Token`. Bundle rebuilt. **(d) Curator-JWT verify v2 deployed:** добавлен `JWT_SECRET` в env-block heys-api-rest (deploy-all.sh), inline `verifyCuratorJwt` функция (копия от `shared/auth-helpers.js`); Case 2 в middleware — полный HS256 verify + payload.role === 'curator' + `SELECT 1 FROM clients WHERE id=$1 AND curator_id=$2` → cross-client → audit `rest_read_curator_cross_client_warn`. Live-пробы 2026-06-14 v2: (1) curator JWT → свой клиент → 200 без warn ✅; (2) curator JWT → random UUID → 200 empty + audit `cross_client_warn curator=6d4dbb32 requested=aaaaaaaa` ✅; (3) spoofed JWT (mangled signature) → audit `invalid_jwt_warn invalid-signature` ✅. **(e) Flip STRICT 2026-06-16** (≥48ч observation): `HEYS_REST_READ_STRICT=1` → mismatch + spoof → 401/403. **Pre-flip checklist:** audit-log за 48ч → 0 unexpected warns; bundle deployed клиентам; curator-JWT verify уже в проде. См. также [[project-write-contexts]]. **Апдейт 2026-07-26 — закрыто:** production `HEYS_REST_READ_STRICT=1`; live IDOR-матрица: anon read B → 401, A→B → 403, A→A и curator→B → 200, cross-write с capability A не изменил B; SEC-024 readiness `ready=true` (`22` § 6Б.4, R0-evidence 2026-07-26).                                                                                                                                                      |
| SEC-025 | 2026-06-14 | L4   | REST / UUID-cast        | Low (DX)                      | `GET /rest/shared_products?select=id,name&id=eq.1` → HTTP 500 `Internal server error` вместо 400/422. Причина: `shared_products.id` это UUID-колонка, `'1'::uuid` cast — exception → handler возвращает 500. SQL-injection попытка failed safely (нет утечки данных), но 500 вместо 400 = плохой DX + лишний шум в логах + потенциальная information disclosure в стэк-трейсах при future error-handling refactoring.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `curl https://api.heyslab.ru/rest/shared_products?select=id,name&id=eq.1` → HTTP 500                                                                                                                                                                                                                                       | open (improvement) — wrap UUID/integer parsing в try/catch в `heys-api-rest/index.js` SELECT path, возвращать 400 «invalid filter value» при type mismatch. Не launch-blocker.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| SEC-027 | 2026-07-31 | L0   | Tenant isolation / RPC  | Medium                        | Остаток SEC-004: strict-env выдаётся только функции `heys-api-rest`, `heys-api-rpc` работает в warn-mode (комментарий в деплое — «до 2026-06-21», флип не подтверждён). Разбор покрытия на 2026-07-31: `validateContextForWrite` в RPC вызывается только для `merge_save_client_kv_by_{session,curator}` и `batch_upsert_client_kv_by_curator`; generic session-пути (`upsert_client_kv_by_session`, `batch_upsert_client_kv_by_session`, `delete_client_kv_by_session`) валидацию не проходят, а `p_context_id` срезается перед SQL-вызовом. **Это не IDOR-дыра:** сигнатуры этих SQL-функций не принимают `client_id`, он резолвится server-side через `require_client_id(p_session_token)`, поэтому запись в чужой scope через session-путь невозможна. Незакрытым остаётся capability-контроль (защита от pollution «правильный клиент, данные из stale-состояния другого») на merge/curator-путях RPC.                                                                                                                                          | `yandex-cloud-functions/deploy-all.sh:474-482`; `yandex-cloud-functions/heys-api-rpc/index.js:739` (STRICT_MODE), `:2711`, `:3648` (call sites), `:4079` (срез `p_context_id`); `database/2025-12-25_p1_kv_session_functions.sql:65-82`; `database/2025-12-24_subscriptions_and_sessions.sql:94-106`                       | **accepted-risk** (2026-07-31, решение владельца — вариант B; вариант A «флип после observation window» рассмотрен и отклонён). **Обоснование:** флип не закрывает то, ради чего нужен — generic session-пути валидацию не вызывают вовсе, а cross-client запись по ним невозможна конструктивно (`client_id` резолвится server-side из session-токена). Ужесточение затронуло бы только merge/curator-пути и добавило риск 403 при `p_context_id = null` — тот же механизм, что уронил облачный синк PIN-клиентов 2026-06-15. **Остаточный риск:** capability-контроль на merge/curator-путях RPC остаётся в warn-mode; pollution-сценарий «правильный клиент, данные из stale-состояния другого» на этих путях логируется, но не блокируется. **Триггеры пересмотра:** (1) появление нового браузерного writer'а на RPC-путь; (2) подтверждённый инцидент pollution в `data_loss_audit` по merge/curator-путям; (3) подготовка к R2 (paid-scale); (4) переход generic session-путей на capability-контекст. См. § «Решение по SEC-027» ниже.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| SEC-026 | 2026-06-14 | L5   | API Gateway / Payments  | High (future-blocker)         | `POST /payments/webhook` в API Gateway сейчас замаплен на STUB-функцию (`d4e0rqplbonspm84pk3n`, health-placeholder). Когда `heys-api-payments` будет задеплоен и заменит function_id, нужно ОБЯЗАТЕЛЬНО прогнать L5.3 ЮKassa webhook-spoofing пробу (curl без подписи, fake payment.succeeded → 403 expected, не 200). Сейчас результат L5.3 = ✅ de-facto, но отражает stub, не реальный payments-flow.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | api-gateway-spec-v2.yaml `/payments/webhook` → `d4e0rqplbonspm84pk3n` (health stub)                                                                                                                                                                                                                                        | open (deferred) — re-probe после payments deploy. До тех пор HMAC-verify коду нужен в `heys-api-payments/index.js` (sign-check от ЮKassa shared secret). См. план 22 §1.2 Payment-readiness blocker.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| SEC-028 | 2026-08-02 | L0   | REST / Shared catalog   | High                          | `POST /rest/shared_products` с валидным телом проходил **без любой аутентификации**: CORS-гейт пропускает `origin === null` (curl), generic INSERT для этой таблицы не проверял роль, а пул подключается под владельцем таблиц (`PG_USER=heys_admin`), поэтому RLS не применялась. Любой мог дописать строку в общий каталог мимо очереди модерации.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `yandex-cloud-functions/heys-api-rest/index.js:648` (гейт); контракт `yandex-cloud-functions/__tests__/rest-shared-products-write-auth.contract.test.cjs` (7/7 pass)                                                                                                                                                       | **fixed** (2026-08-02, commit `7b8f2d42`): write (POST/PATCH/DELETE) в `shared_products` и `shared_products_pending` требует валидный curator JWT (Bearer или HttpOnly cookie), fail-closed; чтение не тронуто. ⚠️ ID: в коммите и в комментарии кода находка изначально была помечена `SEC-025` — номер, уже занятый находкой от 2026-06-14. Комментарий и тест переименованы в `SEC-028` 2026-08-02; в тексте коммита `7b8f2d42` старый номер остаётся историческим. Не задеплоено.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| SEC-029 | 2026-08-02 | L4   | REST / SQL injection    | High                          | **SQL-инъекция через имя колонки.** Имя колонки бралось из ключа query-параметра (фильтры GET/PATCH/DELETE) или из ключа JSON-тела и `on_conflict` (POST) и подставлялось в SQL как идентификатор без экранирования: `"${fieldName}" = $1`. Двойная кавычка внутри имени закрывала идентификатор и позволяла дописать произвольное условие. В ветке `is.` плейсхолдер не добавляется вовсе, поэтому нумерация параметров не ломалась и запрос оставался синтаксически валидным — слепая инъекция на публичном GET-пути каталога. Пример вектора: `?is.x" IS NULL OR (SELECT 1) IS NULL --=null`. Закрывает пункт «SQLi в REST-фильтрах» из чек-листа L4.                                                                                                                                                                                                                                                                                                                                                                                             | `yandex-cloud-functions/heys-api-rest/index.js:467` (`sanitizeFilterColumn`), `:481` (`NON_FILTER_PARAMS`), применение `:918` (GET), `:1638` (POST), `:1778` (PATCH), `:1846` (DELETE); контракт `yandex-cloud-functions/__tests__/rest-filter-column-injection.contract.test.cjs` (11/11 pass)                            | **fixed + live** (закоммичено `8291125c1`, задеплоено 2026-08-02, прод `2026.08.02.0321.9922a2d2`; живая проба: инъекционный ключ → HTTP 400 `invalid_filter_column`, легальный фильтр → 200): имя колонки проходит regex `^[a-zA-Z_][a-zA-Z0-9_]*$` + whitelist `ALLOWED_COLUMNS[table]`, иначе 400. Проверка стоит в начале цикла разбора параметров, а не в каждой из 19 ветвей — новая ветвь не сможет появиться в обход. Регрессия проверена разбором всех вызывающих (apps/web, apps/mobile, scripts, cloud functions, тесты): не-колоночных ключей ровно три группы, все внесены в `NON_FILTER_PARAMS`. ⬜ Остаётся динамическая проба curl'ом по проду после деплоя.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| SEC-030 | 2026-08-02 | L0   | MCP / OAuth             | High                          | **Открытая DCR + страница согласия, скрывающая получателя.** `POST /mcp/register` работает без аутентификации и принимал ЛЮБОЙ https-`redirect_uri`, а `/mcp/authorize` собирает первичные учётные данные (телефон+PIN клиента; email+пароль+TOTP куратора) и не показывает, куда уйдёт код; единственный признак приложения — `client_name` из запроса регистрации. Итог — готовый фишинг на легитимном домене с валидным TLS: обычная защита «проверь домен перед вводом пароля» не срабатывает, потому что домен действительно наш. Отдельно: в согласии заявлен только scope `heys:diary` («доступ к дневнику»), тогда как кураторский вход открывает переписку, заведение клиентов и смену PIN, подписки, заявки и лиды.                                                                                                                                                                                                                                                                                                                        | `yandex-cloud-functions/heys-mcp/lib/oauth.js:56` (allowlist), `:141` (повторная проверка при authorize), `:387` (получатель на странице), `:416` (объём доступа); `yandex-cloud-functions/heys-mcp/__tests__/oauth.test.cjs` (22/22 pass)                                                                                 | **fixed + live** (закоммичено `42660019c`, задеплоено 2026-08-02; живая проба: `POST /mcp/register` с `https://evil.tld/cb` → HTTP 400 `invalid_redirect_uri`, с `https://claude.ai/api/mcp/auth_callback` → 201): allowlist хостов redirect_uri (`claude.ai`/`claude.com`/`anthropic.com` с поддоменами + loopback любой порт), переопределение `MCP_ALLOWED_REDIRECT_HOSTS` (список через запятую, `*.example.com` — поддомены; заменяет список целиком). Проверка продублирована в `validateAuthorizeRequest`, иначе `client_id`, выданные до фикса, работали бы весь свой годичный TTL. Список держится в коде: heys-mcp при деплое получает ровно одну env-переменную (`LOCKBOX_APP_SECRET_ID`), остальное — Lockbox-overlay. На странице показан origin получателя и пометка, что имя приложения HEYS не проверяет; объём кураторского доступа перечислен явно.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| SEC-031 | 2026-08-02 | L0   | MCP / Session mgmt      | Medium                        | **Кураторский JWT перевыпускался на refresh без единой серверной проверки.** `exchangeRefreshToken` подписывала свежий `role: curator` JWT боевым `JWT_SECRET`, не спрашивая, существует ли куратор и не отключён ли он. Refresh-токен живёт 30 суток и выдаётся заново на каждом обмене, `/mcp/token` публичен и без клиентской аутентификации, а `client_id` был необязателен (`if (form.client_id && ...)`) — то есть утёкший refresh-токен давал возобновляемый кураторский доступ, который нельзя прекратить ничем, кроме ротации общего `JWT_SECRET`. При этом страница согласия обещала обратное: «куратору — отключить коннектор в Claude», что не отзывает ничего на сервере; те же ложные утверждения были в шапках `crypto-tokens.js` и `oauth.js`.                                                                                                                                                                                                                                                                                       | `yandex-cloud-functions/heys-api-auth/index.js:1096` (`handleCuratorStatus`), `yandex-cloud-functions/api-gateway-spec.yaml` (`/auth/curator-status`), `yandex-cloud-functions/heys-mcp/lib/heys-api.js:318`, `yandex-cloud-functions/heys-mcp/lib/oauth.js:270`; `heys-mcp/__tests__/curator.test.cjs` (5 новых проверок) | **partially fixed + live** (закоммичено `42660019c`, задеплоено 2026-08-02; живая проба: `GET /auth/curator-status` без токена → HTTP 401, то есть эндпоинт существует и закрыт). Сделано: новый `GET /auth/curator-status` читает `curators.is_active` (единственный признак деактивации в схеме — `database/2025-12-22_curators.sql:14`; `/auth/verify` для этого не годился, он в БД не ходит вовсе); перевыпуск идёт только после положительного ответа сервера, fail-closed при отказе и при недоступности; проверка выполняется под свежевыпущенным JWT, потому что прежний к моменту продления обычно мёртв; `client_id` в refresh-гранте стал обязательным; тексты страницы и шапок приведены к фактам. ⚠️ **Порядок деплоя:** `heys-api-auth` + шлюз строго ДО `heys-mcp`, иначе проверка бьётся в 404 и все кураторские refresh отказывают. **Остаток:** мгновенного отзыва уже выданного кураторского JWT нет (окно до 24 ч) — нужен `token_version` в токене и сверка во всех функциях; заведено в `todo.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| SEC-032 | 2026-08-02 | L0   | REST / Authorization    | Medium                        | Гейт SEC-028 проверял роль, но не владение: куратор A с валидным JWT мог через прямой REST PATCH/DELETE изменить или удалить заявку из очереди модерации куратора B. Штатный путь так не делает — вся модерация идёт через атомарную RPC `moderate_pending_shared_product_by_curator`, где ownership проверяется, — но REST её обходил.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `yandex-cloud-functions/heys-api-rest/index.js:698` (гейт), `:1836` (PATCH), `:1888` (DELETE); контракт `yandex-cloud-functions/__tests__/rest-pending-ownership.contract.test.cjs` (6/6 pass)                                                                                                                             | **fixed + live** (закоммичено `8291125c1`, задеплоено 2026-08-02): чужой `curator_id` в фильтре → 403 `cross_curator_forbidden`, и — главное — сервер сам дописывает `"curator_id" = $n` в WHERE, иначе запрос без этого фильтра (только по `id` заявки) дотянулся бы до чужой строки. `shared_products` под правило не попадает: у общего каталога колонки `curator_id` нет. Семантика подтверждена: `curator_id` в pending — куратор клиента-автора («чья очередь», `database/2025-12-16_shared_products_pending.sql:18`), модератор пишется отдельно в `moderated_by`; общей очереди и супер-модератора в продукте нет.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| SEC-033 | 2026-08-02 | L0   | MCP / Docs              | Low                           | Документация обещала отзыв, которого нет: `heys-mcp/README.md` утверждал «отзыв кураторского доступа: отключить коннектор в Claude». Это тот же ложный текст, что был на странице согласия и убран в SEC-031 — отключение коннектора не отзывает на сервере ничего. Опасно тем, что при подозрении на утечку куратор выполнил бы бесполезное действие и считал вопрос закрытым.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `yandex-cloud-functions/heys-mcp/README.md` (раздел про кураторский режим)                                                                                                                                                                                                                                                 | **fixed in tree** (2026-08-02, не закоммичено): текст переписан по фактам — отключение коннектора действует только на стороне Claude; продление прекращает `curators.is_active = false`, но выданный JWT доживает 24 часа; полный отзыв — задача D5 в `todo.md`. Отдельно оговорено, что для клиентского режима отзыв настоящий (`revoke_session`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| SEC-034 | 2026-08-02 | L0   | Shared / auth-helpers   | Low                           | `verifyCuratorSession` селектит несуществующую колонку: `SELECT curator_id FROM curator_sessions`, тогда как в схеме колонка называется `user_id` (`database/2025-01-10_curator_sessions.sql:17`), а `ALTER TABLE` для неё не найден. Функция экспортируется из двух копий helpers, но не вызывается ни одним обработчиком — мёртвый код. **Не уязвимость:** при попытке использования запрос упал бы с ошибкой неизвестной колонки, то есть поведение fail-closed. Риск в другом — функция выглядит рабочим гейтом кураторской сессии, и следующий, кто на неё обопрётся, получит неработающую проверку вместо ожидаемой. Сама таблица объявлена неиспользуемой при переходе на JWT-only (`database/2026-02-09_admin_functions_jwt_only.sql:319`).                                                                                                                                                                                                                                                                                                  | `yandex-cloud-functions/shared/auth-helpers.js:82-97`, `yandex-cloud-functions/heys-api-payments/shared/auth-helpers.js:136`; вызывающих нет (`rg 'verifyCuratorSession'` даёт только определения и экспорты)                                                                                                              | **fixed in tree** (2026-08-02, не закоммичено): функция удалена из обеих копий вместе с экспортом; на её месте оставлен комментарий, объясняющий почему удалена, а не починена — код выглядел рабочим гейтом кураторской сессии, и следующий, кто на него обопрётся, получил бы неработающую проверку. Валидация куратора остаётся в `verifyCuratorJwt`. Вызывающих не было (`rg` по всему репо), синтаксис обеих функций проверен, наборы тестов 84/84 и 229/229 зелёные. Таблица `curator_sessions` не трогалась — её удаление это отдельная миграция.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| SEC-035 | 2026-08-02 | L5   | Storage / IAM           | High                          | Статический ключ Object Storage сервис-аккаунта `heys-storage-uploader` (`YCAJE0Hk…`, создан 2026-03-29) был раскрыт целиком: его значение попало в транскрипт агентской сессии при отладке leak-сканера. Ключ давал доступ ко всем бакетам, включая `heys-backups` с ежедневными выгрузками клиентских данных. Первопричина не в самом ключе, а в рассинхроне локальных файлов: `yandex-cloud-functions/.env` устарел относительно Lockbox, а `.env.backup-before-phase3-20260522-185935` вопреки имени содержал ДЕЙСТВУЮЩИЕ значения — из-за чего сначала был сделан ошибочный вывод «в backup всё устарело».                                                                                                                                                                                                                                                                                                                                                                                                                                      | Сверка выполнена против Lockbox, а не против локального `.env`; проверка доступа — подписанный SigV4-запрос `GET /` к `storage.yandexcloud.net`                                                                                                                                                                            | **fixed (ротация выполнена 2026-08-02)**: создан новый ключ для того же сервис-аккаунта, оба Lockbox-секрета `heys-s3` (`e6qnjm2ks2n1ubiaiki6`, `e6qsj7eie5446321chr2`) переведены на него новой версией, старый ключ `aje9dvhbpjugvmb5c7da` удалён. Проверено: старый ключ → HTTP 403, новый → доступ есть. Простоя нет: новый ключ действовал до удаления старого. Тёплые инстансы функций могут держать прежние креды до перезапуска — ближайший cron-прогон уже возьмёт новые. **Остаточное:** (1) у `heys-storage-uploader` остаётся 6 активных статических ключей — каждый даёт полный доступ к бакетам с клиентскими данными, набор стоит проредить; (2) файл `.env.backup-before-phase3-20260522-185935` **удалён 2026-08-02 по решению владельца**; в git он не попадал ни разу (`git log --all` по пути пуст), поэтому удаление закрывает только локальную копию, а не факт раскрытия. **Перед удалением сверены все ключи файла, а не пять: живых оказалось шесть.** К `JWT_SECRET`, `HEYS_ENCRYPTION_KEY`, `INTERNAL_CRON_TOKEN`, `VAPID_PRIVATE_KEY` добавились `SESSION_SECRET` и `PG_PASSWORD` — пароль production-БД, совпавший со значением в Lockbox-секрете `heys-database` (`e6q7gdshieo5udoet10f`). Устаревшими оказались `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CLIENT_BOT_TOKEN` и S3-ключи. **Решение владельца 2026-08-02: `JWT_SECRET`, `HEYS_ENCRYPTION_KEY` и `INTERNAL_CRON_TOKEN` не ротировать** — раскрытие частичное (у `JWT_SECRET` скрыто 69 символов), перебор невозможен, а цена ротации несоразмерна: смена `JWT_SECRET` разлогинивает всех, смена ключа шифрования рискует уже зашифрованными данными. **Открыто:** `SESSION_SECRET` и `PG_PASSWORD` в это решение не входили — они выявлены как живые уже после него, и степень их раскрытия в транскриптах не установлена. Это отдельный вопрос к владельцу, а не следствие принятого решения; (3) правило «сверять с Lockbox, а не с `.env`» зафиксировано в `docs/SECURITY_RUNBOOK.md` → «Источник правды при проверке секрета». |
| SEC-036 | 2026-08-02 | L0   | Push / VAPID            | Medium                        | **VAPID-ключ невозможно ротировать без потери push у всех подписчиков.** `apps/web/heys_push_v1.js:155-162` переиспользует существующую подписку по `if (!sub)` и не сверяет её `applicationServerKey` с текущим публичным ключом. После смены VAPID браузер продолжит отдавать старую подписку, клиент никогда не переподпишется, а push-сервис начнёт отклонять доставку — push тихо умрёт навсегда, до ручной очистки данных сайта. Практическое следствие: ключ, который нельзя ротировать, при компрометации оставляет выбор между «жить с утечкой» и «сломать push всем».                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `apps/web/heys_push_v1.js:155-162` (`getSubscription()` → `if (!sub)`), сравнение с текущим публичным ключом отсутствует                                                                                                                                                                                                   | **fixed in tree** (2026-08-02, не закоммичено): клиент теперь спрашивает публичный ключ у сервера (кеш стал запасным вариантом, а не первым), сверяет `applicationServerKey` существующей подписки с текущим ключом и при расхождении пересоздаёт её. Браузер без `sub.options` рабочую подписку не теряет; при недоступности сети существующая подписка тоже сохраняется. Регрессия — `apps/web/__tests__/push-vapid-key-rotation.test.js`, 5/5 pass. **Решение владельца 2026-08-02: ротировать VAPID через 1–2 недели после выката фикса**, когда обновление разойдётся по клиентам; ротация раньше необратимо убьёт push у тех, кто давно не заходил. Условие проверяемое: доля подписок, переподписавшихся на текущий публичный ключ. Исходная формулировка ограничения ниже. **Ротация VAPID по-прежнему не выполнена и требует отдельного решения:** фикс снимает блокировку только для тех, кто откроет приложение после выката, поэтому ротировать ключ имеет смысл не раньше, чем обновление разойдётся по клиентам. Исходно обнаружено при разборе SEC-035: `VAPID_PRIVATE_KEY` тоже был раскрыт целиком, но ротация тогда была отложена — цена (необратимая потеря push у всех текущих подписчиков) несоразмерна риску: приватный VAPID-ключ сам по себе бесполезен без endpoint и ключей шифрования конкретной подписки, а они не раскрывались. Чинить надо в обратном порядке: сначала научить клиент сверять `applicationServerKey` и переподписываться при расхождении, и только потом ротировать ключ.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| SEC-037 | 2026-08-02 | L0   | Tooling / Curator board | Medium                        | Локальная доска куратора `scripts/curator-board/server.mjs` слушает на всех интерфейсах и не проверяет ничего на своём API. `server.listen(PORT, …)` вызван без хоста, поэтому Node биндится на `0.0.0.0`, а обработчик `/api/summary` отдаёт данные сразу, без токена и без проверки источника запроса. Пока процесс запущен, любой в той же сети получает по `http://<ip>:4777` имена активных клиентов, названия приёмов пищи со временем, вес, часы и качество сна, настроение и шаги — то есть сведения о здоровье.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `scripts/curator-board/server.mjs:186` (`listen` без хоста), `:171-181` (обработчик `/api/summary` без проверок), `:101-117` (состав отдаваемых полей)                                                                                                                                                                     | **open** — правка на стороне автора инструмента: либо `listen(PORT, '127.0.0.1')`, либо явный токен на эндпоинт, если доска задумана для просмотра с телефона в своей сети. Не production-контур: инструмент запускается вручную и живёт, пока запущен процесс. **Общее с SEC-035, и это важнее самой строки:** оба дефекта не видны при проверке по описанию. Здесь и шапка файла, и `docs/reference/systems/CURATOR_WORKSPACE.md` называют доску «read-only страницей на localhost», а лог при старте печатает `http://localhost:4777` — при том, что слушает она весь интерфейс. В SEC-035 файл назывался backup'ом и содержал действующие ключи. Вывод для методики аудита: свойство проверяется поведением, а не тем, как объект назван.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| SEC-038 | 2026-08-02 | L0   | Telegram / Webhook gate | Medium                        | **Гейт вебхука бота fail-open на холодном старте.** `verifyWebhookSecret` при отсутствии и `HEYS_START_WEBHOOK_SECRET`, и `HEYS_START_WEBHOOK_SECRET_SHA256` пишет предупреждение «webhook accepts ANY caller» и **возвращает `true`**. При этом в задеплоенной версии функции (`d4epjvoc7a172060rhgc`) ни одна из SHA256-переменных не задана, сырой секрет приходит только Lockbox-overlay'ем, а ветка `/start-bot/webhook` вызывает проверку **до** `ensureRuntimeConfig()`; `initSecrets` на уровне модуля не вызывается нигде — все вызовы внутри async-функций. Значит на холодном старте первый запрос к вебхуку проходит проверку без секрета. Эндпоинт публично доступен через шлюз, поэтому поддельный Telegram-update можно отправить напрямую, дождавшись рециклинга инстанса. Та же ветвь у `/webhook` (`TELEGRAM_WEBHOOK_SECRET`).                                                                                                                                                                                                     | `yandex-cloud-functions/heys-bot-client/index.js:326-356` (fail-open в `verifyWebhookSecret`), `:1961-1971` (проверка до загрузки секретов), `rg '^\s*(await\s+)?(initSecrets\|ensureRuntimeConfig)\('` — вызовов на уровне модуля нет; `yc serverless function version list` — SHA256-переменные не заданы                | **open**. Живой пробой не подтверждено: `POST /start-bot/webhook` без заголовка вернул 403, но это тёплый инстанс — принудительно вызвать холодный старт в проверке не удалось, поэтому вывод сделан по коду, а не по наблюдению. Варианты правки: (1) сделать fail-closed — при отсутствии обоих значений отклонять, а не пропускать; (2) вызывать `ensureRuntimeConfig()` до проверки на webhook-ветках; (3) вернуть `HEYS_START_WEBHOOK_SECRET_SHA256` и `TELEGRAM_WEBHOOK_SECRET_SHA256` в env функции — `deploy-all.sh:462` их уже перечисляет, но `_add` пропускает пустые значения, поэтому в проде они отсутствуют. Пункт (1) обязателен независимо от остальных.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |

> L4/L5 attack-probes прогнаны 2026-06-14 (sub-agent на 8 проб): L4.1/4.2/4.6 ✅
> PASS (SQL-injection safely failed, path-traversal whitelist через
> `sanitizeSegment` + crypto.randomBytes filename); L5.1 ✅ accepted-risk
> Вариант B SEC-006 (heys-photos: anon read=true, list=false); L5.5 ✅ нет HTTP
> cron-endpoints (только timer triggers); L4.3 ⚠️ skipped (RPC name изменился);
> L5.2 ⚠️ skipped (нет sample URL + L5.1 уже подтвердил list=false).

> Следующий `SEC-039`. Добавляй новые находки сюда; не переписывай существующие.

### Решение по SEC-027 — strict-режим для `heys-api-rpc`

**Статус: `accepted-risk`. Решение владельца от 2026-07-31 — вариант B.**
Вариант A (флип после observation window) рассмотрен и отклонён.

**Что установлено (2026-07-31, по source):**

1. `deploy-all.sh:474-482` выдаёт `HEYS_WRITE_CONTEXT_STRICT=1` и
   `HEYS_REST_READ_STRICT` только функции `heys-api-rest`.
2. В `heys-api-rpc/index.js` `STRICT_MODE` читается внутри
   `validateContextForWrite` (`:741`), а сама функция вызывается лишь дважды:
   `:2711` — ветка
   `merge_save_client_kv_by_session | merge_save_client_kv_by_curator`; `:3648`
   — `batch_upsert_client_kv_by_curator`.
3. Generic session-пути (`upsert_client_kv_by_session`,
   `batch_upsert_client_kv_by_session`, `delete_client_kv_by_session`) идут
   через типизированный диспетчер, который срезает `p_context_id` (`:4079`), и
   валидацию контекста не проходят.
4. Cross-client запись по этим путям невозможна конструктивно: SQL-функции
   принимают только `p_session_token`, а `client_id` берётся из
   `require_client_id(p_session_token)` по хешу токена сессии
   (`database/2025-12-25_p1_kv_session_functions.sql:65-82`,
   `database/2025-12-24_subscriptions_and_sessions.sql:94-106`).

**Почему принят риск, а не флип:**

- Флип не закрывает то, ради чего задумывался: на generic session-путях
  валидация не вызывается вовсе, поэтому strict их не затрагивает.
- Ужесточение подействовало бы только на merge-путь обеих ролей и curator-batch,
  где защита от IDOR и так есть.
- Клиент передаёт контекст как `contextId || null`
  (`apps/web/heys_yandex_api_v1.js:2169`, `:2198`). При `null` strict вернёт
  403, запись уйдёт обратно в очередь — это в точности механизм инцидента
  2026-06-15, когда PIN-клиенты перестали синхронизироваться в облако.

**Остаточный принятый риск:** capability-контроль на merge/curator-путях RPC
остаётся в warn-mode. Сценарий «правильный клиент, данные из stale-состояния
другого» на этих путях логируется в `data_loss_audit`, но не блокируется.

**Триггеры пересмотра решения:**

1. появление нового браузерного writer'а на RPC-путь;
2. подтверждённый инцидент pollution в `data_loss_audit` по merge/curator-путям;
3. подготовка к R2 (paid-scale);
4. перевод generic session-путей на capability-контекст — тогда флип начнёт
   давать реальную защиту и вопрос открывается заново.

**Если решение будет пересмотрено**, окно измеряется так (разбивка
`context_missing_warn` по `reason`: `session_phase_b` / `curator_phase_b` —
`scripts/security/strict-readiness-check.mjs:212-221`; критерий флипа — 0
событий за окно):

```bash
pnpm security:strict-readiness -- --gate write --write-since "<начало окна>" --events --fail-on-not-ready
```

Правка деплоя при этом — снять ограничение по имени функции в
`yandex-cloud-functions/deploy-all.sh:479-482`, где strict-env сейчас выдаётся
только при `"$func_name" == "heys-api-rest"`.

**Что нельзя делать без отдельной команды:** менять env-переменные, деплоить
функции или править runtime-код cloud functions. В рамках этого решения ничего
из перечисленного не выполнялось.
...
- **Сейчас**: PBKDF2-SHA512-100k+соль (куратор), pgcrypto PIN, session-токен
  ...
  anti-enumeration (`2025-12-25_p2`). SEC-011 MFA/per-account lockout source+DB
  fixed 2026-06-14, но ждёт commit+deploy `heys-api-auth`. Пробелы → ротация
  JWT, refresh, live-smoke после deploy.

### 7.2 Авторизация / мультитенантная изоляция — 🟡 (P1, было 🔴 P0 до 2026-07-26)
...
- **Сейчас**: `write_contexts` + `issue_write_context_by_*` +
  ...
  identity-guard + table-whitelist; `curator_write_lock` (default-on).
  **Обновление 2026-07-31:** SEC-004 закрыт для `heys-api-rest` (production
  strict write+read с 2026-07-26); остаточный пробел → **SEC-027**:
  `heys-api-rpc` продолжает работать в warn-mode — риск принят осознанно
  (`accepted-risk`, решение владельца 2026-07-31). RLS `auth.uid()` инертен под
  `heys_rpc` — изоляция держится на RPC + write_contexts, не на RLS.
...
- **Сейчас**: `SET search_path=public` системно; least-privilege GRANT+REVOKE
  ...
  `USING(false)`; FK CASCADE по 11 таблицам; SEC-017 включил RLS на 49/49
  ordinary `public` tables без `FORCE`. Пробелы: runtime всё ещё owner-role
  (`heys_admin`) и обходит RLS; `FORCE` безопасен только после явных
  role/context policies; audit-таблицы tamper; колоночное шифрование PII (⚪).
...
- **Сейчас**: shrink-guard, tombstones, central stamper, `isNonClientDataKey`,
  `curator_write_lock`, набор lint'ов. **SEC-007 source-fixed 2026-06-14**:
  после активного server-revision checkpoint no-revision pull блокируется для
  ключей с известной local revision; diagnostic buffer
  `_serverRevisionGateBlocks` показывает такие скипы. Остающийся широкий ceiling
  — write-merge/CAS для device-clock конфликтов.
...
- **Сейчас**: gitleaks, набор lint-гейтов, pnpm-lock; **`dependency-check.js`
  починен (SEC-001) и блокирует CI high/critical**, SAST блокирует новые находки
  вне baseline, SBOM генерируется как CycloneDX artifact. SEC-010 закрыт для
  prod/runtime; остаются dev/build-only low/moderate advisories и регулярный
  review `^`-диапазонов. См. также 7.15.
...
- **Сейчас**: static export, lead API consent/honeypot/rate-limit;
  `next@15.5.18`, `pnpm --dir apps/landing audit --prod --audit-level moderate`
  чистый. Пробелы: FAQ `dangerouslySetInnerHTML` → **SEC-008** (severity зависит
  от источника `items` — проследить); iframe `allow-same-origin`+`allow-scripts`
  → **SEC-012**; нет security-headers.
...
| ------------------ | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dependency audit   | `pnpm audit --prod --json`; `pnpm audit --json` | 2026-06-14: prod audit clean (0 advisories, 0 high/critical) after `esbuild >=0.28.1`; full audit has 0 high/critical and only dev/build-only low/moderate advisories → SEC-010 closed for runtime/build high-risk. |
...
| L2 §4: RLS на таблицах                                      | live `pg_class.relrowsecurity` + `2026-06-14_enable_public_rls_defense_in_depth.sql` | RLS on 49/49 ordinary `public` tables; `FORCE` не включён         |
...
- 2026-07-31 — документационная сверка с фактическим состоянием: SEC-004 закрыт
  для `heys-api-rest`, SEC-024/L3 IDOR закрыт (evidence 2026-07-26), компонент 2
  переведён 🔴 P0 → 🟡 P1, заведена находка SEC-027 (strict-покрытие
  `heys-api-rpc`). В тот же день SEC-027 закрыт решением владельца как
  `accepted-risk` (вариант B; вариант A — флип после observation window —
  отклонён), зафиксированы остаточный риск и четыре триггера пересмотра.
  Исходные записи журнала и срезы 2026-06-08/14 сохранены как история.
