...
_Закрыто 2026-08-31: признаки доезжают до экрана (§15.2, L1), механизм включён
по решению владельца «Б» — признаком стала метка правки (§16)._
...
## 15. Предусловия решений 4 и 7 (2026-08-31)

Пункты 2 и 3 из §13.5. Одно закрыто целиком, второе — наполовину, и остановилось
не на объёме работы, а на решении, которое я принимать не вправе.

### 15.1 Закрыто: масса тела в тоннаже дня (предусловие решения 7)

`computeDayTotalTonnage` и подпись «~N кг объёма» не передавали `bodyWeightKg`,
поэтому упражнения со своим весом давали **ноль**, и одна и та же тренировка
показывала разный тоннаж на дне и в конструкторе.

- `bodyWeightForDay(dayData)` — вес того дня (`weightMorning`), иначе вес
  профиля: тоннаж прошлой тренировки не должен меняться от сегодняшнего
  взвешивания. Правило то же, что у TDEE —
  [heys_day_trainings_v1.js:16](apps/web/heys_day_trainings_v1.js:16).
- `computeDayTotalTonnage` передаёт его в `ks.dayTonnage`; `findPrevDayTonnage`
  наследует через него.
- `calcWorkoutBuilderVolumeKg(wl, bodyWeightKg)` — подпись тренировки на
  карточке; вес приходит из `renderTrainingsBlock`.

Проверено на живой странице: подтягивания 10 повторов при коэффициенте 0,95 и
весе 80 кг дают **760 кг** (столько же показывает конструктор), без массы тела
ядро отдавало **0**.

Что осталось нетронутым намеренно: `plannedVolume` в подписи карточки против
`totalVolume` в тоннаже дня — это разные величины по решению 7 (плановый только
внутри карточки силовой), расхождение здесь не дефект. Локальные
`approachVolumeKg` / `exerciseRecordsFromApproaches` (полоска истории и рекорды)
массы тела по-прежнему не знают — отдельный след, в решениях его нет.

### 15.2 Закрыто наполовину: признак оценённых шагов (предусловие решения 4)

**Сделано (L1).** `buildEnergyContext` теперь отдаёт `stepsResolved`,
`stepsEstimated`, `stepsMissing` —
[heys_day_energy_context_v1.js:45](apps/web/heys_day_energy_context_v1.js:45).
Раньше TDEE их считал, а контекст дня даже не разбирал, и ни один экран не мог
их прочитать. Теперь пометку есть на чём рисовать — как только механизм начнёт
срабатывать.

**Не сделано (L2): механизм по-прежнему не срабатывает на экране дня.** Это не
объём работы, а нерешённый вопрос модели.

Чтобы подставить медиану, движку нужно отличить «прошёл ноль» от «не вносил». В
модели дня оба возможных признака стёрты:

| Признак                                                    | Кто стирает                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `steps: null` — исходный контракт `resolveStepsInput`      | `ensureDay`: `steps: +d.steps \|\| 0` ([heys_models_v1.js:820](apps/web/heys_models_v1.js:820)). Если восстановить — обе merge-функции коерсят обратно: `Math.max(Number(local.steps) \|\| 0, …)` ([heys_sync_merge_v1.js:925](apps/web/heys_sync_merge_v1.js:925), [heys_cloud_merge_v1.js:75](apps/web/heys_cloud_merge_v1.js:75)). Значит после первой синхронизации оценка исчезала бы — недетерминированное поведение хуже отсутствия правки. |
| `stepsUpdatedAt` — метка правки, её ставит каждый писатель | Тот же `ensureDay`: метки в его списке полей нет вовсе (см. 15.3).                                                                                                                                                                                                                                                                                                                                                                                 |

Любой из путей — правка модели дня и слоя синхронизации, а не экрана. Это
решение владельца: **что в модели означает «шаги не вносили»** — отсутствующее
значение (и тогда merge для шагов надо переписывать) или метка правки (и тогда
надо чинить 15.3). До этого решения пометка «оценка по медиане» рисует
состояние, которого не бывает.

Отдельно стоит назвать вслух цену включения: как только механизм заработает,
**«Цель дня» на ещё не заполненном дне вырастет** на оценку шагов (у активного
человека это порядка 200–300 ккал) — именно поэтому решение 4 требует пометку.
Долг это не затронет: `stepsKcalForDebt` намеренно обнуляет оценённые шаги
([heys_tdee_v1.js:286](apps/web/heys_tdee_v1.js:286)).

### 15.3 Найдено по дороге: `ensureDay` стирает метки правки полей

`ensureDay` собирает день перечислением полей, и в списке нет ни одной из
четырёх пофайловых меток: `stepsUpdatedAt`, `weightUpdatedAt`,
`householdUpdatedAt`, `deficitUpdatedAt`. Проверено двумя способами:
`rg "UpdatedAt" heys_models_v1.js` не находит их в `ensureDay`, и живой вызов
`HEYS.utils.ensureDay({steps:0, stepsUpdatedAt:…})` возвращает объект без метки.

Почему это важно: на этих метках держится пофайловое разрешение конфликтов между
устройствами — `heys_sync_merge_v1.js:33` объявляет группу
`{ timestamp: 'stepsUpdatedAt', fields: ['steps'] }`, и без метки merge
сваливается на грубое правило (`Math.max` для шагов). Метки живут только между
правкой и следующей загрузкой дня: `setDay` их добавляет, `ensureDay` на
следующем `heys:day-updated` снимает, автосейв пишет уже обрезанный объект
([heys_day_hooks.js:268](apps/web/heys_day_hooks.js:268)).

**Не чинил:** это правка слоя синхронизации для четырёх полей сразу, с
последствиями для merge между устройствами, а не предусловие вкладки «Актив».
Отдельная задача и отдельное решение. В §9 это новый пункт класса «мёртвое/
стираемое», рядом с M.

### 15.4 Чем подтверждено

[activity-numbers-net-and-plan.test.js](apps/web/__tests__/activity-numbers-net-and-plan.test.js)
— 22 сценария (было 16), `npx vitest run` зелёный:

- **bodyWeightKg** — тоннаж дня равен тоннажу конструктора для упражнения на
  своём весе; берётся вес того дня; без утреннего веса — вес профиля, а не ноль.
- **L1** — контекст дня различает три состояния шагов: факт, оценка, нет данных.

Соседние наборы зелёные (269 тестов), включая `sync-merge-shared` (154),
`day-content-equal` и `heys-display-norms` — то есть модель дня и слой слияния
правкой не задеты.

Локалка: новый `boot-day.bundle.0351d045ca58.js` грузится, консоль чистая, живые
вызовы дают 760 против 0 и `stepsEstimated: true`.

### 15.5 Что осталось из §13.5

- **Решение владельца по 15.2** — чем в модели помечать «шаги не вносили». Без
  него решение 4 (пометка «оценка по медиане») не реализуемо.
- Контракт с числами и кадры на 375 px.
- Сведение экрана и тест сверки.

---

## 16. Решение «Б»: метки доменов и включённая оценка шагов (2026-08-31)

Владелец выбрал вариант Б из §15.2: признаком «шаги не вносили» становится метка
правки, а не отсутствующее значение. Условие этого варианта — сначала починить
метки, потому что их стирал `ensureDay`.

### 16.1 Масштаб оказался втрое больше заявленного

В §15.3 я назвал четыре метки. Их **двенадцать**, и вместе с ними терялись
**четыре поля групп**. Проверено сверкой списка полей `ensureDay` с
`DAY_USER_MUTATION_GROUPS` и живым вызовом на странице:

| Теряли                                                                                                                                                                                                                                                                | Что это                                |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| `stepsUpdatedAt`, `waterUpdatedAt`, `weightUpdatedAt`, `householdUpdatedAt`, `cycleUpdatedAt`, `sleepNoteUpdatedAt`, `dayCommentUpdatedAt`, `dayScoreUpdatedAt`, `supplementsPlannedUpdatedAt`, `supplementsTakenUpdatedAt`, `deficitUpdatedAt`, `dayStatusUpdatedAt` | двенадцать меток доменов правки        |
| `waterEntries`, `cycleStatus`, `cycleAnsweredAt`, `dayScoreManual`                                                                                                                                                                                                    | поля групп, не названные в `ensureDay` |

`waterEntries` — журнал воды, по которому считается разбор «глотка»
([heys_day_water_v1.js:189](apps/web/heys_day_water_v1.js:189)); он
восстанавливался при следующем добавлении воды, поэтому потеря не была заметна.

### 16.2 Что изменилось

- `DAY_MUTATION_PASSTHROUGH` в [heys_models_v1.js](apps/web/heys_models_v1.js) —
  метки и поля групп проносятся **без нормализации**: merge сравнивает их как
  есть, и приведение типов означало бы, что модель тихо переписывает то, по чему
  разрешается спор между устройствами. Список — копия групп слияния, потому что
  модель грузится раньше слоя синхронизации.
- `HEYS.TDEE.hasStepsFact(day)` — один предикат «шаги внесены» на весь расчёт:
  число больше нуля, либо ноль с меткой правки. Им пользуются
  `resolveStepsInput`, `hasAnyStepsFactEver`
  ([heys_tdee_v1.js:169](apps/web/heys_tdee_v1.js:169)) и сборщик истории
  `collectRecentStepsHistory` ([heys_steps_v1.js](apps/web/heys_steps_v1.js), с
  локальным фолбэком на случай порядка загрузки). До этого медиана считалась по
  одному правилу, а решение «подставлять ли её» — по другому.

### 16.3 Что теперь видит человек

**Оценка шагов включилась.** Проверено на живой странице: день без метки при
истории 6000 / 8000 / 10000 даёт `{ steps: 8000, stepsEstimated: true }`.
Практическое следствие, названное заранее в §15.2: **«Цель дня» на ещё не
заполненном дне выросла** на оценку шагов. Пометки на экране пока нет — она
приходит с контрактом (решение 4), и до сведения экрана это состояние без
объяснения. Долг не затронут: `stepsKcalForDebt` оценённые шаги обнуляет.

**Слияние между устройствами заработало как задумано.** Спор по домену теперь
решается меткой домена, а не корневым `updatedAt`, который двигают и фоновые
записи. Это меняет поведение синхронизации для всех двенадцати групп — не только
для шагов.

### 16.4 Чем подтверждено

Новый гейт
[day-mutation-stamps.test.js](apps/web/__tests__/day-mutation-stamps.test.js) —
4 сценария. Он читает `DAY_USER_MUTATION_GROUPS` из самого файла слияния, а не
дублирует список: падает и когда `ensureDay` теряет метку, и когда списки
разошлись. Расхождение списков и было причиной.

[activity-numbers-net-and-plan.test.js](apps/web/__tests__/activity-numbers-net-and-plan.test.js)
— 28 сценариев (было 22): ноль с меткой это факт, ноль без метки — незаполненный
день, медиана считается только по дням с фактом, у нового человека оценки нет
вовсе, оценка не идёт в долг.

**Обновлён чужой тест.** `heys-display-norms.test.js` проверял прежний контракт
(«steps 0 — всегда явный ввод»). Смысл проверки сохранён — явный ноль обязан
отличаться от «не вносил», — изменился признак. Причина названа в самом тесте.

Прогон всего набора: **6309 зелёных**. Падений 15 в 13 файлах; моё из них
**одно** — тот самый `heys-display-norms`, и оно закрыто правкой теста.
Attribution доказана двумя прогонами: без моих правок те же файлы дают 10
падений, с моими — 11, разница ровно в одном тесте. Остальные — приехавший пакет
дизайна (`ui-v4-contract-drift`, `ui-v4-visual-harness`, `widgets-canvas-copy`,
`line-roles-v4`: у зоны `tab-activity` 214 строк контракта без вердиктов, их
ставит сведение) и работа параллельных сессий.

### 16.5 Что осталось

Сведение экрана «Актив» с канвасом и тест сверки. Оба предусловия закрыты:
строка тоннажа — §15.1, кадр «шаги оценены» — этот раздел.
