...
- Production trial-flow согласован на `1.6/1.6/1.5`, а текущий `main` уже
  содержит следующий legal bump `1.7/1.7/1.5`; его публикация требует отдельного
  синхронного app/landing deploy и re-consent smoke.
- Любые новые категории анкеты по-прежнему требуют data-change gate и
  синхронизации health consent до следующего production-релиза.
...
6. **Retention:** 30-дневная очистка отклонённых intake подключена к
   существующему `daily_cleanup`; maintenance задеплоен, revoke/purge smoke
   пройден в production.
...
## Production rollout — выполнен 2026-07-27

Rollout выполнен по отдельной прямой команде пользователя. Основная migration и
re-consent forward-fix применены, `heys-api-rpc` и `heys-maintenance`
задеплоены, source опубликован коммитами `74904b51f`, `b05aa1726` и `d234cbee5`.
Production build `2026.07.27.2016.d234cbee`; полный smoke прошёл, три
синтетических клиента и связанные записи удалены. Таблица ниже остаётся
повторяемым stop-gate runbook для следующего совместимого релиза.
