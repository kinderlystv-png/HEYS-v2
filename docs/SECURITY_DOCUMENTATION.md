...
> **Статус:** исторический обзор, частично перепроверен 2026-07-17<br>
> **Канонический статус находок:** [SECURITY_REVIEW.md](SECURITY_REVIEW.md)<br>
> **Операционная памятка:** [SECURITY_RUNBOOK.md](SECURITY_RUNBOOK.md)<br>
> Зелёные статусы и production/compliance-утверждения ниже не считать
> подтверждёнными без актуального evidence из канонического review.
...
Документ объясняет устройство защиты, но не присваивает системе итоговый
security/compliance-статус.
...
3. 🗄️ Data Security         — шифрование health data в БД; browser localStorage
   скоупится и сжимается, но не шифруется storage layer'ом
...
### Data at Rest — localStorage клиента

| Ключ localStorage    | Описание                  | Шифрование                 |
| -------------------- | ------------------------- | -------------------------- |
| `heys_{id}_profile`  | ПДн + health profile      | JSON/сжатие, не шифрование |
| `heys_{id}_dayv2_*`  | Дневник питания, сон, вес | JSON/сжатие, не шифрование |
| `heys_{id}_hr_zones` | Пульсовые зоны            | JSON/сжатие, не шифрование |
| `heys_{id}_products` | База продуктов питания    | JSON/сжатие                |
| `heys_{id}_norms`    | Нормы питания             | JSON/сжатие                |

Серверное encryption-at-rest не защищает эти копии от JavaScript в origin
приложения. XSS и доступ к устройству — отдельные угрозы.
