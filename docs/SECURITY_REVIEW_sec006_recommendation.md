...
**Статус (2026-08-11): вариант B закрыт, реализован вариант строже A.** Триггер
— не рост клиентской базы (её как раз не было), а находка в ходе разбора
внешнего аудита лендинга: путь `attachment.url` из ответа `/photos/upload`
оседал в `client_messages` и был живым публичным адресом, проверено
экспериментом на проде (`curl` без авторизации отдавал фото/голос целиком).
Вариант A предполагал подписанные GET-ссылки с TTL ~1ч; реализовано строже —
постоянного URL нет вообще, каждое чтение идёт через `/photos/read` с проверкой
сессии и владения per-request (`heys-api-messages` уже вела этот паттерн для
голосовых транскриптов, распространили на чтение самого файла). Оценка «10–15
часов» ниже в этом документе была для рефактора всех мест `photo.url` вручную;
фактически ушло меньше — большая часть контуров уже читала через общий
`HEYS.MessengerAPI`. `ACL: 'public-read'` убран из `PutObjectCommand`
(`heys-api-photos/index.js`), редеплой и закрытие публичного чтения бакета — в
этом же заходе. `Referrer-Policy`/доп. митигации варианта B более не нужны — сам
класс риска (постоянный публичный URL) снят, а не смягчён.
