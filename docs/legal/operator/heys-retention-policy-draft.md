...
owner, evidence и dry-run-first retention job runbook перечислены. Перед
включением в публичные документы нужен юридический sign-off.
...
| Фото еды                 | S3 `heys-photos/<client_id>/...`                                         | пока активен клиент; **не бэкапятся**; orphan после удаления чистить cron после grace 7 дней | `heys-cron-photo-cleanup` с `DRY_RUN=0` в `deploy-all.sh`                       | `photo_cleanup_log`             |
| Backups                  | PG 14 дней + S3 `heys-backups/client-daily/...` **365 дней** (факт кода) | черновик 90 дней не утверждён; смена срока — отдельный legal sign-off                        | lifecycle / `RETENTION_DAYS`; watchdog `backup_chain_gap`                       | `backup_run_log`                |
...
4. Перед paid-scale пройти
   [heys-retention-job-runbook.md](heys-retention-job-runbook.md): dry-run,
   legal/owner review, затем controlled batch delete если сроки утверждены.
5. `heys-cron-photo-cleanup`: фото и голос не бэкапятся. Orphan-удаление
   включается `DRY_RUN=0` в `deploy-all.sh` (grace 7 дней, hard cap 100).
6. Backup-chain не проверять вручную по памяти: каждый daily backup пишет
   `backup_run_log`, а `heys-cron-security-alerts` rule `backup_chain_gap`
   алертит, если последний успешный run старше 30 часов.
