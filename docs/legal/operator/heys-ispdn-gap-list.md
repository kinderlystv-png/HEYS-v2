...
| Запись РКН                      | Одна запись оператора `26-22-005319`; HEYS updateform принят порталом 2026-06-17, публикация в реестре ожидается | 🟡     |
...
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- | --------------------------- | ---------------------------------------------------------------------------------- |
| РКН-изменение по записи `26-22-005319`     | R0: подача выполнена, но нужно дождаться публикации и сверить, что HEYS-цели добавлены без удаления старого блока | ИП Поплавский А.С.          | мониторить реестр; после публикации сверить с `heys-data-register.md` и privacy    |
...
До старта активных заявок и бесплатных триалов РКН-подача уже принята порталом,
но нужно дождаться публикации/сверки записи или явно принять риск старта до
обновления публичного реестра; также зафиксировать рабочее решение по уровню
ИСПДн/модели угроз, задеплоить и переизмерить security fixes `6Б.3/6Б.4`, пройти
Telegram lead smoke без ПДн. До первой оплаты дополнительно нужен платежный
live-smoke R1.
