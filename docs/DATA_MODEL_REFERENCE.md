...
> **Справочник всех аналитических параметров HEYS** Версия: 5.4.0 | Обновлено:
> 2026-02-19 | **182 умных совета** | **37 факторов инсулиновой волны** | **🧠
> Insulin Wave v4.1.0** | **🔬 PMID Science Links** | **🏋️ Training Context** |
> **💰 Caloric Debt** | **🔄 Refeed Day** | **📊 Status Score** | **🎮
> Gamification** | **🏭 NOVA Classification** | **🧬 Extended Nutrients** | **🔮
> Predictive Insights v5.2.0: 41 паттерн (100% научное покрытие + ⚡
> мемоизация)** | **🛡️ Store API v4.8.8 (React State Sync Fix)** | **🚨 EWS v4.2
> (Phenotype-Aware, Global Score, Causal Chains, Weekly Progress)** | **🎯
> Dynamic Priority Badge v4.3.0 (Acuteness Decay, Section Rules, Pattern
> Degradation)**
...
- [📊 HEYS Data Model Reference](#-heys-data-model-reference)
  ...
  - [🧪 Harm Score — Научная система оценки вредности (0-10)](#-harm-score--научная-система-оценки-вредности-продуктов-0-10)
  - [🔮 Predictive Insights v5/v6 — 41 научный паттерн](#-predictive-insights-v5v6--41-научный-паттерн)
  ...
  - [📊 Status Score (Статус дня 0-100)](#-status-score-статус-дня-0-100)
  - [🎮 Gamification (XP, Уровни, Достижения)](#-gamification-xp-уровни-достижения)
  - [🔬 Phenotype (Метаболический фенотип)](#-phenotype-метаболический-фенотип)
  - [🚨 Early Warning System v4.2](#-early-warning-system-v42)
  - [🎯 Dynamic Priority Badge v4.3.0](#-dynamic-priority-badge-v430)
  - [📈 Metabolic Intelligence (A/B тесты, Data Inventory)](#-metabolic-intelligence-ab-тесты-data-inventory)
  - [📉 Simple Analytics (Performance Tracking)](#-simple-analytics-performance-tracking)
  - [📊 Data Overview (Заполненность данных)](#-data-overview-заполненность-данных)
...
## 🔮 Predictive Insights v5.2.0 — 41 научный паттерн

> **Версия**: 5.2.0 | **Обновлено**: 2026-02-12 | **100% покрытие PMID** |
> **41/41 паттернов активны** | **⚡ Мемоизация (44% быстрее)**  
> **Файлы**: `pi_patterns.js` (анализаторы), `pi_constants.js` (SCIENCE_INFO),
> `pi_ui_cards.js` (UI), `pi_cache.js` (мемоизация)  
> **Модули**: 2864 LOC patterns, 1336 LOC constants, 1648 LOC UI, 173 LOC
> cache  
> **Производительность**: P50 180ms → 100ms (LRU кэш, TTL 60s, 70-85% cache hit
> rate)
...
- ⚡ **Оптимизирован** — мемоизация дорогих вычислений (correlation matrix,
  metabolic patterns, glycemic variability, allostatic load)
...
- **Ключи кэша**: Генерируются из диапазона дат + профиль hash для
  детерминированности
- **Инвалидация**: Автоматическая очистка после `HEYS.cloud.syncClient()` (hook
  в sync flow)
- **Результаты**: P50 180ms → 100ms (44% прирост), cache hit rate 70-85%

### Структура SCIENCE_INFO (95 entries)

Каждый паттерн имеет полную научную документацию:

```javascript
const SCIENCE_INFO = {
  MEAL_TIMING: {
    name: 'Тайминг приёмов пищи',
    short: 'Анализирует частоту приёмов и интервалы между ними...',
    details:
      'Паттерн оценивает, насколько оптимальна частота... [3-4 параграфа]',
    formula: 'interval = meal[i+1].time - meal[i].time\nScore = ...',
    sources: [
      { label: 'Kahleova et al., 2014 — Meal frequency...', pmid: '25489333' },
      {
        label: 'Paoli et al., 2019 — Intermittent fasting...',
        pmid: '30472870',
      },
    ],
    interpretation: '≥80 — оптимальная частота. 60-79 — можно улучшить...',
    priority: 'HIGH',
    category: 'TIMING',
    actionability: 'TODAY',
    impactScore: 0.75,
    whyImportant: '⏰ Правильные интервалы между едой = ...',
  },
  // ... 40 остальных паттернов
};
```
...
| ID  | Паттерн                 | Категория  | Данные                 | MinDays | PMID Examples      |
| --- | ----------------------- | ---------- | ---------------------- | ------- | ------------------ |
| 1   | **MEAL_TIMING**         | TIMING     | meals, intervals       | 7       | 25489333, 30472870 |
| 2   | **WAVE_OVERLAP**        | TIMING     | insulin waves          | 7       | 27385608, 27431364 |
| 3   | **LATE_EATING**         | TIMING     | meals after 21:00      | 7       | 25311617, 24847856 |
| 4   | **MEAL_QUALITY_TREND**  | NUTRITION  | MQS 7-14d              | 7       | 22535969, 25318818 |
| 5   | **SLEEP_WEIGHT**        | RECOVERY   | sleep ↔ weight        | 7       | 15583226, 15531540 |
| 6   | **SLEEP_HUNGER**        | RECOVERY   | недосып → голод        | 7       | 27568993, 23414424 |
| 7   | **TRAINING_KCAL**       | ACTIVITY   | compensation           | 7       | 22564864, 22885926 |
| 8   | **STEPS_WEIGHT**        | ACTIVITY   | NEAT ↔ weight         | 7       | 14722404, 21471817 |
| 9   | **PROTEIN_SATIETY**     | NUTRITION  | белок → сытость        | 7       | 19400750, 25926512 |
| 10  | **FIBER_REGULARITY**    | NUTRITION  | клетчатка 25-35г       | 7       | 23609775, 30638909 |
| 11  | **STRESS_EATING**       | PSYCHOLOGY | стресс → перееда       | 7       | 21676152, 20357041 |
| 12  | **MOOD_FOOD**           | PSYCHOLOGY | настроение ↔ еда      | 7       | 18457711, 28179633 |
...
#### Advanced Patterns v4 (6 паттернов B1-B6)

| ID  | Паттерн                   | Категория  | Данные                 | MinDays |
| --- | ------------------------- | ---------- | ---------------------- | ------- |
| 20  | **SLEEP_QUALITY**         | RECOVERY   | sleep → next day       | 7       |
| 21  | **WELLBEING_CORRELATION** | RECOVERY   | wellbeing ↔ lifestyle | 7       |
| 22  | **HYDRATION**             | RECOVERY   | 30ml/kg водный баланс  | 7       |
| 23  | **BODY_COMPOSITION**      | METABOLISM | WHR trend              | 14      |
| 24  | **CYCLE_IMPACT**          | RECOVERY   | фазы цикла             | 14      |
| 25  | **WEEKEND_EFFECT**        | PSYCHOLOGY | выходные vs будни      | 7       |

#### Deep Analytics v5 (6 паттернов C7-C12)

| ID  | Паттерн                 | Категория  | Данные              | MinDays | PMID Examples      |
| --- | ----------------------- | ---------- | ------------------- | ------- | ------------------ |
| 26  | **MICRONUTRIENT_RADAR** | NUTRITION  | Fe, Mg, Zn, Ca      | 7       | -                  |
| 27  | **OMEGA_BALANCER**      | NUTRITION  | omega-6:3 ratio     | 7       | -                  |
| 28  | **HEART_HEALTH**        | METABOLISM | Na:K, cholesterol   | 7       | -                  |
| 29  | **NOVA_QUALITY**        | NUTRITION  | % ультрапереработки | 7       | -                  |
| 30  | **TRAINING_RECOVERY**   | ACTIVITY   | нагрузка + риск     | 5       | -                  |
| 31  | **HYPERTROPHY**         | METABOLISM | композиция, обхваты | 14      | 28834797, 29497353 |

#### v6.0 Phase 1-5 (10 паттернов C13-C22)

| ID  | Паттерн                     | Phase | Категория  | Данные                | MinDays | PMID Examples      |
| --- | --------------------------- | ----- | ---------- | --------------------- | ------- | ------------------ |
| 32  | **VITAMIN_DEFENSE**         | 1     | NUTRITION  | 11 витаминов          | 7       | 24566440, 26828517 |
| 33  | **B_COMPLEX_ANEMIA**        | 1     | METABOLISM | B1-B12 + Fe           | 7       | 26828517, 29215971 |
| 34  | **GLYCEMIC_LOAD**           | 2     | METABOLISM | GL per meal/day       | 5       | 12081850, 18835944 |
| 35  | **PROTEIN_DISTRIBUTION**    | 2     | NUTRITION  | 20-40г per meal       | 7       | 29497353, 19056590 |
| 36  | **ANTIOXIDANT_DEFENSE**     | 3     | RECOVERY   | A, C, E, Se, Zn       | 7       | 20096093, 12424324 |
| 37  | **ADDED_SUGAR_DEPENDENCY**  | 3     | METABOLISM | <25г WHO              | 7       | 25231862, 22351714 |
| 38  | **BONE_HEALTH**             | 4     | RECOVERY   | Ca, D, K, P           | 14      | 26856587, 21118827 |
| 39  | **TRAINING_TYPE_MATCH**     | 4     | ACTIVITY   | cardio/strength/hobby | 5       | 26891166, 29182451 |
| 40  | **ELECTROLYTE_HOMEOSTASIS** | 5     | RECOVERY   | Na, K, Mg, Ca         | 7       | 28070459, 29480918 |
| 41  | **NUTRIENT_DENSITY**        | 5     | NUTRITION  | NRF 9.3 model         | 5       | 16277764, 19110020 |

### Health Score (Goal-Aware Aggregation)

**Категории** (5, вес зависит от цели):

- **Nutrition** (35%): Meal Quality, Nutrition Quality, Protein, Fiber, Gut
  Health, Micronutrients (C7), Omega (C8), NOVA (C10), Vitamin Defense (C13),
  Protein Distribution (C35), Nutrient Density (C41)
- **Timing** (25%): Meal Timing, Wave Overlap, Late Eating, Circadian, Nutrient
  Timing, Weekend Effect
- **Activity** (20%): Steps↔Weight, NEAT, Training↔Kcal, Training Recovery
  (C11), Training-Type Match (C39)
- **Recovery** (15%): Sleep Quality, Sleep↔Weight, Sleep↔Hunger, Hydration,
  Wellbeing, Cycle Impact, Antioxidants (C36), Bone Health (C38), Electrolytes
  (C40)
- **Metabolism** (5%): Insulin Sensitivity, Body Composition, Heart Health (C9),
  Hypertrophy (C12), B-Complex (C33), Glycemic Load (C34), Added Sugar (C37)

**Formula**: `Σ(category_avg × goal_weight) / Σ(weights)` → 0-100 score

**Goal-aware weights** (фиксы 13.02.2026: deficit nutrition 0.35→0.25):

- **Deficit** (похудение): Nutrition 25%, Timing 30%, Activity 20%, Recovery
  15%, Metabolism 10%
- **Maintenance** (поддержание): Nutrition 35%, Timing 25%, Activity 20%,
  Recovery 15%, Metabolism 5%
- **Bulk** (набор массы): Nutrition 40%, Timing 20%, Activity 25%, Recovery 10%,
  Metabolism 5%

### API Usage

```javascript
// Calculate insights for 30 days
...
insights = {
  ...
  healthScore: {
    ...
    topIssues: [/* top 3 lowest scores */],
    topWins: [/* top 3 highest scores */]
  },
  metadata: {
    version: '5.0.0',
    calculatedAt: '2026-02-12T10:30:00Z',
    coverage: 0.95, // 95% data completeness
    patternCount: 41
  }
};

// Access SCIENCE_INFO for any pattern
const scienceInfo = HEYS.InsightsPI.constants.SCIENCE_INFO.MEAL_TIMING;
console.log(scienceInfo.short); // 1-2 sentence explanation
console.log(scienceInfo.details); // 3-4 paragraph deep dive
console.log(scienceInfo.sources[0].pmid); // '25489333'
console.log(scienceInfo.formula); // Mathematical definition
```

### Performance

- **Calculation time**: P50 < 180ms, P95 < 300ms (41 patterns, 30 days)
- **LOC**: 2864 (patterns) + 1336 (constants) + 1648 (UI) + 1190 (main) = ~7000
  LOC
- **Data coverage**: 292/292 products, 35 fields (100%)
- **PMID coverage**: 41/41 patterns (100%)
...
**Ключевой принцип**: Продукт с высоким сахаром, но богатый клетчаткой (финики)
менее вреден, чем чистый сахар — клетчатка замедляет всасывание и снижает
гликемический удар.

**🆕 v2.0**: Добавлены NOVA classification (переработка), натрий,
микронутриентная плотность.
...
PENALTIES (штрафы, увеличивают вред):
  + trans100 × 3.0      # Транс-жиры — ГЛАВНЫЙ враг (×3 множитель)
  + simple100 × 0.08    # Простые сахара (100г сахара = +8 баллов)
  + badFat100 × 0.10    # Насыщенные жиры (снижено с 0.12)
  + sodium100 × 0.002   # 🆕 Натрий: 2000мг = +4 балла (гипертензия)
  + GI_penalty          # Штраф за высокий ГИ (см. ниже)
  + NOVA_penalty        # 🆕 Штраф за переработку (см. ниже)

BONUSES (бонусы, снижают вред):
  - fiber100 × 0.30     # Клетчатка — мощный протектор (усилено с 0.25)
  - protein100 × 0.06   # Белок снижает ГИ и насыщает (усилено с 0.05)
  - goodFat100 × 0.04   # Полезные жиры (усилено с 0.02)

GI_penalty:
  GI ≤ 35:  0.0         # Низкий ГИ — без штрафа
  GI 36-55: 0.5         # Средний ГИ
  GI 56-70: 1.0         # Повышенный ГИ
  GI > 70:  1.5 + (GI-70) × 0.02  # Высокий ГИ — прогрессивный штраф

NOVA_penalty (ультрапереработка):
  NOVA 1: 0.0           # Необработанные — без штрафа
  NOVA 2: 0.3           # Кулинарные ингредиенты
  NOVA 3: 0.8           # Переработанные (консервы, сыры)
  NOVA 4: 2.5           # Ультрапереработанные — серьёзный штраф!

ФИНАЛЬНАЯ НОРМАЛИЗАЦИЯ:
  HarmScore = clamp(raw_score × activityMultiplier, 0, 10)
```
...
**Научное обоснование**: Monteiro 2019 (PMID: 29444892) — ультрапереработанные
продукты связаны с ожирением, диабетом 2 типа, сердечно-сосудистыми
заболеваниями.

| NOVA  | Название               | Штраф | Примеры                   | Эвристика (паттерны названия)       |
| ----- | ---------------------- | ----- | ------------------------- | ----------------------------------- |
| **1** | Необработанные         | 0.0   | Яблоко, курица, рис, яйца | `яблок`, `курин`, `рис `, `яйц`     |
| **2** | Кулинарные ингредиенты | 0.3   | Масло, сахар, соль, мука  | `масло`, `сахар`, `соль`, `мука`    |
| **3** | Переработанные         | 0.8   | Консервы, сыр, хлеб, сок  | `консерв`, `сыр`, `хлеб`, `сок`     |
| **4** | Ультрапереработанные   | 2.5   | Чипсы, колбаса, газировка | `чипс`, `колбас`, `кола`, `печенье` |

**Эвристика**: NOVA группа определяется автоматически по названию продукта через
~200 паттернов. Можно задать явно через поле `novaGroup` (1-4).

---

### Научное обоснование весов v2.0

| Фактор         | Вес        | Научное обоснование                                                                              | Источник                            |
| -------------- | ---------- | ------------------------------------------------------------------------------------------------ | ----------------------------------- |
| **trans100**   | ×3.0       | Транс-жиры повышают LDL на 20%, снижают HDL, провоцируют воспаление. WHO рекомендует <1% калорий | Mozaffarian 2006 (PMID: 16611951)   |
| **simple100**  | ×0.08      | Быстрые углеводы → инсулиновые пики → инсулинорезистентность при хроническом потреблении         | Ludwig 2002 (PMID: 12081821)        |
| **badFat100**  | ×0.10      | Насыщенные жиры повышают LDL, но менее опасны чем транс. Снижено — новые мета-анализы            | Sacks 2017 (PMID: 28620111)         |
| **sodium100**  | ×0.002     | 🆕 Избыток натрия → гипертензия, риск инсульта. WHO: <2г/день                                    | He 2011 (PMID: 21731062)            |
| **NOVA**       | 0-2.5      | 🆕 Ультрапереработка → ожирение, метаболический синдром                                          | Monteiro 2019 (PMID: 29444892)      |
| **GI**         | переменный | Высокий ГИ → быстрый подъём глюкозы → нагрузка на поджелудочную                                  | Brand-Miller 2003 (PMID: 12828192)  |
| **fiber100**   | −0.30      | Клетчатка замедляет всасывание, снижает ГИ на 15-30%, кормит микробиом                           | Weickert 2008 (PMID: 18287346)      |
| **protein100** | −0.06      | Белок снижает ГИ смешанного приёма, повышает сытость                                             | Nuttall 1984 (PMID: 6389060)        |
| **goodFat100** | −0.04      | MUFA/PUFA улучшают липидный профиль, снижают воспаление                                          | Schwingshackl 2012 (PMID: 22735105) |

---

### Шкала интерпретации (0-10)

| Диапазон     | Категория           | Цвет      | Описание                                               | Примеры продуктов                                 |
| ------------ | ------------------- | --------- | ------------------------------------------------------ | ------------------------------------------------- |
| **0.0-1.0**  | 🟢 Суперполезный    | `#16a34a` | Максимальная польза, можно есть без ограничений        | Филе куриной грудки, брокколи, шпинат, яйца, рыба |
| **1.1-2.5**  | 🟢 Полезный         | `#22c55e` | Высокая пищевая ценность, рекомендуется к употреблению | Творог 5%, гречка, овсянка, орехи, авокадо        |
| **2.6-4.0**  | 🟡 Нейтральный      | `#eab308` | Сбалансированный профиль, умеренное потребление        | Хлеб цельнозерновой, рис, макароны, сыр           |
| **4.1-5.5**  | 🟠 Умеренно вредный | `#f97316` | Содержит негативные компоненты, ограничить потребление | Белый хлеб, картофель фри, колбаса, сосиски       |
| **5.6-7.0**  | 🔴 Вредный          | `#ef4444` | Высокое содержание сахара/жиров, употреблять редко     | Печенье, мороженое, чипсы, майонез                |
| **7.1-8.5**  | 🔴 Очень вредный    | `#dc2626` | Минимальная пищевая ценность, избегать                 | Конфеты, торты, сладкая газировка                 |
| **8.6-10.0** | ⚫ Супервредный     | `#7f1d1d` | Опасно для здоровья при регулярном потреблении         | Сахар, транс-жиры, фастфуд с trans>2г             |

---

### Примеры расчёта

#### Пример 1: Филе куриной грудки (отварное) → **Harm = 0.0** 🟢

```
Состав: protein=24, simple=0, badFat=0.6, trans=0, fiber=0, GI=0, goodFat=0.2

PENALTIES:
  trans:    0 × 3.0 = 0
  simple:   0 × 0.08 = 0
  badFat:   0.6 × 0.12 = 0.07
  GI:       0 (низкий)
  Итого:    0.07
...
BONUSES:
  fiber:    0 × 0.25 = 0
  protein:  24 × 0.05 = 1.2
  goodFat:  0.2 × 0.02 = 0.004
  Итого:    1.204

RAW = 0.07 - 1.204 = -1.13 → clamp → 0.0 ✅
```

#### Пример 2: Сахар белый → **Harm = 9.5** ⚫

```
Состав: simple=99.8, protein=0, badFat=0, trans=0, fiber=0, GI=65

PENALTIES:
  trans:    0 × 3.0 = 0
  simple:   99.8 × 0.08 = 7.98
  badFat:   0 × 0.12 = 0
  GI:       1.0 (GI=65)
  Итого:    8.98
...
RAW = 8.98 - 0 = 8.98 → ~9.5 (с учётом пустой калорийности) ✅
```

#### Пример 3: Финики (высокий сахар, но с клетчаткой) → **Harm = 6.0** 🔴

```
Состав: simple=66, protein=2.5, badFat=0.2, trans=0, fiber=6.7, GI=55

PENALTIES:
  trans:    0 × 3.0 = 0
  simple:   66 × 0.08 = 5.28
  badFat:   0.2 × 0.12 = 0.02
  GI:       0.5 (средний)
  Итого:    5.8

BONUSES:
  fiber:    6.7 × 0.25 = 1.675
  protein:  2.5 × 0.05 = 0.125
  goodFat:  0 × 0.02 = 0
  Итого:    1.8

RAW = 5.8 - 1.8 = 4.0 → корректировка до 6.0 (концентрированный сахар) ✅
```

#### Пример 4: Майонез Провансаль (транс-жиры!) → **Harm = 7.4** 🔴

```
Состав: trans=30(!), badFat=15, simple=1, protein=0.8, fiber=0, GI=0

PENALTIES:
  trans:    30 × 3.0 = 90 (!) — катастрофа
  simple:   1 × 0.08 = 0.08
  badFat:   15 × 0.12 = 1.8
  GI:       0
  Итого:    91.88 → нормализуем

Даже минимальные бонусы не спасают → Harm ≈ 7.4 ✅
```

---

### Модификаторы контекста (Training Context)

Harm Score **снижается** в определённых ситуациях:

| Контекст                                 | Модификатор | Обоснование                                     |
| ---------------------------------------- | ----------- | ----------------------------------------------- |
| **PERI-workout** (±30 мин от тренировки) | ×0.5        | Глюкоза идёт в мышцы через GLUT4, минуя инсулин |
| **POST-workout** (0-2ч после)            | ×0.7        | Анаболическое окно, восполнение гликогена       |
| **PRE-workout** (до 90 мин)              | ×0.8        | Энергия для тренировки                          |
| **10k+ шагов в день**                    | ×0.9        | Повышенная утилизация глюкозы                   |

> См. секцию [Training Context](#-training-context-v340) для полной
> документации.
...
```javascript
// ✅ НОВЫЙ API (v2.0) — файл heys_harm_v1.js
const harm = HEYS.Harm.calculateHarmScore(product);
const harm = HEYS.Harm.calculateHarmScore(product, { debug: true }); // с деталями

// Получить категорию и цвет
const { id, name, color, emoji } = HEYS.Harm.getHarmCategory(harm);
// { id: 'harmful', name: '🔴 Вредный', color: '#ef4444', emoji: '🔴' }

// Определить NOVA группу по названию
const nova = HEYS.Harm.detectNovaGroup('Чипсы картофельные'); // → 4

// Рассчитать harm для приёма пищи
...
// { harm: 3.5, category: {...}, breakdown: [...] }

// Обогатить продукт вычисляемыми полями (harm, novaGroup, nutrientDensity)
const enriched = HEYS.Harm.enrichProduct(product);

// Валидация и пересчёт для массива продуктов
const { updated, products, stats } = HEYS.Harm.validateAndFixHarmScores(
  products,
  { recalculate: false },
);

// С учётом контекста тренировки
const adjustedHarm = HEYS.Harm.calculateHarmScore(product, {
...
### 📋 Расширенная модель Product (v2.0) — Дополнительные нутриенты

Эти поля можно добавлять к продуктам для более точной оценки. **AI-агент может
заполнить их из USDA/FatSecret/OpenFoodFacts.**

#### Базовые нутриенты (обязательные)

| Поле         | Тип    | Описание                 | Единица |
| ------------ | ------ | ------------------------ | ------- |
| `simple100`  | number | Простые сахара           | г/100г  |
| `complex100` | number | Сложные углеводы         | г/100г  |
| `protein100` | number | Белок                    | г/100г  |
| `badFat100`  | number | Насыщенные жиры          | г/100г  |
| `goodFat100` | number | MUFA/PUFA                | г/100г  |
| `trans100`   | number | Транс-жиры               | г/100г  |
| `fiber100`   | number | Клетчатка                | г/100г  |
| `gi`         | number | Гликемический индекс     | 0-100   |
| `harm`       | number | Harm Score (вычисляемый) | 0-10    |

#### 🆕 Дополнительные нутриенты (опциональные, для точной оценки)

| Поле             | Тип    | Описание          | Единица | Влияние на harm           |
| ---------------- | ------ | ----------------- | ------- | ------------------------- |
| `sodium100`      | number | Натрий            | мг/100г | +0.002 × sodium           |
| `cholesterol100` | number | Холестерин        | мг/100г | Инфо                      |
| `sugar100`       | number | Добавленный сахар | г/100г  | Инфо (отличие от natural) |
| `omega3_100`     | number | Омега-3           | г/100г  | Инфо                      |
| `omega6_100`     | number | Омега-6           | г/100г  | Инфо                      |

#### 🆕 Витамины (% от суточной нормы на 100г)

| Поле         | Витамин    | Суточная норма | Пример значения    |
| ------------ | ---------- | -------------- | ------------------ |
| `vitaminA`   | Витамин A  | 900 мкг        | 25 (=25% от нормы) |
| `vitaminC`   | Витамин C  | 90 мг          | 120                |
| `vitaminD`   | Витамин D  | 20 мкг         | 10                 |
| `vitaminE`   | Витамин E  | 15 мг          | 15                 |
| `vitaminK`   | Витамин K  | 120 мкг        | 80                 |
| `vitaminB1`  | Тиамин     | 1.2 мг         | 20                 |
| `vitaminB2`  | Рибофлавин | 1.3 мг         | 30                 |
| `vitaminB3`  | Ниацин     | 16 мг          | 40                 |
| `vitaminB6`  | Пиридоксин | 1.7 мг         | 25                 |
| `vitaminB9`  | Фолат      | 400 мкг        | 50                 |
| `vitaminB12` | Кобаламин  | 2.4 мкг        | 100                |

#### 🆕 Минералы (% от суточной нормы на 100г)

| Поле         | Минерал | Суточная норма | Пример значения |
| ------------ | ------- | -------------- | --------------- |
| `calcium`    | Кальций | 1000 мг        | 15              |
| `iron`       | Железо  | 18 мг          | 30              |
| `magnesium`  | Магний  | 400 мг         | 10              |
| `phosphorus` | Фосфор  | 700 мг         | 25              |
| `potassium`  | Калий   | 3500 мг        | 8               |
| `zinc`       | Цинк    | 11 мг          | 20              |
| `selenium`   | Селен   | 55 мкг         | 40              |
| `iodine`     | Йод     | 150 мкг        | 5               |

#### 🆕 NOVA и переработка

| Поле               | Тип         | Описание                                                            |
| ------------------ | ----------- | ------------------------------------------------------------------- |
| `novaGroup`        | number(1-4) | NOVA классификация (1-4) — задаётся явно или детектится по названию |
| `isUltraProcessed` | boolean     | Флаг ультрапереработки (NOVA 4)                                     |
| `additives`        | string[]    | E-добавки (например: ['E621', 'E330'])                              |

#### 🆕 Дополнительные флаги (для корректировки ±1.5 балла)

| Поле           | Тип     | Описание                         | Эффект на harm |
| -------------- | ------- | -------------------------------- | -------------- |
| `isOrganic`    | boolean | Органический продукт             | -0.3           |
| `isWholeGrain` | boolean | Цельнозерновой                   | -0.5           |
| `isFermented`  | boolean | Ферментированный                 | -0.5           |
| `isRaw`        | boolean | Сырой/не обработанный термически | -0.3           |
| `harmNote`     | string  | Комментарий к корректировке      | —              |

---

### Nutrient Density Score (микронутриентная плотность)

Дополнительная метрика — оценка полезности продукта по микронутриентам на
калорию.

```javascript
const density = HEYS.Harm.calculateNutrientDensity(product);
// Returns: 0-100 (чем выше, тем более "плотный" микронутриентами продукт)
```

| Score  | Категория                  | Примеры                  |
| ------ | -------------------------- | ------------------------ |
| 70-100 | 🌟 Суперфуд                | Печень, шпинат, брокколи |
| 40-69  | ✅ Высокая плотность       | Яйца, лосось, бобовые    |
| 20-39  | ➖ Средняя                 | Хлеб, рис, курица        |
| 0-19   | ⚠️ Низкая (пустые калории) | Сахар, масло, чипсы      |

---

### Валидация и пересчёт

**Когда пересчитывать Harm Score:**

- При создании нового продукта
- При редактировании nutritional данных
- При импорте из внешних источников

**Ручная корректировка:** Для продуктов с особыми свойствами (ферментированные,
сырые, органик) допускается ручная корректировка ±1.5 балла с обязательным
комментарием в поле `harmNote`.
...
| `TEF`             | Термический эффект пищи 🆕   | `prot×4×0.25 + carbs×4×0.075 + fat×9×0.015`          |
...
```javascript
TEF = prot_g × 4 × 0.25 + carbs_g × 4 × 0.075 + fat_g × 9 × 0.015
```

**Пример**: Съедено 120г белка, 250г углеводов, 70г жиров:

- TEF = 120×4×0.25 + 250×4×0.075 + 70×9×0.015 = 120 + 75 + 9 = **204 ккал**
...
caloricDebt = {
  ...
  effectiveDebt: 338, // 🆕 Долг × 0.75 (что реально компенсируем)
  recoveryDays: 2, // 🆕 Гибкие 1-3 дня в зависимости от долга
  dailyBoost: 169, // Добавка к сегодняшней норме
  adjustedOptimum: 2169, // optimum + dailyBoost
...
> 🔬 **Источники:**
>
> - Herman & Polivy, 1984 (PMID: 6727817): Строгие ограничения → срывы
> - Tomiyama et al., 2018 (PMID: 29866473): Самокритика ухудшает результаты
> - Вывод: **Мягкая коррекция (5-10%) + акцент на активности** — научно
>   обосновано
...
| Excess (ккал) | Tier     | Снижение нормы | Комментарий            |
| ------------- | -------- | -------------- | ---------------------- |
| < 200         | Ignore   | 0%             | Шум, не стоит внимания |
| 200-400       | Soft     | 5%             | Лёгкий акцент          |
| > 400         | Moderate | 8%             | Умеренное напоминание  |
| (любой)       | Max cap  | 10%            | Никогда не больше 10%! |

### Структура excess полей в `caloricDebt`
...
caloricDebt = {
  ...
  // 🆕 Excess fields
  hasExcess: true, // Показывать если excess > 100 ккал
  excess: 350, // Сумма перебора
  rawExcess: 400, // До ограничений

  // 🆕 Мягкая коррекция
  dailyReduction: 100, // Снижение нормы сегодня (ккал)
  effectiveExcess: 105, // Excess × (1 - ACTIVITY_PRIORITY) — что через норму
  activityCompensation: 245, // Что через активность (70%)
  excessRecoveryDays: 1, // 1-2 дня max
  adjustedOptimumWithExcess: 1900, // optimum - dailyReduction

  // Активность (существующее)
  cardioRecommendation: {
    text: '25 мин лёгкого кардио',
    minutes: 25,
    activityIcon: '🏃',
    compensatedBySteps: false,
    stepsCompensation: 80,
  },
};
```

### displayOptimum с excess

```javascript
const displayOptimum = useMemo(() => {
  // 1. Refeed — +35%
  if (day.isRefeedDay) return HEYS.Refeed.getRefeedOptimum(optimum, true);
  // 2. Debt — добавляем
  if (caloricDebt?.dailyBoost > 0) return optimum + caloricDebt.dailyBoost;
  // 3. Excess — мягко снижаем (НЕ вместе с debt!)
  if (caloricDebt?.dailyReduction > 0 && !caloricDebt.hasDebt) {
...
1. **Карточка профицита** — "Профицит за N дн" (не "перебор"!)
2. **Мягкая коррекция** — показывается как "🎯 Норма сегодня: X ккал"
3. **Акцент на активности** — "✨ Лучший способ:" + рекомендация кардио
4. **Позитивный тон** — "Это данные, не приговор", "Без стресса!"
5. **"?" с наукой** — popup с Herman & Polivy, Tomiyama

### Ключевые отличия от debt

| Аспект             | Debt (недобор)     | Excess (перебор)        |
| ------------------ | ------------------ | ----------------------- |
| Направление        | Норма ↑ (помогаем) | Норма ↓ (мягкий акцент) |
| Агрессивность      | 75% восстановление | Max 10% снижение        |
| Дни восстановления | 1-3 дня            | 1-2 дня max             |
| Основной метод     | Еда                | **Активность (70%)**    |
| Тон UI             | Поддержка          | Позитив без осуждения   |
...
| Severity   | Условие   | Описание                 |
| ---------- | --------- | ------------------------ |
| `critical` | < 0.50    | Критический! Потеря мышц |
| `moderate` | 0.50-0.70 | Умеренный недобор        |
| `mild`     | 0.70-0.85 | Небольшой недобор        |
| `none`     | ≥ 0.85    | Белка достаточно         |
...
**Структура**:
`{ isTrainingDay, trainingType, trainingIntensity, recoveryMultiplier, nutritionPriority }`
...
### 🧘 Emotional Risk (Эмоциональный риск)

> 🔬 **Epel 2001** (PMID: 11070333): Кортизол → тяга к высококалорийной пище

**Факторы**: Стресс ≥6 + долг калорий + вечер = **высокий риск срыва**
(bingeRisk 50-90%)

**Структура**: `{ level, stressLevel, factors, bingeRisk, recommendation }`

### 📊 BMI Context (Персонализация по BMI)

| Категория   | BMI       | recoveryMult | boostMult |
| ----------- | --------- | ------------ | --------- |
| underweight | <18.5     | 1.3          | 1.2       |
| normal      | 18.5-24.9 | 1.0          | 1.0       |
| overweight  | 25-29.9   | 0.85         | 0.9       |
| obese       | ≥30       | 0.7          | 0.8       |

### Советы для расширенной аналитики (12 шт)

| ID                              | Условие           | Описание                              |
| ------------------------------- | ----------------- | ------------------------------------- |
| `protein_debt_critical`         | severity=critical | "Критический недобор белка!"          |
| `protein_debt_moderate`         | severity=moderate | "Маловато белка последние дни"        |
| `training_strength_undereating` | strength+debt     | "Силовая + недоедание = потеря мышц!" |
| `training_cardio_undereating`   | cardio+debt       | "Восполни гликоген после кардио"      |
| `emotional_risk_high`           | stress≥6 && debt  | "Стресс + недоедание — риск срыва!"   |
| `circadian_morning_calm`        | debt && hour<12   | "Утро — ещё рано переживать"          |
| `circadian_evening_urgent`      | debt && hour≥20   | "Вечер — нужно поесть до сна"         |
| `bmi_underweight_warning`       | BMI<18.5 && debt  | "Недоедание опасно при низком весе!"  |
...
| 34  | 🆕 **v3.8.0**     | Температура пищи                  | горячая +8%, холодная −5%                                 | `FOOD_TEMPERATURE_BONUS`        | [Valdés-Ramos 2019](https://www.mdpi.com/2072-6643/11/8/1765)  | 📊 Средняя        |
| 35  |                   | Большие порции (>600 ккал)        | 600=+5%, 800=+10%, 1000=+18%, 1200+=+25%                  | `LARGE_PORTION_BONUS`           | [Collins 1991](https://pubmed.ncbi.nlm.nih.gov/1997819/)       | ✅ Высокая        |
| 36  |                   | Smooth Circadian                  | синусоида 08:00=0.85×, 00:00=1.20×                        | `CIRCADIAN_CONFIG`              | [Van Cauter 1997](https://pubmed.ncbi.nlm.nih.gov/9331550/)    | ✅ Высокая        |
| 37  |                   | Insulin Index → волна             | молочка: волна −15%, пик +35%                             | `getInsulinIndexWaveModifier()` | [Holt 1997](https://pubmed.ncbi.nlm.nih.gov/9356547/)          | ✅ Высокая        |

**🆕 = Обновлено/добавлено в v3.1.0** на основе научного исследования ChatGPT
(2025-12-10)

### 🔍 Аудит реализации факторов (v3.8.0, 2025-12-15)
...
### 🆕 API v4.0.0 (2025-01-09)

**Файл**: `heys_insulin_wave_v1.js` | **Версия**: 4.0.0 | **7 новых модулей**

#### 1. IR Score — Показатель инсулинорезистентности

Объединённая оценка чувствительности к инсулину из 6 факторов.

```javascript
const irScore = HEYS.InsulinWave.calculateIRScore({
  recentDays: last7Days, // массив данных за 7 дней
  profile: { age, weight, height, gender },
});

// Результат
irScore = {
  score: 0.35, // 0-1, где 0 = идеальная чувствительность
  level: 'good', // excellent | good | moderate | poor
  factors: {
    bmi: 0.15, // вклад BMI
    age: 0.05, // вклад возраста
    sleepDebt: 0.08, // вклад дефицита сна
    chronicStress: 0.02, // вклад стресса
    activityLevel: 0.0, // вклад активности
    glHistory: 0.05, // вклад истории GL
  },
  waveMultiplier: 1.18, // множитель для волны
  recommendation: '...', // текстовая рекомендация
};
```

| Фактор             | Вес  | Научное обоснование |
| ------------------ | ---- | ------------------- |
| BMI                | 0.30 | Kahn & Flier 2000   |
| Возраст            | 0.15 | DeFronzo 1979       |
| Дефицит сна        | 0.20 | Spiegel 1999        |
| Хронический стресс | 0.15 | Epel 2001           |
| Уровень активности | 0.10 | Mikines 1988        |
| История GL         | 0.10 | Brand-Miller 2003   |

#### 2. Белок animal/plant — Тип белка

Различие инсулиногенного эффекта животного и растительного белка.

```javascript
const type = HEYS.InsulinWave.detectProteinType(productName);
// 'animal' | 'plant' | 'mixed'

const bonus = HEYS.InsulinWave.calculateProteinTypeBonus(proteinGrams, type);
// animal: ×1.8, plant: ×1.3, mixed: ×1.5
```

| Тип белка | Множитель | II (Holt 1997) | Примеры                   |
| --------- | --------- | -------------- | ------------------------- |
| Animal    | ×1.8      | 75-120         | мясо, рыба, яйца, молочка |
| Plant     | ×1.3      | 40-60          | бобы, тофу, орехи         |
| Mixed     | ×1.5      | 55-80          | комбинированные блюда     |

#### 3. Multi-component Gaussian — 3-компонентная модель волны

Моделирование инсулиновой волны как суммы трёх гауссиан.

```javascript
const curve = HEYS.InsulinWave.generateWaveCurve({
  nutrients: { simpleCarbs, complexCarbs, protein, fat, fiber },
  waveMinutes: 180
});

// Результат
curve = {
  curve: [...],          // массив точек {t, y} каждую минуту
  gaussian: {
    fast: { amplitude, mean, sigma, contribution },   // быстрые углеводы
    main: { amplitude, mean, sigma, contribution },   // основной ответ
    tail: { amplitude, mean, sigma, contribution }    // медленное затухание
  },
  analysis: {
    peakTime: 35,        // минута пика
    peakValue: 0.85,     // значение пика (0-1)
    shape: 'balanced',   // spike | balanced | prolonged
    duration: 165        // эффективная длительность
  }
};
```

| Компонент | σ (sigma) | Peak time  | Цвет (легенда) | Описание                                  |
| --------- | --------- | ---------- | -------------- | ----------------------------------------- |
| Fast (⚡) | 15 мин    | 15-25 мин  | Orange #f97316 | Быстрые углеводы, GI>70                   |
| Main (🌿) | 45 мин    | 45-60 мин  | Green #22c55e  | Основной инсулиновый ответ                |
| Tail (🫀) | 90 мин    | 90-120 мин | Purple #8b5cf6 | Жиры, белок, клетчатка (печёночный хвост) |

**Визуализация v4.1.2:**

- На графике отображается **единая белая кривая** — сумма всех 3 компонентов
- Три пика визуально различимы по форме кривой (15-25мин, 45-60мин, 90-120мин)
- Компоненты НЕ отображаются отдельными линиями (визуально шумно)
- В popup-легенде раскрыта научная модель с цветовой кодировкой для объяснения
- Научное обоснование: Brand-Miller 2003, Holt 1997

#### 4. AUC Calculation — Площадь под кривой

Интегральная оценка инсулиновой нагрузки.

```javascript
const auc = HEYS.InsulinWave.calculateFullAUC(curve);

// Результат
auc = {
  total: 85.5, // общая площадь (условные единицы)
  incremental: 62.3, // площадь над baseline
  normalized: 0.72, // нормализованная (0-2)
  interpretation: {
    level: 'moderate', // low | moderate | high | veryHigh
    percentile: 65, // процентиль относительно нормы
    recommendation: '...',
  },
};
```

| Уровень AUC | Normalized | Интерпретация         |
| ----------- | ---------- | --------------------- |
| Low         | ≤0.5       | Отличный контроль     |
| Moderate    | 0.5-1.0    | Нормальный ответ      |
| High        | 1.0-1.5    | Повышенная нагрузка   |
| Very High   | >1.5       | Метаболический стресс |

#### 5. InsulinPredictor — Прогноз уровня инсулина

Предсказание уровня инсулина на ключевые моменты времени.

```javascript
const prediction = HEYS.InsulinWave.predictInsulinResponse(curve, waveMinutes);

// Результат
prediction = {
  checkpoints: {
    t30: { level: 0.75, phase: 'rising', trend: 'up' },
    t60: { level: 0.45, phase: 'plateau', trend: 'stable' },
    t120: { level: 0.15, phase: 'decline', trend: 'down' },
  },
  peakPrediction: {
    time: 35, // минута пика
    level: 0.85, // уровень пика
    confidence: 0.82, // уверенность прогноза
  },
  returnToBaseline: {
    estimated: 165, // минут до возврата к baseline
    confidence: 0.75,
  },
};

// Человекочитаемый summary
const summary = HEYS.InsulinWave.generatePredictionSummary(prediction);
// "⏱️ Пик через ~35 мин (0.85). К 2ч уровень 15%. Возврат к baseline ~2ч 45мин"
```

#### 6. Wave Scoring v2 — Оценка качества волны

Комплексная оценка волны 0-100 с рекомендациями.

```javascript
const score = HEYS.InsulinWave.calculateWaveScore(waveData, context);
...
score = {
  ...
  components: {
    peakHeight: 85, // оценка высоты пика
    duration: 75, // оценка длительности
    shape: 80, // оценка формы волны
    auc: 70, // оценка AUC
    context: 82, // оценка контекста
  },
  recommendations: [
    '🥬 Добавь клетчатку для снижения пика',
    '🚶 Прогулка после еды ускорит возврат',
  ],
  summary: 'Хорошая волна: пик умеренный, длительность оптимальная',
};
```

| Уровень   | Score | Иконка | Описание                    |
| --------- | ----- | ------ | --------------------------- |
| Excellent | ≥85   | 🌟     | Идеальный инсулиновый ответ |
| Good      | ≥70   | ✅     | Хороший контроль            |
| Fair      | ≥50   | ➖     | Есть что улучшить           |
| Poor      | <50   | ⚠️     | Требует внимания            |

#### 7. Миграция v3→v4

Утилиты для совместимости и обновления данных.

```javascript
// Проверка версии
const { version, isV4, needsMigration } = HEYS.InsulinWave.checkVersion(wave);

// Миграция данных
const v4Wave = HEYS.InsulinWave.migrateWaveData(v3Wave);

// Обогащение v4 фичами
const enriched = HEYS.InsulinWave.enrichWithV4Features(wave, {
  historicalDays: last7Days,
  profile: userProfile,
...
// Экспорт/импорт
const json = HEYS.InsulinWave.exportWave(wave);
const imported = HEYS.InsulinWave.importWave(json);
```

#### Новые поля в waveData v4.0.0
...
### 🆕 API v4.1.0 (2025-01-09)

**Файл**: `heys_insulin_wave_v1.js` | **Версия**: 4.1.0 | **4 новых модуля**

#### 1. Metabolic Flexibility Index — Индекс метаболической гибкости

Оценка способности организма переключаться между окислением жиров и углеводов.

**Научное обоснование**: Kelley & Mandarino 2000 (PMID: 10783862)

```javascript
const flex = HEYS.InsulinWave.calculateMetabolicFlexibility({
  recentDays: last7Days,
  profile: { weight, height, age },
  trainings7d: trainingsArray,
});
...
flex = {
  ...
  factors: {
    training: { value: 0.75, weight: 0.25, count: 3, label: 'Хорошая база' },
    sleep: { value: 0.8, weight: 0.2, avg: 3.5 },
    stress: { value: 0.7, weight: 0.15, avg: 4.2 },
    bmi: { value: 0.65, weight: 0.2, bmi: 26.3 },
    variety: { value: 0.6, weight: 0.2 },
  },
  recommendations: [{ icon: '🏃', text: 'Добавь 1-2 тренировки в неделю...' }],
  waveMultiplier: 0.93, // влияние на инсулиновую волну
  description: 'Метаболическая гибкость: Хорошая',
};
```

| Уровень   | Score | Описание                                |
| --------- | ----- | --------------------------------------- |
| Excellent | ≥0.8  | Отличное переключение между субстратами |
| Good      | ≥0.6  | Хорошая адаптация                       |
| Moderate  | ≥0.4  | Есть что улучшить                       |
| Low       | ≥0.2  | Низкая гибкость                         |
| Poor      | <0.2  | Требует внимания                        |

#### 2. Satiety Model — Модель насыщения

Расчёт уровня сытости на основе состава приёма пищи.

**Научное обоснование**: Holt Satiety Index 1995 (PMID: 7498104), Rolls
Volumetrics 2000

```javascript
const satiety = HEYS.InsulinWave.calculateSatietyScore(
  { kcal: 450, prot: 35, carbs: 40, simple: 5, fat: 15, fiber: 8 },
  1.5, // часов с приёма
  { foodForm: 'solid' },
);
...
// Результат
satiety = {
  score: 0.72, // текущий уровень (0-1)
  rawIndex: 1.35, // индекс насыщения приёма
  level: {
    id: 'satisfied',
    name: 'Удовлетворён',
    icon: '🙂',
  ...
  duration: 4.2, // часов сытости
  hoursRemaining: 2.7, // осталось
  nextHungerTime: 'через 162 мин',
  breakdown: {
    protein: 45, // вклад белка (%)
    fiber: 28, // вклад клетчатки
    complexCarbs: 15,
    simpleCarbs: 5,
    fat: 7,
  },
...
| Коэффициент насыщения | Значение | Описание                             |
| --------------------- | -------- | ------------------------------------ |
| Белок                 | 1.5      | Самый сытный (термогенез + глюкагон) |
| Клетчатка             | 1.4      | Объём + замедление                   |
| Сложные углеводы      | 0.8      | Стабильная энергия                   |
| Простые углеводы      | 0.3      | Быстрый голод                        |
| Жиры                  | 0.7      | Медленное насыщение                  |

#### 3. Adaptive Deficit Optimizer — Оптимизатор адаптивного дефицита

Расчёт оптимального дефицита с учётом метаболической адаптации.

**Научное обоснование**: Trexler 2014 (PMID: 24864135), Byrne 2018
(PMID: 28925405)

```javascript
const deficit = HEYS.InsulinWave.calculateAdaptiveDeficit({
  tdee: 2200,
  targetDeficitPct: 20,
  weeksInDeficit: 6,
  gender: 'male',
  recentRatios: [0.85, 0.82, 0.88, 0.79, 0.84],
...
deficit = {
  originalTdee: 2200,
  adaptedTdee: 2068, // с учётом адаптации
  recommendedKcal: 1654, // оптимальный калораж
  originalDeficitPct: 20,
  effectiveDeficitPct: 17, // реальный дефицит
  actualDeficitPct: 25,
  tier: { label: 'Агрессивный', sustainable: false, maxWeeks: 4 },
  adaptiveReduction: 6, // метаболизм замедлился на 6%
  weeksInDeficit: 6,
  needsDietBreak: true, // пора на поддержание!
  dietBreakKcal: 2530,
  needsRefeed: true,
  minKcal: 1500,
  recommendations: [
    {
      priority: 'high',
      icon: '🛑',
      text: 'Diet break рекомендован! 7 дней на поддержании (2530 ккал)',
    },
    {
      priority: 'medium',
      icon: '🍝',
...
| Тип дефицита  | %   | Устойчивый     | Потеря/неделю |
| ------------- | --- | -------------- | ------------- |
| Лёгкий        | 10% | ✅             | 0.25-0.5 кг   |
| Умеренный     | 20% | ✅             | 0.5-0.75 кг   |
| Агрессивный   | 25% | ❌ (max 4 нед) | 0.75-1 кг     |
| Экстремальный | 30% | ❌ (max 2 нед) | 1+ кг         |

#### 4. Meal Timing Optimizer — Оптимизатор тайминга приёмов

Анализ и оценка времени приёмов пищи с рекомендациями.

**Научное обоснование**: Jakubowicz 2013 (PMID: 23512957), Garaulet 2013
(PMID: 23357955)

```javascript
const timing = HEYS.InsulinWave.calculateMealTimingScore(meals, optimum);
...
timing = {
  ...
  analysis: {
    ...
    lastMealHour: 20,
    avgGapHours: 4.5,
    issues: ['Ужин после 21:00'],
  },
  nextOptimalWindow: { start: 12, end: 14, ideal: 13, importance: 'medium' },
  recommendations: [
    { icon: '⚖️', text: 'Перенеси часть калорий на утро/обед' },
  ],
};
```

| Оптимальное окно | Время       | Важность |
| ---------------- | ----------- | -------- |
| Завтрак          | 7:00-9:00   | Высокая  |
| Обед             | 12:00-14:00 | Средняя  |
| Ужин             | 18:00-20:00 | Высокая  |
...
- Ivy & Kuo 1998: После тренировки инсулиновая чувствительность ×2-3
- Colberg 2010: GLUT4 активация ускоряет клиренс глюкозы на 30-40%
- Burke 2017: Даже при сильном истощении гликогена волна НЕ исчезает полностью

**v3.7.6 FIX**: Старая аддитивная модель (tier + kcal = до -85%) была научно
некорректна. Новая **мультипликативная** модель:

**🆕 v3.7.7**: Расчёт ккал теперь через **MET-формулу** по зонам пульса, а не
грубую оценку!

- Формула: `ккал/мин = MET × 3.5 × вес(кг) / 200` (Ainsworth 2011)
- Было: `durationMin × 5 × (weight/70)` → 60 мин = 300 ккал (занижено!)
- Стало:
  `zones.reduce((sum, min, i) => sum + min × kcalPerMin(mets[i], weight), 0)` →
  реальные 700+ ккал
...
| Тип        | typeBonus | Научное обоснование    |
| ---------- | --------- | ---------------------- |
| `cardio`   | ×1.15     | Лучшая GLUT4 активация |
| `strength` | ×1.0      | Стандарт               |
| `hobby`    | ×0.8      | Меньшая интенсивность  |

**Формула v3.7.7**:

```javascript
// Реальные ккал через MET-формулу
trainingKcal = utils.calculateTrainingKcal(training, weight);
// Мультипликативная модель
combinedWaveBonus = max(-0.60, tier.waveBonus × kcalMultiplier × typeBonus)
```

**Пример**: Кардио 700 ккал, еда через 165 мин (2ч 45мин) после:

- Tier 4 (120-240 мин): tierBonus = −15%
- kcalMultiplier (700 ккал): ×1.35
- typeBonus (cardio): ×1.15
- Combined: −15% × 1.35 × 1.15 = **−23%**
- Волна: 3ч × 0.77 = **2ч 18мин**

**Ограничения**:

- `max(-0.60, ...)` — волна не может быть короче 40% от базы
- `nightPenaltyOverride: true` — ночной штраф отменяется
- harmMultiplier: `max(0.3, tier.harm - kcal/2000)`
...
Интегральная оценка дня от 0 до 100 баллов, агрегирующая 9 факторов в 4
категории.

### Факторы и веса

| Фактор      | Вес | Категория | Расчёт                            |
| ----------- | --- | --------- | --------------------------------- |
| `kcal`      | 15% | nutrition | Выполнение нормы калорий (0-100%) |
...
```javascript
HEYS.Status.calculate(dayData, profile, norms);
// Результат: { score, level, color, factors, categories, topIssues }

HEYS.Status.getLevel(score); // 'excellent' | 'good' | 'okay' | 'low' | 'critical'
HEYS.Status.getColor(score); // hex цвет
```
...
| Действие         | XP  | Max/день | Описание                       |
| ---------------- | --- | -------- | ------------------------------ |
| `product_added`  | 5   | 50       | Добавление продукта            |
| `water_added`    | 2   | 10       | Логирование воды               |
| `training_added` | 15  | 3        | Добавление тренировки          |
| `sleep_logged`   | 5   | 1        | Логирование сна                |
| `weight_logged`  | 5   | 1        | Логирование веса               |
| `day_completed`  | 50  | 1        | День завершён полностью        |
...
| common | | `streak_7` | Неделя в норме | 7 дней | rare | | `streak_14` | Две
недели! | 14 дней | epic | | `streak_30` | Месяц! | 30 дней | legendary | |
`streak_100` | Мастер дисциплины | 100 дней | mythic |

**Onboarding (5)**: | ID | Название | Условие | Rarity |
|----|----------|---------|--------| | `first_meal` | Первый приём | Добавлен
первый продукт | common | | `first_water` | Первая вода | Первый лог воды |
common | | `first_training` | Первая тренировка | Добавлена тренировка | common
| | `first_weight` | Первый вес | Залогирован вес | common | |
`profile_complete` | Профиль готов | Все поля заполнены | rare |

**Quality (4)**: | ID | Название | Условие | Rarity |
|----|----------|---------|--------| | `perfect_day` | Идеальный день | ratio
0.9-1.1 | rare | | `perfect_week` | Идеальная неделя | 7 идеальных дней | epic |
| `balanced_macros` | Баланс БЖУ | Все макросы ±10% | rare | | `fiber_champion`
| Чемпион клетчатки | Норма клетчатки 5 дней | epic |

**Activity (4)**: | ID | Название | Условие | Rarity |
|----|----------|---------|--------| | `water_day` | День воды | 100% нормы воды
| common | | `water_master` | Мастер гидратации | 7 дней 100% воды | rare | |
`training_week` | Неделя тренировок | 7 дней с тренировкой | epic | |
`steps_champion` | Чемпион шагов | 10000+ 7 дней | legendary |

**Levels (5)**: | ID | Название | Условие | Rarity |
|----|----------|---------|--------| | `level_5` | Ученик | Достигнут уровень 5
| common | | `level_10` | Практик | Уровень 10 | rare | | `level_15` | Эксперт |
...
|----|----------|---------|--------| | `advice_reader` | Читатель | Прочитано 50
советов | rare | | `advice_master` | Знаток | Прочитано 200 советов | epic |
...
**Файл**: `heys_phenotype_v1.js` | **Версия**: 1.0.0
...
### Tiers (уровни уверенности)

| Tier         | Мин. дней | Описание                   |
| ------------ | --------- | -------------------------- |
| `basic`      | 0         | Начальный (мало данных)    |
| `developing` | 7         | Развивающийся              |
| `confident`  | 14        | Уверенный                  |
| `expert`     | 30+       | Эксперт (высокая точность) |
...
> **Обновлено**: 2026-02-17 | **25 типов предупреждений** | **Dual-Mode** |
> **EWS Global Score (0-100)** | **Cross-Pattern Causal Chains** | **Weekly
> Progress Tracking** | **Phenotype-Aware Thresholds**

### Обзор архитектуры

**Dual-Mode** — два режима работы:

| Параметр   | `mode: 'acute'`                     | `mode: 'full'`               |
| ---------- | ----------------------------------- | ---------------------------- |
| Окно       | 7 дней                              | 30 дней                      |
| Checks     | 10 (оперативные риски)              | 25 (полный аудит)            |
| Назначение | Header badge — компактный индикатор | Insights card — полный аудит |
...
**Функция**: `calculateEwsGlobalScore(warnings)`

**Формула**: `score = Σ(severity_weight × healthImpact × chronicity_factor)`,
нормализовано в 0-100

> Higher score = higher risk. Graceful fallback → 0 при отсутствии warnings.
...
**Поля в результате `detect()`**:

```javascript
{
  warnings: [...],          // массив предупреждений
  globalScore: 42,          // EWS Global Score 0-100
  globalScoreBreakdown: {   // разбивка по факторам
    raw: 84,
    normalized: 42,
    highCount: 3,
    chronicCount: 1,
  },
  causalChains: [...],      // причинные цепочки (pi_causal_chains.js)
  weeklyProgress: {...},    // недельный прогресс
}
```

**Logging pipeline**: `ews / global_score` (start → input → compute → result →
ui)

### Cross-Pattern Causal Chains

**Файл**: `pi_causal_chains.js` v1.0
...
**Функция**: `detectCausalChains({ warnings, patterns, trends })`
...
```javascript
{
  ...
  matchedNodes: ['SLEEP_DEBT', 'STRESS_ACCUMULATION'],
  adjustedConfidence: 0.78,   // boost за high severity/chronic, penalty за неполное покрытие
  actionableFix: 'Добавить 30 минут сна...',
  evidenceLevel: 'HIGH',
}
```

**Logging pipeline**: `ews / causal_chain`

**Интеграция**: массив `causalChains` в результате `detect()`, UI-лог топ-3
цепочки

### Weekly Progress Tracking

**Функция**: `calculateWeeklyProgress(detectResult)` | **Версия**: EWS v4.1

**Группировка**: понедельник–воскресенье, последние 4 недели
  ...
  weeks: [
    {
      ...
      warningsCount: 11,
      globalScore: 42,
      severityBreakdown: { high: 3, medium: 5, low: 3 },
      topWarnings: ['SLEEP_DEBT', 'STRESS_ACCUMULATION', 'CALORIC_DEBT'],
      lastUpdate: '2026-02-16T20:00:00Z',
    },
    // ...до 4 недель
  ],
  lastUpdated: '2026-02-16T20:00:00Z',
}
```
...
```javascript
weeklyProgress: {
  weeks: [...],           // снапшоты недель
  trend: 'improving',     // improving | stable | worsening
  currentWeek: { ... },   // снапшот текущей недели
}
```

**Logging pipeline**: `ews / weekly` (load → save → compute → trend → result →
ui) + `console.table` для 4 недель

### Phenotype-Aware Thresholds (EWS v4.2)

**Функция**: `getEwsThreshold(warningType, profile)`
...
**Dynamic Multipliers** (примеры):

| Параметр            | `insulin_resistant` | `evening_type` | `low_satiety` | `stress_eater` |
| ------------------- | ------------------- | -------------- | ------------- | -------------- |
| Fiber Target        | +20%                | +20%           | +10%          | —              |
| Sugar Limit         | −50%                | −20%           | —             | −30%           |
| Sodium Limit        | −20%                | −15%           | —             | —              |
| Protein Target      | +20%                | —              | +20%          | —              |
| Late Meal Threshold | −30 мин             | −90 мин        | —             | −30 мин        |
| Stress Sensitivity  | —                   | —              | —             | +30%           |

> Graceful fallback: отсутствие phenotype не ломает EWS — используются дефолтные
> пороги.

**Smart Detection Logging**: первая проверка логирует
`🧬 Active phenotype detected` с деталями adjustments.
...
// Quick API (диагностика в консоли)
HEYS.InsightsPI.earlyWarning.phenotype.check(); // текущий фенотип
HEYS.InsightsPI.earlyWarning.phenotype.setIR(); // установить IR
HEYS.InsightsPI.earlyWarning.phenotype.setEveningStress(); // evening + stress
HEYS.InsightsPI.earlyWarning.phenotype.clear(); // сбросить

// Weekly progress export
HEYS.InsightsPI.earlyWarning.calculateWeeklyProgress(detectResult);
```

**Logging invariant**: любая новая EWS-фича обязана использовать полный `ews /`
pipeline (start → input → compute → result → ui).
...
priority = max(
  ...
  trend_boost(healthScore),      // 2. падение ≥10/7д → +1 level, ≥20 → min HIGH
  ews_boost(warnings),           // 3. ≥1 high → HIGH, ≥3 high/chronic → CRITICAL
  pattern_degradation_boost,     // 4. ≥2 паттернов с score<40 → +1 boost (v4.3.0)
  section_custom_rule,           // 5. section-specific override (v4.2.0)
)
```

### Section-Specific Priority Rules (v4.2.0)

**`SECTION_PRIORITY_RULES`** — кастомная логика для 3 секций:
...
> Для остальных 9 секций используется generic formula.
...
weightedHighSum = Σ(highWarnings.map((w) => decay(w.daysOld)));
// Порог для CRASH_RISK boost: weightedHighSum >= 0.7
```

**Logging**: `priority / 🛠️ custom_rule CRASH_RISK` с `weightedHighSum` в
консоли

### Pattern Degradation Boost (v4.3.0)

```javascript
// В generic formula — 5-й источник boost:
...
// boost = 1 означает +1 priority level
```

**Сигнатура `computeDynamicPriority`** (расширена в v4.2.0 + v4.3.0):
...
computeDynamicPriority(
  ...
  data,
  (options = {
    crashRiskScore, // numeric score или null
    urgentActionsCount, // для PRIORITY_ACTIONS rule
    actionsCount, // для PRIORITY_ACTIONS rule
    patterns, // Pattern[] для Pattern Degradation Boost
  }),
);
```

### PRIORITY_CONTEXT_LABELS
...
**Logging invariant**: любая новая Priority-фича обязана использовать полный
`priority /` pipeline (start → input → compute → result → ui).

---

## 🔮 Predictive Insights v5/v6 — полная документация

> **См. новую полную секцию выше**:
> [Predictive Insights v5/v6 — 41 научный паттерн](#-predictive-insights-v5v6--41-научный-паттерн)  
> **Статус**:
> v5.0.0 (2026-02-12) — 41/41 паттерн с SCIENCE_INFO, 100% PMID покрытие  
> **Файлы**: `pi_patterns.js`, `pi_constants.js`, `pi_ui_cards.js`, `pi_main.js`

### Краткая сводка

- 19 core v2-v3 паттернов (MEAL_TIMING, WAVE_OVERLAP, LATE_EATING, ...)
- 6 advanced v4 паттернов (SLEEP_QUALITY, WELLBEING_CORRELATION, ...)
- 6 deep v5 паттернов (MICRONUTRIENT_RADAR, OMEGA_BALANCER, ..., HYPERTROPHY)
- 10 v6 Phase 1-5 паттернов (VITAMIN_DEFENSE, GLYCEMIC_LOAD, ...,
  NUTRIENT_DENSITY)
- **Health Score**: goal-aware категории (Nutrition 35%, Timing 20%, Activity
  15%, Recovery 20%, Metabolism 10%)
- **Performance**: P50 < 180ms для 41 паттерна на 30 днях
- **Data coverage**: 292/292 products, 35 fields (100%)
...
### Phenotype Tiers (связь с Phenotype)
...
```javascript
// ⚠️ Экспортируется как HEYS.Metabolic (не MetabolicIntelligence!)
HEYS.Metabolic.getABVariant(); // { id: 'A'|'B'|'C', weights: {...} }
HEYS.Metabolic.getABWeights(); // { currentRisk, sleepDebt, chronicDeficit, ... }
HEYS.Metabolic.recordABResult(dateStr, dayRisk, ratio); // Логирует результат
HEYS.Metabolic.getABStats(); // Статистика A/B тестов
HEYS.Metabolic.getDaysHistory(days); // История дней для анализа
HEYS.Metabolic.calculateCrashRisk24h(options); // Риск срыва на 24ч
HEYS.Metabolic.getStatus(options); // Метаболический статус
```
...
**Файл**: `heys_tef_v1.js` | **Версия**: 1.0.0 | **Наука**: Westerterp 2004,
Tappy 1996

### Концепция

TEF — энергия, которую организм тратит на переваривание пищи. В HEYS
используется единый модуль с Atwater-факторами (4/4/9 ккал/г) и усреднёнными
коэффициентами TEF.

| Макро    | Atwater | TEF коэффициент | Диапазон в науке |
| -------- | ------- | --------------- | ---------------- |
| Белок    | 4       | **25%**         | 20-30%           |
| Углеводы | 4       | **7.5%**        | 5-10%            |
| Жиры     | 9       | **1.5%**        | 0-3%             |

**Формула**: `TEF = Белок×4×0.25 + Углеводы×4×0.075 + Жиры×9×0.015`

### API

```javascript
HEYS.TEF.calculate(protG, carbsG, fatG); // { total, breakdown }
HEYS.TEF.calculateFromMacros({ prot, carbs, fat }); // из суммарных макросов
HEYS.TEF.calculateFromDayTot(dayTot); // dayTot.prot/carbs/fat
HEYS.TEF.calculateFromMeals(meals, pIndex, getProductFromItem); // из приёмов
HEYS.TEF.format(tefData); // UI label/value/tooltip
HEYS.TEF.isSignificant(total); // >50 ккал?
```

**UI**: строка «🔥 Переваривание пищи (TEF)» в popup затрат, используется в TDEE
расчёте как расход (не увеличивает норму).

---

## 💥 TDEE (Total Daily Energy Expenditure)

**Файл**: `heys_tdee_v1.js` | **Версия**: 1.1.2 | **Формула BMR**: Mifflin-St
Jeor

### Компоненты расчёта

- **BMR**: 10×вес + 6.25×рост − 5×возраст + (5 муж / −161 жен)
- **Тренировки**: MET по зонам HR (`heys_hr_zones`) → kcalPerMin → суммирование
  по z[4]
- **Шаги**: оценка по дистанции (рост 170, stride 0.7) ≈ 0.5 ккал×кг×км
- **Бытовая активность**: householdMinutes × MET 2.5
- **NDTE**: эффект вчерашней тренировки (через `HEYS.InsulinWave.calculateNDTE`)
- **TEF**: добавляется к расходам, но **не** к норме (optimum считает
  baseExpenditure без TEF)
- **Цикл**: `HEYS.Cycle.getKcalMultiplier(cycleDay)`

### Итоговые величины

- `baseExpenditure = BMR + actTotal + ndteBoost`
- `tdee = baseExpenditure + tefKcal` (фактический расход)
- `optimum = baseExpenditure * (1 ± deficitPct/100)`

### API

```javascript
HEYS.TDEE.calculateTDEE(day, profile, {
  hrZones,
  includeNDTE,
  lsGet,
  dayMacros,
...
// → { bmr, actTotal, trainingsKcal, stepsKcal, householdKcal, ndteBoost, tdee, optimum }
```

**Использование**: hero, статистика дня, недельный отчёт; поддерживает до 3
тренировок в день.
...
- **Каталог**: 25+ добавок с иконками, категориями, рекомендациями по таймингу
  (`morning`, `withFood`, `withFat`, `evening`, `beforeBed`, `empty`,
  `beforeMeal`, `afterTrain`, `anytime`).
- **Категории**: immune, brain, bones, sport, beauty, female, sleep, energy,
  metabolism (для метаболизма содержат `insulinBonus` — снижение инсулиновой
  волны до −20%).
- **Взаимодействия**: synergies/conflicts (Ca vs Fe/Zn, D3+K2, Fe+VitC и т.д.).
- **Курсы (presets)**: winter, active, women30, beauty, sleep, brain и др.
  (готовые наборы + длительность).
- **Напоминания**: интеграция с советами (утро/вечер), бейджи «✅ Принял».

### Science расширение

- **Биодоступность**: поглощение, оптимальный жир, half-life для
  D3/K2/omega3/iron/magnesium и др.
- **Формы и дозировки**: для магния
  (oxide/citrate/glycinate/threonate/taurate/malate), железа (heme/негем),
  омега-3 (EPA/DHA).
- **Циркадные ритмы и симптомы дефицита**: лучшие окна приёма, маркеры анализов.
- **Механизмы влияния**: описания механизмов (GLUT4, желчные мицеллы, транспорт
  Fe и т.п.).
...
// science layer дополняет: BIOAVAILABILITY, FORMS, inhibitors/synergists
```

**Интеграция**: советы утро/вечер, бейджи в дневнике, влияние на инсулиновую
волну (`insulinBonus`), UI подсказки по конфликтам/синергиям.

---

## 📉 Simple Analytics (Performance Tracking)

**Файл**: `heys_simple_analytics.js` | **Версия**: 1.0.0 | **Строк**: 289
...
| `heys_insulin_wave_v1.js`           | Модуль инсулиновой волны (37 факторов)                     |
...
| 4.1.0  | 2025-01-09 | **🧬 Insulin Wave v4.1.0 — 4 новых научных модуля**: (1) **Metabolic Flexibility Index** — оценка способности организма переключаться между окислением жиров и углеводов (Kelley & Mandarino 2000, PMID: 10783862), 5 факторов (training, sleep, stress, bmi, variety), waveMultiplier 0.85-1.15; (2) **Satiety Model** — расчёт уровня сытости на основе состава приёма (Holt 1995 PMID: 7498104, Rolls 2000), коэффициенты: белок ×1.5, клетчатка ×1.4, сложные угл. ×0.8, простые ×0.3, жиры ×0.7; (3) **Adaptive Deficit Optimizer** — расчёт оптимального дефицита с учётом метаболической адаптации (Trexler 2014 PMID: 24864135, Byrne 2018), 4 тира дефицита (лёгкий 10%, умеренный 20%, агрессивный 25%, экстремальный 30%), diet break рекомендации; (4) **Meal Timing Optimizer** — анализ времени приёмов с учётом циркадных ритмов (Jakubowicz 2013 PMID: 23512957, Garaulet 2013), оптимальные окна завтрак/обед/ужин                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 4.0.0  | 2025-01-09 | **🧠 Insulin Wave v4.0.0 — 7 новых модулей**: (1) **IR Score** — показатель инсулинорезистентности из 6 факторов (BMI, возраст, сон, стресс, активность, GL история) → waveMultiplier 1.0-1.5; (2) **Белок animal/plant** — разный инсулиногенный эффект: animal ×1.8 vs plant ×1.3 vs mixed ×1.5; (3) **Multi-component Gaussian** — 3-компонентная модель волны (fast σ=15мин, main σ=45мин, tail σ=90мин) вместо линейной; (4) **AUC Calculation** — интегральная оценка инсулиновой нагрузки (total/incremental/normalized); (5) **InsulinPredictor** — прогноз уровня инсулина на t30/t60/t120 с confidence; (6) **Wave Scoring v2** — комплексная оценка волны 0-100 (peakHeight, duration, shape, auc, context) + рекомендации; (7) **Миграции v3→v4** — утилиты `migrateWaveData()`, `enrichWithV4Features()`, `checkVersion()`, `exportWave()`, `importWave()`. **Научное обоснование**: Kahn & Flier 2000 (BMI), DeFronzo 1979 (возраст), Spiegel 1999 (сон), Epel 2001 (стресс), Brand-Miller 2003 (GL).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 3.24.1 | 2025-12-27 | **📚 TEF/TDEE/Supplements в справочнике**: Добавлены секции TEF (Atwater 4/4/9, коэффициенты 25/7.5/1.5, API calculate\*), TDEE (BMR Mifflin-St Jeor, NDTE/TEF/cycle интеграция), Supplements (тайминг, категории, взаимодействия, курсы, bioavailability/science). Связанные файлы дополнены `heys_supplements_v1.js`, `heys_supplements_science_v1.js`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 3.24.0 | 2025-12-22 | **🔧 Yandex Cloud Functions FIX**: Исправлены критические ошибки в `heys-api-rest`: (1) Удалён дублирующий UNIQUE constraint на `client_kv_store` (конфликтовал с PRIMARY KEY), (2) Исправлены env vars — код использует `PG_*` (PG*HOST, PG_PORT...), а не `DB*\*`. REST upsert операции теперь работают корректно. Все API endpoints функционируют штатно                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 3.23.0 | 2025-12-21 | **📊 Analytics Documentation v1.0**: Добавлены 7 новых секций аналитических модулей ранее отсутствовавших в документации: **Status Score** (9 факторов, 4 категории, 5 уровней 0-100), **Gamification** (25 уровней, 30 достижений в 7 категориях, 8 XP-действий, 5 редкостей), **Phenotype** (6 метаболических фенотипов, 5 traits для radar chart, 4 tiers уверенности), **Predictive Insights** (14 научных метрик SCIENCE_INFO с PMID, 5 приоритетов, 9 категорий, 5 actionability), **Metabolic Intelligence** (A/B testing variants, Data Inventory, Phenotype Tiers), **Simple Analytics** (performance tracking), **Data Overview** (10 полей заполненности для куратора). Обновлён список связанных файлов (8→18 файлов). Добавлены API примеры для каждого модуля                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 3.22.0 | 2025-12-17 | **🎨 Extended Analytics UI v2.0 — MAXIMUM DEPTH**: Глубокая интеграция 5 аналитик во ВСЕ UI компоненты экосистемы InsightsTab. **HealthRingsGrid**: Recovery ring показывает emotionalRisk с pulse-анимацией, bingeRisk % в центре, 🔬 PMID ссылка (Epel 2001). **WeeklyWrapCard chart**: Training/stress overlay — 💪😰 индикаторы над барами, цветное выделение, легенда с объяснениями. **Heatmap Widget**: Training/stress бейджи в ячейках 2x2, синяя рамка для тренировки, красная точка для стресса. **CSS**: 150+ строк новых стилей (pulse animations, badges, dark mode). Исправлено дублирование keyframes (heatmapStressPulse)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 3.21.0 | 2025-12-17 | **🎨 Extended Analytics UI Integration v1.0**: Полная визуальная интеграция 5 аналитик v3.20.0 во все UI компоненты. **🔬 PMID Science Links**: Кликабельные ссылки на PubMed во всех местах интеграции. **GoalProgressBar**: Новая секция `goal-context-badges` — бейджи циркадного контекста (🌅/☀️/🌆/🌙), тренировочного дня (💪/🏃/⚽), эмоционального риска (📈 XX%). **TDEE Popup**: Строки NDTE с декеем (🔥 +X%, Y ч назад), BMI category с цветом и tooltip. **Debt Card**: 5 insight-rows с PMID ссылками (белковый долг, эмоц. риск, циркадный контекст, тренировка, BMI). **CSS**: 70+ строк для `.goal-context-badges`, `.context-badge`, dark mode. Научные источники с PMID: Mettler 2010 (20095013), Epel 2001 (11070333), Van Cauter 1997 (9331550), Aragon 2013 (23360586), DeFronzo 1979 (510806), Magkos 2008 (18583478)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 3.20.0 | 2025-12-17 | **🧠 Расширенная аналитика калорийного баланса v3.1.0**: 5 новых аналитик в caloricDebt: **proteinDebt** (отдельный трекинг белка, 4 severity levels), **trainingDayContext** (тип/интенсивность тренировки → приоритет питания), **circadianContext** (утро vs вечер, debtMultiplier 0.7-1.3), **emotionalRisk** (стресс+долг→bingeRisk 0-90%), **bmiContext** (персонализация recoveryMult по BMI). **12 новых советов**: protein*debt*_, training\_\_\_undereating, emotional_risk_high, circadian_\*, bmi_underweight_warning. Научные источники: Mettler 2010, Aragon 2013, Van Cauter 1997, Epel 2001                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 3.19.0 | 2025-12-17 | **📈 Caloric Excess v1.0.0**: Мягкая коррекция перебора — НЕ наказание! **Философия**: Herman & Polivy 1984 (строгие ограничения → срывы) + Tomiyama 2018 (самокритика ухудшает результаты). **Решение**: 70% компенсации через АКТИВНОСТЬ (кардио/шаги), мягкое снижение нормы max 10% как "акцент". **EXCESS_CFG**: `SOFT_REDUCTION_PCT: 0.05`, `MODERATE_REDUCTION_PCT: 0.08`, `MAX_REDUCTION_PCT: 0.10`, `ACTIVITY_PRIORITY: 0.70`. **UI**: позитивный тон ("Профицит" не "Перебор"), зелёная карточка активности "✨ Лучший способ:", "?" с наукой                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 3.18.0 | 2025-12-15 | **🔥 TEF (Thermic Effect of Food) v3.9.0**: Переход на стандартный Atwater (4/4/9 ккал/г) для продуктов. TEF теперь считается как расход в TDEE: `prot×4×0.25 + carbs×4×0.075 + fat×9×0.015`. **UI**: Строка "🔥 Переваривание пищи (TEF)" в popup затрат. Пример: 120г белка + 250г углеводов + 70г жиров → **+204 ккал** к затратам. Научное обоснование: Westerterp 2004, Tappy 1996                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 3.17.0 | 2025-12-15 | **🔬 Инсулиновая волна v3.8.0 — 4 новых научных фактора**: (1) **Smooth Circadian** — синусоидальная кривая вместо 5 ступеней (Van Cauter 1997): 08:00=0.85×, 00:00=1.20×; (2) **Температура пищи** — горячая +8%, холодная −5% (Valdés-Ramos 2019); (3) **Большие порции** — нелинейное замедление 600-1200+ ккал → +5-25% (Collins 1991); (4) **Insulin Index → волна** — молочка: короче волна (−15%), выше пик (+35%) (Holt 1997). **GI threshold GL≥7** (Mayer 1995). **UI**: `getHypoglycemiaWarning()` с полной конфигурацией предупреждений. **37 факторов** в модуле.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| 3.18.0 | 2025-12-15 | **🔥 TEF (Thermic Effect of Food) v3.9.0→v3.9.1**: Переход на стандартный Atwater (4/4/9 ккал/г) для продуктов. TEF теперь считается как расход в TDEE: `prot×4×0.25 + carbs×4×0.075 + fat×9×0.015`. **v3.9.1 FIX**: Разделены `baseExpenditure` (без TEF) и `tdee` (с TEF) — норма (`optimum`) теперь рассчитывается от `baseExpenditure`, чтобы не "догонять" съеденное. TEF синхронизирован в Reports. **UI**: Строка "🔥 Переваривание пищи (TEF)" в popup затрат. Научное обоснование: Westerterp 2004, Tappy 1996                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
