...
> **Версия**: 1.0.0 | **Обновлено**: 2025-01-17 | **Шкала**: 0 (суперполезный) → 10 (супервредный)
...
PENALTIES (штрафы, увеличивают вред):
  ...
  + badFat100 × 0.12    # Насыщенные жиры
  + GI_penalty          # Штраф за высокий ГИ (см. ниже)

BONUSES (бонусы, снижают вред):
  - fiber100 × 0.25     # Клетчатка — мощный протектор
  - protein100 × 0.05   # Белок снижает ГИ и насыщает
  - goodFat100 × 0.02   # Полезные жиры (омега-3, мононенасыщенные)
...
ФИНАЛЬНАЯ НОРМАЛИЗАЦИЯ:
  HarmScore = clamp(raw_score, 0, 10)  # Ограничение 0-10
```
...
| **badFat100** | ×0.12 | Насыщенные жиры повышают LDL, но менее опасны чем транс | Sacks 2017 (PMID: 28620111) |
| **GI** | переменный | Высокий ГИ → быстрый подъём глюкозы → нагрузка на поджелудочную | Brand-Miller 2003 (PMID: 12828192) |
| **fiber100** | −0.25 | Клетчатка замедляет всасывание, снижает ГИ на 15-30%, кормит микробиом | Weickert 2008 (PMID: 18287346) |
| **protein100** | −0.05 | Белок снижает ГИ смешанного приёма, повышает сытость | Nuttall 1984 (PMID: 6389060) |
| **goodFat100** | −0.02 | Мононенасыщенные жиры улучшают липидный профиль | Schwingshackl 2012 (PMID: 22735105) |
...
```javascript
// Получить harm score продукта
const harm = HEYS.products.calculateHarmScore(product);

// Получить категорию и цвет
const { category, color, emoji } = HEYS.products.getHarmCategory(harm);
// { category: 'Вредный', color: '#ef4444', emoji: '🔴' }

// С учётом контекста тренировки
const adjustedHarm = harm * activityContext.harmMultiplier;
```
