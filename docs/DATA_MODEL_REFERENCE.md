...
> Версия: 4.3.0 | Обновлено: 2026-01-18 | **182 умных совета** | **37 факторов
> инсулиновой волны** | **🧠 Insulin Wave v4.1.0** | **🔬 PMID Science Links** |
> **🏋️ Training Context** | **💰 Caloric Debt** | **🔄 Refeed Day** | **📊
> Status Score** | **🎮 Gamification** | **🏭 NOVA Classification** | **🧬 Extended Nutrients**
...
- [📊 HEYS Data Model Reference](#-heys-data-model-reference)
  ...
  - [🔮 Predictive Insights (Прогнозы и паттерны)](#-predictive-insights-прогнозы-и-паттерны)
...
> **Версия**: 3.0.0 | **Обновлено**: 2026-01-18 | **Шкала**: 0 (суперполезный) → 10 (супервредный)
> **Файл**: `heys_harm_v1.js` | **NOVA**: 284 продуктов классифицированы | **Extended Nutrients**: 29 колонок в БД

### Философия оценки

Harm Score — интегральная оценка **метаболической нагрузки** продукта на организм.
Учитывает не только "плохие" компоненты, но и **защитные факторы** (клетчатка, белок, полезные жиры).

**Ключевой принцип**: Продукт с высоким сахаром, но богатый клетчаткой (финики) менее вреден,
чем чистый сахар — клетчатка замедляет всасывание и снижает гликемический удар.

**🆕 v2.0**: Добавлены NOVA classification (переработка), натрий, микронутриентная плотность.
...
**Научное обоснование**: Monteiro 2019 (PMID: 29444892) — ультрапереработанные продукты
связаны с ожирением, диабетом 2 типа, сердечно-сосудистыми заболеваниями.
...
> См. секцию [Training Context](#-training-context-v340) для полной документации.
...
const mealHarm = HEYS.Harm.calculateMealHarm(meal, productIndex, getProductFromItem, activityContext);
...
const { updated, products, stats } = HEYS.Harm.validateAndFixHarmScores(products, { recalculate: false });

// С учётом контекста тренировки
const adjustedHarm = HEYS.Harm.calculateHarmScore(product, { activityMultiplier: 0.7 });
```
...
Дополнительная метрика — оценка полезности продукта по микронутриентам на калорию.
...
Для продуктов с особыми свойствами (ферментированные, сырые, органик) допускается
ручная корректировка ±1.5 балла с обязательным комментарием в поле `harmNote`.
...
> **Legacy alias**: `harmScore` (устаревший, поддерживается для обратной совместимости)  
...
## 🔮 Predictive Insights (Прогнозы и паттерны)

**Файл**: `heys_predictive_insights_v1.js` | **Версия**: 1.0.0 | **Строк**:
~10,100

### Научные метрики (SCIENCE_INFO)

| Метрика               | PMID     | Категория  | Приоритет | impactScore |
| --------------------- | -------- | ---------- | --------- | ----------- |
| `TEF`                 | —        | METABOLISM | HIGH      | 0.75        |
| `EPOC`                | —        | METABOLISM | MEDIUM    | 0.50        |
| `HORMONES`            | —        | RECOVERY   | CRITICAL  | 0.95        |
| `ADAPTIVE`            | —        | METABOLISM | HIGH      | 0.80        |
| `CIRCADIAN`           | 9331550  | TIMING     | HIGH      | 0.85        |
| `NUTRIENT_TIMING`     | 23360586 | TIMING     | HIGH      | 0.70        |
| `INSULIN_SENSITIVITY` | 510806   | METABOLISM | HIGH      | 0.80        |
| `GUT_HEALTH`          | —        | NUTRITION  | MEDIUM    | 0.50        |
| `STATUS_SCORE`        | —        | COMPOSITE  | CRITICAL  | 1.00        |
| `CRASH_RISK_QUICK`    | —        | RISK       | CRITICAL  | 0.95        |
| `HEALTH_SCORE`        | —        | COMPOSITE  | HIGH      | 0.85        |
| `CORRELATION`         | —        | STATISTICS | INFO      | 0.20        |
| `WEIGHT_PREDICTION`   | —        | PREDICTION | HIGH      | 0.75        |
| `PATTERNS`            | —        | PATTERNS   | HIGH      | 0.70        |

### Приоритеты (PRIORITY_LEVELS)

| Приоритет  | impactScore | Когда используется           |
| ---------- | ----------- | ---------------------------- |
| `CRITICAL` | 0.90-1.00   | Требует немедленных действий |
| `HIGH`     | 0.70-0.85   | Важно для результата         |
| `MEDIUM`   | 0.45-0.55   | Полезно знать                |
| `LOW`      | 0.20-0.40   | Справочная информация        |
...
| Категория    | Описание                                 |
| ------------ | ---------------------------------------- |
| `METABOLISM` | Метаболизм (TEF, EPOC, ADAPTIVE)         |
| `NUTRITION`  | Питание (GUT_HEALTH)                     |
| `TIMING`     | Тайминг (CIRCADIAN, NUTRIENT_TIMING)     |
| `RECOVERY`   | Восстановление (HORMONES)                |
| `RISK`       | Риски (CRASH_RISK_QUICK)                 |
| `PREDICTION` | Прогнозы (WEIGHT_PREDICTION)             |
| `PATTERNS`   | Паттерны                                 |
| `COMPOSITE`  | Композитные (STATUS_SCORE, HEALTH_SCORE) |
| `STATISTICS` | Статистика (CORRELATION)                 |

### Actionability (срочность действия)
...
| `LONG_TERM`     | Долгосрочная работа |
| `INFORMATIONAL` | Для информации      |

### API

```javascript
HEYS.PredictiveInsights.analyze(historyDays, profile, norms);
// Результат: { insights, patterns, predictions, risks }

HEYS.PredictiveInsights.getInsightsByPriority(insights, 'CRITICAL');
HEYS.PredictiveInsights.getInsightsByCategory(insights, 'METABOLISM');
```
