...
> Версия: 3.17.0 | Обновлено: 2025-12-15 | **170 умных советов** | **37 факторов
> инсулиновой волны** | **🏋️ Training Context** | **💰 Caloric Debt** | **🔄
> Refeed Day** | **🆕 Smooth Circadian + Temperature + Large Portions**
...
> ⚠️ **Важно v3.9.1**: `optimum` рассчитывается от `baseExpenditure` (без TEF), чтобы норма не "догоняла" съеденное. TEF показывается в UI как фактические затраты на переваривание.
...
**UI**: В popup "⚡ Затраты энергии" отображается строка "🔥 Переваривание пищи (TEF)".
...
**🆕 v3.7.7**: Расчёт ккал теперь через **MET-формулу** по зонам пульса, а не грубую оценку!
- Формула: `ккал/мин = MET × 3.5 × вес(кг) / 200` (Ainsworth 2011)
- Было: `durationMin × 5 × (weight/70)` → 60 мин = 300 ккал (занижено!)
- Стало: `zones.reduce((sum, min, i) => sum + min × kcalPerMin(mets[i], weight), 0)` → реальные 700+ ккал
