...
5. 🔍 **Проактивные улучшения** — ОБЯЗАТЕЛЬНО предлагай (см. секцию ниже)
6. 🚨 **НИКОГДА НЕ ОТКАТЫВАЙ ФАЙЛЫ** без явного согласия!

---

## 🔍 Проактивные улучшения (ОБЯЗАТЕЛЬНАЯ секция)

**После КАЖДОЙ выполненной задачи** добавляй в конце ответа:
...
### ⚡ Quick Wins (можно сделать за 5 мин)

- [ ] **Что**: краткое описание
  - **Где**: `path/to/file.js:123`
  - **Почему**: польза для UX/DX/производительности

### 🎯 Стратегические улучшения (требуют планирования)

- [ ] **Что**: краткое описание
  - **Влияние**: на какие модули/фичи
  - **Оценка**: ~X часов
  - **Риски**: что может сломаться

### 🐛 Потенциальные баги (заметил при чтении кода)

- [ ] **Где**: `file.js:line` — описание проблемы
```

### Правила генерации улучшений

| Тип               | Когда добавлять                                         | Пример                                    |
| ----------------- | ------------------------------------------------------- | ----------------------------------------- |
| ⚡ Quick Win      | Увидел дубликат кода, устаревший TODO, опечатку         | "Дублируется логика валидации в 3 файлах" |
| 🎯 Стратегическое | Увидел архитектурную проблему, возможность рефакторинга | "Модуль X можно вынести в shared"         |
| 🐛 Баг            | Увидел edge case, race condition, неправильную логику   | "При пустом массиве будет NaN"            |

### Когда НЕ добавлять

- ❌ Задача была чисто информационная (вопрос-ответ)
- ❌ Улучшения не связаны с контекстом работы
- ❌ Уже есть в `todo.md` или известная проблема
...
- [ ] **Удалить мёртвый код**: `heys_day_v12.js:2847` — закомментированная
      функция `oldCalculateWave()` (50 строк)
- [ ] **Типизация**: добавить `@typedef` для `MealQualityScore` — IDE подсказки
      станут точнее

### 🎯 Стратегические

- [ ] **Вынести расчёт TEF в отдельный модуль**
  - Сейчас дублируется в `heys_day_v12.js` и `heys_reports_v12.js`
  - ~2 часа, низкий риск

### 🐛 Потенциальные баги

- [ ] `heys_advice_v1.js:156` — `waterPct` может быть `Infinity` если
      `waterGoal === 0`
```

### Триггер-фразы для AI

Если пользователь хочет **отключить** улучшения для конкретной задачи:

- "без улучшений"
- "только задача"
- "skip insights"

Если хочет **больше деталей** по улучшению:

- "расскажи подробнее про [название]"
- "сделай это улучшение"
