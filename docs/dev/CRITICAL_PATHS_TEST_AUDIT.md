...
| PWA update / cache invalidation                             | Пользователь держит stale bundle или stale SW cache                                        | Добавлены checks на stale path: mismatch `manifest boot hash` vs loaded asset и `SW cache version` mismatch + expected hard reload action                                                                                                                   | **Partial** | `apps/web/__tests__/pwa-update-logic.test.js`, `apps/web/index.html`, `apps/web/public/sw.js`, `apps/web/public/bundle-manifest.json`                                                                                                                            |
...
3. `pwa-update-logic`: уточнить coverage на cache-stale path после смены hashed
   bundle и обновления `bundle-manifest.json`.
