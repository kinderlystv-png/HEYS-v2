...
| Параметр          | Значение                           |
| ----------------- | ---------------------------------- |
| **API ключ**      | хранится во внешнем secret manager |
| **Баланс**        | 548.96 ₽                           |
...
HEYS.sms.init({ apiKey: '<SMSRU_API_KEY>' });

// Вариант 2: Через Yandex Lockbox / runtime env (рекомендуется)
// SMSRU_API_KEY=<SMSRU_API_KEY>

// Затем в Yandex Cloud Function:
HEYS.sms.init({ apiKey: process.env.SMSRU_API_KEY });

// Вариант 3: Через локальный env для dev/staging
// VITE_SMSRU_API_KEY=<SMSRU_API_KEY>
```
...
| Yandex Lockbox / YC env | ✅ Хорошо    | Для продакшена |
