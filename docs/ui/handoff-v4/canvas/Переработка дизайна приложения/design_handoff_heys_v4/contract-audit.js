...
(function () {
  ...
  function findings() {
    ...
    // Маркеры ищутся БЕЗ УЧЁТА РЕГИСТРА (флаг i, 15 сентября). Прежде регулярка знала только
    // «Решение», а соглашение проекта пишет голову строки заглавными — «РЕШЕНИЕ 5 сентября:»,
    // «ЗАКРЫТО», «ПРИВЕДЕНО». Такую голову проверка не видела вовсе: первым совпадением
    // становилось вложенное «Решение 31 августа» в теле строки, то есть проверка ругалась на
    // строку с ВЕРНЫМ порядком и молчала на пяти строках с заглавными маркерами — не потому,
    // что порядок верен, а потому, что маркера не видела. Тот же класс, что «зелёная проверка
    // обязана отличать „сошлось“ от „не смотрели“». Список маркеров пополнен формами, которые
    // соглашение уже использует: ЗАКРЫТО, ПЕРЕСНЯТО, ИСПРАВЛЕНО, ДОБАВЛЕНО, ОТВЕТ,
    // ПОДТВЕРЖДЕНО, ПРОВЕРЕНО, УСЛОВИЕ.
    var LATE = /(?:Правк[аи]|Решени[ея]|Решено|Уточнение|Дополнение|Приведено|Сведено|Снято|Изменено|Закрыто|Переснято|Исправлено|Добавлено|Ответ|Подтверждено|Проверено|Условие)[\s\S]{0,90}?(?:\d{1,2}\s+(?:августа|сентября)|по срезу|владельца)/i;
    var late = [];
