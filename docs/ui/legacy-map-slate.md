...
## 6. Решение владельца, 3 сентября

**F1, F2, F4, F5, F6, F7 — вариант A. F3 отложена, а 3 сентября вечером закрыта
дизайнером вариантом B — см. §7.**

Шесть из семи решением по существу не были: все они устроены одинаково — одна
легаси-ступень служит и текстом, и хромом, — а правило «роли заливки текст не
красят» это уже разделяет. Вариант A применяет правило, B и C либо перекрашивают
экраны ради удобства миграции, либо ломают гейт ролей. Роли, на которые A
опирается, проверены: `--v4-hero`, `--v4-edge`, `--v4-track`, `--v4-c1`
объявлены во всех четырёх наборах.

**F3 отложена, потому что её вариант A нерабочий.** Он ссылается на
`--v4-on-hero`, а такой роли нет ни в одном наборе — проверено. Текст на тёмной
панели (24 места) остаётся на легаси-ступени, пока не решено: заводить роль
«чернила на герое» в палитре или осветлять панели до `--v4-c1`. Первое — правка
файла ролей и дизайнерская территория, второе меняет вид графиков. Ни то ни
другое не делается попутно с миграцией.

**Что это разблокирует:** 486 мест scope минус 67 мест `slate-100` = 419 к
механической миграции, разложенных по восьми файлам (§1.3).

## 7. F3 закрыта дизайнером, 3 сентября (вечер) — вариант B

**Панели осветляются до `--v4-c1`, роли «чернила на герое» не будет.** Дословно:
«путь 2, и это не решение о виде — это вид, который уже принят». Тёмной панели
графика в v4 нет ни в одном кадре: «вид · оси фенотипа», «вид · ярусы порогов»,
«вид · таблица порогов» в `reports-insights`, блок `.cd` на `--c1` радиусом 20.
`--c1` идёт за набором (песочный `#f7efe2`, тёмный `#23201b`, синий `#eef3f9`,
сине-тёмный `#182a3a`), и обычные чернила верны в каждом: главное `--tx`,
вторичное — 56 %.

Опасение «B меняет вид графиков», из-за которого развилку откладывали, дизайнер
снял прямо: тёмные панели «Метрик» — поверхность прежней системы, которую v4
никогда не рисовал, и «это и есть та самая перекраска, ради которой всё
затевалось».

**Оговорка на будущее.** Если найдётся кадр, где панель тёмная **по замыслу**,
роль называется по уже принятому в `v4-canvas.css` правилу пар — заливка и
`--on-<заливка>`, как `--fab/--on-fab`, `--acs/--on-acs`, `--wat/--on-wat`,
`--gr/--on-gr`. Новое имя не изобретается.

**Сделано в тот же день (c1491af24):** `100-metrics-and-graphs.css`, 22 места.
Одной ступенью они не заменяются — разложены по назначению: четыре текста на
тёмной панели → `--v4-ink`, тринадцать фишек и кнопок → `--v4-chip`, разделитель
в 1 px и две отбивки → `--v4-line`, пустой день теплокарты → `--v4-track`,
градиент блока корреляции → `--v4-c1`/`--v4-chip`. Все роли объявлены во всех
четырёх наборах (проверено чтением палитры), оба гейта ролей зелёные.

**Остаток F3:** 45 мест в восьми файлах, из них `500-pwa-and-offline` 14,
`300-modals-and-day` 13, `000-base-and-gamification` 12, `612-training-step` 8,
`610-aps-meal-flow` 3, `200-dark-and-effects` 3, `600-steps-and-aps` 2,
`tailwind.css` 1 (объявление токена — уходит последним).

---

**Чего это НЕ разблокирует.** Политика на остальные четыре карты не переносится,
и обещание разнести её было ошибкой: развилки там другого рода. У `gray` спор о
ступени чернил (56 против 45 против 38 %), а не о тексте против хрома. У `blue`
— о смысле акцента: вода, действие, декоративный уровень редкости или ссылка. У
`red-pink` карта уже предлагает ответ по каждой развилке с обоснованием — там
нужен не выбор, а подтверждение.
