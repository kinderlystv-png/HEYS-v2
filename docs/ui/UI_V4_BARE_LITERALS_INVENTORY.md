...
| ----------------------------- | ---: |
| только hex                    | 1339 |
| hex + непрозрачный `rgb()`    | 1346 |
| hex + `rgba()` с альфой ≥ 0,5 | 1502 |
| все, включая полупрозрачные   | 2238 |

В описи — последняя строка, **2238**.

## Охват

Просмотрены все модули стилей (48) и файлы кода, названные обоснованиями
вердиктов закрытых зон (30). Закрытых зон 7; незакрытой считается та, где ещё
стоят «?» — сейчас это `app-splash`, `curator-cabinet`, `cycle`,
`date-remainders`, `food-meal`, `gamification`, `home-widgets`, `login`,
`norm-correction`, `nutrition-tab`, `product-card`, `questionnaire`,
`registration`, `reports-insights`, `settings-system`, `strength-builder`,
`tips`, `water-add`. Всего голых литералов в этих файлах 15031; тёплых 2238.
Остальные палитре тоже не следуют, но песочными экран не делают, и это отдельный
разговор.
...
ни одной синей — такое место переводится на роль без разговора. Их 17.

Всего мест: **2238** в 40 файлах.
