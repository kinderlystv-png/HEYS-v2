...
**Разметки «место за местом» не будет — решение дизайнера 4 сентября.** Она
неверна по существу: размечать надо **значение, а не место**. 2294 тёплых места
это три разных множества, склеенных признаком «тёплый», и знак «герой» применим
только к одному из них. Прежняя инструкция «пометьте `герой` у семейств» здесь
не действует; вместо неё — три ведра ниже.

Причина отказа не в объёме: строки, написанные по памяти о кадрах, всегда
покрывают часть — за проход вспоминается десяток элементов, а их тысяча.
Разметка 2294 мест глазами это тот же провал, только дороже: она **будет
выглядеть сделанной**. Правило по значению проверяется счётом и повторяется
командой.

## Три ведра — счёт от 4 сентября

Считается командой `node scripts/ui-v4-list-bare-literals.mjs --buckets`:
правило и таблица ролей живут в скрипте, поэтому числа можно повторить.
Старшинство ведра — 1, затем 2, затем 3; место попадает ровно в одно.
...
**Ожидание, что ведро 1 — основной объём, не подтвердилось.** Оно 13 %; основное
— янтарная лестница, которой в песочном наборе нет ни одного тона.

- **Ведро 1** чинится механически и без дизайнера: замена на роль, «герой» не
  ставится нигде. Сравнение идёт с полной таблицей ролей (четыре набора
  продуктовой палитры плюс `v4-canvas.css`), а не только с песочной, — поэтому
  мест больше, чем звёздочек в таблицах ниже.
- **Ведро 2** решается **зоной, а не тоном**: помечать эти тона намеренными
  значило бы легализовать палитру, которую v4 не принимал. Экран, где они стоят,
  перекраску не проходил.
- **Ведро 3** то же, но опаснее: выглядит ролью и потому переживает обе описи.
  Не переименовывать в роль — убирать вместе с зоной, иначе выйдет роль без
  набора. Тёплых таких мест 32 (в прозе выше 139 — это все места со своей
  переменной, без отбора по тёплому).
- **Вне трёх вёдер 290**, из них **43** — тон роли при другой прозрачности.
  Механически они не заменяются: роль такой альфы не даёт.

**Поправка того же дня: ведро 1 сначала было насчитано 383.** В таблицу ролей
попали роли **с именем набора** (`--v4-sand-*`), а они предмет первой описи, а
не роли набора: держат песочное значение и в синих. Из-за этого ведро 1 забрало
себе 80 мест `#fef3c7` — это `--v4-sand-warn-bg` и одновременно тон янтарной
лестницы, которого в песочном наборе нет ни одного. Проверено поимённо: из 27
тонов лестницы ролью объявлен только этот, остальные 26 не объявлены нигде. Роли
с именем набора из таблицы исключены.
...
приёма, снято тем же решением: она переходит с `--v4-sand-act` на `--v4-act`.
Два исключения по одному признаку — это уже не исключение, а вторая система.

### Ведро 2: где оно лежит

Список зон по нему дать нельзя — **1131 место из 1673 стоит в файлах, которых не
назвал ни один вердикт**, и «зона» для них пуста по построению. Единица решения
там — файл:

| мест | файл                             | зоны вердиктов                                  |
| ---: | -------------------------------- | ----------------------------------------------- |
|  201 | `100-metrics-and-graphs.css`     | —                                               |
|  190 | `heys-components.css`            | checkin-morning, pwa-update, spinners, undo-bar |
|  180 | `000-base-and-gamification.css`  | —                                               |
|  163 | `fingers.css`                    | —                                               |
|  141 | `720-predictive-insights.css`    | —                                               |
|  101 | `200-dark-and-effects.css`       | —                                               |
|   84 | `710-refeed.css`                 | —                                               |
|   76 | `730-widgets-dashboard.css`      | spinners                                        |
|   65 | `725-metabolic-intelligence.css` | —                                               |
|   65 | `900-planning.css`               | —                                               |

Полный список — в выводе `--buckets`. Первые четыре файла держат 734 места из
1673, то есть почти половину. `fingers.css` вместе с `drums-finger-trainer.css`
(163 + 23) — это целый продуктовый модуль, который перекраску v4 не проходил
вовсе; зоной вердиктов он не описан, поэтому в списке зон его нет и быть не
могло.
...
8. Мёртвый цвет вычитается: значение в таблице данных, ключ которой никто не
   читает, до экрана не доходит вовсе. Вычтено 3 — разбор в
...
| ----------------------------- | ---: |
| только hex                    | 1334 |
| hex + непрозрачный `rgb()`    | 1341 |
| hex + `rgba()` с альфой ≥ 0,5 | 1495 |
| все, включая полупрозрачные   | 2207 |

В описи — последняя строка, **2207**.

## Охват

Просмотрены все модули стилей (46) и файлы кода, названные обоснованиями
вердиктов закрытых зон (22). Закрытых зон 5; незакрытой считается та, где ещё
стоят «?» — сейчас это `app-splash`, `checkin-morning`, `curator-cabinet`,
`cycle`, `date-remainders`, `food-meal`, `gamification`, `home-widgets`,
`login`, `norm-correction`, `nutrition-tab`, `product-card`, `questionnaire`,
`registration`, `reports-insights`, `settings-system`, `strength-builder`,
`tab-activity`, `tips`, `water-add`. Всего голых литералов в этих файлах 14652;
тёплых 2207. Остальные палитре тоже не следуют, но песочными экран не делают, и
...
ни одной синей — такое место переводится на роль без разговора. Их 24.

Всего мест: **2207** в 38 файлах.
...
Зоны: `pwa-update`, `spinners`, `undo-bar`
...
### `400-water-and-hydration.css` — 56

Зоны: `service-curator`
...
Зоны: `curator-edits`, `service-curator`, `spinners`
...
### `733-ui-v4-login-theme.css` — 59

| семейство                                | мест | литералы                                                 |
| ---------------------------------------- | ---: | -------------------------------------------------------- |
| `.heys-auth-card`                        |   10 | `#efe3cf`, `#23201b`, `#c67139`, `#2b1608`               |
