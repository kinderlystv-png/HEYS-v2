...
Всего мест: **581** в 13 модулях, 256 семействах — счёт на день составления.

**Счёт стареет быстрее, чем документ.** За 31 августа роли переводили несколько
сессий сразу, и общее число выше фактического. Пересчитан только
`733-ui-v4-reports.css` (15 → 10, таблица пересобрана по файлу). Остальные
разделы не сверялись — читать их как верхнюю границу, а не как точное число.
Живой счёт по всем модулям печатает `pnpm ui:v4:check` строкой «Ролей с именем
набора в модулях».
