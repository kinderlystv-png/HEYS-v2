...
Всего мест: **518** в 16 модулях — пересчитано 7 сентября. Было **469** (3
сентября); **+49** за две недели:

| причина                                          |         мест | что делать                                               |
| ------------------------------------------------ | -----------: | -------------------------------------------------------- |
| новые `--v4-sand-*` в коде                       |      **+59** | размечать по описи; три файла в опись не входили         |
| перевод на общие роли (`--v4-act`, `--v4-ink` …) |      **−14** | уже сделано — в таблицах могут торчать ушедшие семейства |
| шапка описи не обновлялась                       | **+49 итог** | этот пересчёт                                            |

Детализация **+59**: новый `613-cycle-ui.css` **+24**, рост
`000-base-and-gamification.css` **+26** (профиль/геймификация), плюс
`300-modals-and-day.css` **+2**, `610-aps-meal-flow.css` **+2**,
`733-ui-v4-login-theme.css` **+2**, `730-widgets-dashboard.css` **+1**,
`400-water-and-hydration.css` **+1**, `100-metrics-and-graphs.css` **+1**.
Детализация **−14**: `732-ui-v4-nutrition.css` **−5**,
`715-yesterday-verify.css` **−5**, `500-pwa-and-offline.css` **−2**,
`600-steps-and-aps.css` **−1**, `611-aps-product-card.css` **−1**.

**Как это посчитано** (чтобы число повторялось, а не спорилось). Считаются
вхождения `var(--v4-sand-*)` и `var(--v4-blue-*)` в файлах
`apps/web/styles/modules/*.css` (сейчас blue = 0). Файл палитры
`002-ui-v4-palette-roles.css` **не в счёте**. Одно место — одно вхождение
`var()`, а не одно правило и не одно семейство.

Тем же способом считает `pnpm ui:v4:check` строкой «Ролей с именем набора в
модулях» (`scripts/ui-v4-check-foreign-fallbacks.mjs`, `scanPaletteRoles`).
Независимый пересчёт 7 сентября: **518** — сходится с гейтом.

**Итог пересчитан, таблицы частично.** У заголовка каждого файла — живой счёт;
где таблица старее, верен заголовок. Три новых файла (`613-cycle-ui`,
`300-modals-and-day`, `100-metrics-and-graphs`) — таблица по файлу на день
пересчёта; остальные разделы ещё несут семейства, часть из которых уже ушла в
общие роли.

**Мёртвый цвет эту опись не задевает.** Свод 37 мёртвых значений
(`UI_V4_DEAD_COLORS.md`, разбор `heys-v2-62`) даёт с ней **ноль пересечений**:
она про роли в модулях стилей, а мёртвый цвет весь в коде. Вычитать перед
разметкой нечего.

## `610-aps-meal-flow.css` — 92 (в таблице ниже — 158 на день составления)
...
## `500-pwa-and-offline.css` — 96 (в таблице ниже — 100 на день составления)
...
## `000-base-and-gamification.css` — 122 (в таблице ниже — 96 на день составления; +26 профиль/геймификация)
...
## `600-steps-and-aps.css` — 34 (в таблице ниже — 49 на день составления)
...
## `611-aps-product-card.css` — 30 (в таблице ниже — 38 на день составления)
...
**Пересчитано 31 августа вечером: 15 → 10.** Пять строк описи были стары.
`.reports-v4-zero-actions` и `.reports-v4-period-pill` ушли вместе с самими
правилами (первое — удалённый компонент, второе — перебранный чип). Осталось
десять мест в девяти семействах; таблица ниже пересобрана по файлу, а не
дописана.

**Одно место переведено вне общей задачи, и вот почему.**
`.heys-score-screen__tier` (ярус «Из чего сложился каскад») стоял на
`--v4-sand-act-mid` со значением `#a75c2b`. Это не случай «песочная роль равна
общей, но не следует набору»: `#a75c2b` не совпадает ни с `--v4-act-text`
(`#8a4a20`), ни с чем, что называет канвас. То есть цвет был неверен и в
песочном наборе тоже — а канвасный шаблон `.tier` прямо задаёт `--ac`, и он
равен `--v4-act-text` во всех четырёх наборах. Это починка одного элемента по
кадру, а не опережение разметки; остальные места ждут решения владельца, как и
записано в `733-ui-v4-reports.css` у `.reports-v4-days__more`.
...
| `.reports-v4-wellbeing`     | 1    | `--v4-sand-surface`                       |
| `.reports-v4-summary-card`  | 1    | `--v4-sand-ink`                           |
| `.reports-v4-stub__count`   | 1    | `--v4-sand-act-deep`                      |
| `.reports-v4-score-slot`    | 1    | `--v4-sand-hero`                          |

## `734-ui-v4-insights.css` — 3 (в таблице ниже — 14 на день составления)
...
## `715-yesterday-verify.css` — 5 (в таблице ниже — 10; пять мест ушли на общие роли)
...
## `732-ui-v4-nutrition.css` — 1 (в таблице ниже — 6; пять мест ушли на общие роли)
...
**Новый файл** (цикл / карточка цикла). Таблица пересобрана 7 сентября по файлу;
все места — `--v4-sand-act-deep` и `--v4-sand-act-soft`.

| семейство                      | мест | роли                                       |
| ------------------------------ | ---: | ------------------------------------------ |
| `.mc-cycle-status-info`        |    2 | `--v4-sand-act-deep`, `--v4-sand-act-soft` |
| `.mc-cycle-auto-hint`          |    2 | `--v4-sand-act-deep`, `--v4-sand-act-soft` |
| `.cycle-card__empty-hint`      |    2 | `--v4-sand-act-deep`, `--v4-sand-act-soft` |
| `.cycle-card__day-btn`         |    2 | `--v4-sand-act-deep`, `--v4-sand-act-soft` |
| `.mc-cycle-toggle`             |    1 | `--v4-sand-act-soft`                       |
| `.mc-cycle-day-label`          |    1 | `--v4-sand-act-soft`                       |
| `.mc-cycle-day-btn`            |    1 | `--v4-sand-act-soft`                       |
| `.mc-cycle-input`              |    1 | `--v4-sand-act-soft`                       |
| `.mc-cycle-hint-text`          |    1 | `--v4-sand-act-soft`                       |
| `.mc-cycle-option-day`         |    1 | `--v4-sand-act-soft`                       |
| `.mc-cycle-change-btn`         |    1 | `--v4-sand-act-soft`                       |
| `.mc-cycle-option-label`       |    1 | `--v4-sand-act-soft`                       |
| `.mc-cycle-option-hint`        |    1 | `--v4-sand-act-soft`                       |
| `.mc-cycle-exact-label`        |    1 | `--v4-sand-act-soft`                       |
| `.mc-cycle-exact-btn`          |    1 | `--v4-sand-act-soft`                       |
| `.cycle-card__day`             |    1 | `--v4-sand-act-deep`                       |
| `.cycle-card`                  |    1 | `--v4-sand-act-deep`                       |
| `.cycle-card__badge`           |    1 | `--v4-sand-act-soft`                       |
| `.cycle-card__day-btn--active` |    1 | `--v4-sand-act-deep`                       |

## `300-modals-and-day.css` — 2

| семейство                   | мест | роли                |
| --------------------------- | ---: | ------------------- |
| `.ma-followup-note-title`   |    1 | `--v4-sand-ok-text` |
| `.ma-followup-answer--done` |    1 | `--v4-sand-ok-text` |

## `100-metrics-and-graphs.css` — 1

| семейство      | мест | роли                  |
| -------------- | ---: | --------------------- |
| `.goal-marker` |    1 | `--v4-sand-ink-slate` |
