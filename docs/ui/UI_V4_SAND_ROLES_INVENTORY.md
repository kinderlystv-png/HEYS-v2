...
Всего мест: **469** в 13 модулях — пересчитано 3 сентября. Было 463: шесть мест
прибавили пустой день Главной и состояния тренда — новые правила взяли роли
активного набора (`--v4-sand-ok-*`, `--v4-sand-act`) вместо литералов. До того
было 471: восемь мест ушли при сведении затронутых зон на общие роли.

**Как это посчитано** (чтобы число повторялось, а не спорилось). Считаются
вхождения `var(--v4-sand-*)` в файлах `apps/web/styles/modules/*.css`. Файл
самой палитры `002-ui-v4-palette-roles.css` **не в счёте**: там роли
объявляются, а не используются, и его 17 объявлений — это не долг. Комментарии
не выбрасываются: упоминаний роли в них нет. Одно место — одно вхождение, а не
одно правило и не одно семейство.

Тем же способом считает `pnpm ui:v4:check` строкой «Ролей с именем набора в
модулях» — два независимых счёта сходились на 471, потом на 463, сейчас на 469.

**Итог пересчитан, разделы — нет.** Шапка выше говорит правду на текущий срез.
Таблицы по файлам собраны днём, когда мест было 581, и с тех пор несколько
сессий переводили роли на общие — **числа внутри разделов читать как верхнюю
границу**. Ниже, у заголовка каждого файла, стоит его живой счёт; где он
расходится с суммой строк таблицы, верен живой.
...
## `610-aps-meal-flow.css` — 90 (в таблице ниже — 158 на день составления)
...
## `500-pwa-and-offline.css` — 98 (в таблице ниже — 100 на день составления)
...
## `600-steps-and-aps.css` — 35 (в таблице ниже — 49 на день составления)
...
## `611-aps-product-card.css` — 31 (в таблице ниже — 38 на день составления)
