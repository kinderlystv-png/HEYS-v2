...
> Документация модуля `heys_cascade_card_v1.js` v3.0.0  
...
> Планируемая версия: v3.0.0 | Статус: design
...
Новая модель (v3.0.0 CRS):
  day → computeCascadeState() → dailyScore → DCS (Daily Contribution Score)
                                               ↓
  30 дней DCS → EMA (α=0.92) → CRS (0..1) → state/progress
                                  ↓
...
function computeDailyContribution(dailyScore, day, normAbs) {
...
### 2. Вычисление CRS (Exponential Moving Average)

**30-дневное окно** с экспоненциальным затуханием (decay α = 0.92):

```
weights: today=1.0, вчера=0.92, 2 дня назад=0.846, ..., 29 дней=0.92²⁹ ≈ 0.08
crs_raw = Σ(weights[i] × dcs[i]) / Σ(weights[i] для дней с данными)
crs = clamp(crs_raw, 0, ceiling)
```
...
| :------------ | :---- | :------------------------------ |
| Сегодня       | 1.000 | 100% — полный вклад             |
| Вчера         | 0.920 | 92% — почти полный              |
| 3 дня назад   | 0.779 | 78% — существенный              |
| 7 дней назад  | 0.558 | 56% — заметный                  |
| 14 дней назад | 0.312 | 31% — фоновый                   |
| 21 день назад | 0.174 | 17% — слабый                    |
| 30 дней назад | 0.082 | 8% — минимальный, скоро выпадет |
...
Сегодня DCS: clamp(-1.84/12, -0.3, 1.0) = -0.15 (ограничен!)

CRS ≈ weighted_avg ≈ 0.68  (вместо BROKEN w/ score=-1.84)
...
- **Цветовая схема (Neon):**
  - Левая часть (от 0 до текущего CRS): **Динамический зелёный**. Чем больше
    значение CRS, тем ярче и зеленее становится начало линии слева (меняется
    прозрачность от 0.1 до 1.0), а также усиливается неоновое свечение.
  - Правая часть (от текущего CRS до 100%): **Динамический цвет**. Если CRS ≤
    50% (много плохой линии), она ярко-красная. Чем выше CRS (чем меньше плохой
    линии), тем больше она плавно переходит в жёлтый/янтарный цвет.
...
5. **CSS:** Классы `.crs-bar-container`, `.crs-bar-green`, `.crs-bar-orange`,
   ...
   `safe-area-inset-bottom`. | **base** | `0.65` | 0.65 | Все начинают здесь | |
   **consistency** | `1 + clamp((1 - CV(dailyScores)) × 0.3, 0, 0.3)` | 1.0 –
   1.3 | Низкая вариативность → выше | | **diversity** |
   `1 + activatedFactorTypes / 10 × 0.15` | 1.0 – 1.15 | Больше типов факторов →
   выше | | **dataDepth** | `+0.03 × полных_недель` (до 4 недель) | 0 – 0.12 |
   Каждая неделя +3% |
...
```
Ключ: heys_{clientId}_cascade_dcs_v1
...
- Для дней без кэшированного DCS — ретроспективно вычисляется из `dayv2` данных
  (упрощённый скоринг: еда + сон + тренировки + чекин)
...
### 6. Прогресс-бар: CRS вместо dailyScore
...
```js
{
  ...
  daysAtPeak:           number         // дней подряд на потолке
}
```

### 9. UI компонента: обновления
...
console.info('[HEYS.cascade.crs] ✅ Cascade Rate Score:', {
  ...
  daysWithData: 18,
  daysAtPeak: 3,
  model: 'EMA α=0.92, 30-day window',
...
│  dcs = clamp(dailyScore / 12.0, -0.3, 1.0)                     │
│  Critical override: night_harm → -0.8, excess → -0.6, both → -1│
├─────────────────────────────────────────────────────────────────┤
│  CASCADE RATE SCORE (CRS)                                       │
│  weights[i] = 0.92^i  (i = 0..29, today = 0)                   │
│  crs_raw = Σ(w[i]×dcs[i]) / Σ(w[i] for days with data)         │
│  crs = clamp(crs_raw, 0, ceiling)                               │
├─────────────────────────────────────────────────────────────────┤
│  CEILING (индивидуальный потолок)                               │
...
| v3.0.0 | TBD        | **Cascade Rate Score (CRS) v1.0 — Кумулятивный импульс.** 30-дневная EMA (α=0.92) заменяет однодневный score для определения состояния и прогресс-бара. Индивидуальный потолок (ceiling) = f(consistency, diversity, dataDepth). DCS (Daily Contribution Score) с защитой инерции (clamp −0.3) и Critical Violation Override (ночное переедание/harm). CRS-driven пороги: STRONG≥0.75, GROWING≥0.45, BUILDING≥0.20. Персистенция DCS в localStorage.                              |
