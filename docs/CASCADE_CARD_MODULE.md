...
Новая модель (v3.0.0 CRS):
  day → computeCascadeState() → dailyScore → DCS (Daily Contribution Score)
                                               ↓
  30 дней DCS → EMA (α=0.95) → CRS (0..1) → state/progress  // v3.3.0: α повышен
                                  ↓
...
**30-дневное окно** с экспоненциальным затуханием (decay α = 0.95, v3.3.0):

```
weights: today=1.0, вчера=0.95, 2 дня назад=0.903, ..., 29 дней=0.95²⁹ ≈ 0.23
crs_raw = Σ(weights[i] × dcs[i]) / Σ(weights[i] для дней с данными)
crs = clamp(crs_raw, 0, ceiling)

v3.3.0 — Partial-day weighting:
  Если i=0 (сегодня) и день ещё не завершён:
  todayProgress = max(0.2, min(1.0, minutesSinceMidnight / 1440))
  weights[0] = 1.0 × todayProgress
  // В 12:00 дня → вес 0.5, в 18:00 → 0.75, в 23:00 → 0.96
```

**Свойства α = 0.95 (v3.3.0, было 0.92):**

| Давность      | Вес   | Влияние                              |
| :------------ | :---- | :----------------------------------- |
| Сегодня       | 1.000 | 100% — полный вклад (× dayProgress)  |
| Вчера         | 0.950 | 95% — почти полный                   |
| 3 дня назад   | 0.903 | 90% — существенный                   |
...
> **v3.3.0**: α повышен с 0.92 до 0.95. Период полураспада: ~14 дней (было ~8).
> Один плохой день среди 14 хороших не обрушит CRS. Более стабильная оценка.
...
Сегодня DCS: clamp(-1.84/10, -0.3, 1.0) = -0.18 (защита инерции −0.3)
...
console.info('[HEYS.cascade.crs] ✅ Cascade Rate Score:', {
  ...
  daysWithData: 18,
  daysAtPeak: 3,
  model: 'EMA α=0.95, 30-day window',  // v3.3.0
...
// Проверить версию:
HEYS.CascadeCard.VERSION; // → "3.3.0"
