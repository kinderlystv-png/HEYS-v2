> **DEPRECATED** (26.02.2026): Описывает нереализованную 4-уровневую архитектуру
> (React → localStorage → IndexedDB → Supabase). IndexedDB и Web Workers для
> sync **не используются**. Supabase SDK удалён 2025-12-24. Текущая архитектура:
> localStorage + Store API + debounced RPC → PostgreSQL. Актуальный справочник:
