# 🧠 HEYS Advice Module — Система умных советов
...
## ⚠️ КРИТИЧЕСКИЕ ЗАМЕЧАНИЯ ДЛЯ РЕАЛИЗАЦИИ

### Z-index и позиционирование
- **Текущий toast**: `z-index: 1000`, `bottom: 80px`
- **Модалки** (time-picker, deficit-picker): перекрывают всё
- **Решение**: НЕ делать панель fixed. Сделать expandable toast — клик на toast раскрывает остальные советы ВНУТРИ того же контейнера

### Side effects в useMemo — ЗАПРЕЩЕНО!
```javascript
// ❌ НЕЛЬЗЯ (в промпте было неправильно):
onShow: () => sessionStorage.setItem('heys_streak7', '1')

// ✅ ПРАВИЛЬНО — вызывать onShow в useEffect при показе:
useEffect(() => {
...
### fmtDate fallback
```javascript
// Inline функция вместо зависимости от window.fmtDate:
const formatDate = (d) => {
...
### 🧠 Smart Timing — Умное время показа (Deep Psychology Edition)

**Ключевая идея**: Советы показываются только когда пользователь **готов их воспринять** и когда они **максимально релевантны**.

---

#### 🌅 Циркадные ритмы — адаптация под время суток

| Время | Состояние пользователя | Какие советы | Тон |
|-------|------------------------|--------------|-----|
| 6:00-9:00 | Просыпается, низкая энергия | Мягкая мотивация | "Доброе утро! Начни день с..." |
| 9:00-12:00 | Активен, принимает решения | Конкретные рекомендации | Деловой, чёткий |
| 12:00-14:00 | Обед, спешит | Максимум 2-3 слова | "Добавь белка 💪" |
| 14:00-17:00 | "Сахарная яма", спад | Предупреждения о сладком | Заботливый |
| 17:00-20:00 | Усталость, домой | Итоги дня, что доесть | Поддерживающий |
| 20:00-23:00 | Расслаблен, рефлексия | Похвала, достижения | Тёплый, хвалящий |
| 23:00-6:00 | **НЕ БЕСПОКОИТЬ!** | НИКАКИХ советов | — |
...
#### 😊 Эмоциональный интеллект — адаптация под состояние

| Состояние | Как определить | Реакция системы |
|-----------|----------------|-----------------|
| **Стресс** | mood < 3 в последних приёмах | Только поддержка, без критики |
| **Срыв** | kcalPct > 1.5 резко | "Бывает! Завтра новый день 💙" |
| **Успех** | streak >= 3 | Праздновать! Confetti! |
| **Возвращение** | lastVisit > 2 дней | "Рады видеть! Продолжим?" |
| **Новичок** | totalDays < 3 | Onboarding советы |
| **Удалил продукт** | action === 'delete' | Молчим (не комментируем) |

**Правило**: Если `mood < 3` или только что был срыв — **НЕ показывать warning**, только `achievement` и `tip`.

---

#### 🔄 Поведенческие паттерны — персонализация

Система запоминает привычки и адаптируется:

| Паттерн | Как распознать | Персонализированный совет |
|---------|----------------|---------------------------|
| **"Забывает завтрак"** | 5+ дней первый приём после 11:00 | "Замечаю, завтрак часто пропускается..." |
| **"Вечерний едок"** | >60% ккал после 18:00 регулярно | "Попробуй сместить калории на день" |
| **"Сладкоежка"** | simplePct > 40% неделю | "Давай найдём альтернативу сладкому?" |
| **"Белковый дефицит"** | proteinPct < 50% неделю | "Белок — твоя зона роста 💪" |
| **"Перфекционист"** | Всегда 95-105% | "Идеально! Можешь иногда расслабиться" |
| **"Выходные = срыв"** | Сб-Вс перебор, Пн-Пт норма | В пятницу: "Выходные близко — помни о целях!" |

---

#### 🚫 Когда НЕ показывать — контекст занятости

| Состояние UI | Показывать? | Причина |
|--------------|-------------|---------|
| Открыт поиск продуктов | ❌ | Пользователь занят |
| Редактирует граммы | ❌ | Фокус на вводе |
| Любая модалка открыта | ❌ | Блокирует внимание |
| Скроллит список | ❌ | Изучает данные |
| Idle 3+ секунд | ✅ | Готов к информации |
| Только добавил продукт | ✅ | Момент обучения |
| Удалил продукт | ❌ | Не комментировать |
...
#### 📅 Временные/социальные факторы

| Фактор | Условие | Особый совет |
|--------|---------|--------------|
| **Понедельник** | day === 1 | "Новая неделя — новый старт!" |
| **Пятница вечер** | day === 5 && hour >= 17 | "Выходные близко — не забывай о целях" |
| **Воскресенье вечер** | day === 0 && hour >= 18 | "Спланируй питание на неделю" |
| **Конец месяца** | date >= 28 | "Финальный рывок месяца!" |
| **1 января** | month === 0 && date === 1 | "Новый год — идеальный старт! 🎉" |
| **День рождения** | (если знаем) | "С днём рождения! Сегодня можно всё 🎂" |

---

#### 🎯 Триггеры показа (итоговая таблица)

| Триггер | Контекст | Советы | TTL | Приоритет |
|---------|----------|--------|-----|-----------|
| `tab_open` | Первое открытие за сессию | Streak, баланс, прогресс | 5 сек | 1 |
| `product_added` | После добавления | БЖУ, клетчатка, ГИ | 4 сек | 2 |
| `meal_opened` | Клик на приём | Специфичные для приёма | 3 сек | 3 |
| `day_changed` | Смена даты | Итоги / план | 5 сек | 1 |
| `idle_5s` | Бездействие 5 сек | Общие советы | 4 сек | 4 |
| `time_evening` | 18:00+ первый раз | Вечерние | 5 сек | 2 |
| `time_night` | 23:00+ | **НИЧЕГО** | — | — |
...
```javascript
const COOLDOWN_MS = 30000;      // Минимум 30 сек между советами
const ADVICE_MEMORY_MS = 300000; // Не повторять совет 5 минут
const MAX_ADVICES_PER_SESSION = 10; // Не надоедать

// Исключения (можно показывать всегда):
...
| Проблема | Описание |
|----------|----------|
| **Один toast** | Показывает только одну рекомендацию (первую по приоритету) |
| **Потеря контекста** | Остальные полезные советы скрыты |
| **Жёсткая логика** | Условия захардкожены в useMemo |
| **Нет категоризации** | Все советы в одной куче |

---

## 🏗️ Архитектура Advice Module

```
┌─────────────────────────────────────────────────────────────┐
│                    ADVICE ENGINE                            │
├─────────────────────────────────────────────────────────────┤
│  Context Collector          →  Собирает ВСЕ данные          │
│  ├── DayContext             (dayTot, normAbs, meals, etc.)  │
│  ├── TimeContext            (hour, isEvening, isWeekend)    │
│  ├── TrendContext           (streak, weight trend)          │
│  └── ProfileContext         (goals, preferences)            │
├─────────────────────────────────────────────────────────────┤
│  Rule Engine                →  Оценивает все правила        │
│  ├── Warning Rules          (критические, priority: 1)      │
│  ├── Achievement Rules      (похвала, priority: 2)          │
│  ├── Nutrition Rules        (БЖУ, клетчатка, ГИ)            │
│  ├── Timing Rules           (время суток, перерывы)         │
│  ├── Training Rules         (контекст тренировки)           │
│  └── Lifestyle Rules        (сон, шаги, вода)               │
├─────────────────────────────────────────────────────────────┤
│  Advice Prioritizer         →  Сортирует и фильтрует        │
│  ├── По severity            (warning > achievement > tip)   │
│  ├── По актуальности        (время суток, контекст)         │
│  └── По новизне             (не повторять часто)            │
├─────────────────────────────────────────────────────────────┤
│  Delivery Layer             →  Как показывать               │
│  ├── Toast (1 главный)      - текущий функционал            │
│  ├── Advice Panel           - раскрывающийся список         │
│  └── Daily Summary          - итоги дня (вечером)           │
└─────────────────────────────────────────────────────────────┘
```
...
| Файл | Описание |
|------|----------|
| `apps/web/heys_advice_v1.js` | **НОВЫЙ** — Модуль советов |
| `apps/web/heys_day_v12.js` | Интеграция модуля, замена macroTip |
| `apps/web/styles/main.css` | Стили для Advice Panel |
 ...
 * @property {string} id - Уникальный ID правила (e.g., 'low_protein', 'streak_7')
 * @property {string} icon - Emoji иконка
 * @property {string} text - Текст совета
 * @property {'warning'|'achievement'|'tip'|'info'} type - Тип
 * @property {number} priority - Приоритет (1-100, меньше = важнее)
 * @property {string} category - Категория (nutrition, timing, training, lifestyle)
 * @property {number} score - Релевантность (0-1)
 * @property {string[]} triggers - Когда показывать: ['tab_open', 'product_added', 'meal_opened', 'day_changed']
 * @property {number} [ttl] - Время показа в мс (default: 5000)
 * @property {boolean} [dismissable] - Можно ли скрыть
 * @property {Function} [onShow] - Callback при показе (для sessionStorage)
 */
...
### Задача 1: Создать файл heys_advice_v1.js

**Расположение**: `apps/web/heys_advice_v1.js`

```javascript
// heys_advice_v1.js — Модуль умных советов HEYS
// Справочник данных: docs/DATA_MODEL_REFERENCE.md
...
(function() {
  ...
  const ALWAYS_SHOW = ['streak_7', 'perfect_day', 'first_day']; // Исключения
  ...
  /**
   * Определяет тон сообщений по времени суток
   * @param {number} hour - Текущий час (0-23)
   * @returns {'silent'|'gentle'|'business'|'brief'|'caring'|'supportive'|'warm'}
   */
  function getToneForHour(hour) {
    if (hour >= 23 || hour < 6) return 'silent';   // Не беспокоить!
    if (hour < 9) return 'gentle';                  // Мягко, утро
    if (hour < 12) return 'business';               // Деловой
    if (hour < 14) return 'brief';                  // Коротко (обед)
    if (hour < 17) return 'caring';                 // Заботливо (спад энергии)
    if (hour < 20) return 'supportive';             // Поддержка (вечер)
    return 'warm';                                  // Тёплый (рефлексия)
  }
   ...
   * @param {Object} uiState - Состояние UI
   * @returns {boolean}
   */
  ...
  /**
   * Определяет особый день (понедельник, праздник и т.д.)
   * @param {Date} date
   * @returns {string|null}
   */
  ...
  // ═══════════════════════════════════════════════════════════
  // SHOWN ADVICES TRACKER — Какие советы уже показаны в сессии
  // ═══════════════════════════════════════════════════════════
  ...
  function wasShownRecently(id, withinMs = ADVICE_MEMORY_MS) {
    if (ALWAYS_SHOW.includes(id)) return false; // Эти показываем всегда
    const shown = getShownAdvices();
  ...
  // ═══════════════════════════════════════════════════════════
  // ADVICE RULES — Все правила советов
  // ═══════════════════════════════════════════════════════════

  /**
   * Создаёт правила советов
   * @param {Object} ctx - Контекст данных
   * @returns {Array<Advice>} Массив активных советов
   */
  function evaluateRules(ctx) {
    ...
    if (specialDay === 'new_year') {
      advices.push({
        id: 'new_year',
        icon: '🎉',
        text: 'С Новым годом! Идеальный день для старта!',
        type: 'achievement',
        priority: 0,
        category: 'special',
        triggers: ['tab_open'],
    ...
    if (emotionalState === 'returning') {
      advices.push({
        id: 'welcome_back',
        icon: '👋',
        text: 'Рады видеть! Продолжим с того места?',
        type: 'achievement',
        priority: 1,
        category: 'emotional',
        triggers: ['tab_open'],
    ...
    if (kcalPct >= 1.25) {
      advices.push({
        ...
        icon: '⚠️',
        text: 'Перебор калорий! Завтра сделай разгрузочный день',
        type: 'warning',
        priority: 11,
        category: 'nutrition',
        score: 1.0,
    ...
    if (simplePct > 0.5 && simpleCarbs > 30) {
      advices.push({
        ...
        icon: '⚠️',
        text: 'Много простых углеводов! Замени сладкое на кашу/овощи',
        type: 'warning',
        priority: 12,
        category: 'nutrition',
        score: simplePct,
    ...
    if (badFatPct > 0.4 && badFat > 20) {
      advices.push({
        id: 'bad_fat_high',
        icon: '⚠️',
        text: 'Много вредных жиров! Замени на рыбу/орехи/авокадо',
        type: 'warning',
        priority: 13,
        category: 'nutrition',
        score: badFatPct,
    ...
    if (avgGI > 70 && dayTot.kcal > 500) {
      advices.push({
        id: 'high_gi',
        icon: '📈',
        text: 'Высокий ГИ — замени быстрые углеводы на сложные',
        type: 'warning',
        priority: 14,
        category: 'nutrition',
        score: avgGI / 100,
    ...
    // Мало калорий вечером — показываем при tab_open
    if (hour >= 18 && dayTot.kcal < 500 && dayTot.kcal > 0) {
      advices.push({
        id: 'too_few_kcal',
        icon: '⚠️',
        text: 'Слишком мало калорий — это вредит метаболизму',
        type: 'warning',
        priority: 15,
        category: 'nutrition',
        score: 0.9,
    ...
    // ─────────────────────────────────────────────────────────
    // 🥬 NUTRITION TIPS (priority: 31-50) — после добавления продукта
    // ─────────────────────────────────────────────────────────
    ...
    if (fiberPct < 0.5 && dayTot.kcal > 500) {
      advices.push({
        id: 'low_fiber',
        icon: '🥬',
        text: 'Добавь клетчатки: овощи, фрукты, каша',
        type: 'fiber',
        priority: 31,
        category: 'nutrition',
        score: 1 - fiberPct,
    ...
    if (carbsPct > 0.7 && proteinPct < 0.4) {
      advices.push({
        id: 'carbs_no_protein',
        icon: '🍽️',
        text: 'Углеводы без белка = быстрый голод. Добавь белок!',
        type: 'tip',
        priority: 32,
        category: 'nutrition',
        score: 0.8,
    ...
    if (fatPct > 0.7 && carbsPct < 0.4) {
      advices.push({
        id: 'fat_no_carbs',
        icon: '⚡',
        text: 'Для энергии нужны углеводы — попробуй кашу',
        type: 'tip',
        priority: 33,
        category: 'nutrition',
        score: 0.8,
    ...
    // Дефициты макросов — при просмотре приёма или после добавления
    if (proteinPct < 0.5 && fatPct >= 0.5 && carbsPct >= 0.5) {
      advices.push({
        id: 'low_protein',
        icon: '🥩',
        text: 'Добавь белка: творог, яйца, курица',
        type: 'protein',
        priority: 41,
        category: 'nutrition',
        score: 1 - proteinPct,
    ...
    if (fatPct < 0.5 && proteinPct >= 0.5 && carbsPct >= 0.5) {
      advices.push({
        id: 'low_fat',
        icon: '🥑',
        text: 'Мало жиров: орехи, авокадо, масло',
        type: 'fat',
        priority: 42,
        category: 'nutrition',
        score: 1 - fatPct,
    ...
    if (carbsPct < 0.5 && proteinPct >= 0.5 && fatPct >= 0.5) {
      advices.push({
        id: 'low_carbs',
        icon: '🍞',
        text: 'Добавь углеводов: каша, хлеб, фрукты',
        type: 'carbs',
        priority: 43,
        category: 'nutrition',
        score: 1 - carbsPct,
    ...
    // ─────────────────────────────────────────────────────────
    // ⏰ TIMING TIPS (priority: 51-70) — контекстные по времени
    // ─────────────────────────────────────────────────────────
    
    // Вечерние простые углеводы — после добавления вечером
    if (hour >= 18 && simplePct > 0.4 && simpleCarbs > 50) {
      advices.push({
        id: 'evening_sugar',
        icon: '🌙',
        text: 'Сладкое вечером → плохой сон. Лучше белок!',
        type: 'tip',
        priority: 51,
        category: 'timing',
        score: 0.7,
    ...
    // Пропущенный завтрак — при открытии вкладки днём
    if (hour >= 12 && mealCount === 0) {
      advices.push({
        id: 'no_breakfast',
        icon: '🌅',
        text: 'Завтрак запускает метаболизм — не пропускай!',
        type: 'tip',
        priority: 52,
        category: 'timing',
        score: 0.8,
    ...
    if (hour >= 14 && mealCount === 1 && dayTot.kcal > 300) {
      advices.push({
        id: 'long_gap',
        icon: '⏰',
        text: 'Большие перерывы замедляют метаболизм — перекуси!',
        type: 'tip',
        priority: 53,
        category: 'timing',
        score: 0.6,
    ...
    if (mealCount === 1 && dayTot.kcal > 800) {
      advices.push({
        id: 'one_big_meal',
        icon: '🍽️',
        text: 'Лучше 3-4 небольших приёма чем 1 большой',
        type: 'tip',
        priority: 54,
        category: 'timing',
        score: 0.5,
    ...
    if (hasTraining && proteinPct < 0.6) {
      advices.push({
        id: 'training_protein',
        icon: '💪',
        text: 'После тренировки важен белок для восстановления',
        type: 'tip',
        priority: 71,
        category: 'training',
        score: 0.8,
    ...
    if (kcalPct >= 1.1 && kcalPct < 1.25) {
      advices.push({
        ...
        icon: '📊',
        text: 'Немного перебор — завтра чуть меньше, и всё ок 😊',
        type: 'tip',
        priority: 81,
        category: 'nutrition',
        score: 0.6,
    ...
    if (kcalPct >= 0.8 && kcalPct < 0.95) {
      ...
      advices.push({
        id: 'kcal_remaining',
        icon: '🎯',
        text: `Осталось ${remaining} ккал — идеально для перекуса`,
        type: 'tip',
        priority: 82,
        category: 'nutrition',
        score: 0.5,
    ...
    if (proteinPct >= 0.8 && fatPct >= 0.8 && carbsPct >= 0.8) {
      advices.push({
        id: 'all_balanced',
        icon: '✅',
        text: 'Отлично! Все макросы в балансе',
        type: 'success',
        priority: 91,
        category: 'nutrition',
        score: 0.9,
  ...
  function useAdviceEngine(params) {
    ...
    // Главный совет для показа (первый из relevantAdvices)
    const primary = relevantAdvices[0] || null;
    
    // Функция отметки показа
    ...
    return {
      primary,             // Главный совет для toast (или null если нет актуальных)
      relevant: relevantAdvices, // Все актуальные для текущего trigger
      all: allAdvices,     // ВСЕ активные советы (для дебага)
      byCategory,          // Сгруппированные по категориям
      count: relevantAdvices.length,
      streak: ctx.currentStreak,
      markShown            // Функция для отметки показа
    };
  }
...
**Где**: `apps/web/heys_day_v12.js` и `apps/web/index.html`

#### 4.1 Добавить скрипт в index.html

**ПЕРЕД** `heys_day_v12.js`:
```html
<script defer src="heys_advice_v1.js?v=1" fetchpriority="low"></script>
```

#### 4.2 В DayTab:
1. Добавить состояние `const [adviceExpanded, setAdviceExpanded] = React.useState(false);`
2. Вызвать `useAdviceEngine` с нужными параметрами
3. Добавить useEffect для onShow (см. Задача 2)
4. Заменить рендеринг toast на expandable версию
5. **Удалить старый macroTip useMemo** — логика теперь в модуле

#### 4.3 Закрытие при открытии модалок

```javascript
// В обработчиках открытия модалок добавить:
setAdviceExpanded(false);
```
...
- ❌ Отдельную fixed панель (конфликт z-index с модалками)
- ❌ Side effects в useMemo (React Strict Mode)
- ❌ Зависимость от window.fmtDate (использовать inline formatDate)
- ❌ Dark theme CSS (light достаточно, toast уже имеет свои цвета)
- ❌ Переусложнять — это не AI engine
- ❌ Добавлять персистентность советов в localStorage
- ❌ Делать советы кликабельными с действиями (пока)
- ❌ Добавлять звуки/вибрацию
- ❌ Создавать отдельную вкладку советов

---

## 🔮 Будущие улучшения (не в этом промпте)

1. **Daily Summary** — итоги дня вечером (после 21:00)
2. **Персонализация** — учёт предпочтений пользователя
3. **Тренды** — советы на основе динамики веса/калорий
4. **Микронутриенты** — когда добавим данные по витаминам
5. **Inline советы** — показ рядом с соответствующими данными (совет о белке → рядом с карточкой белка)

---

## 🔗 Связь с первым промптом

**Зависимость**: Этот промпт требует выполнения [2025-11-28-smart-toast-recommendations.md](./2025-11-28-smart-toast-recommendations.md) сначала:
- Первый промпт: багфикс `prot/protein` + расширение CSS типов toast
- Этот промпт: модуль советов + expandable UI

**После выполнения обоих**: macroTip useMemo полностью удаляется, вся логика в `heys_advice_v1.js`

---

**Время**: ~2-3 часа
**Сложность**: Средняя
**Зависимости**: Выполнить сначала 2025-11-28-smart-toast-recommendations.md
