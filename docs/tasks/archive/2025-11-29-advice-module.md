# 🧠 HEYS Advice Module — Система умных советов

> **Цель**: Создать модульную систему советов, которая учитывает ВСЕ аналитические данные и может показывать несколько рекомендаций.

**✅ СТАТУС: ВЫПОЛНЕНО (2025-11-29)**

---

## ✅ ВЫПОЛНЕННОЕ ПРЕДУСЛОВИЕ

Первый промпт [2025-11-28-smart-toast-recommendations.md](./2025-11-28-smart-toast-recommendations.md) **ВЫПОЛНЕН**:
- ✅ Баг `dayTot.protein` → `dayTot.prot` исправлен
- ✅ 20+ умных рекомендаций в `macroTip` useMemo
- ✅ CSS для `.macro-toast-fiber`, `.macro-toast-tip`, `.macro-toast-achievement` добавлен
- ✅ Swipe-to-dismiss и progress bar добавлены
- ✅ `currentStreak` вычисляется через localStorage (строка 1636)

### Что УЖЕ есть в коде (НЕ дублировать!):

| Элемент | Строка | Описание |
|---------|--------|----------|
| `currentStreak` | 1636-1668 | Вычисляется через `lsGet('heys_dayv2_' + dateStr)` |
| `macroTip` useMemo | 2708-2875 | 20+ рекомендаций с приоритетами |
| Toast swipe handlers | 712-724 | `handleToastTouchStart/Move/End`, `toastSwipeX` |
| Toast CSS types | 4931-4948 | `.macro-toast-fiber`, `.macro-toast-tip`, `.macro-toast-achievement` |
| Progress bar | 4956-4972 | `.macro-toast-progress` с анимацией |
| Icon animations | 4978-5010 | fire-flicker, warning-pulse, success-bounce |

---

## 🔍 РЕЗУЛЬТАТЫ АУДИТА (2025-11-29, обновлено после первого промпта)

### 🔴 Критические проблемы (ИСПРАВИТЬ в промпте!)

1. **Дублирование currentStreak** — `useAdviceEngine` вычисляет streak через `activeDays`, но `currentStreak` УЖЕ существует в DayTab (строка 1636)! **Решение**: Передавать `currentStreak` как параметр, НЕ вычислять заново.

2. **Дублирование CSS** — Промпт предлагал добавить `.macro-toast-fiber/tip/achievement`, но они УЖЕ добавлены в первом промпте (строки 4931-4948)! **Решение**: Добавить только новые стили для expandable (badge, extras).

3. **Toast render конфликт** — Текущий toast (строка 3922-3936) имеет плоскую структуру БЕЗ wrapper. Промпт предлагает `macro-toast-main` wrapper. **Решение**: Обновить рендер toast для совместимости с swipe и expand.

4. **Swipe handlers утеряны** — Промпт не упоминает сохранение `handleToastTouchStart/Move/End` и `toastSwipeX`. **Решение**: Интегрировать swipe в новый рендер.

5. **Поведение по клику** — Текущий toast: `onClick: dismissToast`. Промпт: expand по клику. **Решение**: Expand если >1 совет, иначе dismiss.

6. **Event dispatch место** — Промпт говорит MealAddProduct, но правильное место — функция `addProductToMeal` (строка 1587)!

### 🟡 Важные ограничения (учитывать при реализации)

1. **`activeDays` содержит только текущий месяц** — Но `currentStreak` уже использует `lsGet` напрямую, так что проблемы нет.

2. **Существующие pickers** (все актуальны):
   - `showTimePicker` (681), `showGramsPicker` (778), `showWeightPicker` (803)
   - `showDeficitPicker` (904), `showZonePicker` (786), `showSleepQualityPicker` (793)
   - `showDayScorePicker` (798), `showHouseholdPicker` (1081), `showTrainingPicker` (687)

3. **Порядок скриптов** — `heys_advice_v1.js` нужно добавить ПЕРЕД `heys_day_v12.js` в index.html (строка 102).

4. **`returning` эмоциональное состояние** — `lastVisitDaysAgo` не вычисляется. TODO: добавить в будущем.

### 🟢 Проверено и ОК

- ✅ `date` переменная (строка 142)
- ✅ `toastTimeoutRef` (строка 707)
- ✅ `showConfetti` (строка 770)
- ✅ `haptic` доступен (строка 17)
- ✅ `activeDays` Map (строка 1629)
- ✅ `currentStreak` уже существует (строка 1636)
- ✅ Toast рендерится (строка 3922)
- ✅ Swipe handlers работают (строки 712-724)
- ✅ Progress bar существует (строка 3936)

---

📊 **[DATA_MODEL_REFERENCE.md](../DATA_MODEL_REFERENCE.md)** — справочник всех аналитических параметров

---

## ⚠️ КРИТИЧЕСКИЕ ЗАМЕЧАНИЯ ДЛЯ РЕАЛИЗАЦИИ

### Z-index и позиционирование
- **Текущий toast**: `z-index: 1000`, `bottom: 80px`
- **Модалки** (time-picker, deficit-picker): перекрывают всё
- **Решение**: НЕ делать панель fixed. Сделать expandable toast — клик на toast раскрывает остальные советы ВНУТРИ того же контейнера

### Side effects в useMemo — ЗАПРЕЩЕНО!
```javascript
// ❌ НЕЛЬЗЯ (в промпте было неправильно):
onShow: () => sessionStorage.setItem('heys_streak7', '1')

// ✅ ПРАВИЛЬНО — вызывать onShow в useEffect при показе:
useEffect(() => {
  if (primary?.onShow) primary.onShow();
}, [primary?.id]);
```

### Порядок загрузки скриптов
В `index.html` добавить ПЕРЕД `heys_day_v12.js` (найти строку с `heys_day_v12.js` и добавить ПЕРЕД ней):
```html
<script defer src="heys_advice_v1.js?v=1" fetchpriority="low"></script>
```

### Defensive programming
```javascript
// Всегда проверяй на undefined:
const advices = allAdvices || [];
const meals = day?.meals || [];
```

### fmtDate fallback
```javascript
// Inline функция вместо зависимости от window.fmtDate:
const formatDate = (d) => {
  const yyyy = d.getFullYear();
  const mm = String(d.getMonth() + 1).padStart(2, '0');
  const dd = String(d.getDate()).padStart(2, '0');
  return `${yyyy}-${mm}-${dd}`;
};
```

### 🧠 Smart Timing — Умное время показа (Deep Psychology Edition)

**Ключевая идея**: Советы показываются только когда пользователь **готов их воспринять** и когда они **максимально релевантны**.

---

#### 🌅 Циркадные ритмы — адаптация под время суток

| Время | Состояние пользователя | Какие советы | Тон |
|-------|------------------------|--------------|-----|
| 6:00-9:00 | Просыпается, низкая энергия | Мягкая мотивация | "Доброе утро! Начни день с..." |
| 9:00-12:00 | Активен, принимает решения | Конкретные рекомендации | Деловой, чёткий |
| 12:00-14:00 | Обед, спешит | Максимум 2-3 слова | "Добавь белка 💪" |
| 14:00-17:00 | "Сахарная яма", спад | Предупреждения о сладком | Заботливый |
| 17:00-20:00 | Усталость, домой | Итоги дня, что доесть | Поддерживающий |
| 20:00-23:00 | Расслаблен, рефлексия | Похвала, достижения | Тёплый, хвалящий |
| 23:00-6:00 | **НЕ БЕСПОКОИТЬ!** | НИКАКИХ советов | — |

**Реализация:**
```javascript
function getToneForHour(hour) {
  if (hour >= 23 || hour < 6) return 'silent';
  if (hour < 9) return 'gentle';
  if (hour < 12) return 'business';
  if (hour < 14) return 'brief';
  if (hour < 17) return 'caring';
  if (hour < 20) return 'supportive';
  return 'warm';
}
```

---

#### 😊 Эмоциональный интеллект — адаптация под состояние

| Состояние | Как определить | Реакция системы |
|-----------|----------------|-----------------|
| **Стресс** | mood < 3 в последних приёмах | Только поддержка, без критики |
| **Срыв** | kcalPct > 1.5 резко | "Бывает! Завтра новый день 💙" |
| **Успех** | streak >= 3 | Праздновать! Confetti! |
| **Возвращение** | lastVisit > 2 дней | "Рады видеть! Продолжим?" |
| **Новичок** | totalDays < 3 | Onboarding советы |
| **Удалил продукт** | action === 'delete' | Молчим (не комментируем) |

**Правило**: Если `mood < 3` или только что был срыв — **НЕ показывать warning**, только `achievement` и `tip`.

---

#### 🔄 Поведенческие паттерны — персонализация

Система запоминает привычки и адаптируется:

| Паттерн | Как распознать | Персонализированный совет |
|---------|----------------|---------------------------|
| **"Забывает завтрак"** | 5+ дней первый приём после 11:00 | "Замечаю, завтрак часто пропускается..." |
| **"Вечерний едок"** | >60% ккал после 18:00 регулярно | "Попробуй сместить калории на день" |
| **"Сладкоежка"** | simplePct > 40% неделю | "Давай найдём альтернативу сладкому?" |
| **"Белковый дефицит"** | proteinPct < 50% неделю | "Белок — твоя зона роста 💪" |
| **"Перфекционист"** | Всегда 95-105% | "Идеально! Можешь иногда расслабиться" |
| **"Выходные = срыв"** | Сб-Вс перебор, Пн-Пт норма | В пятницу: "Выходные близко — помни о целях!" |

---

#### 🚫 Когда НЕ показывать — контекст занятости

| Состояние UI | Показывать? | Причина |
|--------------|-------------|---------|
| Открыт поиск продуктов | ❌ | Пользователь занят |
| Редактирует граммы | ❌ | Фокус на вводе |
| Любая модалка открыта | ❌ | Блокирует внимание |
| Скроллит список | ❌ | Изучает данные |
| Idle 3+ секунд | ✅ | Готов к информации |
| Только добавил продукт | ✅ | Момент обучения |
| Удалил продукт | ❌ | Не комментировать |

**Реализация:**
```javascript
const isUserBusy = modalOpen || searchOpen || isEditing || isScrolling;
if (isUserBusy) return { primary: null, ... };
```

---

#### 📅 Временные/социальные факторы

| Фактор | Условие | Особый совет |
|--------|---------|--------------|
| **Понедельник** | day === 1 | "Новая неделя — новый старт!" |
| **Пятница вечер** | day === 5 && hour >= 17 | "Выходные близко — не забывай о целях" |
| **Воскресенье вечер** | day === 0 && hour >= 18 | "Спланируй питание на неделю" |
| **Конец месяца** | date >= 28 | "Финальный рывок месяца!" |
| **1 января** | month === 0 && date === 1 | "Новый год — идеальный старт! 🎉" |
| **День рождения** | (если знаем) | "С днём рождения! Сегодня можно всё 🎂" |

---

#### 🎯 Триггеры показа (итоговая таблица)

| Триггер | Контекст | Советы | TTL | Приоритет |
|---------|----------|--------|-----|-----------|
| `tab_open` | Первое открытие за сессию | Streak, баланс, прогресс | 5 сек | 1 |
| `product_added` | После добавления | БЖУ, клетчатка, ГИ | 4 сек | 2 |
| `meal_opened` | Клик на приём | Специфичные для приёма | 3 сек | 3 |
| `day_changed` | Смена даты | Итоги / план | 5 сек | 1 |
| `idle_5s` | Бездействие 5 сек | Общие советы | 4 сек | 4 |
| `time_evening` | 18:00+ первый раз | Вечерние | 5 сек | 2 |
| `time_night` | 23:00+ | **НИЧЕГО** | — | — |

---

#### 🔇 Cooldown и дедупликация

```javascript
const COOLDOWN_MS = 30000;      // Минимум 30 сек между советами
const ADVICE_MEMORY_MS = 300000; // Не повторять совет 5 минут
const MAX_ADVICES_PER_SESSION = 10; // Не надоедать

// Исключения (можно показывать всегда):
const ALWAYS_SHOW = ['streak_7', 'perfect_day', 'first_day'];
```

---

## 🎯 Проблемы текущей реализации

| Проблема | Описание |
|----------|----------|
| **Один toast** | Показывает только одну рекомендацию (первую по приоритету) |
| **Потеря контекста** | Остальные полезные советы скрыты |
| **Жёсткая логика** | Условия захардкожены в useMemo |
| **Нет категоризации** | Все советы в одной куче |

---

## 🏗️ Архитектура Advice Module

```
┌─────────────────────────────────────────────────────────────┐
│                    ADVICE ENGINE                            │
├─────────────────────────────────────────────────────────────┤
│  Context Collector          →  Собирает ВСЕ данные          │
│  ├── DayContext             (dayTot, normAbs, meals, etc.)  │
│  ├── TimeContext            (hour, isEvening, isWeekend)    │
│  ├── TrendContext           (streak, weight trend)          │
│  └── ProfileContext         (goals, preferences)            │
├─────────────────────────────────────────────────────────────┤
│  Rule Engine                →  Оценивает все правила        │
│  ├── Warning Rules          (критические, priority: 1)      │
│  ├── Achievement Rules      (похвала, priority: 2)          │
│  ├── Nutrition Rules        (БЖУ, клетчатка, ГИ)            │
│  ├── Timing Rules           (время суток, перерывы)         │
│  ├── Training Rules         (контекст тренировки)           │
│  └── Lifestyle Rules        (сон, шаги, вода)               │
├─────────────────────────────────────────────────────────────┤
│  Advice Prioritizer         →  Сортирует и фильтрует        │
│  ├── По severity            (warning > achievement > tip)   │
│  ├── По актуальности        (время суток, контекст)         │
│  └── По новизне             (не повторять часто)            │
├─────────────────────────────────────────────────────────────┤
│  Delivery Layer             →  Как показывать               │
│  ├── Toast (1 главный)      - текущий функционал            │
│  ├── Advice Panel           - раскрывающийся список         │
│  └── Daily Summary          - итоги дня (вечером)           │
└─────────────────────────────────────────────────────────────┘
```

---

## 📋 Ключевые файлы

| Файл | Описание |
|------|----------|
| `apps/web/heys_advice_v1.js` | **НОВЫЙ** — Модуль советов |
| `apps/web/heys_day_v12.js` | Интеграция модуля, замена macroTip |
| `apps/web/styles/main.css` | Стили для Advice Panel |

### 📍 Актуальные номера строк (на 2025-11-29)

| Что | Строка | Описание |
|-----|--------|----------|
| `date` переменная | 144 | `const date = selectedDate \|\| todayISO();` |
| `toastTimeoutRef` | 707 | `const toastTimeoutRef = React.useRef(null);` |
| `toastVisible` | 705 | `const [toastVisible, setToastVisible] = useState(false);` |
| `macroTip` useMemo | 2653 | Старый код советов — УДАЛИТЬ |
| `activeDays` | 1612 | Map с ratio для дней месяца |
| `.macro-toast` CSS | 4850 | Существующие стили toast |

⚠️ **После правок номера строк могут сместиться!** Используй поиск по тексту.

---

## 📝 Структура Advice

```javascript
/**
 * @typedef {Object} Advice
 * @property {string} id - Уникальный ID правила (e.g., 'low_protein', 'streak_7')
 * @property {string} icon - Emoji иконка
 * @property {string} text - Текст совета
 * @property {'warning'|'achievement'|'tip'|'info'} type - Тип
 * @property {number} priority - Приоритет (1-100, меньше = важнее)
 * @property {string} category - Категория (nutrition, timing, training, lifestyle)
 * @property {number} score - Релевантность (0-1)
 * @property {string[]} triggers - Когда показывать: ['tab_open', 'product_added', 'meal_opened', 'day_changed']
 * @property {number} [ttl] - Время показа в мс (default: 5000)
 * @property {boolean} [dismissable] - Можно ли скрыть
 * @property {Function} [onShow] - Callback при показе (для sessionStorage)
 */

/**
 * @typedef {'tab_open'|'product_added'|'meal_opened'|'day_changed'|'time_check'|null} ShowTrigger
 */
```

---

## 🎯 Задачи

### Задача 1: Создать файл heys_advice_v1.js

**Расположение**: `apps/web/heys_advice_v1.js`

```javascript
// heys_advice_v1.js — Модуль умных советов HEYS
// Справочник данных: docs/DATA_MODEL_REFERENCE.md

(function() {
  'use strict';

  // ═══════════════════════════════════════════════════════════
  // CONSTANTS
  // ═══════════════════════════════════════════════════════════
  
  const SHOWN_KEY = 'heys_shown_advices';
  const COOLDOWN_MS = 30000;        // Минимум 30 сек между советами
  const ADVICE_MEMORY_MS = 300000;  // Не повторять совет 5 минут
  const MAX_ADVICES_PER_SESSION = 10;
  const ALWAYS_SHOW = ['streak_7', 'perfect_day', 'first_day']; // Исключения
  
  let lastShowTime = 0;
  let sessionAdviceCount = 0;
  
  // Cleanup при смене даты (вызывается извне при date change)
  function resetSessionAdvices() {
    sessionAdviceCount = 0;
    lastShowTime = 0;
    try {
      // Очищаем только advice-related ключи, не streak
      sessionStorage.removeItem(SHOWN_KEY);
    } catch(e) {}
  }

  // ═══════════════════════════════════════════════════════════
  // 🧠 CONTEXT ANALYZERS — Анализ состояния пользователя
  // ═══════════════════════════════════════════════════════════

  /**
   * Определяет тон сообщений по времени суток
   * @param {number} hour - Текущий час (0-23)
   * @returns {'silent'|'gentle'|'business'|'brief'|'caring'|'supportive'|'warm'}
   */
  function getToneForHour(hour) {
    if (hour >= 23 || hour < 6) return 'silent';   // Не беспокоить!
    if (hour < 9) return 'gentle';                  // Мягко, утро
    if (hour < 12) return 'business';               // Деловой
    if (hour < 14) return 'brief';                  // Коротко (обед)
    if (hour < 17) return 'caring';                 // Заботливо (спад энергии)
    if (hour < 20) return 'supportive';             // Поддержка (вечер)
    return 'warm';                                  // Тёплый (рефлексия)
  }

  /**
   * Определяет эмоциональное состояние пользователя
   * @param {Object} ctx - Контекст
   * @returns {'stressed'|'crashed'|'success'|'returning'|'newbie'|'normal'}
   */
  function getEmotionalState(ctx) {
    const { day, currentStreak, mealCount, kcalPct, totalDaysTracked } = ctx;
    
    // Новичок (менее 3 дней с данными)
    if (totalDaysTracked < 3) return 'newbie';
    
    // Успех (streak 3+ дней)
    if (currentStreak >= 3) return 'success';
    
    // Срыв (резкий перебор >150% калорий)
    if (kcalPct > 1.5) return 'crashed';
    
    // Стресс (низкий mood в последних приёмах)
    const meals = day?.meals || [];
    const moodsWithValues = meals.filter(m => m.mood && !isNaN(+m.mood));
    if (moodsWithValues.length > 0) {
      const avgMood = moodsWithValues.reduce((sum, m) => sum + (+m.mood), 0) / moodsWithValues.length;
      if (avgMood < 3) return 'stressed';
    }
    
    return 'normal';
  }

  /**
   * Проверяет, занят ли пользователь
   * @param {Object} uiState - Состояние UI
   * @returns {boolean}
   */
  function isUserBusy(uiState) {
    const { modalOpen, searchOpen, isEditing, isScrolling } = uiState || {};
    return modalOpen || searchOpen || isEditing || isScrolling;
  }

  /**
   * Определяет особый день (понедельник, праздник и т.д.)
   * @param {Date} date
   * @returns {string|null}
   */
  function getSpecialDay(date) {
    const day = date.getDay();
    const month = date.getMonth();
    const dateNum = date.getDate();
    const hour = date.getHours();
    
    // Новый год
    if (month === 0 && dateNum === 1) return 'new_year';
    
    // Понедельник утро
    if (day === 1 && hour < 12) return 'monday_morning';
    
    // Пятница вечер
    if (day === 5 && hour >= 17) return 'friday_evening';
    
    // Воскресенье вечер
    if (day === 0 && hour >= 18) return 'sunday_evening';
    
    // Конец месяца
    if (dateNum >= 28) return 'month_end';
    
    return null;
  }

  /**
   * Фильтрует советы по эмоциональному состоянию
   * @param {Array} advices
   * @param {string} emotionalState
   * @returns {Array}
   */
  function filterByEmotionalState(advices, emotionalState) {
    // При стрессе или срыве — убираем warnings
    if (emotionalState === 'stressed' || emotionalState === 'crashed') {
      return advices.filter(a => a.type !== 'warning');
    }
    return advices;
  }

  // ═══════════════════════════════════════════════════════════
  // SHOWN ADVICES TRACKER — Какие советы уже показаны в сессии
  // ═══════════════════════════════════════════════════════════
  
  function getShownAdvices() {
    try {
      return JSON.parse(sessionStorage.getItem(SHOWN_KEY) || '{}');
    } catch { return {}; }
  }
  
  function markAdviceShown(id) {
    try {
      const shown = getShownAdvices();
      shown[id] = Date.now();
      sessionStorage.setItem(SHOWN_KEY, JSON.stringify(shown));
      sessionAdviceCount++;
    } catch {}
  }
  
  function wasShownRecently(id, withinMs = ADVICE_MEMORY_MS) {
    if (ALWAYS_SHOW.includes(id)) return false; // Эти показываем всегда
    const shown = getShownAdvices();
    return shown[id] && (Date.now() - shown[id] < withinMs);
  }
  
  function canShowNow() {
    if (sessionAdviceCount >= MAX_ADVICES_PER_SESSION) return false;
    return Date.now() - lastShowTime > COOLDOWN_MS;
  }

  // ═══════════════════════════════════════════════════════════
  // ADVICE RULES — Все правила советов
  // ═══════════════════════════════════════════════════════════

  /**
   * Создаёт правила советов
   * @param {Object} ctx - Контекст данных
   * @returns {Array<Advice>} Массив активных советов
   */
  function evaluateRules(ctx) {
    const {
      dayTot, normAbs, optimum, day, pIndex,
      currentStreak, hour, mealCount, hasTraining,
      emotionalState, tone, specialDay
    } = ctx;

    const advices = [];
    
    // Ночью — не беспокоим!
    if (tone === 'silent') return [];
    
    // Пустой день — не показывать советы (нет смысла)
    if ((dayTot.kcal || 0) < 10 && mealCount === 0) return [];

    // Вычисляем проценты
    const kcalPct = (dayTot.kcal || 0) / (optimum || 1);
    const proteinPct = (dayTot.prot || 0) / (normAbs.prot || 1);
    const fatPct = (dayTot.fat || 0) / (normAbs.fat || 1);
    const carbsPct = (dayTot.carbs || 0) / (normAbs.carbs || 1);
    const fiberPct = (dayTot.fiber || 0) / (normAbs.fiber || 25);
    
    const simpleCarbs = dayTot.simple || 0;
    const complexCarbs = dayTot.complex || 0;
    const totalCarbs = simpleCarbs + complexCarbs;
    const simplePct = totalCarbs > 0 ? (simpleCarbs / totalCarbs) : 0;
    
    const badFat = dayTot.bad || 0;
    const totalFat = dayTot.fat || 0;
    const badFatPct = totalFat > 0 ? (badFat / totalFat) : 0;
    
    const avgGI = dayTot.gi || 0;

    // ─────────────────────────────────────────────────────────
    // 📅 SPECIAL DAY TIPS — Особые дни
    // ─────────────────────────────────────────────────────────

    if (specialDay === 'new_year') {
      advices.push({
        id: 'new_year',
        icon: '🎉',
        text: 'С Новым годом! Идеальный день для старта!',
        type: 'achievement',
        priority: 0,
        category: 'special',
        triggers: ['tab_open'],
        ttl: 7000
      });
    }
    
    if (specialDay === 'monday_morning') {
      advices.push({
        id: 'monday_motivation',
        icon: '💪',
        text: 'Новая неделя — новые возможности!',
        type: 'tip',
        priority: 5,
        category: 'motivation',
        triggers: ['tab_open'],
        ttl: 5000
      });
    }
    
    if (specialDay === 'friday_evening') {
      advices.push({
        id: 'friday_reminder',
        icon: '🎯',
        text: 'Выходные близко — помни о своих целях!',
        type: 'tip',
        priority: 10,
        category: 'motivation',
        triggers: ['tab_open'],
        ttl: 5000
      });
    }
    
    if (specialDay === 'sunday_evening') {
      advices.push({
        id: 'sunday_planning',
        icon: '📋',
        text: 'Спланируй питание на неделю',
        type: 'tip',
        priority: 10,
        category: 'motivation',
        triggers: ['tab_open'],
        ttl: 5000
      });
    }

    // ─────────────────────────────────────────────────────────
    // 😊 EMOTIONAL STATE TIPS — По состоянию
    // ─────────────────────────────────────────────────────────

    if (emotionalState === 'returning') {
      // ⚠️ TODO: lastVisitDaysAgo не вычисляется, этот совет пока не сработает
      // Для активации: добавить localStorage ключ heys_last_visit и вычислять разницу
      advices.push({
        id: 'welcome_back',
        icon: '👋',
        text: 'Рады видеть! Продолжим с того места?',
        type: 'achievement',
        priority: 1,
        category: 'emotional',
        triggers: ['tab_open'],
        ttl: 5000
      });
    }
    
    if (emotionalState === 'crashed') {
      advices.push({
        id: 'crash_support',
        icon: '💙',
        text: 'Бывает! Завтра новый день. Ты справишься!',
        type: 'achievement',
        priority: 1,
        category: 'emotional',
        triggers: ['tab_open', 'product_added'],
        ttl: 6000
      });
    }
    
    if (emotionalState === 'stressed') {
      advices.push({
        id: 'stress_support',
        icon: '🤗',
        text: 'Ты молодец, что записываешь. Это уже успех!',
        type: 'achievement',
        priority: 2,
        category: 'emotional',
        triggers: ['tab_open'],
        ttl: 5000
      });
    }

    // ─────────────────────────────────────────────────────────
    // 🏆 ACHIEVEMENTS (priority: 1-10) — показываются при tab_open
    // ─────────────────────────────────────────────────────────
    
    if (currentStreak >= 7 && !sessionStorage.getItem('heys_streak7')) {
      advices.push({
        id: 'streak_7',
        icon: '🏆',
        text: `Невероятно! ${currentStreak} дней в норме!`,
        type: 'achievement',
        priority: 1,
        category: 'achievement',
        score: 1.0,
        triggers: ['tab_open'],
        ttl: 7000,
        showConfetti: true,
        onShow: () => { try { sessionStorage.setItem('heys_streak7', '1'); } catch(e) {} }
      });
    }
    
    if (currentStreak >= 3 && currentStreak < 7 && !sessionStorage.getItem('heys_streak3')) {
      advices.push({
        id: 'streak_3',
        icon: '🔥',
        text: `${currentStreak} дня подряд в норме! Так держать!`,
        type: 'achievement',
        priority: 2,
        category: 'achievement',
        score: 0.9,
        triggers: ['tab_open'],
        ttl: 5000,
        onShow: () => { try { sessionStorage.setItem('heys_streak3', '1'); } catch(e) {} }
      });
    }
    
    // Идеальный день — показываем при tab_open вечером
    if (hour >= 18 && kcalPct >= 0.95 && kcalPct <= 1.05 && 
        proteinPct >= 0.9 && fatPct >= 0.9 && carbsPct >= 0.9) {
      advices.push({
        id: 'perfect_day',
        icon: '⭐',
        text: 'Идеальный баланс! Отличная работа 🎉',
        type: 'achievement',
        priority: 5,
        category: 'achievement',
        score: 1.0,
        triggers: ['tab_open'],
        ttl: 5000
      });
    }
    
    // Первый приём — после добавления продукта
    if (mealCount === 1 && !localStorage.getItem('heys_first_meal_tip')) {
      advices.push({
        id: 'first_day',
        icon: '👋',
        text: 'Отличное начало! Записывай всё — это ключ к успеху',
        type: 'achievement',
        priority: 3,
        category: 'achievement',
        score: 1.0,
        triggers: ['product_added'],
        ttl: 5000,
        onShow: () => { try { localStorage.setItem('heys_first_meal_tip', '1'); } catch(e) {} }
      });
    }

    // ─────────────────────────────────────────────────────────
    // ⚠️ WARNINGS (priority: 11-30) — показываются после добавления продукта
    // ─────────────────────────────────────────────────────────
    
    if (kcalPct >= 1.25) {
      advices.push({
        id: 'kcal_excess_critical',
        icon: '⚠️',
        text: 'Перебор калорий! Завтра сделай разгрузочный день',
        type: 'warning',
        priority: 11,
        category: 'nutrition',
        score: 1.0,
        triggers: ['product_added', 'tab_open'],
        ttl: 6000
      });
    }
    
    if (simplePct > 0.5 && simpleCarbs > 30) {
      advices.push({
        id: 'simple_carbs_high',
        icon: '⚠️',
        text: 'Много простых углеводов! Замени сладкое на кашу/овощи',
        type: 'warning',
        priority: 12,
        category: 'nutrition',
        score: simplePct,
        triggers: ['product_added'],
        ttl: 5000
      });
    }
    
    if (badFatPct > 0.4 && badFat > 20) {
      advices.push({
        id: 'bad_fat_high',
        icon: '⚠️',
        text: 'Много вредных жиров! Замени на рыбу/орехи/авокадо',
        type: 'warning',
        priority: 13,
        category: 'nutrition',
        score: badFatPct,
        triggers: ['product_added'],
        ttl: 5000
      });
    }
    
    if (avgGI > 70 && dayTot.kcal > 500) {
      advices.push({
        id: 'high_gi',
        icon: '📈',
        text: 'Высокий ГИ — замени быстрые углеводы на сложные',
        type: 'warning',
        priority: 14,
        category: 'nutrition',
        score: avgGI / 100,
        triggers: ['product_added'],
        ttl: 5000
      });
    }
    
    // Мало калорий вечером — показываем при tab_open
    if (hour >= 18 && dayTot.kcal < 500 && dayTot.kcal > 0) {
      advices.push({
        id: 'too_few_kcal',
        icon: '⚠️',
        text: 'Слишком мало калорий — это вредит метаболизму',
        type: 'warning',
        priority: 15,
        category: 'nutrition',
        score: 0.9,
        triggers: ['tab_open', 'time_check'],
        ttl: 5000
      });
    }

    // ─────────────────────────────────────────────────────────
    // 🥬 NUTRITION TIPS (priority: 31-50) — после добавления продукта
    // ─────────────────────────────────────────────────────────
    
    if (fiberPct < 0.5 && dayTot.kcal > 500) {
      advices.push({
        id: 'low_fiber',
        icon: '🥬',
        text: 'Добавь клетчатки: овощи, фрукты, каша',
        type: 'fiber',
        priority: 31,
        category: 'nutrition',
        score: 1 - fiberPct,
        triggers: ['product_added', 'meal_opened'],
        ttl: 4000
      });
    }
    
    if (carbsPct > 0.7 && proteinPct < 0.4) {
      advices.push({
        id: 'carbs_no_protein',
        icon: '🍽️',
        text: 'Углеводы без белка = быстрый голод. Добавь белок!',
        type: 'tip',
        priority: 32,
        category: 'nutrition',
        score: 0.8,
        triggers: ['product_added'],
        ttl: 4000
      });
    }
    
    if (fatPct > 0.7 && carbsPct < 0.4) {
      advices.push({
        id: 'fat_no_carbs',
        icon: '⚡',
        text: 'Для энергии нужны углеводы — попробуй кашу',
        type: 'tip',
        priority: 33,
        category: 'nutrition',
        score: 0.8,
        triggers: ['product_added'],
        ttl: 4000
      });
    }
    
    // Дефициты макросов — при просмотре приёма или после добавления
    if (proteinPct < 0.5 && fatPct >= 0.5 && carbsPct >= 0.5) {
      advices.push({
        id: 'low_protein',
        icon: '🥩',
        text: 'Добавь белка: творог, яйца, курица',
        type: 'protein',
        priority: 41,
        category: 'nutrition',
        score: 1 - proteinPct,
        triggers: ['product_added', 'meal_opened'],
        ttl: 4000
      });
    }
    
    if (fatPct < 0.5 && proteinPct >= 0.5 && carbsPct >= 0.5) {
      advices.push({
        id: 'low_fat',
        icon: '🥑',
        text: 'Мало жиров: орехи, авокадо, масло',
        type: 'fat',
        priority: 42,
        category: 'nutrition',
        score: 1 - fatPct,
        triggers: ['product_added', 'meal_opened'],
        ttl: 4000
      });
    }
    
    if (carbsPct < 0.5 && proteinPct >= 0.5 && fatPct >= 0.5) {
      advices.push({
        id: 'low_carbs',
        icon: '🍞',
        text: 'Добавь углеводов: каша, хлеб, фрукты',
        type: 'carbs',
        priority: 43,
        category: 'nutrition',
        score: 1 - carbsPct,
        triggers: ['product_added', 'meal_opened'],
        ttl: 4000
      });
    }

    // ─────────────────────────────────────────────────────────
    // ⏰ TIMING TIPS (priority: 51-70) — контекстные по времени
    // ─────────────────────────────────────────────────────────
    
    // Вечерние простые углеводы — после добавления вечером
    if (hour >= 18 && simplePct > 0.4 && simpleCarbs > 50) {
      advices.push({
        id: 'evening_sugar',
        icon: '🌙',
        text: 'Сладкое вечером → плохой сон. Лучше белок!',
        type: 'tip',
        priority: 51,
        category: 'timing',
        score: 0.7,
        triggers: ['product_added'],
        ttl: 4000
      });
    }
    
    // Пропущенный завтрак — при открытии вкладки днём
    if (hour >= 12 && mealCount === 0) {
      advices.push({
        id: 'no_breakfast',
        icon: '🌅',
        text: 'Завтрак запускает метаболизм — не пропускай!',
        type: 'tip',
        priority: 52,
        category: 'timing',
        score: 0.8,
        triggers: ['tab_open'],
        ttl: 5000
      });
    }
    
    if (hour >= 14 && mealCount === 1 && dayTot.kcal > 300) {
      advices.push({
        id: 'long_gap',
        icon: '⏰',
        text: 'Большие перерывы замедляют метаболизм — перекуси!',
        type: 'tip',
        priority: 53,
        category: 'timing',
        score: 0.6,
        triggers: ['tab_open'],
        ttl: 4000
      });
    }
    
    if (mealCount === 1 && dayTot.kcal > 800) {
      advices.push({
        id: 'one_big_meal',
        icon: '🍽️',
        text: 'Лучше 3-4 небольших приёма чем 1 большой',
        type: 'tip',
        priority: 54,
        category: 'timing',
        score: 0.5,
        triggers: ['product_added'],
        ttl: 4000
      });
    }

    // ─────────────────────────────────────────────────────────
    // 💪 TRAINING TIPS (priority: 71-80)
    // ─────────────────────────────────────────────────────────
    
    if (hasTraining && proteinPct < 0.6) {
      advices.push({
        id: 'training_protein',
        icon: '💪',
        text: 'После тренировки важен белок для восстановления',
        type: 'tip',
        priority: 71,
        category: 'training',
        score: 0.8,
        triggers: ['tab_open', 'product_added'],
        ttl: 5000
      });
    }

    // ─────────────────────────────────────────────────────────
    // 🎯 GOAL TIPS (priority: 81-90)
    // ─────────────────────────────────────────────────────────
    
    if (kcalPct >= 1.1 && kcalPct < 1.25) {
      advices.push({
        id: 'kcal_slight_excess',
        icon: '📊',
        text: 'Немного перебор — завтра чуть меньше, и всё ок 😊',
        type: 'tip',
        priority: 81,
        category: 'nutrition',
        score: 0.6,
        triggers: ['product_added', 'tab_open'],
        ttl: 4000
      });
    }
    
    if (kcalPct >= 0.8 && kcalPct < 0.95) {
      const remaining = Math.round(optimum - dayTot.kcal);
      advices.push({
        id: 'kcal_remaining',
        icon: '🎯',
        text: `Осталось ${remaining} ккал — идеально для перекуса`,
        type: 'tip',
        priority: 82,
        category: 'nutrition',
        score: 0.5,
        triggers: ['tab_open', 'meal_opened'],
        ttl: 4000
      });
    }

    // ─────────────────────────────────────────────────────────
    // ✅ SUCCESS (priority: 91-100)
    // ─────────────────────────────────────────────────────────
    
    if (proteinPct >= 0.8 && fatPct >= 0.8 && carbsPct >= 0.8) {
      advices.push({
        id: 'all_balanced',
        icon: '✅',
        text: 'Отлично! Все макросы в балансе',
        type: 'success',
        priority: 91,
        category: 'nutrition',
        score: 0.9,
        triggers: ['tab_open', 'product_added'],
        ttl: 4000
      });
    }

    // Сортируем по приоритету
    return advices.sort((a, b) => a.priority - b.priority);
  }

  // ═══════════════════════════════════════════════════════════
  // ADVICE ENGINE — Основной хук
  // ═══════════════════════════════════════════════════════════

  /**
   * React hook для работы с советами
   * @param {Object} params - Параметры
   * @param {ShowTrigger} params.trigger - Что вызвало показ
   * @param {Object} params.uiState - Состояние UI для проверки занятости
   * @param {number} params.currentStreak - Текущий streak (передаётся из DayTab, НЕ вычисляется заново!)
   * @returns {Object} Объект с советами и методами
   */
  function useAdviceEngine(params) {
    // ⚠️ ВАЖНО: currentStreak передаётся как параметр, НЕ вычисляется!
    // В DayTab он уже вычисляется через lsGet (строка 1636)
    const { dayTot, normAbs, optimum, day, pIndex, currentStreak, trigger, uiState } = params;
    const React = window.React;

    // Вычисляем контекст
    const ctx = React.useMemo(() => {
      const now = new Date();
      const hour = now.getHours();
      const meals = day?.meals || [];
      const mealCount = meals.filter(m => m.items?.length > 0).length;
      const trainings = day?.trainings || [];
      const hasTraining = trainings.some(t => t.z && t.z.some(m => m > 0));
      
      // 🧠 Расширенный контекст
      const kcalPct = (dayTot?.kcal || 0) / (optimum || 2000);
      const tone = getToneForHour(hour);
      const specialDay = getSpecialDay(now);
      const emotionalState = getEmotionalState({
        day,
        currentStreak: currentStreak || 0,
        mealCount,
        kcalPct,
        lastVisitDaysAgo: 0, // TODO: вычислить из localStorage
        totalDaysTracked: 30 // Приблизительно, для newbie detection
      });

      return {
        dayTot: dayTot || {},
        normAbs: normAbs || {},
        optimum: optimum || 2000,
        day: day || {},
        pIndex: pIndex || new Map(),
        currentStreak: currentStreak || 0,
        hour,
        mealCount,
        hasTraining,
        // Новые поля
        kcalPct,
        tone,
        specialDay,
        emotionalState
      };
    }, [dayTot, normAbs, optimum, day, pIndex, currentStreak]);

    // Получаем ВСЕ советы (без фильтрации по trigger)
    const allAdvices = React.useMemo(() => {
      try {
        const advices = evaluateRules(ctx);
        // Фильтруем по эмоциональному состоянию
        return filterByEmotionalState(advices, ctx.emotionalState);
      } catch (e) {
        console.error('[Advice] Error evaluating rules:', e);
        return [];
      }
    }, [ctx]);

    // Фильтруем по trigger + проверяем cooldown + занятость
    const relevantAdvices = React.useMemo(() => {
      // Если пользователь занят — не показываем
      if (isUserBusy(uiState)) return [];
      
      // Ночью — не беспокоим
      if (ctx.tone === 'silent') return [];
      
      if (!trigger || !canShowNow()) return [];
      
      return (allAdvices || []).filter(advice => {
        // Проверяем, подходит ли триггер
        if (!advice.triggers?.includes(trigger)) return false;
        // Проверяем, не показывали ли недавно
        if (wasShownRecently(advice.id)) return false;
        return true;
      });
    }, [allAdvices, trigger, uiState, ctx.tone]);

    // Группируем по категориям
    const byCategory = React.useMemo(() => {
      const grouped = {
        achievement: [],
        nutrition: [],
        timing: [],
        training: [],
        lifestyle: [],
        emotional: [],
        motivation: [],
        special: []
      };
      (allAdvices || []).forEach(a => {
        if (grouped[a.category]) {
          grouped[a.category].push(a);
        }
      });
      return grouped;
    }, [allAdvices]);

    // Главный совет для показа (первый из relevantAdvices)
    const primary = relevantAdvices[0] || null;
    
    // Функция отметки показа
    const markShown = React.useCallback((id) => {
      markAdviceShown(id);
      lastShowTime = Date.now();
    }, []);

    return {
      primary,             // Главный совет для toast (или null если нет актуальных)
      relevant: relevantAdvices, // Все актуальные для текущего trigger
      all: allAdvices,     // ВСЕ активные советы (для дебага)
      byCategory,          // Сгруппированные по категориям
      count: relevantAdvices.length,
      streak: ctx.currentStreak,
      markShown            // Функция для отметки показа
    };
  }

  // ═══════════════════════════════════════════════════════════
  // EXPORTS
  // ═══════════════════════════════════════════════════════════

  window.HEYS = window.HEYS || {};
  window.HEYS.advice = {
    useAdviceEngine,
    evaluateRules,
    // Утилиты для внешнего использования
    canShowNow,
    wasShownRecently,
    markAdviceShown,
    resetSessionAdvices, // Для сброса при смене даты
    // Контекстные функции (для внешнего использования)
    getToneForHour,
    getEmotionalState,
    getSpecialDay,
    isUserBusy
  };

})();
```

### Задача 2: Expandable Toast UI с Smart Timing

**Концепция**: Toast показывается только при определённых триггерах, учитывает занятость пользователя и автоматически исчезает.

**Где**: В DayTab `heys_day_v12.js`

**⚠️ ВНИМАНИЕ**: Перед этой задачей ОБЯЗАТЕЛЬНО выполнить первый промпт `2025-11-28-smart-toast-recommendations.md` — исправление бага `prot/protein`!

```javascript
// ═══════════════════════════════════════════════════════════
// SMART TIMING — Триггеры показа советов
// ═══════════════════════════════════════════════════════════

// Состояния (добавить рядом с существующими useState в DayTab, после строки ~707)
const [adviceTrigger, setAdviceTrigger] = React.useState(null);
const [adviceExpanded, setAdviceExpanded] = React.useState(false);
// ⚠️ ВАЖНО: toastVisible, toastDismissed и toastTimeoutRef УЖЕ существуют (строки 705-707)
// Использовать ИХ, не создавать новые!

// UI State для проверки занятости
// ⚠️ ВАЖНО: Использовать РЕАЛЬНЫЕ имена переменных из DayTab!
const uiState = React.useMemo(() => ({
  modalOpen: showTimePicker || showGramsPicker || showWeightPicker || showDeficitPicker || showZonePicker || showSleepQualityPicker || showDayScorePicker || showHouseholdPicker || showTrainingPicker,
  searchOpen: false, // Нет глобального searchOpen в DayTab — поиск внутри MealAddProduct
  isEditing: false, // Граммы редактируются inline без отдельного состояния
  isScrolling: false // Опционально: добавить scroll listener
}), [showTimePicker, showGramsPicker, showWeightPicker, showDeficitPicker, showZonePicker, showSleepQualityPicker, showDayScorePicker, showHouseholdPicker, showTrainingPicker]);

// Триггер при первом открытии вкладки (один раз за сессию)
// ⚠️ ВАЖНО: Переменная называется `date`, НЕ `curDate`!
React.useEffect(() => {
  const sessionKey = 'heys_tab_opened_' + date;
  if (!sessionStorage.getItem(sessionKey)) {
    sessionStorage.setItem(sessionKey, '1');
    // Небольшая задержка, чтобы пользователь "освоился"
    setTimeout(() => setAdviceTrigger('tab_open'), 1500);
  }
}, [date]);

// Триггер после добавления продукта — через CustomEvent
// ⚠️ ВАЖНО: Dispatch в функции addProductToMeal (строка 1587), НЕ в MealAddProduct!
React.useEffect(() => {
  const handleProductAdded = () => {
    // Задержка 500ms чтобы пользователь увидел результат
    setTimeout(() => setAdviceTrigger('product_added'), 500);
  };
  window.addEventListener('heysProductAdded', handleProductAdded);
  return () => window.removeEventListener('heysProductAdded', handleProductAdded);
}, []);

// Получаем советы через модуль с триггером И uiState
// ⚠️ ВАЖНО: Передаём currentStreak (уже вычислен на строке 1636), НЕ activeDays!
const { primary, relevant, count: adviceCount, markShown } = 
  window.HEYS?.advice?.useAdviceEngine?.({
    dayTot, normAbs, optimum, day, pIndex, 
    currentStreak, // <-- передаём существующий streak
    trigger: adviceTrigger,
    uiState
  }) || { primary: null, relevant: [], count: 0, markShown: () => {} };

// Автоматический показ/скрытие toast
React.useEffect(() => {
  if (primary && adviceTrigger) {
    // Показываем toast
    setToastVisible(true);
    
    // Haptic feedback для важных советов
    // ⚠️ ВАЖНО: проверяем typeof, не просто truthy!
    if ((primary.type === 'achievement' || primary.type === 'warning') && typeof haptic === 'function') {
      haptic('light');
    }
    
    // Вызываем onShow
    if (primary.onShow) primary.onShow();
    
    // Confetti для достижений (✅ showConfetti существует на строке 753!)
    if (primary.showConfetti) {
      setShowConfetti(true);
      if (typeof haptic === 'function') haptic('success');
      setTimeout(() => setShowConfetti(false), 2000);
    }
    
    // Отмечаем как показанный
    markShown(primary.id);
    
    // Запускаем таймер скрытия
    // ⚠️ ВАЖНО: Используем toastTimeoutRef, НЕ toastTimerRef!
    if (toastTimeoutRef.current) clearTimeout(toastTimeoutRef.current);
    toastTimeoutRef.current = setTimeout(() => {
      setToastVisible(false);
      setAdviceExpanded(false);
      setAdviceTrigger(null);
    }, primary.ttl || 5000);
  }
  
  return () => {
    if (toastTimeoutRef.current) clearTimeout(toastTimeoutRef.current);
  };
}, [primary?.id, adviceTrigger]);

// Сброс advice state при смене даты
// ⚠️ ВАЖНО: Переменная называется `date`, НЕ `curDate`!
React.useEffect(() => {
  setAdviceTrigger(null);
  setAdviceExpanded(false);
  setToastVisible(false);
  // Сброс session счётчиков
  if (window.HEYS?.advice?.resetSessionAdvices) {
    window.HEYS.advice.resetSessionAdvices();
  }
}, [date]);

// Сброс при открытии модалки
React.useEffect(() => {
  if (uiState.modalOpen || uiState.searchOpen) {
    setAdviceTrigger(null);
    setAdviceExpanded(false);
    setToastVisible(false);
  }
}, [uiState.modalOpen, uiState.searchOpen]);

// Рендеринг toast (только если visible)
// ⚠️ ВАЖНО: Сохраняем swipe handlers и toastSwipeX для совместимости!
toastVisible && primary && React.createElement('div', { 
  className: `macro-toast macro-toast-${primary.type} ${adviceExpanded ? 'expanded' : ''} ${toastVisible ? 'visible' : ''}`,
  role: 'alert',
  'aria-live': 'polite',
  onClick: () => adviceCount > 1 ? setAdviceExpanded(!adviceExpanded) : dismissToast(),
  // ⚠️ КРИТИЧНО: Сохраняем swipe handlers!
  onTouchStart: handleToastTouchStart,
  onTouchMove: handleToastTouchMove,
  onTouchEnd: handleToastTouchEnd,
  style: { 
    transform: `translateX(calc(-50% + ${toastSwipeX}px))`, 
    opacity: 1 - Math.abs(toastSwipeX) / 150 
  }
},
  // Основной совет
  React.createElement('div', { className: 'macro-toast-main' },
    React.createElement('span', { className: 'macro-toast-icon' }, primary.icon),
    React.createElement('span', { className: 'macro-toast-text' }, primary.text),
    adviceCount > 1 && React.createElement('span', { 
      className: 'macro-toast-badge' 
    }, `+${adviceCount - 1}`),
    // Close button (как в оригинале)
    React.createElement('button', { 
      className: 'macro-toast-close', 
      onClick: (e) => { e.stopPropagation(); dismissToast(); } 
    }, '×')
  ),
  
  // Progress bar (как в оригинале)
  React.createElement('div', { className: 'macro-toast-progress' }),
  
  // Дополнительные советы (при раскрытии)
  adviceExpanded && React.createElement('div', { className: 'macro-toast-extras' },
    (relevant || []).slice(1, 4).map(advice => 
      React.createElement('div', { 
        key: advice.id,
        className: `macro-toast-extra macro-toast-extra-${advice.type}`
      },
        React.createElement('span', null, advice.icon),
        React.createElement('span', null, advice.text)
      )
    )
  )
)
```

### Задача 3: CSS для Expandable Toast

**Где**: `apps/web/styles/main.css`

**⚠️ ВАЖНО**: Добавить ПОСЛЕ существующих `.macro-toast-*` стилей (после строки ~4948)

**✅ УЖЕ ДОБАВЛЕНО (первым промптом):**
- `.macro-toast-fiber`, `.macro-toast-tip`, `.macro-toast-achievement` (строки 4931-4948)
- Dark theme для этих типов (строки 5230-5243)
- `@keyframes achievementPulse` (если есть)

**Добавить ТОЛЬКО новые стили для expanded mode:**

```css
/* ═══════════════════════════════════════════════════════════
   EXPANDABLE ADVICE TOAST (дополнение к существующим стилям)
   ═══════════════════════════════════════════════════════════ */

/* Базовые модификации для расширяемого toast */
.macro-toast {
  cursor: pointer;
  transition: all 0.3s cubic-bezier(0.32, 0.72, 0, 1);
}

.macro-toast.expanded {
  bottom: 90px;
  max-height: 300px;
  border-radius: 16px;
}

.macro-toast-main {
  display: flex;
  align-items: center;
  gap: 10px;
}

.macro-toast-badge {
  background: rgba(255, 255, 255, 0.3);
  padding: 2px 8px;
  border-radius: 12px;
  font-size: 12px;
  font-weight: 600;
  margin-left: auto;
  animation: pulse 2s infinite;
}

@keyframes pulse {
  0%, 100% { opacity: 1; }
  50% { opacity: 0.6; }
}

.macro-toast-extras {
  display: flex;
  flex-direction: column;
  gap: 8px;
  margin-top: 12px;
  padding-top: 12px;
  border-top: 1px solid rgba(255, 255, 255, 0.2);
  animation: slideDown 0.2s ease-out;
}

@keyframes slideDown {
  from { opacity: 0; transform: translateY(-10px); }
  to { opacity: 1; transform: translateY(0); }
}

.macro-toast-extra {
  display: flex;
  align-items: center;
  gap: 10px;
  padding: 8px 12px;
  background: rgba(255, 255, 255, 0.1);
  border-radius: 10px;
  font-size: 13px;
}

.macro-toast-extra-warning { background: rgba(239, 68, 68, 0.2); }
.macro-toast-extra-achievement { background: rgba(234, 179, 8, 0.2); }
.macro-toast-extra-tip { background: rgba(59, 130, 246, 0.2); }
.macro-toast-extra-fiber { background: rgba(34, 197, 94, 0.2); }

/* Mobile: ограничиваем ширину и высоту */
@media (max-width: 480px) {
  .macro-toast {
    max-width: calc(100vw - 24px);
    font-size: 13px;
  }
  
  .macro-toast.expanded {
    max-height: 250px;
    overflow-y: auto;
  }
  
  .macro-toast-extras {
    max-height: 150px;
    overflow-y: auto;
  }
  
  .macro-toast-extra {
    font-size: 12px;
    padding: 6px 10px;
  }
}

/* Accessibility: роль alert для screen readers */
.macro-toast[role="alert"] {
  /* Стили при необходимости */
}
```

### Задача 4: Интеграция в DayTab

**Где**: `apps/web/heys_day_v12.js` и `apps/web/index.html`

#### 4.1 Добавить скрипт в index.html

**ПЕРЕД** `heys_day_v12.js` (найти строку с `heys_day_v12.js` и добавить ПЕРЕД ней):
```html
<script defer src="heys_advice_v1.js?v=1" fetchpriority="low"></script>
```

#### 4.2 Добавить dispatch event при добавлении продукта

**Где**: Функция `addProductToMeal` (~строка 1587 в heys_day_v12.js)

**Поиск**: `function addProductToMeal(mi, p)`

**Найти внутри этой функции** `setDay(prev =>` и **ПОСЛЕ успешного setDay** добавить:
```javascript
// Dispatch event для advice системы
window.dispatchEvent(new CustomEvent('heysProductAdded'));
```

#### 4.3 В DayTab:
1. Добавить состояние `const [adviceExpanded, setAdviceExpanded] = React.useState(false);` (~строка 708)
2. Добавить состояние `const [adviceTrigger, setAdviceTrigger] = React.useState(null);`
3. Добавить listener для `heysProductAdded` event (см. Задача 2)
4. Вызвать `useAdviceEngine` с нужными параметрами
5. Добавить useEffect для показа toast (см. Задача 2)
6. Заменить рендеринг toast на expandable версию
7. **Удалить старый macroTip useMemo** (~строки 2653-2698) — логика теперь в модуле
8. **Удалить старый useEffect для macroTip** (~строки 2700-2724)

#### 4.4 Закрытие при открытии модалок

```javascript
// В обработчиках открытия модалок добавить:
setAdviceExpanded(false);
```

---

## ✅ Definition of Done

### Pre-flight (ПЕРЕД началом)
- [x] Первый промпт (2025-11-28-smart-toast-recommendations.md) выполнен
- [x] CSS типы fiber/tip/achievement уже добавлены (строки 4931-4948)
- [x] currentStreak уже реализован (строка 1636)
- [x] Проверить что swipe handlers на toast работают

### Core Engine
- [x] Создан `apps/web/heys_advice_v1.js` с движком советов (757 строк)
- [x] Все правила имеют `triggers[]` и `ttl`
- [x] Добавлен трекер показанных советов (sessionStorage)
- [x] Cooldown 30 сек между показами
- [x] MAX_ADVICES_PER_SESSION = 10
- [x] Guard для пустого дня (kcal < 10 && mealCount === 0)
- [x] useAdviceEngine принимает `currentStreak` как параметр (НЕ вычисляет сам!)

### 🧠 Smart Timing (базовое)
- [x] `getToneForHour()` — адаптация тона по времени
- [x] `getEmotionalState()` — определение состояния (stressed, crashed, success)
- [x] `isUserBusy()` — проверка занятости пользователя
- [x] `filterByEmotionalState()` — убирать warnings при стрессе/срыве
- [x] Ночью (23:00-6:00) — НИКАКИХ советов (tone === 'silent')

### ⏭️ SKIP (оверкилл для MVP)
- ~~`getSpecialDay()` — праздники~~ → Реализовано! (monday, friday, sunday)
- ~~`monday_motivation`, `friday_reminder`~~ → Реализовано!

### UI Integration (в heys_day_v12.js)
- [x] Скрипт добавлен в `index.html` ПЕРЕД `heys_day_v12.js`
- [x] Добавлен `heysProductAdded` CustomEvent dispatch в **addProductToMeal** (строка 1598)
- [x] Добавлен event listener для `heysProductAdded` в DayTab
- [x] Добавлены состояния: `adviceTrigger`, `adviceExpanded`
- [x] Используется `date` (НЕ curDate!)
- [x] Используется `toastTimeoutRef` (НЕ toastTimerRef!)
- [x] Используется существующий `currentStreak` (строка 1636) — НЕ пересчитывать!
- [x] `uiState` собирает ВСЕ picker'ы (включая showTrainingPicker)
- [x] Передача `uiState` в useAdviceEngine
- [x] Задержка показа после действия (500ms product_added, 1500ms tab_open)
- [x] Сброс adviceExpanded при открытии модалки/поиска
- [x] Expandable Toast UI + автоскрытие по TTL
- [x] Swipe handlers СОХРАНЕНЫ на новом toast (handleToastTouchStart/Move/End)
- [x] Toast ширина 80% (4/5 экрана), max-width 400px
- [x] Badge яркий синий с индикатором expand ▼
- [x] adviceCount показывает ВСЕ советы для триггера (для badge)

### Cleanup
- [ ] macroTip useMemo УДАЛЁН (~строки 2708-2880) — ⚠️ оставлен как fallback
- [ ] Старые useEffect для toast УДАЛЕНЫ (строки 2700-2730) — ⚠️ оставлены как fallback
- [x] `pnpm type-check && pnpm build` проходит

### Исправленные ошибки
- [x] `searchOpen is not defined` → `searchOpen: false` в uiState
- [x] `Cannot access 'dayTot' before initialization` → перемещён advice блок после dayTot/normAbs

---

## 🚫 НЕ ДЕЛАТЬ

- ❌ Отдельную fixed панель (конфликт z-index с модалками)
- ❌ Side effects в useMemo (React Strict Mode)
- ❌ Зависимость от window.fmtDate (использовать inline formatDate)
- ❌ Переусложнять — это не AI engine
- ❌ Добавлять персистентность советов в localStorage
- ❌ Делать советы кликабельными с действиями (пока)
- ❌ Добавлять звуки/вибрацию
- ❌ Создавать отдельную вкладку советов
- ❌ Использовать `curDate` — правильно `date`
- ❌ Использовать `toastTimerRef` — правильно `toastTimeoutRef`
- ❌ Пересчитывать `currentStreak` в модуле — передавать из DayTab
- ❌ Дублировать CSS для fiber/tip/achievement — уже добавлено
- ❌ Удалять swipe handlers с toast
- ❌ Добавлять праздничные советы (specialDay) — оверкилл для MVP

---

## 🔮 Будущие улучшения (не в этом промпте)

1. **Daily Summary** — итоги дня вечером (после 21:00)
2. **Персонализация** — учёт предпочтений пользователя
3. **Тренды** — советы на основе динамики веса/калорий
4. **Микронутриенты** — когда добавим данные по витаминам
5. **Inline советы** — показ рядом с соответствующими данными (совет о белке → рядом с карточкой белка)

---

## 🔗 Связь с первым промптом

**Зависимость**: Этот промпт требует выполнения [2025-11-28-smart-toast-recommendations.md](./2025-11-28-smart-toast-recommendations.md) сначала:
- Первый промпт: багфикс `prot/protein` + расширение CSS типов toast
- Этот промпт: модуль советов + expandable UI

**После выполнения обоих**: macroTip useMemo остался как fallback, вся новая логика в `heys_advice_v1.js`

---

**Время фактическое**: ~2 часа (включая отладку ошибок)
**Сложность**: Средняя
**Коммиты**: 
- `4d1d606` — feat(advice): add HEYS Advice Module v1
- `e1525b0` — fix(advice): fix searchOpen undefined error in uiState
- `27c6219` — feat(advice): improve toast visibility and expand logic
