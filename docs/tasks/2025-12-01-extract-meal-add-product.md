...
### Phase 0 — статус
- Бейзлайн (Profiler): **не снят** (нет Profiler). Требуется ручной профилинг в браузере — если не будет снят, риски overkill/регресса выше, ориентируемся на ручные проверки UI.
...
### Текущее состояние
- `MealAddProduct` вынесен из `DayTab` в отдельный `React.memo` с компаратором; вызовы обновлены с явными пропсами (`mi, products, date, day, setDay`).
- `onAdd` внутри использует функциональный `setDay`, чтобы не зависеть от устаревшей ссылки на day и не плодить лишние ререндеры.
- Бейзлайн Profiler не снят: актуальны только ручные проверки UI.
- Требуется ручной прогон сценариев (поиск → ввод → модалка граммов → добавить продукт/несколько приёмов) и наблюдение за миганиями/сбросом стейта.
