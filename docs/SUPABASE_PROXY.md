# Supabase Proxy на Vercel

> **Дата создания**: 2025-12-06  
> **Статус**: ✅ Работает  
> **Причина**: Обход блокировки Supabase в РФ
...
Vercel **не поддерживает catch-all `[...path].js`** в глубоко вложенных папках (`/api/supabase/rest/v1/`).
...
⚠️ **Важно**: Можно использовать как `SUPABASE_*` так и `VITE_SUPABASE_*` — код проверяет оба варианта.
...
**Решение**: Node serverless runtime (убрали `export const config = { runtime: 'edge' }`)
...
**Причина**: Дублирование `content-encoding: gzip` — Supabase возвращает сжатый ответ, мы распаковываем через `arrayBuffer()`, но проксируем заголовок сжатия  
