...
(function (global) {
    ...
    function calculateDayTotals(day) {
        if (!isValidDay(day) || !day.meals.length) {
            return { kcal: 0, prot: 0, protein: 0, fat: 0, carbs: 0, simple: 0, complex: 0, bad: 0, good: 0, trans: 0, fiber: 0 };
        }
        ...
        let totalSimple = 0, totalComplex = 0, totalBad = 0, totalGood = 0, totalTrans = 0, totalFiber = 0;
    ...
    Stats.getRecentAverage = function (daysCount = 7) {
        try {
            const recentDays = [];
    ...
    // Вычисление трендов
    Stats.calculateTrends = function (days) {
        ...
        for (const day of sortedDays) {
            if (isValidDay(day)) {
                ...
                const weight = typeof day.weight === 'number' ? day.weight :
                    typeof day.weightMorning === 'number' ? day.weightMorning :
    ...
    Stats.getWeeklyStats = function (days) {
        if (!Array.isArray(days) || days.length === 0) {
            return { weeks: [] };
        }
        const sortedDays = [...days].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    ...
    // Сравнение периодов
    Stats.comparePeriods = function (period1, period2) {
    ...
    Stats.findAnomalies = function (days, threshold = 2) {
        ...
        const stdKcal = Math.sqrt(kcalValues.reduce((sum, val) => sum + Math.pow(val - avgKcal, 2), 0) / kcalValues.length);
...
export {};
//# sourceMappingURL=heys_stats_v1.js.map