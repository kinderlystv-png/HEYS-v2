...
(function (global) {
    ...
    function logError(error, errorInfo, additionalData) {
        try {
            const errorReport = {
                error: {
                    name: error.name,
    ...
    class ErrorBoundary extends React.Component {
        ...
        render() {
            if (this.state.hasError) {
                ...
                }, '🚨 Произошла ошибка'), React.createElement('p', null, 'Извините, что-то пошло не так. Мы зафиксировали эту ошибку и работаем над её исправлением.'), React.createElement('details', {
                    style: { marginTop: '15px', marginBottom: '15px' }
                }, React.createElement('summary', {
                    style: { cursor: 'pointer', fontWeight: 'bold' }
                }, 'Техническая информация'), React.createElement('div', {
                    style: {
                        marginTop: '10px',
                        padding: '10px',
                ...
                }, React.createElement('div', null, `Error ID: ${errorId}`), React.createElement('div', null, `Error: ${error?.name}: ${error?.message}`), errorInfo && React.createElement('div', null, `Component: ${errorInfo.componentStack?.split('\n')[1]?.trim() || 'Unknown'}`), React.createElement('div', null, `Retry Count: ${retryCount}`))), React.createElement('div', {
                ...
                }, `Попробовать снова ${retryCount > 0 ? `(${retryCount}/3)` : ''}`), React.createElement('button', {
                ...
                }, 'Перезагрузить страницу'), React.createElement('button', {
                    onClick: () => {
    ...
    const SimpleErrorBoundary = ({ children, message = 'Произошла ошибка' }) => {
        return React.createElement(ErrorBoundary, {
            children,
            fallback: ({ error }) => React.createElement('div', {
                style: {
                    padding: '10px',
    ...
    HEYS.SimpleErrorBoundary = SimpleErrorBoundary;
    HEYS.withErrorBoundary = withErrorBoundary;
    HEYS.ErrorReporting = ErrorReporting;
    HEYS.logError = logError;
    console.log('🛡️ HEYS Error Boundary v1 (TypeScript) загружен');
})(window);
export {};
//# sourceMappingURL=heys_error_boundary_v1.js.map