...
6. **🚨 НИКОГДА НЕ ОТКАТЫВАЙ ФАЙЛЫ** через `git checkout <file>`, `git restore`, `git reset` без **явного согласия пользователя**! Пользователь работает с несколькими агентами одновременно — откат может уничтожить работу других агентов.
