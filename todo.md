...
## ✅ Триал-машина — ПОЛНОСТЬЮ ГОТОВА К РАБОТЕ

### 🎫 Backend (выполнено 2025-12-26/27)
...
- [x] **Auth Bugfixes** (Bug #7-8, 2025-01-03):
  - [x] Curator login 500 → `on_conflict: 'client_id,k'` (был
        'user_id,client_id,k')
  - [x] PIN auth infinite loop → session_token preservation + retry guard
  - [x] Database migration: `client_kv_store` PK теперь `(client_id, k)`

### 🖥️ Frontend (готов, протестирован)
...
✅ Curator sync (REST upsert) — 200 OK (Bug #7 fixed)
✅ PIN sync (RPC batch_upsert) — success (Bug #8 fixed)
✅ Read-only режим — subscription_required блокирует записи
```
...
### 1. Onboarding Tutorial — Обучение новых пользователей

**Файл**:
[2025-01-06-onboarding-tutorial.md](./docs/tasks/2025-01-06-onboarding-tutorial.md)  
**Описание**:
Интерактивный тур по приложению для новых пользователей после регистрации.
Spotlight-эффект + пошаговое объяснение Hero-карточек и графика калорий.  
**Время**: ~4-6 часов
...
Система триал-машины полностью готова:

- ✅ Куратор может логиниться и синхронизировать данные клиентов
- ✅ Клиенты могут логиниться по PIN и использовать приложение
- ✅ Триал автоматически стартует при регистрации
- ✅ Read-only режим блокирует записи при истёкшем триале
- ✅ Trial Queue работает (ожидание мест)
...
**После разблокировки (техническая часть ~2-4 часа):**
...
- [x] **🔐 Auth Bugfixes** — curator 500 + PIN infinite loop (2025-01-03)
- [x] **🎫 Триал-машина** — backend + frontend + smoke tests (2025-12-26/27)
...
### 🔬 Инсулиновая Волна v2.0 (Science Upgrade)

**Файл**:
[2026-01-08-insulin-wave-v2.md](./docs/tasks/2026-01-08-insulin-wave-v2.md)
**Описание**: Внедрение научной модели Insuline Wave v2.0 (GL, II,
Multi-component curve, AUC scoring). **Время**: ~8-12 часов

**Технически система готова!** 🎉

Единственный блокер — **бизнес-решения по оплатам:**

1. **Решить юр.схему** (с бухгалтером): ПСН+УСН или только УСН
2. **Зарегистрироваться в ЮKassa** → получить shopId + secretKey
3. **Настроить фискализацию** → облачная касса или "Чеки от ЮKassa"

После этого — деплой платежей займёт **~2-4 часа**.
